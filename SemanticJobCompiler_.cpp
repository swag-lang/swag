...
bool SemanticJob::resolveCompilerImport(SemanticContext* context)
{
    ...
    const auto& moduleName = node->name;
    auto        sourceFile = context->sourceFile;
    auto        module     = sourceFile->module;
    auto        workspace  = module->workspace;

    // If the module is not being compiled, then it should be accessible from the workspace
    auto it = workspace->mapModulesNames.find(moduleName);
    if (it == workspace->mapModulesNames.end())
    {
        internalError(context, "dependency to an external module (resolveCompilerImport)");
        return false;
    }

    // Otherwise we have a dependency
    auto otherModule = it->second;

    {
        scoped_lock lk(otherModule->mutexOutputState);
        if (otherModule->outputState == OutputState::None)
        {
            otherModule->dependentOutputJobs.push_back(context->job);
            context->result = SemanticResult::Pending;
            return true;
        }
    }
    ...
    auto path = workspace->cachePath.string() + moduleName + ".swg";
    if (!fs::exists(path))
    {
