...
bool SemanticJob::resolveImplFor(SemanticContext* context)
{
    ...
    TypeInfoParam* typeParamItf = nullptr;
    {
        ...
        if (!typeParamItf)
        {
            typeParamItf = g_Allocator.alloc0<TypeInfoParam>();
...
bool SemanticJob::resolveStruct(SemanticContext* context)
{
    ...
    for (int i = 0; i < childs.size(); i++)
    {
        ...
        if (!(node->flags & AST_FROM_GENERIC))
        {
            typeParam             = g_Allocator.alloc0<TypeInfoParam>();
...
bool SemanticJob::resolveInterface(SemanticContext* context)
{
    ...
    // itable
    auto typeITable        = g_Allocator.alloc0<TypeInfoStruct>();
    ...
    if (!(node->flags & AST_FROM_GENERIC))
    {
        auto typeParam      = g_Allocator.alloc0<TypeInfoParam>();
