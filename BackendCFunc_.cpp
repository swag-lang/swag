...
void BackendC::emitFuncSignatureH(TypeInfoFuncAttr* typeFunc, AstFuncDecl* node)
{
    if (!(node->attributeFlags & ATTRIBUTE_PUBLIC))
        return;

    bufferH.addString("SWAG_EXTERN SWAG_IMPEXP ");
    bufferH.addString(swagTypeToCType(typeFunc->returnType));
    bufferH.addString(" ");
    bufferH.addString(module->name);
    bufferH.addString("_");
    bufferH.addString(node->name.c_str());
    bufferH.addString("(");
    ...
    if (node->parameters)
    {
        ...
        for (auto param : node->parameters->childs)
        {
            ...
            bufferH.addString(swagTypeToCType(param->typeInfo));
            bufferH.addString(" ");
            bufferH.addString(param->name.c_str());
        }
    }
...
void BackendC::emitFuncSignatureC(TypeInfoFuncAttr* typeFunc, AstFuncDecl* node)
{
...
{
    bufferSwg.addString("/****************************** PROTOTYPES ****************************/\n");
    bufferH.addString("/****************************** PROTOTYPES ****************************/\n");
    bufferC.addString("/****************************** PROTOTYPES ****************************/\n");
    bufferSwg.addString("#[foreign]\n");
    bufferSwg.addString("{\n");
    ...
    for (auto one : module->byteCodeFunc)
    {
        ...
        emitFuncSignatureC(typeFunc, node);
        bufferC.addString(";\n");

        emitFuncSignatureH(typeFunc, node);
