...
bool BackendC::emitInternalFunction(TypeInfoFuncAttr* typeFunc, AstFuncDecl* node)
{
    ...
    for (uint32_t i = 0; i < bc->numInstructions; i++, ip++)
    {
        ...
        case ByteCodeOp::BoundCheckV:
            bufferC.addString(format("__assert(r%u.u32 <= %u, \"%s\", %d, \": error: index out of range\");", ip->a.u32, ip->b.u32, normalizePath(module->files[ip->sourceFileIdx]->path).c_str(), ip->startLocation.line + 1));
            break;
        ...
        case ByteCodeOp::DeRefString:
            bufferC.addString(format("r%u.pointer = *(swag_uint8_t**) r%u.pointer; ", ip->a.u32, ip->a.u32));
            bufferC.addString(format("r%u.u64 = ((swag_uint64_t*) r%u.pointer)[1];", ip->b.u32, ip->a.u32));
            break;
