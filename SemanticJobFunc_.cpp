    ...
    if (node->content && node->returnType && node->returnType->typeInfo != g_TypeMgr.typeInfoVoid)
    {
        if (!(node->flags & AST_SCOPE_HAS_RETURN))
        {
            if (node->flags & AST_FCT_HAS_RETURN)
                return context->errorContext.report({node->sourceFile, node->token, format("not all control paths of function '%s' return a value", node->name.c_str())});
            return context->errorContext.report({node->sourceFile, node->token, format("function '%s' must return a value", node->name.c_str())});
        }
    }
...
bool SemanticJob::resolveFuncDeclType(SemanticContext* context)
{
    ...
    SWAG_VERIFY(funcNode->name != "opInit" || funcNode->flags & AST_GENERATED || funcNode->attributeFlags & ATTRIBUTE_FOREIGN, sourceFile->report({sourceFile, funcNode->token, "'opInit' function cannot be user defined, as it is generated by the compiler"}));
