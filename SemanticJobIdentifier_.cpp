...
anotherTry:
    ...
    if (matches.size() > 1)
    {
        if (hasGenericErrors)
        {
            Diagnostic                diag{sourceFile, node->token, format("ambiguous generic resolution to %s '%s'", SymTable::getNakedKindName(symbol->kind), symbol->name.c_str())};
            vector<const Diagnostic*> notes;
        ...
        {
            Diagnostic                diag{sourceFile, node->token, format("ambiguous resolution of %s '%s'", SymTable::getNakedKindName(symbol->kind), symbol->name.c_str())};
            vector<const Diagnostic*> notes;
            for (auto match : matches)
            {
                auto overload   = match.symbolOverload;
...
void SemanticJob::collectScopeHierarchy(SemanticContext* context, vector<Scope*>& scopes, vector<AlternativeScope>& scopesVars, Scope* startScope)
{
    auto  job       = context->job;
...
bool SemanticJob::checkSymbolGhosting(SemanticContext* context, Scope* startScope, AstNode* node, SymbolKind kind)
{
    ...
    SemanticJob::collectScopeHierarchy(context, job->cacheScopeHierarchy, job->cacheScopeHierarchyVars, startScope);
