                                      ...
                                      TypeInfoStruct*             typeInterface)
{
    ...
    for (uint32_t idx = 0; idx < numFctInterface; idx++)
    {
        ...
        if (!itfRef.itf)
        {
            ...
            if (!defaultExportNode)
            {
                Diagnostic err{node, node->getTokenName(), formErr(Err0448, typeBaseInterface->name.cstr(), typeStruct->getDisplayNameC())};
        ...
        for (uint32_t i = 1; i < typeFunc->parameters.size(); i++)
        {
            ...
            if (type->isNonNullable())
                content += form("p%d: nn %s", i, type->scopedNameExport.cstr());
            else
