        ...
        for (uint32_t i = 0; i < typeLeftStruct->fields.size(); i++)
        {
            ...
            if (typeField->typeInfo->isArray())
            {
                ...
                if (!typeArr->finalType->isStruct())
                    content += form("if @memcmp(&%s[0], &o.%s[0], @sizeof(%s)) != 0 do return false\n", leftN, rightN, leftN);
                else
                {
                    content += form("loop i: %s do ", leftN);
                    content += form("if %s[i] != o.%s[i] do return false\n", leftN, rightN);
                }

                continue;
            }

            content += form("if %s != o.%s do return false\n", leftN, rightN);
        }
                                      ...
                                      TypeInfoStruct*             typeInterface)
{
    ...
    for (uint32_t idx = 0; idx < numFctInterface; idx++)
    {
        ...
        if (!itfRef.itf)
        {
            ...
            if (!defaultExportNode)
            {
                Diagnostic err{node, node->getTokenName(), formErr(Err0508, typeBaseInterface->name.c_str(), typeStruct->getDisplayNameC())};
