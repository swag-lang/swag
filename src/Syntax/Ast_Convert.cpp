...
bool Ast::convertLiteralTupleToStructVar(JobContext* context, TypeInfo* toType, AstNode* fromNode, bool fromType)
{
    ...
    if (!fromType && typeStruct->isTuple())
    {
        ...
        {
            Diagnostic err{fromNode->children[maxCount], formErr(Err0641, maxCount, countParams)};
        ...
        if (countParams < maxCount)
        {
            Diagnostic err{fromNode->lastChild(), formErr(Err0602, maxCount, countParams)};
            const auto errNode = destStruct->originalParent ? destStruct->originalParent : destStruct;
            err.addNote(Diagnostic::note(errNode, toNte(Nte0078)));
            return context->report(err);
        }
...
bool Ast::convertLiteralTupleToStructType(JobContext* context, AstNode* paramNode, TypeInfoStruct* toType, AstNode* fromNode)
{
    ...
    if (countParams > maxCount)
    {
        Diagnostic err{fromNode->firstChild()->children[maxCount], formErr(Err0641, maxCount, countParams)};
        const auto errNode = destStruct->originalParent ? destStruct->originalParent : destStruct;
        err.addNote(Diagnostic::note(errNode, toNte(Nte0078)));
        return context->report(err);
    }
    if (countParams < maxCount)
    {
        Diagnostic err{fromNode->firstChild()->lastChild(), formErr(Err0602, maxCount, countParams)};
        const auto errNode = destStruct->originalParent ? destStruct->originalParent : destStruct;
        err.addNote(Diagnostic::note(errNode, toNte(Nte0078)));
        return context->report(err);
    }
    ...
    // on the name (if specified).
    {
        ...
        for (const auto p : toType->fields)
        {
            ...
            if (typeField->isListTuple())
                return context->report({fromNode, toErr(Err0746)});
