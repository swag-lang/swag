...
bool Tokenizer::doIdentifier(TokenParse& tokenParse)
{
    ...
    else if (tokenParse.token.text[0] == '#')
    {
        ...
        else
        {
            ...
            Diagnostic err{sourceFile, tokenParse, formErr(Err0189, tokenParse.token.c_str())};
    ...
    {
        tokenParse.token.endLocation = location;
        Diagnostic err{sourceFile, tokenParse, formErr(Err0234, tokenParse.token.c_str())};
