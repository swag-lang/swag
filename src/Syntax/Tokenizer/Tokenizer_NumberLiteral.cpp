...
bool Tokenizer::doBinLiteral(TokenParse& tokenParse)
{
    ...
    while (c == '0' || c == '1' || SWAG_IS_NUM_SEP(c))
    {
        ...
        {
            if (!acceptSep)
                return error(tokenParse, toErr(Err0418));
            acceptSep = false;
            c         = peekChar(offset);
            continue;
        }
        ...
        acceptSep = true;
        SWAG_VERIFY(!(tokenParse.literalValue.u64 & 0x80000000'00000000), error(tokenParse, toErr(Err0316)));
        SWAG_VERIFY(rank != 64, error(tokenParse, toErr(Err0314)));
    ...
    if (!acceptSep)
        return error(tokenParse, toErr(Err0415));
...
bool Tokenizer::doHexLiteral(TokenParse& tokenParse)
{
    ...
    while (SWAG_IS_ALPHA_HEX(c) || SWAG_IS_DIGIT(c) || SWAG_IS_NUM_SEP(c))
    {
        ...
        acceptSep = true;
        SWAG_VERIFY(!(tokenParse.literalValue.u64 & 0xF0000000'00000000), error(tokenParse, toErr(Err0316)));
        SWAG_VERIFY(rank != 16, error(tokenParse, toErr(Err0315)));
    ...
    if (SWAG_IS_ALPHA(c))
    {
        ...
        return error(tokenParse, toErr(Err0684));
    }
...
bool Tokenizer::doFloatLiteral(TokenParse& token, uint32_t c)
{
    ...
    while (SWAG_IS_DIGIT(c) || SWAG_IS_NUM_SEP(c))
    {
        ...
        if (SWAG_IS_NUM_SEP(c))
        {
            SWAG_CHECK(acceptSep || rank == 0 || error(token, toErr(Err0418)));
            acceptSep = false;
            c         = peekChar(offset);
            continue;
        }
        ...
        const auto val = c - '0';
        SWAG_VERIFY(token.literalValue.u64 <= 18446744073709551615ULL - val, error(token, toErr(Err0316)));
    ...
    // Be sure we don't have a number with a separator at its end
    if (!acceptSep)
        return error(token, toErr(Err0415));
...
bool Tokenizer::doIntFloatLiteral(TokenParse& tokenParse, uint32_t c)
{
    ...
    if (hasDot)
    {
        ...
        c                             = peekChar(offset);
        SWAG_VERIFY(SWAG_IS_NOT_NUM_SEP(c), error(tokenFrac, toErr(Err0417)));
        if (SWAG_IS_DIGIT(c))
        {
            eatChar(c, offset);
    ...
    if (c == 'e' || c == 'E')
    {
        ...
        tokenExponent.token.startLocation = location;
        SWAG_VERIFY(SWAG_IS_NOT_NUM_SEP(c), error(tokenExponent, toErr(Err0416)));
