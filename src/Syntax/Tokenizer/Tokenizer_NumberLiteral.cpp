...
bool Tokenizer::doBinLiteral(TokenParse& tokenParse)
{
    ...
    while (c == '0' || c == '1' || SWAG_IS_NUM_SEP(c))
    {
        ...
        {
            if (!acceptSep)
                return error(tokenParse, toErr(Err0301));
            acceptSep = false;
            c         = peekChar(offset);
            continue;
        }
        ...
        SWAG_VERIFY(rank != 64, error(tokenParse, toErr(Err0563)));
    ...
    if (!acceptSep)
        return error(tokenParse, toErr(Err0300));
...
bool Tokenizer::doHexLiteral(TokenParse& tokenParse)
{
    ...
    while (SWAG_IS_ALPHA_HEX(c) || SWAG_IS_DIGIT(c) || SWAG_IS_NUM_SEP(c))
    {
        ...
        SWAG_VERIFY(rank != 16, error(tokenParse, toErr(Err0566)));
    ...
    if (SWAG_IS_ALPHA(c))
    {
        ...
        return error(tokenParse, toErr(Err0655));
    }
...
bool Tokenizer::doFloatLiteral(TokenParse& token, uint32_t c)
{
    ...
    while (SWAG_IS_DIGIT(c) || SWAG_IS_NUM_SEP(c))
    {
        ...
        if (SWAG_IS_NUM_SEP(c))
        {
            SWAG_CHECK(acceptSep || rank == 0 || error(token, toErr(Err0301)));
            acceptSep = false;
            c         = peekChar(offset);
            continue;
        }
    ...
    // Be sure we don't have a number with a separator at its end
    if (!acceptSep)
        return error(token, toErr(Err0300));
...
bool Tokenizer::doIntFloatLiteral(TokenParse& tokenParse, uint32_t c)
{
    ...
    if (hasDot)
    {
        ...
        c                             = peekChar(offset);
        SWAG_VERIFY(SWAG_IS_NOT_NUM_SEP(c), error(tokenFrac, toErr(Err0303)));
        if (SWAG_IS_DIGIT(c))
        {
            eatChar(c, offset);
    ...
    if (c == 'e' || c == 'E')
    {
        ...
        tokenExponent.token.startLocation = location;
        SWAG_VERIFY(SWAG_IS_NOT_NUM_SEP(c), error(tokenExponent, toErr(Err0302)));
        SWAG_VERIFY(SWAG_IS_DIGIT(c), error(tokenExponent, toErr(Err0428)));
        eatChar(c, offset);
...
bool Tokenizer::doNumberLiteral(TokenParse& tokenParse, uint32_t c)
{
    if (c == '0')
    {
        ...
        if (SWAG_IS_ALPHA(c))
        {
            ...
            return error(tokenParse, toErr(Err0729));
        }
    }
