    ...
    for (auto f : subDecl)
    {
        ...
        switch (sub->kind)
        {
            case AstNodeKind::FuncDecl:
            {
                ...
                if (sym)
                {
                    const Diagnostic err{nodeFunc, nodeFunc->tokenName, formErr(Err0233, "function", sub->token.cstr())};
            ...
            case AstNodeKind::StructDecl:
            {
                ...
                if (sym)
                {
                    const Diagnostic err{nodeStruct, nodeStruct->tokenName, formErr(Err0233, "struct", sub->token.cstr())};
                    return context->report(err);
                }
            ...
            case AstNodeKind::InterfaceDecl:
                if (sym)
                {
                    const Diagnostic err{sub, sub->token, formErr(Err0233, "interface", sub->token.cstr())};
                    return context->report(err);
                }
