    ...
    for (auto f : subDecl)
    {
        ...
        switch (sub->kind)
        {
            case AstNodeKind::FuncDecl:
            {
                ...
                if (sym)
                {
                    const Diagnostic err{nodeFunc, nodeFunc->tokenName, formErr(Err0527, "function", sub->token.c_str())};
            ...
            case AstNodeKind::StructDecl:
            {
                ...
                if (sym)
                {
                    const Diagnostic err{nodeStruct, nodeStruct->tokenName, formErr(Err0527, "struct", sub->token.c_str())};
                    return context->report(err);
                }
            ...
            case AstNodeKind::InterfaceDecl:
                if (sym)
                {
                    const Diagnostic err{sub, sub->token, formErr(Err0527, "interface", sub->token.c_str())};
                    return context->report(err);
                }
