...
bool Parser::checkIsValidUserName(AstNode* node, const Token* loc) const
{
    ...
    if (!testIsValidUserName(node))
        return error(loc ? *loc : node->token, formErr(Err0577, node->token.c_str()));
...
bool Parser::checkIsSingleIdentifier(AstNode* node, const char* msg) const
{
    ...
    if (node->is(AstNodeKind::IdentifierRef))
        return error(node, formErr(Err0255, msg));

    return error(node, formErr(Err0227, msg));
}
...
bool Parser::doIdentifier(AstNode* parent, IdentifierFlags identifierFlags)
{
    ...
    if (tokenParse.is(TokenId::CompilerUp))
    {
        ...
        if (tokenParse.is(TokenId::SymLeftParen))
        {
            ...
            if (tokenParse.literalType != LiteralType::TypeUntypedInt && tokenParse.literalType != LiteralType::TypeUnsigned8)
                return error(tokenParse, formErr(Err0124, tokenParse.token.c_str()));
            if (tokenParse.literalValue.u64 > 255)
                return error(tokenParse, formErr(Err0556, tokenParse.literalValue.u64));
            if (tokenParse.literalValue.u8 == 0)
                return error(tokenParse, formErr(Err0123, tokenParse.token.c_str()));
    ...
    {
        Diagnostic err{sourceFile, tokenParse, toErr(Err0228)};
    ...
    {
        Diagnostic err{identifier, formErr(Err0521, identifier->token.c_str())};
    ...
    {
        SWAG_VERIFY(parent->ownerStructScope, context->report({identifier, toErr(Err0397)}));
    ...
    if (!tokenParse.flags.has(TOKEN_PARSE_EOL_BEFORE) && !identifierFlags.has(IDENTIFIER_NO_FCT_PARAMS))
    {
        if (tokenParse.is(TokenId::SymLeftParen))
        {
            ...
            {
                Diagnostic err{identifier, tokenParse.token, toErr(Err0307)};
                return context->report(err);
            }
...
bool Parser::doDiscard(AstNode* parent, AstNode** result)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        default:
            ...
            return error(tokenParse, toErr(Err0132));
    }
...
bool Parser::doTryCatchAssume(AstNode* parent, AstNode** result, bool afterDiscard)
{
    ...
    *result = node;
    SWAG_VERIFY(node->ownerFct, error(node, formErr(Err0446, node->token.c_str())));
    SWAG_CHECK(eatToken());
    ...
    if (tokenParse.is(TokenId::SymLeftCurly))
    {
        ...
        SWAG_VERIFY(!afterDiscard, error(tokenParse, toErr(Err0178)));
    ...
    {
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdTry), error(tokenParse, formErr(Err0445, tokenParse.token.c_str(), node->token.c_str())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdCatch), error(tokenParse, formErr(Err0445, tokenParse.token.c_str(), node->token.c_str())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdAssume), error(tokenParse, formErr(Err0445, tokenParse.token.c_str(), node->token.c_str())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdThrow), error(tokenParse, formErr(Err0445, tokenParse.token.c_str(), node->token.c_str())));
        SWAG_CHECK(checkIsIdentifier(tokenParse, formErr(Err0077, node->token.c_str())));
...
bool Parser::doTypeAlias(AstNode* parent, AstNode** result)
{
    ...
    *result = node;
    ...
    SWAG_CHECK(checkIsIdentifier(tokenParse, formErr(Err0252, node->token.c_str())));
