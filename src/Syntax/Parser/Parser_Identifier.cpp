...
bool Parser::checkIsValidUserName(AstNode* node, const Token* loc) const
{
    ...
    if (node->token.text == "_")
        return error(loc ? *loc : node->token, toErr(Err0546));
    if (!testIsValidUserName(node))
        return error(loc ? *loc : node->token, formErr(Err0547, node->token.cstr()));
...
bool Parser::checkIsSingleIdentifier(AstNode* node, const char* msg) const
{
    ...
    if (node->is(AstNodeKind::IdentifierRef))
        return error(node, formErr(Err0191, msg));

    return error(node, formErr(Err0219, msg));
}
...
bool Parser::doIdentifier(AstNode* parent, IdentifierFlags identifierFlags)
{
    ...
    if (tokenParse.is(TokenId::CompilerUp))
    {
        ...
        if (tokenParse.is(TokenId::SymLeftParen))
        {
            ...
            if (tokenParse.literalType != LiteralType::TypeUntypedInt && tokenParse.literalType != LiteralType::TypeUnsigned8)
                return error(tokenParse, formErr(Err0141, tokenParse.cstr()));
            if (tokenParse.literalValue.u64 > 255)
                return error(tokenParse, formErr(Err0526, tokenParse.literalValue.u64));
            if (tokenParse.literalValue.u8 == 0)
                return error(tokenParse, formErr(Err0140, tokenParse.cstr()));
    ...
    {
        Diagnostic err{sourceFile, tokenParse, toErr(Err0221)};
    ...
    {
        const Diagnostic err{identifier, formErr(Err0466, "intrinsic", identifier->token.cstr())};
        return context->report(err);
    }
    ...
    {
        SWAG_VERIFY(parent->ownerStructScope, context->report({identifier, toErr(Err0349)}));
    ...
    if (!identifierFlags.has(IDENTIFIER_NO_CALL_PARAMS))
    {
        if (Tokenizer::isJustAfterPrevious(tokenParse))
        {
            ...
            if (tokenParse.is(TokenId::SymLeftParen))
            {
                ...
                {
                    Diagnostic err{identifier, tokenParse.token, toErr(Err0262)};
                    return context->report(err);
                }
...
bool Parser::doDiscard(AstNode* parent, AstNode** result)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        default:
            ...
            return error(tokenParse, toErr(Err0654));
    }
...
bool Parser::doTryCatchAssume(AstNode* parent, AstNode** result, bool afterDiscard)
{
    ...
    *result = node;
    SWAG_VERIFY(node->ownerFct, error(node, formErr(Err0421, node->token.cstr())));
    SWAG_CHECK(eatToken());
    ...
    if (tokenParse.is(TokenId::SymLeftCurly))
    {
        ...
        SWAG_VERIFY(!afterDiscard, error(tokenParse, toErr(Err0407)));
    ...
    {
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdTry), error(tokenParse, formErr(Err0420, tokenParse.cstr(), node->token.cstr())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdCatch), error(tokenParse, formErr(Err0420, tokenParse.cstr(), node->token.cstr())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdAssume), error(tokenParse, formErr(Err0420, tokenParse.cstr(), node->token.cstr())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdThrow), error(tokenParse, formErr(Err0420, tokenParse.cstr(), node->token.cstr())));
        SWAG_CHECK(checkIsIdentifier(tokenParse, formErr(Err0185, node->token.cstr())));
...
bool Parser::doTypeAlias(AstNode* parent, AstNode** result)
{
    ...
    *result = node;
    ...
    SWAG_CHECK(checkIsIdentifier(tokenParse, formErr(Err0660, node->token.cstr())));
...
bool Parser::doNameAlias(AstNode* parent, AstNode** result)
{
    ...
    *result = node;
    ...
    SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0659)));
