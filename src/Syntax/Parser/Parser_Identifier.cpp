...
bool Parser::checkIsValidUserName(AstNode* node, const Token* loc) const
{
    ...
    if (node->token.text == "_")
        return error(loc ? *loc : node->token, toErr(Err0540));
    if (!testIsValidUserName(node))
        return error(loc ? *loc : node->token, formErr(Err0541, node->token.cstr()));
...
bool Parser::checkIsSingleIdentifier(AstNode* node, const char* msg) const
{
    ...
    if (node->is(AstNodeKind::IdentifierRef))
        return error(node, formErr(Err0162, msg));

    return error(node, formErr(Err0190, msg));
}
...
bool Parser::doIdentifier(AstNode* parent, IdentifierFlags identifierFlags)
{
    ...
    if (tokenParse.is(TokenId::CompilerUp))
    {
        ...
        if (tokenParse.is(TokenId::SymLeftParen))
        {
            ...
            if (tokenParse.isNot(TokenId::LiteralNumber))
                return error(tokenParse, toErr(Err0644));
            if (tokenParse.literalType != LiteralType::TypeUntypedInt && tokenParse.literalType != LiteralType::TypeUnsigned8)
                return error(tokenParse, formErr(Err0118, tokenParse.cstr()));
            if (tokenParse.literalValue.u64 > 255)
                return error(tokenParse, formErr(Err0502, tokenParse.literalValue.u64));
            if (tokenParse.literalValue.u8 == 0)
                return error(tokenParse, formErr(Err0117, tokenParse.cstr()));
    ...
    {
        const Diagnostic err{identifier, formErr(Err0442, "intrinsic", identifier->token.cstr())};
        return context->report(err);
    }
    ...
    {
        SWAG_VERIFY(parent->ownerStructScope, context->report({identifier, toErr(Err0325)}));
    ...
    if (!tokenParse.flags.has(TOKEN_PARSE_EOL_BEFORE) && !identifierFlags.has(IDENTIFIER_NO_FCT_PARAMS))
    {
        if (tokenParse.is(TokenId::SymLeftParen))
        {
            ...
            {
                Diagnostic err{identifier, tokenParse.token, toErr(Err0233)};
                return context->report(err);
            }
...
bool Parser::doDiscard(AstNode* parent, AstNode** result)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        default:
            ...
            {
                Diagnostic err{sourceFile, tokenParse, formErr(Err0756, tokenParse.cstr())};
                err.addNote(sourceFile, discardToken.token, toNte(Nte0161));
                err.addNote(toNte(Nte0010));
                return context->report(err);
            }

            return error(tokenParse, toErr(Err0646));
    }
...
bool Parser::doTryCatchAssume(AstNode* parent, AstNode** result, bool afterDiscard)
{
    ...
    *result = node;
    SWAG_VERIFY(node->ownerFct, error(node, formErr(Err0396, node->token.cstr())));
    SWAG_CHECK(eatToken());
    ...
    if (tokenParse.is(TokenId::SymLeftCurly))
    {
        ...
        SWAG_VERIFY(!afterDiscard, error(tokenParse, toErr(Err0382)));
    ...
    {
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdTry), error(tokenParse, formErr(Err0395, tokenParse.cstr(), node->token.cstr())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdCatch), error(tokenParse, formErr(Err0395, tokenParse.cstr(), node->token.cstr())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdAssume), error(tokenParse, formErr(Err0395, tokenParse.cstr(), node->token.cstr())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdThrow), error(tokenParse, formErr(Err0395, tokenParse.cstr(), node->token.cstr())));
        SWAG_CHECK(checkIsIdentifier(tokenParse, formErr(Err0156, node->token.cstr())));
...
bool Parser::doTypeAlias(AstNode* parent, AstNode** result)
{
    ...
    *result = node;
    ...
    SWAG_CHECK(checkIsIdentifier(tokenParse, formErr(Err0651, node->token.cstr())));
...
bool Parser::doNameAlias(AstNode* parent, AstNode** result)
{
    ...
    *result = node;
    ...
    SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0650)));
