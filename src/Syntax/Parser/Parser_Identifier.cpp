...
bool Parser::checkIsValidUserName(AstNode* node, const Token* loc) const
{
    ...
    if (node->token.text == "_")
        return error(loc ? *loc : node->token, toErr(Err0543));
    if (!testIsValidUserName(node))
        return error(loc ? *loc : node->token, formErr(Err0544, node->token.cstr()));
...
bool Parser::checkIsSingleIdentifier(AstNode* node, const char* msg) const
{
    ...
    if (node->is(AstNodeKind::IdentifierRef))
        return error(node, formErr(Err0164, msg));

    return error(node, formErr(Err0192, msg));
}
...
bool Parser::doIdentifier(AstNode* parent, IdentifierFlags identifierFlags)
{
    ...
    if (tokenParse.is(TokenId::CompilerUp))
    {
        ...
        if (tokenParse.is(TokenId::SymLeftParen))
        {
            ...
            if (tokenParse.isNot(TokenId::LiteralNumber))
                return error(tokenParse, toErr(Err0647));
            ...
            if (tokenParse.literalValue.u64 > 255)
                return error(tokenParse, formErr(Err0505, tokenParse.literalValue.u64));
            if (tokenParse.literalValue.u8 == 0)
                return error(tokenParse, formErr(Err0119, tokenParse.cstr()));
    ...
    {
        Diagnostic err{sourceFile, tokenParse, toErr(Err0194)};
    ...
    {
        const Diagnostic err{identifier, formErr(Err0445, "intrinsic", identifier->token.cstr())};
        return context->report(err);
    }
    ...
    {
        SWAG_VERIFY(parent->ownerStructScope, context->report({identifier, toErr(Err0328)}));
...
bool Parser::doDiscard(AstNode* parent, AstNode** result)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        default:
            ...
            {
                Diagnostic err{sourceFile, tokenParse, formErr(Err0759, tokenParse.cstr())};
...
bool Parser::doTryCatchAssume(AstNode* parent, AstNode** result, bool afterDiscard)
{
    ...
    *result = node;
    SWAG_VERIFY(node->ownerFct, error(node, formErr(Err0399, node->token.cstr())));
    SWAG_CHECK(eatToken());
    ...
    if (tokenParse.is(TokenId::SymLeftCurly))
    {
        ...
        SWAG_VERIFY(!afterDiscard, error(tokenParse, toErr(Err0385)));
    ...
    {
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdTry), error(tokenParse, formErr(Err0398, tokenParse.cstr(), node->token.cstr())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdCatch), error(tokenParse, formErr(Err0398, tokenParse.cstr(), node->token.cstr())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdAssume), error(tokenParse, formErr(Err0398, tokenParse.cstr(), node->token.cstr())));
        SWAG_VERIFY(tokenParse.isNot(TokenId::KwdThrow), error(tokenParse, formErr(Err0398, tokenParse.cstr(), node->token.cstr())));
        SWAG_CHECK(checkIsIdentifier(tokenParse, formErr(Err0158, node->token.cstr())));
...
bool Parser::doTypeAlias(AstNode* parent, AstNode** result)
{
    ...
    *result = node;
    ...
    SWAG_CHECK(checkIsIdentifier(tokenParse, formErr(Err0654, node->token.cstr())));
