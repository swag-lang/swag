...
bool Parser::doFuncCallArguments(AstNode* parent, AstFuncCallParams** result, TokenId closeToken)
{
    ...
    *result         = callParams;
    ...
    if (closeToken == TokenId::SymRightParen && tokenParse.is(TokenId::SymVertical))
    {
        ...
        {
            SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0162)));
            ...
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymVertical), error(tokenParse, toErr(Err0104)));
        }
        ...
        while (true)
        {
            ...
            if (tokenParse.is(TokenId::SymColon))
            {
                if (paramExpression->isNot(AstNodeKind::IdentifierRef) || paramExpression->childCount() != 1)
                    return context->report({paramExpression, toErr(Err0271)});
            ...
            if (tokenParse.is(closeToken))
                return context->report({callParams, tokenComma.token, toErr(Err0105)});
        }
    }
...
bool Parser::doFuncDeclParameterSelf(AstVarDecl* paramNode)
{
    ...
    if (tokenParse.is(TokenId::KwdConst))
    {
        ...
        {
            const Diagnostic err{sourceFile, tokenParse, toErr(Err0398)};
    ...
    SWAG_VERIFY(paramNode->ownerStructScope, error(tokenParse, toErr(Err0412)));
    SWAG_CHECK(eatToken());
    ...
    {
        SWAG_VERIFY(paramNode->ownerStructScope->is(ScopeKind::Struct), error(tokenParse, toErr(Err0412)));
        const auto typeNode = Ast::newTypeExpression(nullptr, paramNode);
    ...
    {
        const Diagnostic err(paramNode, tokenParse.token, toErr(Err0200));
        return context->report(err);
    }
...
bool Parser::doFuncDeclParameter(AstNode* parent, bool acceptMissingType, bool* hasMissingType)
{
    ...
    if (tokenParse.is(TokenId::SymDotDotDot) || tokenParse.is(TokenId::NativeType))
        return context->report({sourceFile, tokenParse.token, formErr(Err0516, tokenParse.token.c_str())});
    ...
    if (tokenParse.isNot(TokenId::KwdConst))
        SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0292)));
    ...
    else
    {
        ...
        if (tokenParse.is(TokenId::SymComma))
        {
            ...
            while (tokenParse.is(TokenId::SymComma))
            {
                ...
                SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(tokenParse, formErr(Err0108, tokenParse.token.c_str())));
                SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0292)));
                SWAG_CHECK(eatToken());
        ...
        if (tokenParse.is(TokenId::SymEqual))
        {
            if (unnamedTokens.size() == parent->childCount())
            {
                Diagnostic err{sourceFile, tokenParse, toErr(Err0176)};
        ...
        if (!hasType && !hasAssignment)
        {
            ...
            {
                Diagnostic err{sourceFile, tokenParse, toErr(Err0535)};
...
{
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly), error(tokenParse, toErr(Err0496)));
        ...
        while (tokenParse.isNot(TokenId::SymRightParen))
        {
            ...
            if (acceptMissingType)
            {
                ...
                if (oneParamDone && !*hasMissingType && missingTypes)
                    return error(allParams->lastChild(), toErr(Err0534));
            ...
            if (tokenParse.is(TokenId::SymRightParen))
                return context->report({allParams, tokenComma.token, toErr(Err0108)});
        }
    }
...
bool Parser::doGenericDeclParameters(AstNode* parent, AstNode** result)
{
    ...
    *result              = allParams;
    ...
    const auto startLoc = tokenParse.token.startLocation;
    eatToken();
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(tokenParse, toErr(Err0500)));
    ...
    while (tokenParse.isNot(TokenId::SymRightParen))
    {
        ...
        SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0247)));
        ...
        if (tokenParse.is(TokenId::SymColon))
        {
            ...
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly), error(tokenParse, toErr(Err0642)));
        ...
        if (isConstant && !oneParam->type && !oneParam->assignment)
            return error(tokenParse, toErr(Err0533));

        if (tokenParse.isNot(TokenId::SymComma))
            break;
        SWAG_CHECK(eatToken());
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(tokenParse, toErr(Err0108)));
    }
    ...
    *result               = funcNode;
    ...
    if (isMethod)
    {
        ...
        if (!funcNode->ownerStructScope)
            return error(tokenParse, toErr(Err0409));
        if (funcNode->ownerStructScope->is(ScopeKind::Enum))
            return error(tokenParse, toErr(Err0410));
    ...
    else
    {
        ...
        if (tokenParse.is(TokenId::KwdConst))
        {
            SWAG_VERIFY(isMethod, error(tokenParse, toErr(Err0399)));
            isConstMethod = true;
            SWAG_CHECK(eatToken());
        }
        ...
        if (isIntrinsic)
            SWAG_VERIFY(sourceFile->acceptsInternalStuff(), error(tokenParse, formErr(Err0234, tokenParse.token.c_str())));
        else
            SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0235)));
    ...
    else if (funcNode->hasAttribute(ATTRIBUTE_MESSAGE_FUNC))
    {
        ...
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(funcNode, toErr(Err0470)));
    ...
    else if (tokenParse.is(TokenId::SymLeftParen))
    {
        ...
        if (funcNode->hasAttribute(ATTRIBUTE_MAIN_FUNC))
            note = toNte(Nte0201);
        return error(tokenParse, formErr(Err0687, funcNode->getDisplayNameC()), note.c_str());
    }
    ...
    if (!funcForCompiler)
    {
        if (tokenParse.is(TokenId::SymMinusGreat))
        {
            ...
            ParserPushFct   scopedFct(this, funcNode);
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdRetVal), error(tokenParse, toErr(Err0661)));
            AstNode* typeExpression;
    ...
    {
        SWAG_VERIFY(!funcForCompiler, error(tokenParse, formErr(Err0068, funcNode->getDisplayNameC())));
    ...
    // Content of function
    {
        ...
        {
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdReturn), error(tokenParse, formErr(Err0660)));
...
bool Parser::doFuncDeclBody(AstNode* node, AstNode** result, FuncDeclFlags flags)
{
    if (tokenParse.isNot(TokenId::SymLeftCurly))
    {
        ...
        {
            const Diagnostic err{sourceFile, tokenParse, formErr(Err0072, node->token.c_str())};
            return context->report(err);
        }

        const Diagnostic err{sourceFile, tokenParse, toErr(Err0073)};
        return context->report(err);
    }
...
bool Parser::doClosureCaptureBlock(TypeInfoFuncAttr* typeInfo, AstFuncCallParams* capture)
{
    ...
    else
    {
        ...
        while (tokenParse.isNot(TokenId::SymVertical))
        {
            ...
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymVertical), error(tokenParse, toErr(Err0480)));
        }
    ...
    SWAG_VERIFY(tokenParse.is(TokenId::SymLeftParen), error(tokenParse, toErr(Err0476)));
...
bool Parser::doLambdaExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    if (!lambda->ownerFct && lambdaDecl->captureParameters)
        return error(lambdaDecl, toErr(Err0440));
