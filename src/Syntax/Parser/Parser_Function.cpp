...
bool Parser::doFuncCallArguments(AstNode* parent, AstFuncCallParams** result, TokenId closeToken)
{
    ...
    *result               = callParams;
    ...
    if (closeToken == TokenId::SymRightParen && tokenParse.is(TokenId::SymVertical))
    {
        ...
        {
            SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0608)));
            callParams->aliasNames.push_back(tokenParse.token);
            SWAG_CHECK(eatToken());
            if (tokenParse.is(TokenId::SymVertical))
                break;
            SWAG_CHECK(eatTokenError(TokenId::SymComma, toErr(Err0093)));
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymVertical), error(tokenParse, toErr(Err0094)));
        }
        ...
        while (true)
        {
            ...
            if (tokenParse.is(TokenId::SymColon))
            {
                if (paramExpression->isNot(AstNodeKind::IdentifierRef) || paramExpression->childCount() != 1)
                    return context->report({paramExpression, toErr(Err0610)});
            ...
            if (forAttrUse && tokenParse.is(TokenId::SymRightSquare))
                return error(tokenParse, toErr(Err0594));

            {
                ...
                if (callParams->hasSpecFlag(AstFuncCallParams::SPEC_FLAG_CALL_FOR_STRUCT) || closeToken == TokenId::SymRightCurly)
                    ...
                    SWAG_CHECK(eatTokenError(TokenId::SymComma, toErr(Err0099)));
                else
                    SWAG_CHECK(eatTokenError(TokenId::SymComma, toErr(Err0095)));
            }
...
bool Parser::doFuncDeclParameterSelf(AstVarDecl* paramNode)
{
    ...
    if (tokenParse.is(TokenId::KwdConst))
    {
        ...
        {
            const Diagnostic err{sourceFile, tokenParse, toErr(Err0313)};
    ...
    SWAG_VERIFY(paramNode->ownerStructScope, error(tokenParse, toErr(Err0335)));
    SWAG_CHECK(eatToken());
    ...
    {
        const Diagnostic err(paramNode, tokenParse.token, toErr(Err0376));
        return context->report(err);
    }

    if (tokenParse.is(TokenId::SymColon))
    {
        const Diagnostic err(paramNode, tokenParse.token, toErr(Err0651));
        return context->report(err);
    }
...
bool Parser::doFuncDeclParameter(AstNode* parent, bool acceptMissingType, bool* hasMissingType)
{
    ...
    if (tokenParse.is(TokenId::SymDotDotDot) || tokenParse.is(TokenId::NativeType))
        return context->report({sourceFile, tokenParse.token, formErr(Err0454, tokenParse.cstr())});
    ...
    if (tokenParse.isNot(TokenId::KwdConst))
        SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0639)));
    ...
    else
    {
        ...
        if (tokenParse.is(TokenId::SymColon))
        {
            ...
            {
                Diagnostic err{sourceFile, tokenParse, toErr(Err0652)};
        ...
        if (tokenParse.is(TokenId::SymEqual))
        {
            if (unnamedTokens.size() == parent->childCount())
            {
                Diagnostic err{sourceFile, tokenParse, toErr(Err0351)};
        ...
        if (!hasType && !hasAssignment)
        {
            ...
            {
                Diagnostic err{sourceFile, tokenParse, toErr(Err0455)};
...
{
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly), error(tokenParse, formErr(Err0438, parent->token.cstr())));
        ...
        while (tokenParse.isNot(TokenId::SymRightParen))
        {
            ...
            if (acceptMissingType)
            {
                ...
                if (oneParamDone && !*hasMissingType && missingTypes)
                    return error(allParams->lastChild(), toErr(Err0450));
...
bool Parser::doGenericDeclParameters(AstNode* parent, AstNode** result)
{
    ...
    *result              = allParams;
    ...
    const auto startLoc = tokenParse.token.startLocation;
    eatToken();
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(tokenParse, toErr(Err0442)));
    ...
    while (tokenParse.isNot(TokenId::SymRightParen))
    {
        ...
        if (tokenParse.is(TokenId::SymColon))
        {
            if (isType)
            {
                Diagnostic err{sourceFile, tokenParse, toErr(Err0192)};
            ...
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly), error(tokenParse, toErr(Err0581)));
        ...
        if (isConstant && !oneParam->type && !oneParam->assignment)
            return error(tokenParse, toErr(Err0432));
    ...
    *result               = funcNode;
    ...
    if (isMethod)
    {
        ...
        if (!funcNode->ownerStructScope)
            return error(tokenParse, toErr(Err0328));
        if (funcNode->ownerStructScope->is(ScopeKind::Enum))
            return error(tokenParse, toErr(Err0330));
    ...
    else
    {
        ...
        if (tokenParse.is(TokenId::KwdConst))
        {
            SWAG_VERIFY(isMethod, error(tokenParse, toErr(Err0314)));
            isConstMethod = true;
            SWAG_CHECK(eatToken());
        }
    ...
    else if (tokenParse.is(TokenId::SymLeftParen))
    {
        ...
        if (funcNode->hasAttribute(ATTRIBUTE_MAIN_FUNC))
            note = toNte(Nte0063);
        return error(tokenParse, formErr(Err0640, funcNode->getDisplayNameC()), note.cstr());
    }
    ...
    {
        SWAG_VERIFY(!funcForCompiler, error(tokenParse, formErr(Err0073, funcNode->getDisplayNameC())));
    ...
    // Content of function
    {
        ...
        {
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdReturn), error(tokenParse, formErr(Err0331)));
...
bool Parser::doFuncDeclBody(AstNode* node, AstNode** result, FuncDeclFlags flags)
{
    if (tokenParse.isNot(TokenId::SymLeftCurly))
    {
        ...
        {
            const Diagnostic err{sourceFile, tokenParse, formErr(Err0071, node->token.cstr())};
            return context->report(err);
        }
        ...
        if (Tokenizer::isSymbol(tokenParse.token.id))
        {
            ...
            {
                const Diagnostic err{sourceFile, tokenParse, toErr(Err0644)};
                return context->report(err);
            }
        }
        ...
        {
            const Diagnostic err{sourceFile, tokenParse, toErr(Err0643)};
            return context->report(err);
        }

        const Diagnostic err{sourceFile, tokenParse, toErr(Err0072)};
        return context->report(err);
    }
...
bool Parser::doClosureCaptureBlock(TypeInfoFuncAttr* typeInfo, AstFuncCallParams* capture)
{
    ...
    else
    {
        ...
        while (tokenParse.isNot(TokenId::SymVertical))
        {
            ...
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymVertical), error(tokenParse, toErr(Err0419)));
        }
    ...
    SWAG_VERIFY(tokenParse.is(TokenId::SymLeftParen), error(tokenParse, toErr(Err0415)));
...
bool Parser::doLambdaExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    if (!lambda->ownerFct && lambdaDecl->captureParameters)
        return error(lambdaDecl, toErr(Err0372));
