...
bool Parser::doAttrDecl(AstNode* parent, AstNode** result)
{
    ...
    *result             = attrNode;
    ...
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymMinusGreat), error(tokenParse, toErr(Err0644), toNte(Nte0019)));
    SWAG_VERIFY(tokenParse.isNot(TokenId::KwdThrow), error(tokenParse, toErr(Err0339), toNte(Nte0019)));
