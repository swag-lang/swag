...
bool Parser::doLiteral(AstNode* parent, AstNode** result)
{
    ...
    *result            = node;
    ...
    if (tokenParse.is(TokenId::SymQuote))
    {
        ...
        {
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.is(TokenId::Identifier) || tokenParse.is(TokenId::NativeType), error(tokenParse, toErr(Err0443)));
        ...
        else
        {
            switch (node->token.id)
            {
                ...
                case TokenId::KwdFalse:
                    return error(tokenParse, formErr(Err0750, "on [[bool]] literal"));
                case TokenId::LiteralString:
                    return error(tokenParse, formErr(Err0750, "on [[string]] literal"));
                case TokenId::KwdNull:
                    return error(tokenParse, formErr(Err0750, "on [[null]]"));
                default:
                    return error(tokenParse, formErr(Err0750, "for that kind of literal"));
            }
        }
    }
...
bool Parser::doArrayPointerIndex(AstNode** exprNode)
{
    ...
    if (tokenParse.is(TokenId::KwdTo) || tokenParse.is(TokenId::KwdUntil))
    {
        ...
        else
        {
            ...
            {
                const Diagnostic err{sourceFile, tokenParse, toErr(Err0488)};
...
bool Parser::doIntrinsicProp(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    SWAG_CHECK(eatTokenError(TokenId::SymLeftParen, formErr(Err0348, node->token.cstr())));
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(tokenParse, formErr(Err0347, node->token.cstr())));
...
bool Parser::doParenthesisExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    const auto startLoc = tokenParse.token.startLocation;
    SWAG_CHECK(eatToken());
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(startLoc, tokenParse.token.endLocation, toErr(Err0426)));
...
bool Parser::doPrimaryExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    {
        SWAG_VERIFY(exprFlags.has(EXPR_FLAG_IN_CALL), error(tokenParse, toErr(Err0324)));
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::SymTilde:
        {
            ...
            *result           = node;
            ...
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.token.isNot(prevTokenParse.token.id), error(tokenParse, formErr(Err0254, tokenParse.cstr())));
...
bool Parser::doModifiers(const Token& forNode, TokenId tokenId, ModifierFlags& mdfFlags, AstNode* /*node*/)
{
    ...
    while (Tokenizer::isModifier(tokenParse.token.id))
    {
        if (tokenParse.is(TokenId::ModifierBit))
        {
            ...
            {
                case TokenId::KwdCast:
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_WRAP), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_bit.cstr(), g_LangSpec->name_sharp_wrap.cstr())));
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_UN_CONST), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_bit.cstr(), g_LangSpec->name_sharp_unconst.cstr())));
                    break;
                default:
                    return error(tokenParse, formErr(Err0721, tokenParse.cstr(), forNode.cstr()));
            }

            SWAG_VERIFY(!mdfFlags.has(MODIFIER_BIT), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierUnConst))
        {
            ...
            {
                case TokenId::KwdCast:
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_WRAP), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_unconst.cstr(), g_LangSpec->name_sharp_wrap.cstr())));
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_BIT), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_unconst.cstr(), g_LangSpec->name_sharp_bit.cstr())));
                    break;
                default:
                    return error(tokenParse, formErr(Err0721, tokenParse.cstr(), forNode.cstr()));
            }

            SWAG_VERIFY(!mdfFlags.has(MODIFIER_UN_CONST), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierErr))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_ERR), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierNoErr))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_NO_ERR), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierPromote))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_PROM), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierWrap))
        {
            ...
            {
                case TokenId::KwdCast:
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_BIT), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_wrap.cstr(), g_LangSpec->name_sharp_bit.cstr())));
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_UN_CONST), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_wrap.cstr(), g_LangSpec->name_sharp_unconst.cstr())));
                    break;
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_WRAP), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierNoDrop))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_NO_LEFT_DROP), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierRef))
        {
            ...
            {
                case TokenId::SymEqual:
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_CONST_REF), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_ref.cstr(), g_LangSpec->name_sharp_constref.cstr())));
                    break;
                default:
                    return error(tokenParse, formErr(Err0721, tokenParse.cstr(), forNode.cstr()));
            }

            SWAG_VERIFY(!mdfFlags.has(MODIFIER_REF), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierConstRef))
        {
            ...
            {
                case TokenId::SymEqual:
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_REF), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_constref.cstr(), g_LangSpec->name_sharp_ref.cstr())));
                    break;
                default:
                    return error(tokenParse, formErr(Err0721, tokenParse.cstr(), forNode.cstr()));
            }

            SWAG_VERIFY(!mdfFlags.has(MODIFIER_CONST_REF), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierReverse))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_REVERSE), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierMove))
        {
            ...
            {
                case TokenId::SymEqual:
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE_RAW), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_move.cstr(), g_LangSpec->name_sharp_moveraw.cstr())));
                    break;
                default:
                    return error(tokenParse, formErr(Err0721, tokenParse.cstr(), forNode.cstr()));
            }

            SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierMoveRaw))
        {
            ...
            {
                case TokenId::SymEqual:
                    SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE), error(tokenParse, formErr(Err0039, g_LangSpec->name_sharp_moveraw.cstr(), g_LangSpec->name_sharp_move.cstr())));
                    break;
                default:
                    return error(tokenParse, formErr(Err0721, tokenParse.cstr(), forNode.cstr()));
            }

            SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE_RAW), error(tokenParse, formErr(Err0240, tokenParse.cstr())));
...
bool Parser::doCompareExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    if (!exprFlags.has(EXPR_FLAG_NAMED_PARAM) || leftNode->isNot(AstNodeKind::IdentifierRef))
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymEqual), error(tokenParse, toErr(Err0529)));
...
bool Parser::doExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::CompilerFuncTest:
            return error(tokenParse, formErr(Err0535, tokenParse.cstr()));
...
bool Parser::doExpressionListArray(AstNode* parent, AstNode** result)
{
    ...
    *result             = node;
    ...
    if (tokenParse.is(TokenId::SymRightSquare))
        return error(tokenParse, toErr(Err0417));
    ...
    switch (tokenParse.token.id)
    {
        case TokenId::SymLeftParen:
        {
            ...
            *result             = multi;
            ...
            while (true)
            {
                ...
                {
                    SWAG_VERIFY(tokenParse.is(TokenId::Identifier) || tokenParse.is(TokenId::SymQuestion), error(tokenParse, toErr(Err0753)));
                ...
                if (tokenParse.is(TokenId::SymRightParen))
                    break;
                SWAG_CHECK(eatTokenError(TokenId::SymComma, toErr(Err0351)));
                SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(tokenParse, toErr(Err0492)));
            }
        ...
        case TokenId::CompilerUp:
        {
            ...
            while (true)
            {
                ...
                SWAG_CHECK(eatToken());
                SWAG_VERIFY(tokenParse.isNot(TokenId::SymEqual) && tokenParse.isNot(TokenId::SymSemiColon), error(tokenParse, toErr(Err0492)));
                SWAG_VERIFY(tokenParse.is(TokenId::Identifier) || tokenParse.is(TokenId::SymDot), invalidIdentifierError(tokenParse, toErr(Err0493)));
        ...
        default:
            return invalidIdentifierError(tokenParse, toErr(Err0494));
    }
