...
bool Parser::doLiteral(AstNode* parent, AstNode** result)
{
    ...
    *result            = node;
    ...
    if (tokenParse.is(TokenId::SymQuote))
    {
        ...
        {
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.is(TokenId::Identifier) || tokenParse.is(TokenId::NativeType), error(tokenParse, toErr(Err0268)));
        ...
        else
        {
            switch (node->token.id)
            {
                ...
                case TokenId::KwdFalse:
                    return error(tokenParse, formErr(Err0706, "[[bool]] literals"));
                case TokenId::LiteralString:
                    return error(tokenParse, formErr(Err0706, "[[string]] literals"));
                case TokenId::KwdNull:
                    return error(tokenParse, formErr(Err0706, "[[null]]"));
                default:
                    return error(tokenParse, formErr(Err0706, "that kind of literal"));
            }
        }
    }
...
bool Parser::doArrayPointerIndex(AstNode** exprNode)
{
    ...
    if (tokenParse.is(TokenId::SymDotDot) || tokenParse.is(TokenId::SymDotDotLess))
    {
        ...
        else
        {
            ...
            {
                const Diagnostic err{sourceFile, tokenParse, toErr(Err0504)};
        ...
        while (true)
        {
            ...
            *exprNode = arrayNode;
            if (tokenParse.is(TokenId::SymRightSquare))
                break;
            SWAG_CHECK(eatTokenError(TokenId::SymComma, toErr(Err0127)));
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightSquare), error(tokenParse, toErr(Err0128)));
        }
...
bool Parser::doIntrinsicProp(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(tokenParse, formErr(Err0488, node->token.cstr())));
...
bool Parser::doParenthesisExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    const auto startLoc = tokenParse.token.startLocation;
    SWAG_CHECK(eatToken());
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(startLoc, tokenParse.token.endLocation, toErr(Err0068)));
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::SymDot:
        {
            ...
            if (tokenParse.isNot(TokenId::Identifier))
                return error(tokenParse, formErr(Err0220, ".").cstr());
...
bool Parser::doPrimaryExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    {
        SWAG_VERIFY(exprFlags.has(EXPR_FLAG_IN_CALL), error(tokenParse, toErr(Err0364)));
...
bool Parser::doModifiers(const Token& forNode, TokenId tokenId, ModifierFlags& mdfFlags, AstNode* /*node*/)
{
    ...
    while (Tokenizer::isModifier(tokenParse.token.id))
    {
        if (tokenParse.is(TokenId::ModifierPromote))
        {
            switch (opId)
            {
                ...
                case TokenId::SymPercent:
                    break;
                default:
                    return error(tokenParse, formErr(Err0688, tokenParse.cstr(), forNode.cstr()));
            }

            SWAG_VERIFY(!mdfFlags.has(MODIFIER_PROM), error(tokenParse, formErr(Err0058, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierOver))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_OVERFLOW), error(tokenParse, formErr(Err0058, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierNoDrop))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_NO_LEFT_DROP), error(tokenParse, formErr(Err0058, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierRef))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_REF), error(tokenParse, formErr(Err0058, tokenParse.cstr())));
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_CONST_REF), error(tokenParse, formErr(Err0053, "#ref", "#constref")));
        ...
        if (tokenParse.is(TokenId::ModifierConstRef))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_CONST_REF), error(tokenParse, formErr(Err0058, tokenParse.cstr())));
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_REF), error(tokenParse, formErr(Err0053, "#constref", "#ref")));
        ...
        if (tokenParse.is(TokenId::ModifierBack))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_BACK), error(tokenParse, formErr(Err0058, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierMove))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE), error(tokenParse, formErr(Err0058, tokenParse.cstr())));
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE_RAW), error(tokenParse, formErr(Err0053, "#move", "#moveraw")));
        ...
        if (tokenParse.is(TokenId::ModifierMoveRaw))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE_RAW), error(tokenParse, formErr(Err0058, tokenParse.cstr())));
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE), error(tokenParse, formErr(Err0053, "#moveraw", "#move")));
...
bool Parser::doCompareExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    if (!exprFlags.has(EXPR_FLAG_NAMED_PARAM) || leftNode->isNot(AstNodeKind::IdentifierRef))
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymEqual), error(tokenParse, toErr(Err0658)));
...
bool Parser::doExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::CompilerFuncTest:
            return error(tokenParse, formErr(Err0202, tokenParse.cstr()));
...
bool Parser::doExpressionListArray(AstNode* parent, AstNode** result)
{
    ...
    *result             = node;
    ...
    if (tokenParse.is(TokenId::SymRightSquare))
        return error(tokenParse, toErr(Err0065));
    ...
    while (tokenParse.isNot(TokenId::SymRightSquare))
    {
        ...
        SWAG_CHECK(eatTokenError(TokenId::SymComma, toErr(Err0129)));
    ...
    switch (tokenParse.token.id)
    {
        case TokenId::SymLeftParen:
        {
            ...
            *result             = multi;
            ...
            while (true)
            {
                SWAG_VERIFY(tokenParse.is(TokenId::Identifier) || tokenParse.is(TokenId::SymQuestion), error(tokenParse, toErr(Err0701)));
                ...
                if (tokenParse.is(TokenId::SymRightParen))
                    break;
                SWAG_CHECK(eatTokenError(TokenId::SymComma, toErr(Err0133)));
                SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightParen), error(tokenParse, toErr(Err0134)));
            }
        ...
        case TokenId::CompilerSelf:
        {
            ...
            while (true)
            {
                if (tokenParse.is(TokenId::SymDot))
                {
                    SWAG_VERIFY(withNode, error(tokenParse, toErr(Err0429)));
                    prependWith = true;
                    eatToken();
                }
                ...
                SWAG_CHECK(eatToken());
                SWAG_VERIFY(tokenParse.isNot(TokenId::SymEqual) && tokenParse.isNot(TokenId::SymSemiColon), error(tokenParse, toErr(Err0135)));
                SWAG_VERIFY(tokenParse.is(TokenId::Identifier) || tokenParse.is(TokenId::SymDot), invalidIdentifierError(tokenParse, toErr(Err0296)));
        ...
        default:
            return invalidIdentifierError(tokenParse, toErr(Err0295));
    }
