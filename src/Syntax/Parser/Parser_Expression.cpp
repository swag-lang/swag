...
bool Parser::doModifiers(const Token& forNode, TokenId tokenId, ModifierFlags& mdfFlags, AstNode* /*node*/)
{
    ...
    {
        if (tokenParse.is(TokenId::ModifierCall))
        {
            switch (opId)
            {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_CALL), error(tokenParse, formErr(Err0041, tokenParse.cstr())));
        ...
        if (tokenParse.is(TokenId::ModifierErr))
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_ERR), error(tokenParse, formErr(Err0041, tokenParse.cstr())));
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_NO_ERR), error(tokenParse, formErr(Err0036, "#err", "#noerr")));
