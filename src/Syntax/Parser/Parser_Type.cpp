...
bool Parser::doLambdaClosureParameters(AstTypeLambda* node, bool inTypeVarDecl, AstNode* params)
{
    ...
    while (true)
    {
        ...
        if (!forceType && tokenParse.is(TokenId::Identifier))
        {
            ...
            {
                Diagnostic err(sourceFile, tokenParse.token, toErr(Err0679));
    ...
    *result         = node;
    ...
    if (tokenParse.is(TokenId::SymLeftSquare))
    {
        ...
        if (tokenParse.flags.has(TOKEN_PARSE_EOL_BEFORE))
        {
            ...
            {
                if (inTypeVarDecl)
                    return context->report({sourceFile, rightSquareToken.token, toErr(Err0667)});
                const Diagnostic err{sourceFile, rightSquareToken.token, toErr(Err0667)};
                return context->report(err);
            }
            return error(rightSquareToken.token, toErr(Err0667));
        }
