...
bool Parser::doLambdaClosureParameters(AstTypeLambda* node, bool inTypeVarDecl, AstNode* params)
{
    ...
    while (true)
    {
        ...
        if (!forceType && tokenParse.is(TokenId::Identifier))
        {
            ...
            {
                Diagnostic err(sourceFile, tokenParse.token, toErr(Err0708));
                return context->report(err);
            }
            else
            {
                SWAG_VERIFY(inTypeVarDecl, error(tokenName.token, toErr(Err0696)));
                SWAG_VERIFY(!isConst, error(constToken, toErr(Err0665)));
        ...
        {
            curIsAlone = false;
            SWAG_VERIFY(currentStructScope, error(tokenParse.token, toErr(Err0472)));
            ...
            if (tokenParse.is(TokenId::SymEqual))
                return error(tokenParse.token, toErr(Err0253));
        }
        // ...
        else if (tokenParse.is(TokenId::SymDotDotDot))
        {
            ...
            SWAG_CHECK(eatToken());
            if (tokenParse.is(TokenId::SymEqual))
                return error(tokenParse.token, toErr(Err0683));
        }
        // cvarargs
        ...
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymEqual) || inTypeVarDecl, error(tokenParse.token, toErr(Err0254)));
...
bool Parser::doSingleTypeExpression(AstTypeExpression* node, ExprFlags exprFlags)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::KwdMethod:
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdMethod), context->report({sourceFile, tokenParse.token, toErr(Err0671)}));
    ...
    if (tokenParse.is(TokenId::SymLeftSquare))
    {
        ...
        while (true)
        {
            ...
            if (node->arrayDim == 254)
                return error(tokenParse.token, toErr(Err0609));
            node->arrayDim++;
            SWAG_CHECK(doExpression(node, EXPR_FLAG_NONE, &dummyResult));
            if (tokenParse.isNot(TokenId::SymComma))
                break;
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymRightSquare), error(tokenParse.token, toErr(Err0119)));
        }
        ...
        if (tokenParse.flags.has(TOKEN_PARSE_EOL_BEFORE))
        {
            ...
            {
                if (inTypeVarDecl)
                    return context->report({sourceFile, rightSquareToken.token, toErr(Err0698)});
                const Diagnostic err{sourceFile, rightSquareToken.token, toErr(Err0698)};
                return context->report(err);
            }
            return error(rightSquareToken.token, toErr(Err0698));
        }
