    ...
    while (true)
    {
        ...
        if (!forceType && tokenParse.is(TokenId::Identifier))
        {
            ...
            else
            {
                ...
                SWAG_VERIFY(!isConst, error(constToken, toErr(Err0659)));
        ...
        {
            curIsAlone = false;
            SWAG_VERIFY(currentStructScope, error(tokenParse, toErr(Err0465)));
            ...
            if (tokenParse.is(TokenId::SymEqual))
                return error(tokenParse, toErr(Err0250));
        }
        // ...
        ...
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymEqual) || inTypeVarDecl, error(tokenParse, toErr(Err0251)));
...
bool Parser::doSubTypeExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    *result         = node;
    ...
    if (tokenParse.is(TokenId::KwdConst))
    {
        ...
        {
            const Diagnostic err{sourceFile, tokenParse, toErr(Err0246)};
            return context->report(err);
        }
    }
    ...
    if (tokenParse.is(TokenId::SymLeftSquare))
    {
        ...
        if (tokenParse.flags.has(TOKEN_PARSE_EOL_BEFORE))
        {
            ...
            {
                if (inTypeVarDecl)
                    return context->report({sourceFile, rightSquareToken.token, toErr(Err0691)});
                const Diagnostic err{sourceFile, rightSquareToken.token, toErr(Err0691)};
                return context->report(err);
            }
            return error(rightSquareToken.token, toErr(Err0691));
        }

        if (tokenParse.is(TokenId::SymComma))
        {
            const Diagnostic err{sourceFile, tokenParse, toErr(Err0395)};
            return context->report(err);
        }
