...
bool Parser::doLambdaClosureParameters(AstTypeLambda* node, bool inTypeVarDecl, AstNode* params)
{
    ...
    while (true)
    {
        ...
        if (tokenParse.is(TokenId::CompilerType))
        {
            ...
            curIsAlone  = false;
            SWAG_CHECK(eatToken());
            SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0476)));
        }
        ...
        if (!forceType && tokenParse.is(TokenId::Identifier))
        {
            ...
            {
                Diagnostic err(sourceFile, tokenParse.token, toErr(Err0687));
                return context->report(err);
            }
            else
            {
                SWAG_VERIFY(inTypeVarDecl, error(tokenName.token, toErr(Err0671)));
                SWAG_VERIFY(!isConst, error(constToken, toErr(Err0317)));
        ...
        {
            curIsAlone = false;
            SWAG_VERIFY(currentStructScope, error(tokenParse, toErr(Err0340)));
            ...
            if (tokenParse.is(TokenId::SymEqual))
                return error(tokenParse, toErr(Err0379));
        }
        // ...
        else if (tokenParse.is(TokenId::SymDotDotDot))
        {
            ...
            SWAG_CHECK(eatToken());
            if (tokenParse.is(TokenId::SymEqual))
                return error(tokenParse, toErr(Err0653));
        }
        // cvarargs
        ...
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymEqual) || inTypeVarDecl, error(tokenParse, toErr(Err0380)));
        ...
        if (inTypeVarDecl)
        {
            ...
            if (lastWasAlone && !curIsAlone && !thisIsAType)
            {
                ...
                auto       note = Diagnostic::note(lastParameter, formNte(Nte0001, lastParameter->type->token.c_str()));
                note->hint      = formNte(Nte0048, lastParameter->type->token.c_str(), lastParameter->type->token.c_str());
                err.addNote(note);
                return context->report(err);
            }
...
bool Parser::doSingleTypeExpression(AstTypeExpression* node, ExprFlags exprFlags)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::KwdMethod:
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdMethod), context->report({sourceFile, tokenParse.token, toErr(Err0333)}));
    ...
    if (tokenParse.is(TokenId::SymLeftParen))
        ...
        err.addNote(formNte(Nte0132, tokenParse.token.c_str()));
...
bool Parser::doSubTypeExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    *result         = node;
    ...
    if (tokenParse.is(TokenId::KwdConst))
    {
        ...
        {
            const Diagnostic err{sourceFile, tokenParse, toErr(Err0153)};
            return context->report(err);
        }
    }
    ...
    if (tokenParse.is(TokenId::SymLeftSquare))
    {
        ...
        while (true)
        {
            ...
            if (node->arrayDim == 254)
                return error(tokenParse, toErr(Err0495));
            node->arrayDim++;
        ...
        if (tokenParse.flags.has(TOKEN_PARSE_EOL_BEFORE))
        {
            ...
            {
                if (inTypeVarDecl)
                    return context->report({sourceFile, rightSquareToken.token, toErr(Err0672)});
                const Diagnostic err{sourceFile, rightSquareToken.token, toErr(Err0672)};
                return context->report(err);
            }
            return error(rightSquareToken.token, toErr(Err0672));
        }
