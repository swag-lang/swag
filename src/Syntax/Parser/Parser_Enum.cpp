...
bool Parser::doEnum(AstNode* parent, AstNode** result)
{
    ...
    *result                                   = enumNode;
    ...
    SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0254)));
    ...
    Scope* newScope = nullptr;
    {
        ...
        if (newScope->isNot(ScopeKind::Enum))
        {
            if (newScope->owner->is(AstNodeKind::Impl))
            {
                ...
                Diagnostic err{implNode, formErr(Err0004, Naming::aKindName(newScope->kind).c_str(), implNode->token.c_str(), Naming::aKindName(ScopeKind::Enum).c_str())};
            ...
            Diagnostic err{enumNode->token.sourceFile, tokenParse.token, formErr(Err0005, "symbol", "enum", enumNode->token.c_str(), asA.c_str())};
...
{
    SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0226)));
