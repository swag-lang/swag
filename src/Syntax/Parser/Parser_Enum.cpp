...
bool Parser::doEnum(AstNode* parent, AstNode** result)
{
    ...
    *result                                   = enumNode;
    ...
    SWAG_CHECK(eatToken());
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymColon), error(tokenParse, toErr(Err0546)));
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly), error(tokenParse, toErr(Err0545)));
    SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0263)));
    ...
    Scope* newScope = nullptr;
    {
        ...
        if (newScope->isNot(ScopeKind::Enum))
        {
            if (newScope->owner->is(AstNodeKind::Impl))
            {
                ...
                Diagnostic err{implNode, formErr(Err0008, Naming::aKindName(newScope->kind).c_str(), implNode->token.c_str(), Naming::aKindName(ScopeKind::Enum).c_str())};
            ...
            Diagnostic err{enumNode->token.sourceFile, tokenParse.token, formErr(Err0623, "enum", enumNode->token.c_str(), asA.c_str())};
