...
bool Parser::doEnum(AstNode* parent, AstNode** result)
{
    ...
    *result                                   = enumNode;
    ...
    SWAG_CHECK(eatToken());
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymColon), error(tokenParse, toErr(Err0515)));
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly), error(tokenParse, toErr(Err0514)));
    ...
    Scope* newScope = nullptr;
    {
        ...
        if (newScope->isNot(ScopeKind::Enum))
        {
            ...
            Diagnostic err{enumNode->token.sourceFile, tokenParse.token, formErr(Err0023, "enum", enumNode->token.c_str(), asA.c_str())};
...
{
    SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0205)));
