...
bool Parser::eatToken(TokenId id, const char* msg)
{
    ...
    {
        const Diagnostic err{sourceFile, tokenParse, formErr(Err0071, Naming::tokenToName(id).c_str(), Naming::tokenToName(id).c_str(), msg)};
...
bool Parser::eatCloseToken(TokenId id, const SourceLocation& start, const char* msg, AstNode* parent)
{
    ...
    if (tokenParse.is(TokenId::EndOfFile))
        errMsg = formErr(Err0465, Naming::tokenToName(id).c_str(), Naming::tokenToName(id).c_str(), msg);
    else
        errMsg = formErr(Err0466, Naming::tokenToName(id).c_str(), Naming::tokenToName(id).c_str(), msg);
    errMsg.replace(" $$$", "");
    ...
    if (tokenParse.isNot(TokenId::SymSemiColon) && tokenParse.isNot(TokenId::EndOfFile) && !tokenParse.flags.has(TOKEN_PARSE_EOL_BEFORE))
    {
        if (tokenParse.is(TokenId::SymAsterisk))
        {
            ...
            if (tokenParse.is(TokenId::SymSlash))
            {
                ...
                return error(tokenParse, formErr(Err0673, msg));
            }

            tokenParse = st;
        }

        return error(tokenParse, formErr(Err0490, msg));
    }
