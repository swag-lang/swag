...
bool Parser::eatToken(TokenId id, const char* msg)
{
    ...
    {
        const Diagnostic err{sourceFile, tokenParse, formErr(Err0074, Naming::tokenToName(id).c_str(), Naming::tokenToName(id).c_str(), msg)};
...
bool Parser::eatCloseToken(TokenId id, const SourceLocation& start, const char* msg, const AstNode* parent)
{
    ...
    if (tokenParse.is(TokenId::EndOfFile))
        errMsg = formErr(Err0475, Naming::tokenToName(id).c_str(), Naming::tokenToName(id).c_str(), msg);
    else
        errMsg = formErr(Err0476, Naming::tokenToName(id).c_str(), Naming::tokenToName(id).c_str(), msg);
    errMsg.replace(" $$$", "");
    ...
    if (tokenParse.isNot(TokenId::SymSemiColon) && tokenParse.isNot(TokenId::EndOfFile) && !tokenParse.flags.has(TOKEN_PARSE_EOL_BEFORE))
    {
        if (tokenParse.is(TokenId::SymAsterisk))
        {
            ...
            if (tokenParse.is(TokenId::SymSlash))
            {
                ...
                return error(tokenParse, formErr(Err0364, msg));
            }

            tokenParse = st;
        }

        return error(tokenParse, formErr(Err0496, msg));
    }
