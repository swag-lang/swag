...
bool Parser::doImpl(AstNode* parent, AstNode** result)
{
    ...
    *result               = implNode;
    ...
    SWAG_VERIFY(module->acceptsCompileImpl, context->report({implNode, toErr(Err0186)}));
    ...
    if (tokenParse.is(TokenId::KwdFor))
    {
        ...
        {
            Diagnostic err{implNode, tokenParse.token, toErr(Err0338)};
        ...
        SWAG_VERIFY(!last->genericParameters, context->report({last->genericParameters, toErr(Err0671)}));
    }
    else
    {
    ...
    {
        Diagnostic err{implNode, formErr(Err0298, Naming::aKindName(scopeKind).cstr(), implNode->token.cstr(), Naming::aKindName(newScope->kind).cstr())};
...
bool Parser::doStructContent(AstStruct* structNode, SyntaxStructType structType)
{
    ...
    SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0236)));
    ...
    Scope* newScope = nullptr;
    {
        ...
        if (newScope->isNot(ScopeKind::Struct))
        {
            if (newScope->owner->is(AstNodeKind::Impl))
            {
                ...
                Diagnostic err{implNode, formErr(Err0298, Naming::aKindName(newScope->kind).cstr(), implNode->token.cstr(), Naming::aKindName(ScopeKind::Struct).cstr())};
            ...
            Diagnostic err{structNode->token.sourceFile, tokenParse.token, formErr(Err0522, "symbol", "struct", structNode->token.cstr(), asA.cstr())};
    ...
    if (tokenParse.is(TokenId::KwdWhere))
    {
        ...
        SWAG_VERIFY(structNode->whereExpression->isNot(AstNodeKind::WhereCallConstraint), error(structNode->whereExpression->token, toErr(Err0360)));
    }
...
bool Parser::doInterfaceMtdDecl(AstNode* parent, AstNode** result)
{
    ...
    SWAG_VERIFY(!funcNode->genericParameters, error(funcNode->genericParameters, toErr(Err0676)));
...
bool Parser::doStructBody(AstNode* parent, SyntaxStructType structType, AstNode** result)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::KwdUsing:
        {
            ParserPushAstNodeFlags scopedFlags(this, AST_STRUCT_MEMBER);
            auto                   savedToken = tokenParse;
            SWAG_VERIFY(structType != SyntaxStructType::Interface, context->report({parent, tokenParse.token, toErr(Err0358)}));
            SWAG_CHECK(eatToken());
        ...
        case TokenId::KwdVar:
        {
            const Diagnostic err{parent, tokenParse.token, toErr(Err0359)};
            return context->report(err);
        }

        case TokenId::KwdMethod:
        case TokenId::KwdFunc:
            SWAG_VERIFY(structType == SyntaxStructType::Interface, error(tokenParse, toErr(Err0401)));
        ...
        default:
        {
            SWAG_VERIFY(structType != SyntaxStructType::Interface, error(tokenParse, toErr(Err0183)));
