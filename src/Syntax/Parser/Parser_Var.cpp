...
bool Parser::checkIsValidVarName(AstNode* node) const
{
    ...
    if (node->is(AstNodeKind::Identifier))
    {
        ...
        if (identifier->genericParameters)
            return error(identifier->genericParameters, formErr(Err0404, identifier->token.c_str()));
        if (identifier->callParameters)
            return error(identifier->callParameters, formErr(Err0403, identifier->token.c_str()));
    }
    ...
    if (node->token.text.length() >= 4)
    {
        ...
        if (node->token.text.find(g_LangSpec->name_sharpmix) == 0)
        {
            if (node->token.is(g_LangSpec->name_sharpmix))
                return error(node->token, toErr(Err0514));
            ...
            if (num >= 10)
                return error(node->token, formErr(Err0595, num));
            if (node->ownerFct)
        ...
        if (node->token.text.find(g_LangSpec->name_sharpalias) == 0)
        {
            if (node->token.is(g_LangSpec->name_sharpalias))
                return error(node->token, toErr(Err0504));
            ...
            if (num >= 32)
                return error(node->token, formErr(Err0594, num));
            if (node->ownerFct)
    ...
    return error(node->token, formErr(Err0399, node->token.c_str()));
}
...
{
    SWAG_VERIFY(acceptDeref, error(leftNode, formErr(Err0500, Naming::aKindName(currentScope->kind).c_str())));
...
bool Parser::doVarDecl(AstNode* parent, AstNode** result, AstNodeKind kind, bool forStruct, bool forLet)
{
    ...
    while (true)
    {
        ...
        if (tokenParse.isNot(TokenId::SymColon) && tokenParse.isNot(TokenId::SymEqual))
        {
            ...
            {
                if (kind == AstNodeKind::ConstDecl)
                    err.addNote(Diagnostic::hereIs(leftNode, form(toNte(Nte0066), "constant", leftNode->token.c_str())));
                else
                    err.addNote(Diagnostic::hereIs(leftNode, form(toNte(Nte0066), "variable", leftNode->token.c_str())));
            }
            else
            {
                err.addNote(Diagnostic::hereIs(leftNode, form(toNte(Nte0063), "declaration")));
            }
        ...
        if (tokenParse.is(TokenId::SymColon))
        {
                ...
                type->is(AstNodeKind::TypeExpression))
            {
                ...
                if (typeExpr->identifier)
                {
                    ...
                    err.addNote(formNte(Nte0197, typeExpr->identifier->token.c_str(), typeExpr->identifier->token.c_str()));
                    err.addNote(toNte(Nte0192));
                    return context->report(err);
                }
            }
        }
