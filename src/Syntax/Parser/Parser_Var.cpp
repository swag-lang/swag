...
bool Parser::checkIsValidVarName(AstNode* node) const
{
    ...
    if (node->is(AstNodeKind::Identifier))
    {
        ...
        if (identifier->genericParameters)
            return error(identifier->genericParameters, formErr(Err0672, identifier->token.cstr()));
        if (identifier->callParameters)
            return error(identifier->callParameters, formErr(Err0182, identifier->token.cstr(), "a variable name"));
    }

    if (node->token.text == g_LangSpec->name_self)
        return error(node->token, toErr(Err0266));
    ...
    if (node->token.text.length() >= 4)
    {
        ...
        if (node->token.text.find(g_LangSpec->name_sharp_mix) == 0)
        {
            ...
            {
                if (!SWAG_IS_DIGIT(*pz))
                    return error(node->token, formErr(Err0117, node->token.cstr() + 4));
                num *= 10;
                num += *pz - '0';
                pz++;
            }
            ...
            if (num >= 10)
                return error(node->token, formErr(Err0504, num));
            if (node->ownerFct)
        ...
        if (node->token.text.find(g_LangSpec->name_sharp_alias) == 0)
        {
            ...
            {
                if (!SWAG_IS_DIGIT(*pz))
                    return error(node->token, formErr(Err0116, node->token.cstr() + 6));
                num *= 10;
                num += *pz - '0';
                pz++;
            }
            ...
            if (num >= 32)
                return error(node->token, formErr(Err0503, num));
            if (node->ownerFct)
    ...
    return error(node->token, formErr(Err0267, node->token.cstr()));
}
...
{
    SWAG_VERIFY(acceptDeref, error(leftNode, formErr(Err0411, Naming::aKindName(currentScope->kind).cstr())));
...
bool Parser::doVarDecl(AstNode* parent, AstNode** result)
{
    ...
    if (tokenParse.is(TokenId::KwdConst))
    {
        ...
        {
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdVar), error(tokenParse, formErr(Err0321, "var", "const", "var")));
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdLet), error(tokenParse, formErr(Err0321, "let", "const", "let")));
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdConst), error(tokenParse, formErr(Err0322, "const", "const", "const")));
    ...
    else if (tokenParse.is(TokenId::KwdLet))
    {
        ...
        if (tokenParse.isNot(TokenId::SymLeftParen))
        {
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdConst), error(tokenParse, formErr(Err0321, "const", "let", "const")));
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdVar), error(tokenParse, formErr(Err0321, "var", "let", "var")));
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdLet), error(tokenParse, formErr(Err0322, "let", "let", "let")));
    ...
    else
    {
        ...
        if (tokenParse.isNot(TokenId::SymLeftParen))
        {
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdConst), error(tokenParse, formErr(Err0321, "const", "var", "const")));
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdLet), error(tokenParse, formErr(Err0321, "let", "var", "let")));
            SWAG_VERIFY(tokenParse.isNot(TokenId::KwdVar), error(tokenParse, formErr(Err0322, "var", "var", "var")));
...
bool Parser::doVarDecl(AstNode* parent, AstNode** result, AstNodeKind kind, bool forStruct, bool forLet)
{
    ...
    if (!forStruct || tokenParse.isNot(TokenId::SymRightCurly))
    {
        ...
        {
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymEqualEqual), error(tokenParse, toErr(Err0662)));
