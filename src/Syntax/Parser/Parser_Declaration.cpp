...
bool Parser::doCheckPublicInternalPrivate(const Token& tokenAttr) const
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::SymAttrStart:
            return error(tokenParse, formErr(Err0422, tokenAttr.c_str(), tokenAttr.c_str()));
        ...
        default:
        {
            Diagnostic err{sourceFile, tokenAttr, formErr(Err0421, tokenAttr.c_str(), tokenParse.token.c_str())};
...
bool Parser::doPublicInternal(AstNode* parent, AstNode** result, bool forGlobal)
{
    ...
    SWAG_VERIFY(currentScope->isGlobalOrImpl(), error(tokenParse, formErr(Err0420, tokenParse.token.c_str())));
    SWAG_VERIFY(!sourceFile->hasFlag(FILE_FORCE_EXPORT), error(tokenParse, formErr(Err0576, tokenParse.token.c_str())));
...
bool Parser::doUsing(AstNode* parent, AstNode** result, bool isGlobal)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::KwdVar:
        {
            ...
            {
                const Diagnostic err{varNode->token.sourceFile, varNode->firstChild()->token.startLocation, varNode->lastChild()->token.endLocation, toErr(Err0236)};
                return context->report(err);
            }
...
bool Parser::doNamespaceOnName(AstNode* parent, AstNode** result, bool forGlobal, bool forUsing, const Token* privName)
{
    ...
    while (true)
    {
        ...
        switch (tokenParse.token.id)
        {
            ...
            case TokenId::SymLeftCurly:
                if (!forPrivate)
                    return error(tokenParse, toErr(Err0518));
                break;
            case TokenId::SymSemiColon:
                return error(tokenParse, toErr(Err0517));
            default:
                if (!forPrivate)
                    return error(tokenParse, toErr(Err0260));
                break;
        }

        // Be sure this is not the swag namespace, except for a runtime file
        if (!sourceFile->acceptsInternalStuff())
            SWAG_VERIFY(!namespaceNode->token.text.compareNoCase(g_LangSpec->name_Swag), error(tokenParse, formErr(Err0579, tokenParse.token.c_str())));
    ...
    else
    {
        ...
        {
            return error(tokenParse, toErr(Err0268));
        }
        else
        {
            ParserPushScope scoped(this, newScope);
...
bool Parser::doCompilerScopeBreakable(AstNode* parent, AstNode** result)
{
    ...
    *result                = labelNode;
    ...
    if (tokenParse.isNot(TokenId::SymLeftCurly))
    {
        ...
        SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0225)));
...
bool Parser::doEmbeddedInstruction(AstNode* parent, AstNode** result)
{
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::KwdInternal:
        case TokenId::KwdPrivate:
            return error(tokenParse, formErr(Err0420, tokenParse.token.c_str()));
        ...
        case TokenId::SymDot:
        {
            ...
            eatToken();
            SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0294)));
