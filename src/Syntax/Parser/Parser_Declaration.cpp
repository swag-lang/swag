...
bool Parser::doCheckPublicInternalPrivate(const Token& tokenAttr) const
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::SymAttrStart:
            return error(tokenParse, formErr(Err0485, tokenAttr.c_str(), tokenAttr.c_str()));
        ...
        default:
        {
            Diagnostic err{sourceFile, tokenAttr, formErr(Err0466, tokenAttr.c_str(), tokenParse.token.c_str())};
...
bool Parser::doPublicInternal(AstNode* parent, AstNode** result, bool forGlobal)
{
    ...
    SWAG_VERIFY(currentScope->isGlobalOrImpl(), error(tokenParse, formErr(Err0467, tokenParse.token.c_str())));
    SWAG_VERIFY(!sourceFile->hasFlag(FILE_FORCE_EXPORT), error(tokenParse, formErr(Err0613, tokenParse.token.c_str())));
...
bool Parser::doUsing(AstNode* parent, AstNode** result, bool isGlobal)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::KwdVar:
        {
            ...
            {
                Diagnostic err{varNode->token.sourceFile, varNode->firstChild()->token.startLocation, varNode->lastChild()->token.endLocation, toErr(Err0394)};
...
bool Parser::doNamespaceOnName(AstNode* parent, AstNode** result, bool forGlobal, bool forUsing, const Token* privName)
{
    ...
    while (true)
    {
        ...
        switch (tokenParse.token.id)
        {
            ...
            case TokenId::SymLeftCurly:
                if (!forPrivate)
                    return error(tokenParse, formErr(Err0564, "{"));
                break;
        ...
        if (!sourceFile->acceptsInternalStuff())
            SWAG_VERIFY(!namespaceNode->token.text.compareNoCase(g_LangSpec->name_Swag), error(tokenParse, formErr(Err0616, tokenParse.token.c_str())));
...
bool Parser::doScopedStatement(AstNode* parent, const Token& forToken, AstNode** result, bool mustHaveDo)
{
    ...
    *result                   = statement;
    ...
    if (mustHaveDo)
    {
        ...
        if (tokenParse.is(TokenId::SymLeftCurly))
        {
            const Diagnostic err{sourceFile, tokenDo.token, toErr(Err0448)};
            return context->report(err);
        }
    }
...
bool Parser::doEmbeddedInstruction(AstNode* parent, AstNode** result)
{
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::KwdInternal:
        case TokenId::KwdPrivate:
            return error(tokenParse, formErr(Err0467, tokenParse.token.c_str()));
        ...
        case TokenId::SymDot:
        {
            ...
            eatToken();
            SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0352)));
