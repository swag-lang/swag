...
bool Parser::doIf(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    if (tokenParse.is(TokenId::KwdVar) || tokenParse.is(TokenId::KwdConst) || tokenParse.is(TokenId::KwdLet))
    {
        ...
        SWAG_VERIFY(varDecl->childCount() == 1, error(varDecl->lastChild()->token, toErr(Err0235)));
...
bool Parser::doSwitch(AstNode* parent, AstNode** result)
{
    ...
    *result                 = switchNode;
    ...
    {
        SWAG_VERIFY(tokenParse.is(TokenId::KwdCase) || tokenParse.is(TokenId::KwdDefault), error(tokenParse, toErr(Err0135)));
        ...
        if (!isDefault)
        {
            ...
            while (tokenParse.isNot(TokenId::SymColon))
            {
                ...
                if (tokenParse.isNot(TokenId::SymComma))
                    break;
                SWAG_CHECK(eatToken());
                SWAG_VERIFY(tokenParse.isNot(TokenId::SymColon), error(tokenParse, toErr(Err0104)));
            }
        }
...
bool Parser::doWith(AstNode* parent, AstNode** result)
{
    ...
    *result         = node;
    ...
    if (tokenParse.is(TokenId::KwdVar) || tokenParse.is(TokenId::KwdLet))
    {
        ...
        {
            const Diagnostic err{id->token.sourceFile, id->firstChild()->token.startLocation, id->lastChild()->token.endLocation, toErr(Err0237)};
    ...
    else
    {
        ...
        {
            const Diagnostic err{id, formErr(Err0139, Naming::aKindName(id).c_str())};
            return context->report(err);
        }
...
bool Parser::doDefer(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    if (tokenParse.is(TokenId::SymColon))
    {
        ...
        {
            return error(tokenParse, formErr(Err0127, tokenParse.token.c_str()));
        }
    }
...
bool Parser::doBreak(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    {
        SWAG_CHECK(checkIsIdentifier(tokenParse, toErr(Err0125)));
