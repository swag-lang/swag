...
bool Parser::doIf(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    SWAG_CHECK(eatToken());
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly) && tokenParse.isNot(TokenId::SymSemiColon), error(tokenParse, toErr(Err0528)));
    ...
    if (tokenParse.is(TokenId::KwdVar) || tokenParse.is(TokenId::KwdConst) || tokenParse.is(TokenId::KwdLet))
    {
        ...
        SWAG_VERIFY(varDecl->childCount() == 1, error(varDecl->lastChild()->token, toErr(Err0399)));
...
bool Parser::doWhile(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
...

    {
        ...
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly) && tokenParse.isNot(TokenId::SymSemiColon), error(tokenParse, formErr(Err0533, tokenParse.token.c_str())));
        SWAG_CHECK(doExpression(node, EXPR_FLAG_NONE, &node->boolExpression));
...
bool Parser::doSwitch(AstNode* parent, AstNode** result)
{
    ...
    *result                 = switchNode;
    ...
    while (tokenParse.isNot(TokenId::SymRightCurly) && tokenParse.isNot(TokenId::EndOfFile))
    {
        ...
        if (!isDefault)
        {
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymColon), error(tokenParse, formErr(Err0525, tokenParse.token.c_str())));
...
bool Parser::doVisit(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    // Variable to visit
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly) && tokenParse.isNot(TokenId::SymSemiColon), error(tokenParse, formErr(Err0531, tokenParse.token.c_str())));
    ...
    if (tokenParse.is(TokenId::SymColon) || tokenParse.is(TokenId::SymComma))
    {
        ...
        while (tokenParse.isNot(TokenId::SymColon))
        {
            ...
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymColon), error(prevTokenParse.token, toErr(Err0532)));
        ...
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly), error(tokenParse, toErr(Err0531)));
    ...
    *result                                = node;
    ...
    else
    {
        ...
        if (tokenParse.is(TokenId::SymColon))
        {
            ...
            node->expression->release();
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly) && tokenParse.isNot(TokenId::SymSemiColon), error(tokenParse, formErr(Err0530, tokenParse.token.c_str())));
            SWAG_CHECK(doExpression(node, EXPR_FLAG_SIMPLE, &node->expression));
        }
        else
        {
            ...
            {
                return error(tokenParse, formErr(Err0524, node->expression->lastChild()->token.c_str()));
            }
        }
    }
...
bool Parser::doWith(AstNode* parent, AstNode** result)
{
    ...
    *result         = node;
    ...
    if (tokenParse.is(TokenId::KwdVar) || tokenParse.is(TokenId::KwdLet))
    {
        ...
        {
            Diagnostic err{id->token.sourceFile, id->firstChild()->token.startLocation, id->lastChild()->token.endLocation, toErr(Err0401)};
