...
bool Parser::doCompilerWhere(AstNode* parent, AstNode** result)
{
    ...
    *result         = node;
    ...
    if (tokenParse.is(TokenId::SymColon))
    {
        ...
        {
            return error(tokenParse, formErr(Err0139, tokenParse.token.c_str()));
        }
    }
    ...
    {
        return error(node, formErr(Err0667, parent->token.c_str()));
    }
...
bool Parser::doCompilerGlobal(AstNode* parent, AstNode** result)
{
    ...
    {
        return context->report({sourceFile, tokenParse.token, formErr(Err0116, tokenParse.token.c_str())});
    }
        ...
        sourceFile->acceptsInternalStuff())
    {
        while (true)
        {
            ...
            if (tokenParse.token.is(g_LangSpec->name_version))
            {
                ...
                SWAG_VERIFY(tokenParse.is(TokenId::LiteralString), error(tokenParse, toErr(Err0118)));
