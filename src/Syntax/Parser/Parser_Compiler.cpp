...
bool Parser::doCompilerIfFor(AstNode* parent, AstNode** result, AstNodeKind kind)
{
    ...
    *result         = node;
    ...
    // Expression
    {
        ...
        SWAG_CHECK(eatToken());
        SWAG_VERIFY(tokenParse.isNot(TokenId::SymLeftCurly) && tokenParse.isNot(TokenId::SymSemiColon), error(tokenParse, toErr(Err0466)));
...
bool Parser::doCompilerIfStatementFor(AstNode* parent, AstNode** result, AstNodeKind kind, bool forIf)
{
    if (tokenParse.is(TokenId::CompilerDo))
    {
        ...
        {
            const Diagnostic err{sourceFile, tokenDo, toErr(Err0374)};
...
{
    SWAG_VERIFY(tokenParse.isNot(TokenId::SymSemiColon), error(tokenParse, toErr(Err0210), toNte(Nte0209)));
...
bool Parser::doCompilerMixin(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    {
        const auto startLoc = tokenParse.token.startLocation;
        SWAG_VERIFY(node->hasOwnerBreakable(), error(tokenParse, toErr(Err0385)));
        SWAG_CHECK(eatToken());
...
bool Parser::doCompilerForeignLib(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    SWAG_CHECK(eatToken());
    SWAG_VERIFY(tokenParse.is(TokenId::LiteralString), error(tokenParse, toErr(Err0463)));
...
{
    SWAG_VERIFY(!afterGlobal, error(tokenParse, toErr(Err0378)));
    auto savedToken = tokenParse;
    SWAG_CHECK(eatToken());
    ...
    else if (tokenParse.token.is(g_LangSpec->name_testpass))
    {
        ...
        {
            context->report({sourceFile, tokenParse.token, formErr(Err0293, tokenParse.token.c_str())});
            return false;
        }
    ...
    else if (tokenParse.token.is(g_LangSpec->name_testerror) || tokenParse.token.is(g_LangSpec->name_testwarning))
    {
        ...
        {
            SWAG_VERIFY(sourceFile->module->is(ModuleKind::Test) || sourceFile->hasFlag(FILE_FOR_FORMAT), context->report({sourceFile, tokenParse.token, toErr(Err0375)}));
            SWAG_CHECK(eatToken());
    ...
    else if (tokenParse.is(TokenId::KwdUsing))
    {
        ...
        {
            const Diagnostic err{sourceFile, tokenParse, toErr(Err0377)};
            return context->report(err);
        }
...
bool Parser::doCompilerDependencies(AstNode* parent)
{
    ...
    {
        const Diagnostic err{sourceFile, tokenParse, toErr(Err0372)};
        return context->report(err);
    }

    SWAG_VERIFY(parent->is(AstNodeKind::File), context->report({sourceFile, tokenParse.token, toErr(Err0373)}));
...
{
    SWAG_VERIFY(sourceFile->hasFlag(FILE_CFG) || sourceFile->hasFlag(FILE_SCRIPT), context->report({sourceFile, tokenParse.token, toErr(Err0382)}));
    ...
    SWAG_VERIFY(scan, context->report({sourceFile, tokenParse.token, toErr(Err0383)}));
...
bool Parser::doCompilerImport(AstNode* parent)
{
    ...
    if (!sourceFile->hasFlag(FILE_GENERATED))
    {
        ...
        SWAG_VERIFY(scan, context->report({sourceFile, tokenParse.token, toErr(Err0380)}));
    }

    const auto node = Ast::newNode<AstCompilerImport>(AstNodeKind::CompilerImport, this, parent);
    SWAG_CHECK(eatToken());
    SWAG_VERIFY(tokenParse.is(TokenId::LiteralString), context->report({sourceFile, tokenParse, toErr(Err0467)}));
    node->inheritTokenName(tokenParse.token);
    node->inheritTokenLocation(tokenParse.token);
    SWAG_CHECK(eatToken());
...
{
    SWAG_VERIFY(currentScope->isGlobalOrImpl(), context->report({sourceFile, tokenParse.token, toErr(Err0386)}));

    const auto node = Ast::newNode<AstNode>(AstNodeKind::CompilerPlaceHolder, this, parent);
    SWAG_CHECK(eatToken());
    SWAG_VERIFY(tokenParse.is(TokenId::Identifier), error(tokenParse, toErr(Err0472)));
    node->inheritTokenName(tokenParse.token);
    node->inheritTokenLocation(tokenParse.token);
    SWAG_CHECK(eatToken());
    SWAG_CHECK(eatFormat(node));
