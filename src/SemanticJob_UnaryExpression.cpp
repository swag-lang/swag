    ...
    if (child->flags & AST_VALUE_COMPUTED && !(child->doneFlags & AST_DONE_NEG_EATEN))
    {
        ...
        case NativeTypeKind::S8:
            if (child->computedValue->reg.s8 <= INT8_MIN)
                return context->report({child, Fmt(g_E[Err0829], child->computedValue->reg.s8, -child->computedValue->reg.s8)});
            child->computedValue->reg.s64 = -child->computedValue->reg.s8;
            break;
        case NativeTypeKind::S16:
            if (child->computedValue->reg.s16 <= INT16_MIN)
                return context->report({child, Fmt(g_E[Err0830], child->computedValue->reg.s16, -child->computedValue->reg.s16)});
            child->computedValue->reg.s64 = -child->computedValue->reg.s16;
            break;
        case NativeTypeKind::S32:
            if (child->computedValue->reg.s32 <= INT32_MIN)
                return context->report({child, Fmt(g_E[Err0831], child->computedValue->reg.s32, -child->computedValue->reg.s32)});
        ...
        case NativeTypeKind::Int:
            if (child->computedValue->reg.s64 <= INT64_MIN)
                return context->report({child, Fmt(g_E[Err0832], child->computedValue->reg.s64, -child->computedValue->reg.s64)});
