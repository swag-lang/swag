    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        if (it == longNameArgs.end())
        {
            ...
            if (it == shortNameArgs.end())
            {
                ...
                {
                    Report::error(FMT(Err(Fat0007), command.c_str()));
                    result = false;
                }
                continue;
            }
...
Utf8 CommandLineParser::buildString() const
{
    ...
    ranges::sort(list, [](const CommandLineArgument* a, const CommandLineArgument* b) { return strcmp(a->longName.c_str(), b->longName.c_str()) < 0; });
