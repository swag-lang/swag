...
void CommandLineParser::setup(CommandLine* cmdLine)
{
    ...
    addArg("bu cl", "--cfg-debug", nullptr, CommandLineType::EnumString, &cmdLine->buildCfgDebug, "true|false|default", "force the build configuration to have (or not) debug informations");
    addArg("bu cl", "--cfg-safety", nullptr, CommandLineType::EnumString, &cmdLine->buildCfgSafety, "true|false|default", "force the build configuration to have (or not) safety guards");
    ...
    addArg("bu cl", "--cfg-optim-speed", nullptr, CommandLineType::EnumString, &cmdLine->buildCfgOptimSpeed, "true|false|default", "force the build configuration to be optimized (or not) for speed");
    addArg("bu cl", "--cfg-optim-size", nullptr, CommandLineType::EnumString, &cmdLine->buildCfgOptimSize, "true|false|default", "force the build configuration to be optimized (or not) for size");
    addArg("bu cl", "--cfg-stack-trace", nullptr, CommandLineType::EnumString, &cmdLine->buildCfgStackTrace, "true|false|default", "force the build configuration to have (or not) call trace for errors");
...
bool CommandLineParser::process(const string& swagCmd, int argc, const char* argv[])
{
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        if (it == longNameArgs.end())
        {
            ...
            {
                g_Log.error(Utf8::format(g_E[Msg0720], command.c_str()));
                result = false;
                continue;
            }
        }
        ...
        case CommandLineType::EnumInt:
        {
            ...
            {
                g_Log.error(Utf8::format(g_E[Msg0722], it->first.c_str(), arg->param));
                result = false;
                continue;
            }
        }
        ...
        case CommandLineType::Bool:
            ...
            {
                g_Log.error(Utf8::format(g_E[Msg0724], it->first.c_str(), argument.c_str()));
                result = false;
                continue;
            }
            break;
        ...
        case CommandLineType::String:
        {
            if (argument.empty())
            {
                g_Log.error(Utf8::format(g_E[Msg0725], it->first.c_str(), argument.c_str()));
                result = false;
                continue;
            }
        ...
        case CommandLineType::Int:
        {
            ...
            {
                if (argument.empty())
                    g_Log.error(Utf8::format(g_E[Msg0727], it->first.c_str()));
                else
                    g_Log.error(Utf8::format(g_E[Msg0728], it->first.c_str(), argument.c_str()));
                result = false;
                continue;
            }
