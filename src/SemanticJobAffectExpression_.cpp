...
bool SemanticJob::checkRelativePointerOverflow(SemanticContext* context, TypeInfo* typeInfo, AstNode* right)
{
    ...
    {
    case 1:
        SWAG_VERIFY(value.reg.s64 >= INT8_MIN && value.reg.s64 <= INT8_MAX, context->report({right, format("relative pointer arithmetic out of range ('%d')", right->computedValue.reg.s64)}));
        break;
    case 2:
        SWAG_VERIFY(value.reg.s64 >= INT16_MIN && value.reg.s64 <= INT16_MAX, context->report({right, format("relative pointer arithmetic out of range ('%d')", right->computedValue.reg.s64)}));
        break;
    case 4:
        SWAG_VERIFY(value.reg.s64 >= INT32_MIN && value.reg.s64 <= INT32_MAX, context->report({right, format("relative pointer arithmetic out of range ('%d')", right->computedValue.reg.s64)}));
        break;
    case 8:
        SWAG_VERIFY(value.reg.s64 >= INT64_MIN && value.reg.s64 <= INT64_MAX, context->report({right, format("relative pointer arithmetic out of range ('%d')", right->computedValue.reg.s64)}));
        break;
    }
...
bool SemanticJob::resolveAffect(SemanticContext* context)
{
    ...
    case TokenId::SymEqual:
        if (leftTypeInfo->kind != TypeInfoKind::Native &&
            ...
            return context->report({left, format("affect not allowed on %s '%s'", TypeInfo::getNakedKindName(leftTypeInfo), leftTypeInfo->getDisplayName().c_str())});
        if (rightTypeInfo->kind != TypeInfoKind::Native &&
            ...
            return context->report({right, format("affect not allowed, '%s' is %s", rightTypeInfo->getDisplayName().c_str(), TypeInfo::getArticleKindName(rightTypeInfo))});
