...
bool SyntaxJob::doIf(AstNode* parent, AstNode** result)
{
    ...
    SWAG_CHECK(eatToken());
    SWAG_CHECK(verifyError(node->token, token.id != TokenId::SymLeftCurly, g_E[Msg0863]));
...
bool SyntaxJob::doWhile(AstNode* parent, AstNode** result)
{
...

    {
        ...
        SWAG_CHECK(verifyError(node->token, token.id != TokenId::SymLeftCurly, g_E[Msg0864]));
        SWAG_CHECK(doExpression(node, EXPR_FLAG_NONE, &node->boolExpression));
...
bool SyntaxJob::doSwitch(AstNode* parent, AstNode** result)
{
    ...
    {
        SWAG_CHECK(verifyError(token, token.id == TokenId::KwdCase || token.id == TokenId::KwdDefault, g_E[Msg0865]));
        bool isDefault = token.id == TokenId::KwdDefault;
        SWAG_CHECK(verifyError(token, !isDefault || !hasDefault, g_E[Msg0866]));
        if (isDefault)
            hasDefault = true;
        ...
        if (!isDefault)
        {
            SWAG_CHECK(verifyError(token, token.id != TokenId::SymColon, g_E[Msg0867]));
            SWAG_CHECK(verifyError(token, token.id != TokenId::KwdBreak, g_E[Msg0868]));
        ...
        // Content
        {
            ...
            if (token.id == TokenId::KwdCase || token.id == TokenId::KwdDefault)
                return error(previousToken, g_E[Msg0869]);
            if (token.id == TokenId::SymRightCurly)
                return error(previousToken, g_E[Msg0823]);
...
bool SyntaxJob::doVisit(AstNode* parent, AstNode** result)
{
    ...
    {
        SWAG_CHECK(eatToken());
        SWAG_CHECK(verifyError(token, token.id == TokenId::Identifier, g_E[Msg0870]));
    ...
    // Variable to visit
    SWAG_CHECK(verifyError(node->token, token.id != TokenId::SymLeftCurly, g_E[Msg0871]));
    {
    ...
    if (token.id == TokenId::SymColon || token.id == TokenId::SymComma)
    {
        ...
        while (token.id != TokenId::SymColon)
        {
...

            {
                ...
                SWAG_CHECK(verifyError(prevToken, token.id != TokenId::SymColon, g_E[Msg0872]));
            }
        ...
        {
            SWAG_CHECK(verifyError(token, token.id != TokenId::SymLeftCurly, g_E[Msg0871]));
    ...
    if (token.id != TokenId::SymLeftCurly)
    {
        ...
        else
        {
            ...
            if (token.id == TokenId::SymColon)
            {
                ...
                {
                    SWAG_CHECK(verifyError(token, token.id != TokenId::SymLeftCurly, g_E[Msg0874]));
...
bool SyntaxJob::doBreak(AstNode* parent, AstNode** result)
{
    ...
    {
        SWAG_CHECK(verifyError(token, token.id == TokenId::Identifier, g_E[Msg0876]));
...
bool SyntaxJob::doContinue(AstNode* parent, AstNode** result)
{
    ...
    if (token.id != TokenId::SymSemiColon)
    {
        SWAG_CHECK(verifyError(token, token.id == TokenId::Identifier, g_E[Msg0877]));
        node->label = move(token.text);
        SWAG_CHECK(eatToken());
    }
