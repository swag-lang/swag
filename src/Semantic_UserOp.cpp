...
bool Semantic::checkFuncPrototypeOp(SemanticContext* context, AstFuncDecl* node)
{
    ...
    {
        Diagnostic err{node, node->getTokenName(), FMT(Err(Err0427), node->token.c_str())};
    ...
    if (!node->ownerScope->owner->hasAttribute(ATTRIBUTE_PUBLIC) && node->hasAttribute(ATTRIBUTE_PUBLIC))
    {
        Diagnostic err{node, node->getTokenName(), FMT(Err(Err0428), node->token.c_str())};
        ...
        name == g_LangSpec->name_opAffectLiteral)
    {
        ...
        SWAG_VERIFY(firstGen->hasSpecFlag(AstVarDecl::SPEC_FLAG_GENERIC_CONSTANT), context->report({firstGen, FMT(Err(Err0306), name.c_str(), firstGen->typeInfo->getDisplayNameC())}));
    ...
    {
        SWAG_VERIFY(node->genericParameters, context->report({node, node->getTokenName(), FMT(Err(Err0559), name.c_str())}));
        SWAG_VERIFY(node->genericParameters->children.size() >= 2, context->report({node->genericParameters, FMT(Err(Err0594), name.c_str())}));
        ...
        SWAG_VERIFY(secondGen->hasSpecFlag(AstVarDecl::SPEC_FLAG_GENERIC_CONSTANT), context->report({secondGen, FMT(Err(Err0306), name.c_str(), secondGen->typeInfo->getDisplayNameC())}));
    ...
    {
        SWAG_VERIFY(!node->genericParameters || node->genericParameters->children.size() == 1, context->report({node->genericParameters, FMT(Err(Err0632), name.c_str(), node->genericParameters->children.size())}));
    }
    ...
    else
    {
        ...
        Diagnostic err{node, node->getTokenName(), FMT(Err(Err0616), name.c_str())};
        if (appendMsg.empty())
        {
...
bool Semantic::hasUserOp(SemanticContext* context, const Utf8& name, TypeInfoStruct* leftStruct, SymbolName** result)
{
    *result = nullptr;
    ...
    if (results.size() > 1)
    {
        Diagnostic err{context->node, FMT(Err(Err0020), name.c_str())};
        err.addNote(context->node, FMT(Nte(Nte0144), name.c_str()));
        for (const auto& f : results)
            err.addNote(f.usingField->declNode, FMT(Nte(Nte0009), name.c_str(), f.parentStruct->getDisplayNameC()));
        return context->report(err);
    }
...
bool Semantic::resolveUserOpAffect(SemanticContext* context, TypeInfo* leftTypeInfo, TypeInfo* rightTypeInfo, AstNode* left, AstNode* right)
{
    ...
    if (context->node->kind == AstNodeKind::VarDecl)
    {
        ...
        if (varDecl->assignToken.token.id != TokenId::Invalid)
            varDecl->token = varDecl->assignToken.token;
    }
    ...
    if (right->hasSemFlag(SEMFLAG_LITERAL_SUFFIX))
    {
        ...
        if (!symbol)
        {
            ...
            err.addNote(right->children.front(), FMT(Nte(Nte0166), suffix.c_str()));
    ...
    else
    {
        ...
        if (!symbol)
        {
            ...
            err.addNote(context->node, context->node->token, FMT(Nte(Nte0144), g_LangSpec->name_opAffect.c_str()));
            ...
            if (!(ropFlags & ROP_SIMPLE_CAST))
            {
                ...
                if (makePtrL && toTypeRef && toTypeRef->isClosure())
                {
                    ...
                    if (convert)
                    {
                        ...
                        auto varNode = Ast::newVarDecl(FMT("__ctmp_%d", g_UniqueID.fetch_add(1)), nullptr, nodeCall);
