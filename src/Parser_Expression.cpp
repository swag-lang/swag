...
bool Parser::doLiteral(AstNode* parent, AstNode** result)
{
    ...
    *result            = node;
    ...
    if (tokenParse.token.id == TokenId::SymQuote)
    {
        ...
        {
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.token.id == TokenId::Identifier || tokenParse.token.id == TokenId::NativeType, error(tokenParse.token, form(Err(Err0404), tokenParse.token.c_str())));
        ...
        else
        {
            switch (node->token.id)
            {
                ...
                case TokenId::KwdFalse:
                    return error(tokenParse.token, form(Err(Err0703), "[[bool]] literals"));
                case TokenId::LiteralString:
                    return error(tokenParse.token, form(Err(Err0703), "[[string]] literals"));
                case TokenId::KwdNull:
                    return error(tokenParse.token, form(Err(Err0703), "[[null]]"));
                default:
                    return error(tokenParse.token, form(Err(Err0703), "that kind of literal"));
            }
        }
    }
...
bool Parser::doArrayPointerIndex(AstNode** exprNode)
{
    ...
    if (tokenParse.token.id == TokenId::SymDotDot || tokenParse.token.id == TokenId::SymDotDotLess)
    {
    ...
    if (tokenParse.token.id == TokenId::SymDotDot || tokenParse.token.id == TokenId::SymDotDotLess)
    {
        ...
        if (tokenParse.token.id == TokenId::SymDotDotLess)
        ...
        else
        {
            ...
            {
                const Diagnostic err{sourceFile, tokenParse.token, Err(Err0580)};
        ...
        while (true)
        {
            ...
            *exprNode = arrayNode;
            if (tokenParse.token.id != TokenId::SymComma)
                break;
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.token.id != TokenId::SymRightSquare, error(tokenParse.token, Err(Err0128)));
        }
...
bool Parser::doIntrinsicProp(AstNode* parent, AstNode** result)
{
    ...
    *result           = node;
    ...
    SWAG_VERIFY(tokenParse.token.id != TokenId::SymRightParen, error(tokenParse.token, Err(Err0568)));
...
bool Parser::doSinglePrimaryExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::SymLeftParen:
        {
            const auto startLoc = tokenParse.token.startLocation;
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.token.id != TokenId::SymRightParen, error(startLoc, tokenParse.token.endLocation, Err(Err0679)));
            AstNode* expr;
...
bool Parser::doPrimaryExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    // Dereference pointer
    else if (tokenParse.token.id == TokenId::KwdDeRef)
    {
        SWAG_CHECK(doDeRef(parent, &exprNode));
    }

    // Force ref pointer
    else if (tokenParse.token.id == TokenId::KwdConst)
    {
        const auto startLoc = tokenParse.token.startLocation;
        tokenizer.saveState(tokenParse);
        eatToken();
        if (tokenParse.token.id == TokenId::KwdRef)
        {
    ...
    else if (tokenParse.token.id == TokenId::KwdMoveRef)
    {
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::SymTilde:
        {
            ...
            *result           = node;
            ...
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(tokenParse.token.id != prevTokenParse.token.id, error(tokenParse.token, form(Err(Err0071), tokenParse.token.c_str())));
            SWAG_VERIFY(tokenParse.token.id != TokenId::KwdDeRef, error(tokenParse.token, form(Err(Err0282), prevTokenParse.token.c_str(), tokenParse.token.c_str(), prevTokenParse.token.c_str())));
...
bool Parser::doModifiers(const Token& forNode, TokenId tokenId, ModifierFlags& mdfFlags, AstNode* node)
{
    ...
    mdfFlags = 0;
    while (tokenParse.token.id == TokenId::SymComma && !tokenParse.flags.has(TOKEN_PARSE_LAST_BLANK) && !tokenParse.flags.has(TOKEN_PARSE_LAST_EOL))
    {
        SWAG_CHECK(eatToken());
        ...
        if (tokenParse.token.text == g_LangSpec->name_up)
        {
            switch (opId)
            {
                ...
                case TokenId::SymPercent:
                    break;
                default:
                    return error(tokenParse.token, form(Err(Err0694), tokenParse.token.c_str(), forNode.c_str()));
            }

            SWAG_VERIFY(!mdfFlags.has(MODIFIER_UP), error(tokenParse.token, form(Err(Err0070), tokenParse.token.c_str())));
        ...
        if (tokenParse.token.text == g_LangSpec->name_over)
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_OVERFLOW), error(tokenParse.token, form(Err(Err0070), tokenParse.token.c_str())));
        ...
        if (tokenParse.token.text == g_LangSpec->name_nodrop)
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_NO_LEFT_DROP), error(tokenParse.token, form(Err(Err0070), tokenParse.token.c_str())));
        ...
        if (tokenParse.token.text == g_LangSpec->name_back)
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_BACK), error(tokenParse.token, form(Err(Err0070), tokenParse.token.c_str())));
        ...
        if (tokenParse.token.text == g_LangSpec->name_bit)
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_BIT), error(tokenParse.token, form(Err(Err0070), tokenParse.token.c_str())));
        ...
        if (tokenParse.token.text == g_LangSpec->name_unconst)
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_UN_CONST), error(tokenParse.token, form(Err(Err0070), tokenParse.token.c_str())));
        ...
        if (tokenParse.token.text == g_LangSpec->name_move)
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE), error(tokenParse.token, form(Err(Err0070), tokenParse.token.c_str())));
        ...
        if (tokenParse.token.text == g_LangSpec->name_moveraw)
        {
            ...
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_NO_RIGHT_DROP), error(tokenParse.token, form(Err(Err0070), tokenParse.token.c_str())));
            SWAG_VERIFY(!mdfFlags.has(MODIFIER_MOVE), error(tokenParse.token, form(Err(Err0070), g_LangSpec->name_move.c_str())));
            mdfFlags.add(MODIFIER_MOVE | MODIFIER_NO_RIGHT_DROP);
            SWAG_CHECK(eatToken());
            continue;
        }

        return error(tokenParse.token, form(Err(Err0352), tokenParse.token.c_str()));
    }
...
bool Parser::doFactorExpression(AstNode** parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    bool isBinary = false;
    if (tokenParse.token.id == TokenId::SymPlus ||
        ...
        tokenParse.token.id == TokenId::SymCircumflex)
    {
        ...
        if (tokenParse.token.id == TokenId::SymGreaterGreater || tokenParse.token.id == TokenId::SymLowerLower)
    ...
    else if (tokenParse.token.id == TokenId::SymEqualEqual ||
...
bool Parser::doCompareExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    if (exprFlags.has(EXPR_FLAG_STOP_AFFECT) && tokenParse.token.id == TokenId::SymEqual)
    {
        Ast::addChildBack(parent, leftNode);
        *result = leftNode;
        return true;
    ...
    if (!exprFlags.has(EXPR_FLAG_NAMED_PARAM) || leftNode->kind != AstNodeKind::IdentifierRef)
        SWAG_VERIFY(tokenParse.token.id != TokenId::SymEqual, error(tokenParse.token, Err(Err0674), Nte(Nte0086)));
...
bool Parser::doBoolExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    bool isBinary = false;
    if (tokenParse.token.id == TokenId::KwdOr || tokenParse.token.id == TokenId::KwdAnd)
    {
    ...
    else if (tokenParse.token.id == TokenId::SymAmpersandAmpersand || tokenParse.token.id == TokenId::SymVerticalVertical)
    {
...
bool Parser::doExpression(AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    switch (tokenParse.token.id)
    {
        ...
        case TokenId::CompilerFuncTest:
            return error(tokenParse.token, form(Err(Err0285), tokenParse.token.c_str()));
    ...
    // A ? B : C
    if (tokenParse.token.id == TokenId::SymQuestion)
    {
        SWAG_CHECK(eatToken());
    ...
    // A orelse B
    else if (tokenParse.token.id == TokenId::KwdOrElse)
    {
...
bool Parser::doExpressionListTuple(AstNode* parent, AstNode** result)
{
    ...
    *result               = initNode;
    ...
    while (tokenParse.token.id != TokenId::SymRightCurly)
    {
        if (tokenParse.token.id == TokenId::SymLeftCurly)
            SWAG_CHECK(doExpressionListTuple(initNode, &dummyResult));
        else if (tokenParse.token.id == TokenId::SymLeftSquare)
        ...
        else
        {
            ...
            {
                SWAG_VERIFY(paramExpression->kind == AstNodeKind::IdentifierRef, error(paramExpression, form(Err(Err0310), tokenParse.token.c_str())));
...
bool Parser::doExpressionListArray(AstNode* parent, AstNode** result)
{
    ...
    *result               = initNode;
    ...
    if (tokenParse.token.id == TokenId::SymRightSquare)
        return error(tokenParse.token, form(Err(Err0077)));

    while (tokenParse.token.id != TokenId::SymRightSquare)
    {
        if (tokenParse.token.id == TokenId::SymLeftSquare)
            SWAG_CHECK(doExpressionListArray(initNode, &dummyResult));
        else if (tokenParse.token.id == TokenId::SymLeftCurly)
            SWAG_CHECK(doExpressionListTuple(initNode, &dummyResult));
        else
...
bool Parser::doInitializationExpression(const TokenParse& forToken, AstNode* parent, ExprFlags exprFlags, AstNode** result)
{
    ...
    if (tokenParse.token.id == TokenId::KwdUndefined)
    {
    ...
    switch (tokenParse.token.id)
    {
        case TokenId::SymLeftParen:
        {
            ...
            *result             = multi;
            ...
            while (true)
            {
                SWAG_VERIFY(tokenParse.token.id == TokenId::Identifier || tokenParse.token.id == TokenId::SymQuestion, error(tokenParse.token, form(Err(Err0698), tokenParse.token.c_str())));
                SWAG_CHECK(doIdentifierRef(multi, &dummyResult, identifierFlags | IDENTIFIER_ACCEPT_QUESTION));
                if (tokenParse.token.id != TokenId::SymComma)
                    break;
                SWAG_CHECK(eatToken());
                SWAG_VERIFY(tokenParse.token.id != TokenId::SymRightParen, error(tokenParse.token, Err(Err0133)));
            }
        ...
        case TokenId::CompilerSelf:
        {
            ...
            while (true)
            {
                if (tokenParse.token.id == TokenId::SymDot)
                {
                    SWAG_VERIFY(withNode, error(tokenParse.token, Err(Err0507)));
                    prePrendWith = true;
                    eatToken();
                }
                ...
                if (tokenParse.token.id != TokenId::SymComma)
                    break;
                SWAG_CHECK(eatToken());
                SWAG_VERIFY(tokenParse.token.id != TokenId::SymEqual && tokenParse.token.id != TokenId::SymSemiColon, error(tokenParse.token, Err(Err0134)));
        ...
        default:
        {
            Diagnostic err{sourceFile, tokenParse.token, form(Err(Err0408), tokenParse.token.c_str())};
...
bool Parser::doAffectExpression(AstNode* parent, AstNode** result, const AstWith* withNode)
{
        ...
        tokenParse.token.id == TokenId::SymGreaterGreaterEqual)
    {
        ...
        // Modifiers
        ModifierFlags mdfFlags = 0;
        if (savedtoken.token.id != TokenId::SymEqual)
        {
    ...
    if (tokenParse.token.id != TokenId::SymLeftCurly && tokenParse.token.id != TokenId::KwdDo)
...
bool Parser::doRange(AstNode* parent, AstNode* expression, AstNode** result)
{
    ...
    *result                = rangeNode;
    ...
    if (tokenParse.token.id == TokenId::KwdUntil)
