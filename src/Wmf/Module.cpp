...
bool Module::addDependency(AstNode* importNode, const Token& tokenLocation, const Token& tokenVersion)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        *setVer = splits[i].toInt();
        ...
        {
            case 1:
                SWAG_VERIFY(dep->verNum != UINT32_MAX, Report::report({importNode, tokenVersion, formErr(Err0102, dep->revNum)}));
                break;
            case 2:
                SWAG_VERIFY(dep->revNum != UINT32_MAX, Report::report({importNode, tokenVersion, formErr(Err0101, dep->buildNum)}));
                break;
            default:
                break;
        }
...
bool Module::compileString(const Utf8& text)
{
    ...
    {
        Report::report({ip->node, ip->node->token, toErr(Err0096)});
        return false;
    }
