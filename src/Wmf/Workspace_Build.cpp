...
Diagnostic* Workspace::errorPendingJob(Job* prevJob, const Job* depJob)
{
    ...
    if (depNode)
    {
        msg = formNte(Nte0117, Naming::kindName(prevNode).c_str(), prevNode->token.c_str(), Naming::kindName(depNode).c_str(), depNode->token.c_str());
    }
    else if (prevNode && prevJob->waitingType)
    {
        msg  = formNte(Nte0057, Naming::kindName(prevNode).c_str(), prevNode->token.c_str(), prevJob->waitingType->getDisplayNameC());
        hint = Diagnostic::isType(prevNode->typeInfo);
    }
    else if (prevJob->waitingType && dynamic_cast<TypeGenStructJob*>(prevJob))
    {
        msg = formNte(Nte0204, prevJob->waitingType->getDisplayNameC());
    }
    ...
    {
        msg  = formNte(Nte0203, Naming::kindName(prevNode).c_str(), prevNode->token.c_str());
        hint = Diagnostic::isType(prevNode->typeInfo);
    }
...
void Workspace::errorPendingJobs(const Vector<PendingJob>& pendingJobs)
{
    for (const auto& it : pendingJobs)
    {
        ...
        if (isCycle)
        {
            ...
            for (const auto depJob : cycle)
            {
                if (prevJob->nodes.size() > 1 && prevJob->originalNode->is(AstNodeKind::FuncDecl))
                {
                    ...
                    auto       msg    = formNte(Nte0117, Naming::kindName(front).c_str(), front->token.c_str(), Naming::kindName(back).c_str(), back->token.c_str());
            ...
            Diagnostic err{prevNodeLocal, prevNodeLocal->token, formErr(Err0575, Naming::kindName(prevNodeLocal).c_str(), prevNodeLocal->token.c_str())};
        ...
        if (pendingJob->waitingJobs.empty())
        {
            ...
            if (!note)
                continue;
            Diagnostic err{note->sourceFile, note->startLocation, note->endLocation, toErr(Err0730)};
            err.addNote(note);
            Report::report(err);
        }
    }
