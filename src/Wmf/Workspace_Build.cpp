...
void Workspace::errorPendingJobs(const Vector<PendingJob>& pendingJobs)
{
    for (const auto& it : pendingJobs)
    {
        ...
        if (isCycle)
        {
            ...
            Diagnostic err{prevNodeLocal, prevNodeLocal->token, formErr(Err0582, Naming::kindName(prevNodeLocal).c_str(), prevNodeLocal->token.c_str())};
        ...
        if (pendingJob->waitingJobs.empty())
        {
            ...
            if (!note)
                continue;
            Diagnostic err{note->sourceFile, note->startLocation, note->endLocation, toErr(Err0732)};
            err.addNote(note);
            Report::report(err);
        }
    }
