...
void Workspace::errorPendingJobs(const Vector<PendingJob>& pendingJobs)
{
    for (const auto& it : pendingJobs)
    {
        ...
        if (isCycle)
        {
            ...
            Diagnostic err{prevNodeLocal, prevNodeLocal->token, formErr(Err0556, Naming::kindName(prevNodeLocal).cstr(), prevNodeLocal->token.cstr())};
        ...
        if (pendingJob->waitingJobs.empty())
        {
            ...
            if (!note)
                continue;
            Diagnostic err{note->sourceFile, note->startLocation, note->endLocation, toErr(Err0767)};
            err.addNote(note);
            Report::report(err);
        }
    }
