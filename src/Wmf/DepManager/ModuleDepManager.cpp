...
bool ModuleDepManager::fetchModuleCfgLocal(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName)
{
    ...
    if (!std::filesystem::exists(remotePath, err))
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0716, SWAG_CFG_FILE, remotePath.cstr())});
    ...
    {
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0718, remotePath.cstr())});
    }
    ...
    {
        (void) fclose(fsrc);
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0712, SWAG_CFG_FILE, dep->name.cstr())});
    }
...
bool ModuleDepManager::fetchModuleCfgSwag(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName, bool fetch)
{
    ...
    if (!std::filesystem::exists(remotePath, err))
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0724, remotePath.cstr())});
    if (!fetch)
        return true;
...
bool ModuleDepManager::fetchModuleCfg(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName, bool fetch)
{
    if (dep->location.empty())
        return Report::report({dep->node, formErr(Err0770, dep->name.cstr())});
    ...
    if (tokens.size() != 2)
    {
        if (dep->isLocalToWorkspace)
            return Report::report({dep->node, toErr(Err0729)});
        return Report::report({dep->node, dep->tokenLocation, toErr(Err0729)});
    }

    // Check mode
    if (tokens[0] != g_LangSpec->name_swag && tokens[0] != g_LangSpec->name_disk)
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0766, tokens[0].cstr())});
...
bool ModuleDepManager::resolveModuleDependency(const Module* srcModule, ModuleDependency* dep)
{
    ...
    else if (cfgModule->fetchDep != dep)
    {
        ...
        switch (cmp)
        {
            ...
            {
                Diagnostic err{dep->node, formErr(Err0767, dep->name.cstr(), dep->verNum, cfgModule->fetchDep->verNum)};
...
bool ModuleDepManager::execute()
{
    ...
    for (const auto val : allModules | std::views::values)
    {
        ...
        if (cmp != CompareVersionResult::Equal)
        {
            ...
            {
                Diagnostic err{dep->node, formErr(Err0768, dep->name.cstr(), dep->version.cstr())};
                Report::report(err);
            }
            else
            {
                Diagnostic err{dep->node, formErr(Err0769, dep->name.cstr(), dep->version.cstr(), dep->resolvedLocation.cstr())};
                Report::report(err);
            }
            ok = false;
        }
        ...
        for (const auto val : allModules | std::views::values)
        {
            ...
            switch (val->fetchDep->fetchKind)
            {
                ...
                case DependencyFetchKind::Swag:
                {
                    ...
                    {
                        Report::errorOS(formErr(Err0723, pathSrc.cstr()));
                        ok = false;
                        continue;
                    }
