...
bool ModuleDepManager::fetchModuleCfgLocal(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName)
{
    ...
    if (!std::filesystem::exists(remotePath, err))
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0072, SWAG_CFG_FILE, remotePath.c_str())});
    ...
    {
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0074, remotePath.c_str())});
    }
    ...
    {
        (void) fclose(fsrc);
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0068, SWAG_CFG_FILE, dep->name.c_str())});
    }
...
bool ModuleDepManager::fetchModuleCfgSwag(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName, bool fetch)
{
    ...
    if (!std::filesystem::exists(remotePath, err))
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0080, remotePath.c_str())});
    if (!fetch)
        return true;
...
bool ModuleDepManager::fetchModuleCfg(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName, bool fetch)
{
    ...
    if (tokens.size() != 2)
    {
        if (dep->isLocalToWorkspace)
            return Report::report({dep->node, toErr(Err0182)});
        return Report::report({dep->node, dep->tokenLocation, toErr(Err0182)});
    }
        ...
        for (const auto val : allModules | std::views::values)
        {
            ...
            switch (val->fetchDep->fetchKind)
            {
                ...
                case DependencyFetchKind::Swag:
                {
                    ...
                    {
                        Report::errorOS(formErr(Err0079, pathSrc.c_str()));
                        ok = false;
                        continue;
                    }
