...
bool ModuleDepManager::fetchModuleCfgLocal(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName)
{
    ...
    if (!std::filesystem::exists(remotePath, err))
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0090, SWAG_CFG_FILE, remotePath.c_str())});
    ...
    {
        (void) fclose(fsrc);
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0086, SWAG_CFG_FILE, dep->name.c_str())});
    }
...
bool ModuleDepManager::resolveModuleDependency(const Module* srcModule, ModuleDependency* dep)
{
    ...
    else if (cfgModule->fetchDep != dep)
    {
        ...
        switch (cmp)
        {
            ...
            {
                Diagnostic err{dep->node, formErr(Err0057, dep->name.c_str(), dep->verNum, cfgModule->fetchDep->verNum)};
...
bool ModuleDepManager::execute()
{
    ...
    for (const auto val : allModules | std::views::values)
    {
        ...
        if (cmp != CompareVersionResult::Equal)
        {
            ...
            {
                Diagnostic err{dep->node, formErr(Err0059, dep->name.c_str(), dep->version.c_str())};
                Report::report(err);
            }
            else
            {
                Diagnostic err{dep->node, formErr(Err0058, dep->name.c_str(), dep->version.c_str(), dep->resolvedLocation.c_str())};
                Report::report(err);
            }
            ok = false;
        }
        ...
        for (const auto val : allModules | std::views::values)
        {
            ...
            switch (val->fetchDep->fetchKind)
            {
                ...
                case DependencyFetchKind::Swag:
                {
                    ...
                    {
                        Report::errorOS(formErr(Err0097, pathSrc.c_str()));
                        ok = false;
                        continue;
                    }
