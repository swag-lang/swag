...
void ModuleDepManager::registerCfgFile(SourceFile* file)
{
    ...
    {
        Report::error(formErr(Fat0012, moduleName.cstr(), moduleFolder.cstr()));
        OS::exit(-1);
    }
...
bool ModuleDepManager::fetchModuleCfgLocal(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName)
{
    ...
    if (!std::filesystem::exists(remotePath, err))
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0075, SWAG_CFG_FILE, remotePath.cstr())});
    ...
    {
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0077, remotePath.cstr())});
    }
    ...
    cfgFileName         = form("module%u.swg", cacheNum++).cstr();
    destPath.append(cfgFileName);
    if (fopen_s(&fdest, destPath, "wbN"))
    {
        (void) fclose(fsrc);
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0071, SWAG_CFG_FILE, dep->name.cstr())});
    }
...
bool ModuleDepManager::fetchModuleCfgSwag(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName, bool fetch)
{
    ...
    if (!std::filesystem::exists(remotePath, err))
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0083, remotePath.cstr())});
    if (!fetch)
        return true;
...
bool ModuleDepManager::fetchModuleCfg(ModuleDependency* dep, Path& cfgFilePath, Utf8& cfgFileName, bool fetch)
{
    if (dep->location.empty())
        return Report::report({dep->node, formErr(Err0744, dep->name.cstr())});
    ...
    if (tokens.size() != 2)
    {
        if (dep->isLocalToWorkspace)
            return Report::report({dep->node, toErr(Err0195)});
        return Report::report({dep->node, dep->tokenLocation, toErr(Err0195)});
    }

    // Check mode
    if (tokens[0] != g_LangSpec->name_swag && tokens[0] != g_LangSpec->name_disk)
        return Report::report({dep->node, dep->tokenLocation, formErr(Err0730, tokens[0].cstr())});
...
bool ModuleDepManager::resolveModuleDependency(const Module* srcModule, ModuleDependency* dep)
{
    ...
    else if (cfgModule->fetchDep != dep)
    {
        ...
        switch (cmp)
        {
            ...
            {
                Diagnostic err{dep->node, formErr(Err0741, dep->name.cstr(), dep->verNum, cfgModule->fetchDep->verNum)};
                err.addNote(cfgModule->fetchDep->node, toNte(Nte0194));
                Report::report(err);
                return false;
            }
...
bool ModuleDepManager::execute()
{
    ...
    for (const auto val : allModules | std::views::values)
    {
        ...
        if (cmp != CompareVersionResult::Equal)
        {
            ...
            {
                Diagnostic err{dep->node, formErr(Err0742, dep->name.cstr(), dep->version.cstr())};
                Report::report(err);
            }
            else
            {
                Diagnostic err{dep->node, formErr(Err0743, dep->name.cstr(), dep->version.cstr(), dep->resolvedLocation.cstr())};
                Report::report(err);
            }
            ok = false;
        }
