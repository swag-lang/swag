...
void ByteCodeRun::executeGetFromStackSI(ByteCodeRunContext* context, const ByteCodeInstruction* ip)
{
    ...
    {
        callInternalCompilerError(context, ip, form(Err(Err0031), solved->symbol->name.c_str()));
        return;
    }
    ...
    if (solved->typeInfo->isVariadic() || solved->typeInfo->isTypedVariadic())
    {
        ...
        {
            callInternalCompilerError(context, ip, form(Err(Err0039), solved->typeInfo->getDisplayNameC()));
            return;
        }
