...
bool SemanticJob::setSymbolMatchCallParams(SemanticContext* context, AstIdentifier* identifier, OneMatch& oneMatch)
{
    ...
    for (int i = 0; i < maxParams; i++)
    {
        ...
        if (i >= typeInfoFunc->parameters.size() - 1 && (typeInfoFunc->flags & TYPEINFO_VARIADIC))
        {
            ...
            SWAG_CHECK(typeTable.makeConcreteTypeInfo(context, concreteType, nullptr, &nodeCall->computedValue->storageSegment, &nodeCall->computedValue->storageOffset, CONCRETE_ZERO));
        }
    }
    ...
    for (auto& dep : scopeHierarchyVars)
    {
        ...
        if (getIt)
        {
            if (dependentVar)
            {
                ...
                Diagnostic note{dependentVar, Msg0884, DiagnosticLevel::Note};
                return context->report(diag, &note);
            }
