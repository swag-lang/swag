...
bool CommandLine::check()
{
    ...
    {
        Report::error(form(Err(Fat0006), Utf8::toNiceSize(limitStackRT).c_str(), Utf8::toNiceSize(SWAG_LIMIT_MIN_STACK).c_str(), Utf8::toNiceSize(SWAG_LIMIT_MAX_STACK).c_str()));
        return false;
    }
    ...
    {
        Report::error(form(Err(Fat0006), Utf8::toNiceSize(limitStackBC).c_str(), Utf8::toNiceSize(SWAG_LIMIT_MIN_STACK).c_str(), Utf8::toNiceSize(SWAG_LIMIT_MAX_STACK).c_str()));
        return false;
    }
    ...
    if (backendGenType == BackendGenType::SCBE)
    {
        ...
        {
            Report::error(form(Err(Fat0009), Backend::getOsName(target)));
            return false;
        }

        if (target.arch != SwagTargetArch::X86_64)
        {
            Report::error(form(Err(Fat0008), Backend::getArchName(target)));
            return false;
        }
    }
    ...
    if (!g_CommandLine.scriptName.empty())
    {
        ...
        {
            Report::error(form(Err(Fat0025), p.extension().c_str()));
            return false;
        }
    }
