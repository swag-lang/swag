...
bool Parser::doEnum(AstNode* parent, AstNode** result)
{
    ...
    *result                                   = enumNode;
    ...
    SWAG_CHECK(checkIsIdentifier(tokenParse, FMT(Err(Err0266), tokenParse.token.c_str())));
    ...
    Scope* newScope = nullptr;
    {
        ...
        if (newScope->kind != ScopeKind::Enum)
        {
            ...
            Diagnostic err{enumNode->token.sourceFile, tokenParse.token, FMT(Err(Err0626), "enum", enumNode->token.c_str(), asA.c_str())};
...
{
    SWAG_CHECK(checkIsIdentifier(tokenParse, FMT(Err(Err0265), tokenParse.token.c_str())));
