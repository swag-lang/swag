...
bool FormatAst::outputChildrenAffectEqual(FormatContext& context, AstNode* node, uint32_t start, uint32_t& processed)
{
    ...
    for (const auto child : nodes)
    {
        ...
        if (!context.keepSameLineAffect)
            concat->addEol();
        else if (const auto parse = getTokenParse(child))
        {
            if (parse->flags.has(TOKEN_PARSE_EOL_AFTER) || !parse->flags.has(TOKEN_PARSE_SEMI_COL))
                concat->addEol();
            else
            {
                concat->addChar(';');
