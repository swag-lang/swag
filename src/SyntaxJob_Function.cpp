        ...
        while (true)
        {
            ...
            if (token.id == TokenId::SymColon)
            {
                if (paramExpression->kind != AstNodeKind::IdentifierRef || paramExpression->childs.size() != 1)
                    return sourceFile->report({paramExpression, Utf8::format(g_E[Err0403], token.text.c_str())});
...
bool SyntaxJob::doFuncDeclParameter(AstNode* parent, bool acceptMissingType)
{
    ...
    else
    {
        ...
        {
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(token.id == TokenId::Identifier, error(token, Utf8::format(g_E[Err0408], token.text.c_str())));
    ...
    if (token.id != TokenId::SymRightParen || isMethod || isConstMethod)
    {
        ...
        while (token.id != TokenId::SymRightParen)
        {
            ...
            if (token.id == TokenId::SymRightParen)
                break;
            SWAG_CHECK(eatToken(TokenId::SymComma));
            SWAG_VERIFY(token.id == TokenId::Identifier || token.id == TokenId::KwdUsing, error(token, Utf8::format(g_E[Err0410], token.text.c_str())));
        }
    }
...
bool SyntaxJob::doFuncDecl(AstNode* parent, AstNode** result, TokenId typeFuncId)
{
    ...
    else
    {
        ...
        if (isIntrinsic)
        {
            SWAG_VERIFY(sourceFile->isBootstrapFile || sourceFile->isRuntimeFile, error(token, Utf8::format(g_E[Err0413], token.text.c_str()), g_E[Hlp0008]));
        }
        else
        {
