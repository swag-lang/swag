...
bool SyntaxJob::doFuncCallParameters(AstNode* parent, AstFuncCallParams** result, TokenId closeToken)
{
    ...
    *result         = callParams;
    ...
    if (closeToken == TokenId::SymRightParen && token.id == TokenId::SymVertical)
    {
        ...
        {
            SWAG_VERIFY(token.id == TokenId::Identifier, error(token, g_E[Msg0401]));
            callParams->aliasNames.push_back(token);
            SWAG_CHECK(eatToken());
            if (token.id == TokenId::SymVertical)
                break;
            SWAG_VERIFY(token.id == TokenId::SymComma, error(token, g_E[Msg0402]));
            SWAG_CHECK(eatToken());
        }
        ...
        while (true)
        {
            ...
            if (token.id == TokenId::SymColon)
            {
                if (paramExpression->kind != AstNodeKind::IdentifierRef || paramExpression->childs.size() != 1)
                    return sourceFile->report({paramExpression, Utf8::format(g_E[Msg0403], token.text.c_str())});
...
bool SyntaxJob::doFuncDeclParameter(AstNode* parent, bool acceptMissingType)
{
    ...
    SWAG_VERIFY(token.id == TokenId::Identifier || token.id == TokenId::KwdConst, error(token, Utf8::format(g_E[Msg0410], token.text.c_str())));
    ...
    if (token.id == TokenId::KwdConst || paramNode->token.text == g_LangSpec->name_self)
    {
        ...
        if (token.id == TokenId::KwdConst)
        {
            isConst = true;
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(token.id == TokenId::Identifier && token.text == g_LangSpec->name_self, error(token, g_E[Msg0405]));
            paramNode->token.text = g_LangSpec->name_self;
        }

        SWAG_CHECK(eatToken());
        SWAG_VERIFY(paramNode->ownerStructScope, error(token, g_E[Msg0406]));
        ...
        {
            SWAG_VERIFY(paramNode->ownerStructScope->kind == ScopeKind::Struct, error(token, g_E[Msg0406]));
            auto typeNode         = Ast::newTypeExpression(sourceFile, paramNode);
            typeNode->ptrCount    = 1;
    ...
    else
    {
        ...
        {
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(token.id == TokenId::Identifier, error(token, Utf8::format(g_E[Msg0408], token.text.c_str())));
        ...
        if (!acceptMissingType && !hasType && !hasAssignment)
            return error(token, g_E[Msg0409]);
...
{
    SWAG_CHECK(verifyError(token, token.id != TokenId::SymLeftCurly, g_E[Msg0883]));
    ...
    if (token.id != TokenId::SymRightParen || isMethod)
    {
        ...
        while (token.id != TokenId::SymRightParen)
        {
            ...
            if (token.id == TokenId::SymRightParen)
                break;
            SWAG_CHECK(eatToken(TokenId::SymComma));
            SWAG_VERIFY(token.id == TokenId::Identifier || token.id == TokenId::KwdUsing, error(token, Utf8::format(g_E[Msg0410], token.text.c_str())));
        }
    }
...
bool SyntaxJob::doGenericDeclParameters(AstNode* parent, AstNode** result)
{
    ...
    SWAG_CHECK(eatToken(TokenId::SymLeftParen));
    SWAG_VERIFY(token.id != TokenId::SymRightParen, error(token, g_E[Msg0411]));

    while (token.id != TokenId::SymRightParen)
    {
        SWAG_VERIFY(token.id == TokenId::Identifier, error(token, g_E[Msg0412]));
        ...
        {
            SWAG_CHECK(eatToken());
            SWAG_VERIFY(token.id != TokenId::SymLeftCurly, error(token, g_E[Msg0555]));
...
bool SyntaxJob::doFuncDecl(AstNode* parent, AstNode** result, TokenId typeFuncId)
{
    ...
    if (isMethod)
    {
        SWAG_VERIFY(funcNode->ownerStructScope && funcNode->ownerStructScope->kind == ScopeKind::Struct, error(token, g_E[Msg0407]));
    }
    ...
    else
    {
        ...
        if (isIntrinsic)
        {
            SWAG_VERIFY(sourceFile->isBootstrapFile || sourceFile->isRuntimeFile, error(token, g_E[Msg0413]));
        }
        else
        {
            Tokenizer::relaxIdentifier(token);
            SWAG_VERIFY(token.id == TokenId::Identifier, error(token, Utf8::format(g_E[Msg0414], token.text.c_str())));
        }
    ...
    if (!funcForCompiler)
    {
        if (token.id == TokenId::SymMinusGreat)
        {
            ...
            SWAG_VERIFY(token.id != TokenId::KwdRetVal, error(token, g_E[Msg0560]));
            AstNode* typeExpression;
    ...
    {
        SWAG_VERIFY(!funcForCompiler, error(token, Utf8::format(g_E[Msg0416], funcNode->getDisplayName().c_str())));
