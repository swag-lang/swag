...
bool SemanticJob::resolveIntrinsicKindOf(SemanticContext* context)
{
    ...
    if (expr->typeInfo->isNative(NativeTypeKind::Any) || expr->typeInfo->kind == TypeInfoKind::Interface)
    {
        ...
        SWAG_CHECK(typeTable.makeConcreteTypeInfo(context, expr->typeInfo, node->computedValue->storageSegment, &node->computedValue->reg.u32, CONCRETE_SHOULD_WAIT, &node->typeInfo));
        if (context->result != ContextResult::Done)
            return true;
