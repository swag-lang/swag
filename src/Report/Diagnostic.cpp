...
Utf8 Diagnostic::isType(TypeInfo* typeInfo)
{
    ...
    auto str = formNte(Nte0217, typeInfo->getDisplayNameC());
...
Utf8 Diagnostic::isType(const SymbolOverload* overload)
{
    if (!overload || !overload->typeInfo)
        return "";
    return formNte(Nte0162, Naming::kindName(overload).cstr(), overload->typeInfo->getDisplayNameC());
}
