...
void initErrors()
{
    ...
    SWAG_ERROR(Err0109, "incomplete list index                             $ expected another index after the [[,]] $ either provide an additional index or remove the trailing [[,]]");
    ...
    SWAG_ERROR(Err0114, "invalid [[#alias]] number                         $ can't use [[%s]] at the end of an [[#alias]] variable name $ an [[#alias]] variable name should end with a number");
    ...
    SWAG_ERROR(Err0122, "invalid [[#up]] count                             $ expected an untyped integer in the range [1, 255] for the [[#up]] count, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0130, "invalid [[discard]] call                          $ expected a call after [[discard]], got $$TKN$$ instead");
    SWAG_ERROR(Err0131, "invalid [[impl]]                                  $ expected a struct name after [[for]] but [[%s]] is %s");
    SWAG_ERROR(Err0132, "invalid [[impl]]                                  $ expected a struct or an enum after [[impl]] but [[%s]] is [[%s]]");
    SWAG_ERROR(Err0133, "invalid [[impl]]                                  $ expected an interface after [[impl]] but [[%s]] is %s");
    SWAG_ERROR(Err0134, "invalid [[namealias]]                             $ can't use [[namealias]] on a struct member");
    ...
    SWAG_ERROR(Err0137, "invalid [[where]] mode                            $ can't recognize [[%s]] as a valid [[where]] mode $ the only valid mode is [[call]]");
    SWAG_ERROR(Err0138, "invalid [[with]]                                  $ can't use [[with]] on an enum variable (type is [[%s]]) $ you can use [[with]] on an enum type, but not a variable");
    SWAG_ERROR(Err0139, "invalid [[with]]                                  $ can't use [[with]] on type [[%s]] $ expected a namespace, an enum, a struct or a pointer to struct");
    SWAG_ERROR(Err0140, "invalid [[with]] expression                       $ expected an identifier or a variable declaration as the [[with]] expression");
    ...
    SWAG_ERROR(Err0143, "invalid address                                   $ can't take the address because the function [[%s]] returns nothing");
    ...
    SWAG_ERROR(Err0155, "invalid align value                               $ expected a power of two argument for [[#[Swag.Align]]], got [[%d]] instead");
    SWAG_ERROR(Err0156, "invalid argument                                  $ can't recognize [[\"%s\"]] as a valid [[location]] mode $ the acceptable modes are [[\"swag\"]] and [[\"disk\"]]");
    SWAG_ERROR(Err0157, "invalid argument                                  $ can't recognize [[\"%s\"]] as a valid [[#[Swag.ExportType]]] argument $ the acceptable values are [[\"methods\"]] and [[\"nozero\"]]");
    SWAG_ERROR(Err0158, "invalid argument                                  $ can't recognize [[\"%s\"]] as a valid [[#[Swag.Match]]] argument $ the acceptable values are [[\"where\"]] and [[\"self\"]]");
    SWAG_ERROR(Err0159, "invalid argument                                  $ can't recognize [[\"%s\"]] as a valid [[#[Swag.Optim]]] argument $ the acceptable values are [[\"bytecode\"]] and [[\"backend\"]]");
    ...
    SWAG_ERROR(Err0161, "invalid argument                                  $ expected a positive value as the [[@countof]] argument, got [[%I64d]] instead");
    SWAG_ERROR(Err0162, "invalid argument                                  $ expected a positive value as the [[@countof]] argument, got [[%d]] instead");
    SWAG_ERROR(Err0163, "invalid argument                                  $ expected a constant value as the [[@gettag]] last argument, got [[%s]] instead");
    SWAG_ERROR(Err0164, "invalid argument                                  $ can't use [[null]] as the [[@mkany]] first argument");
    SWAG_ERROR(Err0165, "invalid array dimension                           $ can't deduce the array dimension because of a missing initialization");
    SWAG_ERROR(Err0166, "invalid array dimension                           $ can't have an array dimension of 0");
    ...
    SWAG_ERROR(Err0175, "invalid bit-cast                                  $ can't bit-cast to the type [[%s]] $ expected an integer, a rune or a float");
    ...
    SWAG_ERROR(Err0178, "invalid call                                      $ can't have call arguments after the variable name [[%s]]");
    SWAG_ERROR(Err0179, "invalid call function                             $ can't have call arguments because [[%s]] is [[%s]], not a function");
    SWAG_ERROR(Err0180, "invalid call function                             $ can't have call arguments because [[%s]] has the type [[%s]] which is not a function");
    ...
    SWAG_ERROR(Err0184, "invalid capture                                   $ can't capture [[%s]] because it is %s $ capturing %s type is not possible");
    ...
    SWAG_ERROR(Err0186, "invalid character                                 $ unexpected character [[%s]]");
    SWAG_ERROR(Err0187, "invalid character literal                         $ can't convert a character literal to the type [[%s]]");
    SWAG_ERROR(Err0188, "invalid character literal                         $ can't convert [['%s']] to a character literal $ this looks like a string and not a character");
    ...
    SWAG_ERROR(Err0190, "invalid compiler instruction                      $ can't recognize [[%s]] as a valid compiler instruction $ a compiler instruction starts with the symbol [['#']]");
    SWAG_ERROR(Err0191, "invalid scope compiler instruction                $ unexpected scoping of a compiler instruction $ compiler instructions do not belong to a specific scope");
    SWAG_ERROR(Err0192, "invalid const move reference                      $ can't associate [[const]] with the move reference type [[&&]] $ consider removing the [[const]] or using a simple reference [[&]] instead");
    SWAG_ERROR(Err0193, "invalid constant                                  $ can't create the constant because of the generic type [[%s]]");
    SWAG_ERROR(Err0194, "invalid default parameters                        $ can't redefine the default parameters of a lambda or a closure");
    SWAG_ERROR(Err0195, "invalid default value                             $ can't define a default value to [[self]]");
    SWAG_ERROR(Err0196, "invalid default value                             $ can't define a default value to lambda parameters");
    ...
    SWAG_ERROR(Err0200, "invalid dereference                               $ can't dereference [[%s]] like a struct or a pointer because the type is [[%s]]");
    ...
    SWAG_ERROR(Err0204, "invalid escape code                               $ can't recognize [['%c']] as a valid escape code");
    ...
    SWAG_ERROR(Err0213, "invalid generated [[impl]] block                  $ can't generate an [[impl]] block outside a [[#message]] with [[Swag.CompilerMsgMask.AttributeGen]]");
    ...
    SWAG_ERROR(Err0219, "invalid generic arguments                         $ expected a type, got a value instead");
    SWAG_ERROR(Err0220, "invalid generic arguments                         $ expected a value, got a type instead");
    SWAG_ERROR(Err0221, "invalid generic arguments                         $ invalid usage of %s ([[%s]]) as a generic argument $ a generic argument must be a literal or a type");
    SWAG_ERROR(Err0222, "invalid generic arguments                         $ unexpected generic arguments after the variable [[%s]]");
    SWAG_ERROR(Err0223, "invalid generic parameter                         $ expected a literal as a generic parameter of the special function [[%s]], got type [[%s]] instead");
    ...
    SWAG_ERROR(Err0229, "invalid import location                           $ unexpected [[#import]] location format $ expecting [[location=\"mode@accesspath\"]] where mode is [[swag]] or [[disk]]");
    SWAG_ERROR(Err0230, "invalid import version                            $ unexpected [[#import]] version format");
    SWAG_ERROR(Err0231, "invalid initialization                            $ can't initialize a constant array (the type is [[%s]]) with one single value $ only variables can be initialized that way");
    SWAG_ERROR(Err0232, "invalid scope instrinsic                          $ unexpected scoping of an intrinsic $ intrinsics do not belong to a specific scope");
    SWAG_ERROR(Err0233, "invalid instruction modifier                      $ can't recognize [[%s]] as a valid instruction modifier");
    SWAG_ERROR(Err0234, "invalid interface conversion                      $ can't cast from [[%s]] to [[%s]] because of a missing implementation");
    SWAG_ERROR(Err0235, "invalid interface function                        $ expected [[self]] as the first parameter, got type [[%s]] instead $ consider declaring the interface function with [[mtd]] instead of [[func]]");
    SWAG_ERROR(Err0236, "invalid interface function                        $ expected at least [[self]] as the first parameter of the interface function [[%s]] $ consider declaring the interface function with [[mtd]] instead of [[func]]");
    SWAG_ERROR(Err0237, "invalid intrinsic                                 $ can't recognize [[%s]] as a valid intrinsic starting with [[@]]");
    SWAG_ERROR(Err0238, "invalid list variables                            $ can't declare multiple variables in [[if]] $ [[if]] should be followed by a single variable declaration");
    ...
    SWAG_ERROR(Err0247, "invalid move                                      $ can't use [[move]] on an immutable expression (type is [[%s]])");
    ...
    SWAG_ERROR(Err0261, "invalid name module                               $ can't have an empty [[#[Swag.Foreign]]] module name");
    ...
    SWAG_ERROR(Err0265, "invalid name pass                                 $ can't recognize [[%s]] as a valid compiler pass name");
    ...
    SWAG_ERROR(Err0271, "invalid namespace body                            $ expected the namespace body or an instruction, got [[;]] instead");
    ...
    SWAG_ERROR(Err0278, "invalid number format binary                      $ can't have an empty binary literal $ binary literals require at least one [[0]] or [[1]] digit");
    SWAG_ERROR(Err0279, "invalid number format binary                      $ expected a binary digit, got $$TKN$$ instead $ binary literals can only contain [[0]] or [[1]]");
    SWAG_ERROR(Err0280, "invalid number format float                       $ expected a scientific notation exponent to have at least one digit");
    SWAG_ERROR(Err0281, "invalid number format float                       $ can't have [[_]] at the start of the exponent part");
    SWAG_ERROR(Err0282, "invalid number format float                       $ can't have [[_]] at the start of the fractional part");
    SWAG_ERROR(Err0283, "invalid number format hexadecimal                 $ expected an hexadecimal digit, got $$TKN$$ instead $ hexadecimal literals can only contain digits [[0-9]], [[A-F]], and [[a-f]]");
    SWAG_ERROR(Err0284, "invalid number format hexadecimal                 $ can't have an empty hexadecimal literal $ hexadecimal literals require at least one valid digit ([[0-9]], [[A-F]], [[a-f]])");
    SWAG_ERROR(Err0285, "invalid number prefix                             $ expected [[0x]] for hexadecimal or [[0b]] for binary, got 0$$TKN$$ instead");
    ...
    SWAG_ERROR(Err0288, "invalid packing value                             $ expected 0 or a power of two [[#[Swag.Pack]]] argument, got [[%d]]");
    SWAG_ERROR(Err0289, "invalid parameter                                 $ expected a type [[Swag.CompilerMsgMask]] as the [[#message]] parameter, got type [[%s]] instead");
    ...
    SWAG_ERROR(Err0301, "invalid return value assignment                   $ can't assign because the call does not return a value");
    SWAG_ERROR(Err0302, "invalid runtime typeinfo                          $ can't convert [[%s]] to a runtime typeinfo");
    SWAG_ERROR(Err0303, "invalid safety argument                           $ can't recognize [[\"%s\"]] as a valid safety value $ the acceptable values are [[\"boundcheck|overflow|math|switch|unreachable|any|bool|nan|sanity|null\"]]");
    ...
    SWAG_ERROR(Err0322, "invalid type                                      $ expected [[bool]] as the generic parameter of the special function [[%s]], got type [[%s]] instead");
    SWAG_ERROR(Err0323, "invalid type                                      $ expected [[string]] as the generic parameter of the special function [[%s]], got type [[%s]] instead");
    ...
    SWAG_ERROR(Err0330, "invalid type [[orelse]]                           $ can't have a [[struct]] as an [[orelse]] argument");
    SWAG_ERROR(Err0331, "invalid type [[orelse]]                           $ can't have the type [[%s]] as an [[orelse]] argument");
    SWAG_ERROR(Err0332, "invalid type [[switch]]                           $ can't use the type [[%s]] as a [[switch]] expression");
    ...
    SWAG_ERROR(Err0340, "invalid type enum                                 $ expected the enum array type [[%s]] to be [[const]] $ this should be designated as [[const %s]]");
    SWAG_ERROR(Err0341, "invalid type enum                                 $ expected the enum array type [[%s]] to have specified dimensions");
    ...
    SWAG_ERROR(Err0367, "literal overflow                                  $ can't fit the number [[%I64u]] in the type [[%s]]");
    SWAG_ERROR(Err0368, "mismatch access                                   $ can't make %s [[%s]] public because %s [[%s]] has [[%s]] access");
    SWAG_ERROR(Err0369, "mismatch access                                   $ can't make the special function [[%s]] internal because the corresponding struct has [[public]] access");
    SWAG_ERROR(Err0370, "mismatch access                                   $ can't make the special function [[%s]] public because the corresponding struct has [[internal]] access");
    SWAG_ERROR(Err0371, "mismatch enum types                               $ expected an enum of type [[%s]], got type [[%s]] instead");
    SWAG_ERROR(Err0372, "mismatch function signature                       $ unexpected [[%s]] function signature for interface [[%s]]");
    ...
    SWAG_ERROR(Err0380, "misplaced [[#global]]                             $ expected [[#global]] to be placed at the very top of the file");
    SWAG_ERROR(Err0381, "misplaced [[#import]]                             $ can't use [[#import]] outside [[module.swg]] or [[.swgs]] files");
    SWAG_ERROR(Err0382, "misplaced [[#import]]                             $ can't use [[#import]] outside the [[#dependencies]] block");
    SWAG_ERROR(Err0383, "misplaced [[#index]]                              $ can't use the keyword [[#index]] outside a breakable block such as [[loop]], [[visit]], [[for]], etc.");
    SWAG_ERROR(Err0384, "misplaced [[#load]]                               $ can't use [[#load]] outside [[module.swg]] or [[.swgs]] files");
    SWAG_ERROR(Err0385, "misplaced [[#load]]                               $ can't use [[#load]] outside the [[#dependencies]] block");
    SWAG_ERROR(Err0386, "misplaced [[#macro]]                              $ can't use [[#macro]] outside a [[#[Swag.Macro]]] function");
    SWAG_ERROR(Err0387, "misplaced [[#mixin]] block                        $ can't use a [[#mixin]] block outside a breakable block such as [[loop]], [[visit]], [[for]], etc.");
    SWAG_ERROR(Err0388, "misplaced [[#placeholder]]                        $ can't use [[#placeholder]] outside a top level scope");
    SWAG_ERROR(Err0389, "misplaced [[#self]]                               $ can't use [[#self]] outside functions");
    SWAG_ERROR(Err0390, "misplaced [[#test]]                               $ can't use [[#test]] outside test modules (in the [[./tests]] folder)");
    SWAG_ERROR(Err0391, "misplaced [[#up]]                                 $ can't use [[#up]] outside a [[#[Swag.Macro]]] function");
    SWAG_ERROR(Err0392, "misplaced [[%s]]                                  $ can't use [[%s]] outside a function marked with [[throw]], but [[%s]] isn't");
    ...
    SWAG_ERROR(Err0396, "misplaced [[@cvastart]]                           $ expected the intrinsic [[@cvastart]] to be used in a function with a last parameter of type [[cvarargs]]");
    SWAG_ERROR(Err0397, "misplaced [[@spread]]                             $ expected the intrinsic [[@spread]] to be used as a direct function argument");
    ...
    SWAG_ERROR(Err0414, "misplaced [[self]]                                $ can't use [[self]] outside an [[impl]] block of an enum or a struct");
    ...
    SWAG_ERROR(Err0423, "misplaced attribute                               $ expected [[AttributeUsage.Gen]] to be associated with [[AttributeUsage.Struct]] or [[AttributeUsage.Enum]]");
    ...
    SWAG_ERROR(Err0465, "missing [[#foreignlib]] string argument           $ expected a library name after [[#foreignlib]], got $$TKN$$ instead $ use [[#foreignlib \"path/to/library\"]] to specify the library to import");
    ...
    SWAG_ERROR(Err0468, "missing [[#if]] boolean expression                $ expected a boolean expression after [[#if]], got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0472, "missing [[#message]] parameter                    $ expected a parameter of type [[Swag.CompilerMsgMask]] for [[#message]]");
    SWAG_ERROR(Err0473, "missing [[#mix]] number                           $ can't use a [[#mix]] variable name without a trailing number, such as [[#mix0]], [[#mix1]], etc.");
    ...
    SWAG_ERROR(Err0477, "missing [[(]] before arguments                    $ expected [[(]] to start the list of arguments, got $$TKN$$ because");
    SWAG_ERROR(Err0478, "missing [[(]] before parameters                   $ expected [[(]] to start the list of parameters, got $$TKN$$ because ");
    SWAG_ERROR(Err0479, "missing [[)]]                                     $ expected [[)]] to end the list of attribute arguments");
    ...
    SWAG_ERROR(Err0487, "missing [[switch]] value                          $ expected the value [[%s.%s]] to be treated in the [[switch]]");
    SWAG_ERROR(Err0488, "missing [[visit]] expression                      $ expected the element to visit before [[%s]]");
    SWAG_ERROR(Err0489, "missing [[visit]] variable                        $ expected another [[visit]] variable name between the [[comma]] ',' and the [[colon]] ':' $ the first [[visit]] variable is the [[value]], the second one is the [[index]]");
    ...
    SWAG_ERROR(Err0503, "missing generic parameters                        $ can't declare special function [[%s]] without generic parameters");
    ...
    SWAG_ERROR(Err0522, "missing parameters                                $ expected [[(]] after the intrinsic [[%s]] because it is a function");
    SWAG_ERROR(Err0523, "missing parameters                                $ can't have a special function [[%s]] without parameters");
    SWAG_ERROR(Err0524, "missing return type                               $ can't have a [[public]] function without an explicitly defined return type");
    SWAG_ERROR(Err0525, "missing return type                               $ expected a return value because the function [[%s]] has the [[#[Swag.Discardable]]] attribute");
    ...
    SWAG_ERROR(Err0546, "missing arguments                                 $ expected the %s to have arguments between [[()]]");
    ...
    SWAG_ERROR(Err0555, "out of range [[#alias]] number                    $ can't use [[%u]] for an [[#alias]] number $ it should be in the range [0, 31]");
    SWAG_ERROR(Err0556, "out of range [[#mix]] number                      $ can't use [[%u]] for a [[#mix]] number $ it should be in the range [0, 31]");
    SWAG_ERROR(Err0557, "out of range [[#up]] count                        $ can't use [[%u]] for the [[#up]] count $ it should be in the range [1, 255]");
    SWAG_ERROR(Err0558, "out of range array dimensions                     $ can't have more than [[254]] array dimensions");
    SWAG_ERROR(Err0559, "out of range call level                           $ unexpected call level $ the limit defined with [[--limit-recurse-bc:%d]] has been reached");
    SWAG_ERROR(Err0560, "out of range callback parameters                  $ can't create a callback if the function has more than [[%d]] parameters (got [[%d]])");
    ...
    SWAG_ERROR(Err0563, "out of range compiler limit                       $ unexpected size of the %s (maximum size is [[0x%I64x]] bytes)");
    SWAG_ERROR(Err0564, "out of range compiler limit                       $ unexpected size of the data segment [[%s]] (maximum size is [[0x%I64x]] bytes)");
    ...
    SWAG_ERROR(Err0572, "out of range variadic arguments                   $ can't have more than [[%d]] variadic arguments, got [[%d]] instead");
    SWAG_ERROR(Err0573, "overflow bytecode stack                           $ can't have a stack bigger than the limit [[%s]] given by [[--stack-size]]");
    ...
    SWAG_ERROR(Err0575, "overload mismatch                                 $ can't match any of the [[%d]] overloads for [[%s]]");
    ...
    SWAG_ERROR(Err0589, "too many generic parameters                       $ unexpected number of generic parameters for the special function [[%s]]");
    ...
    SWAG_ERROR(Err0593, "too many initializers                             $ unexpected number of initializers for the type [[%s]]");
    SWAG_ERROR(Err0594, "too many parameters                               $ unexpected number of parameters for [[%s]] ([[%d]] expected, [[%d]] provided)");
    SWAG_ERROR(Err0595, "tuple type mismatch                               $ can't use the source tuple with the requested tuple; they are not compatible");
    SWAG_ERROR(Err0596, "type mismatch                                     $ can't initialize the type [[%s]] from the type [[%s]]");
    ...
    SWAG_ERROR(Err0603, "type mismatch                                     $ can't use the type [[%s]] with the type [[%s]] defined in the command line for [[%s]]; they are incompatible");
    ...
    SWAG_ERROR(Err0606, "type mismatch argument                            $ can't use the type [[%s]] for [[#include]] $ it requires an argument of type [[string]]");
    SWAG_ERROR(Err0607, "type mismatch argument                            $ can't use the type [[%s]] for [[#mixin]] $ it requires an argument of type [[code]]");
    SWAG_ERROR(Err0608, "type mismatch argument                            $ can't use the type [[%s]] for [[where]] $ it requires an argument of type [[bool]]");
    SWAG_ERROR(Err0609, "type mismatch argument                            $ expected an argument of type [[%s]], got type [[%s]] instead");
    SWAG_ERROR(Err0610, "type mismatch argument                            $ expected an integer for the [[%s]] count, got type [[%s]] instead");
    SWAG_ERROR(Err0611, "type mismatch argument                            $ expected a block pointer as the first argument of [[%s]] if the number of values is greater than one (type is [[%s]])");
    SWAG_ERROR(Err0612, "type mismatch argument                            $ expected a block pointer as the first argument of [[%s]] if the number of values is variable (type is [[%s]])");
    SWAG_ERROR(Err0613, "type mismatch argument                            $ expected a block pointer as the first argument of [[%s]], got type [[%s]] instead");
    SWAG_ERROR(Err0614, "type mismatch argument                            $ expected a pointer as the first argument of [[%s]], got type [[%s]] instead");
    SWAG_ERROR(Err0615, "type mismatch argument                            $ expected the type [[string]] as the argument of [[%s]], got type [[%s]] instead");
    SWAG_ERROR(Err0616, "type mismatch argument                            $ can't use a tuple as the [[@countof]] argument");
    SWAG_ERROR(Err0617, "type mismatch argument                            $ expected an integer as the [[@countof]] argument, got type [[%s]] instead");
    ...
    SWAG_ERROR(Err0620, "type mismatch argument                            $ expected a [[typeinfo]] as the [[@mkany]] second argument, got type [[%s]] instead");
    SWAG_ERROR(Err0621, "type mismatch argument                            $ expected a pointer to a function as the [[@mkcallback]] argument, got type [[%s]] instead");
    SWAG_ERROR(Err0622, "type mismatch argument                            $ expected a pointer or a struct as the [[@mkinterface]] first argument, got [[%s]] instead");
    SWAG_ERROR(Err0623, "type mismatch argument                            $ expected a [[typeinfo]] as the [[@mkinterface]] second argument, got type [[%s]] instead");
    SWAG_ERROR(Err0624, "type mismatch argument                            $ expected an interface name as the [[@mkinterface]] third argument, got type [[%s]] instead");
    SWAG_ERROR(Err0625, "type mismatch argument                            $ expected a pointer to [[u8]] as the [[@mkstring]] first argument, got type [[%s]] instead");
    SWAG_ERROR(Err0626, "type mismatch argument                            $ expected a [[string]] as the [[@runes]] argument, got type [[%s]] instead");
    ...
    SWAG_ERROR(Err0655, "type mismatch return                              $ can't return the type [[%s]] because the return value has already been deduced to be nothing");
    SWAG_ERROR(Err0656, "type mismatch return                              $ can't return the type [[%s]] because the return value has already been deduced to be [[%s]]");
    ...
    SWAG_ERROR(Err0679, "unexpected generic arguments                      $ unexpected generic arguments after the name");
    ...
    SWAG_ERROR(Err0692, "unexpected return value                           $ can't declare a return value in a function with the [[#[Swag.CalleeReturn]]] attribute");
    ...
    SWAG_ERROR(Err0714, "unknown identifier                                $ can't find an [[enum]] or a [[with]] to match with the identifier [[%s]]");
    ...
    SWAG_ERROR(Err0741, "already defined field name                        $ can't redefine the field name [[%s]] because it already exists");
