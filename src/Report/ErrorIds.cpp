...
void initErrors()
{
    ...
    SWAG_ERROR(Err0077, "failed [[%s]] constraint                          $ the %s [[%s]] can't be used because of a failed [[%s]] constraint");
    SWAG_ERROR(Err0078, "failed generic instantiation                      $ the variable creation failed due to the generic type [[%s]]");
    SWAG_ERROR(Err0079, "fetch error                                       $ can't fetch file [[%s]] for module dependency [[%s]]");
    SWAG_ERROR(Err0080, "file delete error                                 $ failed to delete file [[%s]]");
    SWAG_ERROR(Err0081, "file format error                                 $ the file format must be ASCII, UTF-8, or UTF-8-BOM");
    SWAG_ERROR(Err0082, "file not found                                    $ the documentation page [[%s]] can't be found");
    SWAG_ERROR(Err0083, "file not found                                    $ the file [[%s]] can't be found in the module folder [[%s]]");
    SWAG_ERROR(Err0084, "file open error                                   $ failed to access file [[%s]]");
    SWAG_ERROR(Err0085, "file open error                                   $ failed to open file [[%s]] for writing");
    SWAG_ERROR(Err0086, "file open error                                   $ failed to open file [[%s]]");
    SWAG_ERROR(Err0087, "file open error                                   $ failed to open the [[#include]] file [[%s]]");
    SWAG_ERROR(Err0088, "file read error                                   $ failed to read source file [[%s]]");
    SWAG_ERROR(Err0089, "file write error                                  $ failed to write to file [[%s]]");
    SWAG_ERROR(Err0090, "folder error                                      $ failed to create directory [[%s]]");
    SWAG_ERROR(Err0091, "folder error dependency                           $ the dependency module folder [[\"%s\"]] does not exist");
    SWAG_ERROR(Err0092, "forbidden [[@postcopy]]                           $ the intrinsic [[@postcopy]] can't be called on type [[%s]] because of [[#[Swag.NoCopy]]]");
    SWAG_ERROR(Err0093, "forbidden [[opDrop]]                              $ unexpected [[opDrop]] special function for [[%s]] because the struct is marked with [[#[Swag.ConstExpr]]]");
    SWAG_ERROR(Err0094, "forbidden [[opPostCopy]]                          $ unexpected [[opPostCopy]] special function for struct [[%s]] because the struct is marked with [[#[Swag.NoCopy]]]");
    SWAG_ERROR(Err0095, "forbidden call                                    $ a direct call to [[opDrop]] is forbidden $ consider using [[@drop]] instead]]");
    SWAG_ERROR(Err0096, "forbidden call                                    $ a direct call to [[opPostCopy]] is forbidden $ consider using [[@postcopy]] instead]]");
    SWAG_ERROR(Err0097, "forbidden call                                    $ a direct call to [[opPostMove]] is forbidden $ consider using [[@postmove]] instead]]");
    SWAG_ERROR(Err0098, "forbidden call                                    $ the [[compileString]] function is not accessible in this context $ this compiler stage does not allow meta-programmation");
    SWAG_ERROR(Err0099, "forbidden const assign                            $ can't assign because the left expression is immutable");
    SWAG_ERROR(Err0100, "forbidden const assign                            $ can't assign to [[%s]] because it's immutable");
    SWAG_ERROR(Err0101, "forbidden const assign                            $ the global variable [[%s]] is immutable at compile-time because it's initialized to 0");
    SWAG_ERROR(Err0102, "forbidden copy                                    $ the copy of [[%s]] is disabled because the struct is marked with [[#[Swag.NoCopy]]]");
    SWAG_ERROR(Err0103, "incomplete import version                         $ the [[#import]] version has build number [[%d]] but lacks a revision");
    SWAG_ERROR(Err0104, "incomplete import version                         $ the [[#import]] version has revision [[%d]] but lacks a major version");
    ...
    SWAG_ERROR(Err0113, "infinite [[while]]                                $ the while condition is always [[true]], which creates an infinite loop $ if you intentionally want to create an infinite loop, consider using a loop without argument [[loop { ... }]]");
    SWAG_ERROR(Err0114, "invalid [[#alias]] number                         $ a [[#alias]] variable name should end with a number, got [[%s]] instead");
    SWAG_ERROR(Err0115, "invalid [[#global]] instruction                   $ [[%s]] is not recognized as a valid [[#global]] instruction");
    SWAG_ERROR(Err0116, "invalid [[#import]] location                      $ expected a [[#import]] location path, got $$TKN$$ instead");
    SWAG_ERROR(Err0117, "invalid [[#import]] location                      $ expected a [[#import]] version as a string, got $$TKN$$ instead");
    SWAG_ERROR(Err0118, "invalid [[#mix]] number                           $ a [[#mix]] variable name should end with a number, got [[%s]] instead");
    SWAG_ERROR(Err0119, "invalid [[#mixin]] replacement keyword            $ a [[#mixin]] block accepts only [[break]] and [[continue]] as replacements, got $$TKN$$ instead");
    SWAG_ERROR(Err0120, "invalid [[#up]] count                             $ expected an integer literal for the [[#up]] count, got $$TKN$$ instead");
    SWAG_ERROR(Err0121, "invalid [[#up]] count                             $ the [[#up]] count can't be zero");
    SWAG_ERROR(Err0122, "invalid [[#up]] count                             $ the [[#up]] count should be an untyped integer in the range [1, 255], got [[%s]] instead");
    SWAG_ERROR(Err0123, "invalid [[#up]] move                              $ [[#up]] can't move [[%d]] scopes up");
    ...
    SWAG_ERROR(Err0135, "invalid [[retval]]                                $ the [[retval]] special type is not allowed in a function that returns nothing");
    SWAG_ERROR(Err0136, "invalid [[switch]] block content                  $ expected [[case]] or [[default]], got $$TKN$$ instead");
    SWAG_ERROR(Err0137, "invalid [[where]] mode                            $ [[%s]] is not a valid [[where]] mode $ the only valid mode is [[call]]");
    SWAG_ERROR(Err0138, "invalid [[with]]                                  $ [[with]] is invalid on an enum variable (type is [[%s]])");
    SWAG_ERROR(Err0139, "invalid [[with]]                                  $ [[with]] is invalid on type [[%s]] $ expected a namespace, an enum, a struct or a pointer to struct");
    ...
    SWAG_ERROR(Err0141, "invalid access from runtime                       $ the compile-time %s [[%s]] can't be accessed from the runtime %s");
    SWAG_ERROR(Err0142, "invalid access from runtime                       $ the compile-time %s can't be accessed from the runtime %s");
    ...
    SWAG_ERROR(Err0155, "invalid align value                               $ the [[#[Swag.Align]]] argument must be a power of two, got [[%d]]");
    SWAG_ERROR(Err0156, "invalid argument                                  $ invalid [[location]] mode [[%s]] $ the acceptable values are [[\"swag\"]] and [[\"disk\"]]");
    SWAG_ERROR(Err0157, "invalid argument                                  $ invalid argument [[%s]] for the attribute [[#[Swag.ExportType]]] $ the acceptable values are [[\"methods\"]] and [[\"nozero\"]]");
    SWAG_ERROR(Err0158, "invalid argument                                  $ invalid argument [[%s]] for the attribute [[#[Swag.Match]]] $ the acceptable values are [[\"where\"]] and [[\"self\"]]");
    SWAG_ERROR(Err0159, "invalid argument                                  $ invalid argument [[%s]] for the attribute [[#[Swag.Optim]]] $ the acceptable values are [[\"bytecode\"]] and [[\"backend\"]]");
    ...
    SWAG_ERROR(Err0165, "invalid array dimension                           $ the array dimension can't be deduced because of a missing initialization");
    SWAG_ERROR(Err0166, "invalid array dimension                           $ the array dimension is 0");
    SWAG_ERROR(Err0167, "invalid assign                                    $ a function pointer can't be assigned to a constant");
    SWAG_ERROR(Err0168, "invalid assign                                    $ assignments are not allowed for unnamed parameters");
    SWAG_ERROR(Err0169, "invalid assign                                    $ can't assign because the left expression isn't a value");
    SWAG_ERROR(Err0170, "invalid assign                                    $ the affectation to the tuple is not possible because the right expression is not compatible");
    ...
    SWAG_ERROR(Err0173, "invalid bit-cast                                  $ bit-cast from type [[%s]] is not allowed $ expected an integer, a rune, a float or a pointer");
    SWAG_ERROR(Err0174, "invalid bit-cast                                  $ bit-cast to a larger type is not allowed ([[%s]] to [[%s]])");
    SWAG_ERROR(Err0175, "invalid bit-cast                                  $ bit-cast to the type [[%s]] is not allowed $ expected integer, rune, or float");
    SWAG_ERROR(Err0176, "invalid block start                               $ an [[{]] (start of a block) is not allowed after [[discard try/assume/catch]]");
    ...
    SWAG_ERROR(Err0181, "invalid call function                             $ call to function [[%s]] requires arguments between [[()]] and not [[{}]]");
    SWAG_ERROR(Err0182, "invalid call function                             $ can't call [[%s]] because the function is declared but not defined");
    SWAG_ERROR(Err0183, "invalid call lambda                               $ invalid lambda call because [[%s]] is not a variable, it is %s");
    SWAG_ERROR(Err0184, "invalid capture                                   $ can't capture [[%s]] because it's %s $ capturing %s type is not supported");
    SWAG_ERROR(Err0185, "invalid capture                                   $ can't capture [[%s]] because it's not a plain old data struct $ a struct is not plain old data if it contains [[opDrop]], [[opPostCopy]] or [[opPostMove]]");
    ...
    SWAG_ERROR(Err0189, "invalid character syntax                          $ characters should be delimited with back-ticks and not quotes $ consider using the syntax [[`%s`]] instead");
    SWAG_ERROR(Err0190, "invalid compiler instruction                      $ [[%s]] is not a valid compiler instruction starting with the symbol [['#']]");
    SWAG_ERROR(Err0191, "invalid compiler instruction scope                $ compiler instructions do not belong to a specific scope");
    ...
    SWAG_ERROR(Err0197, "invalid dereference                               $ can't dereference [[%s]] by index because the special function [[opIndex]] can't be found in [[%s]]");
    SWAG_ERROR(Err0198, "invalid dereference                               $ dereferencing the type [[%s]] is not valid");
    SWAG_ERROR(Err0199, "invalid dereference                               $ the type of [[%s]] is [[%s]], so it can't be dereferenced by index because pointer arithmetic is not allowed");
    SWAG_ERROR(Err0200, "invalid dereference                               $ the type of [[%s]] is [[%s]], so it can't be dereferenced like a struct or a pointer to a struct");
    SWAG_ERROR(Err0201, "invalid dereference null                          $ attempt to dereference a null pointer");
    ...
    SWAG_ERROR(Err0211, "invalid expression                                $ the instruction [[%s]] can't be used as an expression $ this instruction can only be used as a statement");
    ...
    SWAG_ERROR(Err0214, "invalid generic                                   $ a generic struct instantiation of a partial type alias ([[%s]]) is not supported");
    ...
    SWAG_ERROR(Err0219, "invalid generic argument                          $ a type is expected but got a value instead");
    SWAG_ERROR(Err0220, "invalid generic argument                          $ a value is expected but got a type instead");
    ...
    SWAG_ERROR(Err0222, "invalid generic arguments                         $ unexpected generic arguments after the variable name [[%s]]");
    ...
    SWAG_ERROR(Err0224, "invalid generic type                              $ a generic type does not accept a specific type starting with [[:]]");
    ...
    SWAG_ERROR(Err0376, "misplaced [[#do]]                                 $ unnecessary usage of [[#do]] before [[{]] $ consider removing it");
    ...
    SWAG_ERROR(Err0393, "misplaced [[%s]]                                  $ [[%s]] can't be used in a struct special function ([[%s]])");
    ...
    SWAG_ERROR(Err0401, "misplaced [[const]] qualifier                     $ the [[const]] qualifier is not allowed for [[func]], only for [[mtd]]");
    SWAG_ERROR(Err0402, "misplaced [[continue]]                            $ [[continue]] is only valid within a breakable block such as [[loop]], [[visit]], [[for]], etc.");
    SWAG_ERROR(Err0403, "misplaced [[do]]                                  $ unnecessary usage of [[do]] before [[{]] $ consider removing it");
    ...
    SWAG_ERROR(Err0426, "misplaced attribute                               $ a %s can't have the [[#[Swag.Macro]]] attribute");
    SWAG_ERROR(Err0427, "misplaced attribute                               $ a %s can't have the [[#[Swag.Mixin]]] attribute");
    SWAG_ERROR(Err0428, "misplaced attribute                               $ a %s can't have the [[#[Swag.NotGeneric]]] attribute");
    SWAG_ERROR(Err0429, "misplaced attribute                               $ a [[switch]] without an expression can't be [[#[Swag.Complete]]]");
    SWAG_ERROR(Err0430, "misplaced attribute                               $ an interface member relocation with [[#[Swag.Offset]]] is not allowed");
    SWAG_ERROR(Err0431, "misplaced attribute                               $ incorrect attribute usage");
    SWAG_ERROR(Err0432, "misplaced attribute                               $ the %s can't have the [[#[Swag.Implicit]]] attribute because it is generic");
    SWAG_ERROR(Err0433, "misplaced attribute                               $ the [[#[Swag.CalleeReturn]]] attribute can't be applied to function [[%s]] $ [[#[Swag.CalledReturn]]] can only be applied to a macro ([[#[Swag.Macro]]]) or a mixin ([[#[Swag.Mixin]]])");
    SWAG_ERROR(Err0434, "misplaced attribute                               $ the [[#[Swag.Complete]]] attribute can't be applied to function [[%s]] $ #[Swag.Complete] can only be applied to [[opAffect]] and [[opAffectLiteral]]");
    SWAG_ERROR(Err0435, "misplaced attribute                               $ the [[#[Swag.Discardable]]] attribute can only be applied to lambda variables, got type [[%s]]");
    SWAG_ERROR(Err0436, "misplaced attribute                               $ the [[#[Swag.Implicit]]] attribute can't be applied to function [[%s]] $ [[#[Swag.Implicit]]] can only be applied to [[opAffect]], [[opAffectLiteral]] and [[opCast]]");
    ...
    SWAG_ERROR(Err0444, "misplaced compiler instruction                    $ [[#callerlocation]] can only be used when declaring a function parameter");
    ...
    SWAG_ERROR(Err0447, "misplaced error catching                          $ treating a return error with [[%s]] can only be done inside a function, macro or mixin");
    ...
    SWAG_ERROR(Err0451, "misplaced move reference                          $ a move reference type [[&&]] is only valid when declaring a function parameter");
    SWAG_ERROR(Err0452, "misplaced range                                   $ can't use a range in a switch without an expression");
    SWAG_ERROR(Err0453, "misplaced reference to [[with]] block             $ statements starting with [[.]] are only valid within a [[with]] block");
    SWAG_ERROR(Err0454, "misplaced special function                        $ the struct special function [[%s]] should be declared within the corresponding [[impl]] block $ an [[impl for]] block can only be used to implement interfaces");
    SWAG_ERROR(Err0455, "misplaced special function                        $ the struct special function [[%s]] should be declared within the corresponding [[impl]] block");
    SWAG_ERROR(Err0456, "misplaced top-level [[using]]                     $ a top-level [[using]] must be placed before any other declarations");
    SWAG_ERROR(Err0457, "misplaced tuple unpacking                         $ tuple unpacking is not allowed in %s");
    SWAG_ERROR(Err0458, "misplaced type                                    $ the type [[%s]] is only valid when declaring a function parameter");
    SWAG_ERROR(Err0459, "misplaced type                                    $ the type [[code]] is only valid within a [[#[Swag.Macro]]] or [[#[Swag.Mixin]]] function");
    SWAG_ERROR(Err0460, "misplaced variadic argument                       $ a variadic argument must be the last one");
    SWAG_ERROR(Err0461, "misplaced variadic parameter                      $ a variadic parameter must be the last one");
    SWAG_ERROR(Err0462, "misplaced variadic parameter                      $ an inline function can't have variadic parameters");
    SWAG_ERROR(Err0463, "missing [[#alias]] number                         $ an [[#alias]] variable name must end with a number such as [[#alias0]], [[#alias1]], etc.");
    SWAG_ERROR(Err0464, "missing [[#do]]                                   $ missing either [[#do]] or [[{]] before $$TKN$$");
    SWAG_ERROR(Err0465, "missing [[#foreignlib]] string argument           $ expected the library name after [[#foreignlib]], got $$TKN$$ instead $ use [[#foreignlib \"path/to/library\"]] to specify the library to import");
    SWAG_ERROR(Err0466, "missing [[#global testerror]] string argument     $ expected the error string to check after [[#global testerror]], got $$TKN$$ instead");
    SWAG_ERROR(Err0467, "missing [[#global testwarning]] string argument   $ expected the warning string to check after [[#global testwarning]], got $$TKN$$ instead");
    SWAG_ERROR(Err0468, "missing [[#if]] boolean expression                $ expected the boolean expression after [[#if]], got $$TKN$$ instead");
    SWAG_ERROR(Err0469, "missing [[#import]] string argument               $ expected the module name to import after [[#import]], got $$TKN$$ instead");
    SWAG_ERROR(Err0470, "missing [[#include]] string argument              $ expected the file name to include after [[#include]], got $$TKN$$ instead");
    SWAG_ERROR(Err0471, "missing [[#main]]                                 $ the program entry point [[#main]] is missing");
    SWAG_ERROR(Err0472, "missing [[#message]] parameter                    $ [[#message]] requests a parameter of type [[Swag.CompilerMsgMask]]");
    SWAG_ERROR(Err0473, "missing [[#mix]] number                           $ a [[#mix]] variable name must end with a number such as [[#mix0]], [[#mix1]], etc.");
    SWAG_ERROR(Err0474, "missing [[#placeholder]] identifier               $ expected an identifier after [[#placeholder]], got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0477, "missing [[(]] before arguments                    $ expected [[(]] to start the list of arguments, got $$TKN$$ instead");
    SWAG_ERROR(Err0478, "missing [[(]] before parameters                   $ expected [[(]] to start the list of parameters, got $$TKN$$ instead");
    SWAG_ERROR(Err0479, "missing [[)]]                                     $ missing [[)]] to end the attribute arguments list");
    SWAG_ERROR(Err0480, "missing [[:]]                                     $ a [[:]] is expected here after the [[loop]] variable name [[%s]] $ the syntax is [[loop [variable:] count]]");
    SWAG_ERROR(Err0481, "missing [[case]] expression                       $ expected the [[case]] expression before [[%s]]");
    ...
    SWAG_ERROR(Err0483, "missing [[do]]                                    $ missing either [[do]] or [[{]] before $$TKN$$");
    SWAG_ERROR(Err0484, "missing [[if]] boolean expression                 $ expected a boolean expression after [[if]], got $$TKN$$ instead");
    SWAG_ERROR(Err0485, "missing [[impl]]                                  $ the function [[%s]] is part of the interface [[%s]] but lacks an [[impl]] marker");
    SWAG_ERROR(Err0486, "missing [[loop]] count                            $ expected the [[loop]] count expression before [[%s]]");
    SWAG_ERROR(Err0487, "missing [[switch]] value                          $ the value [[%s.%s]] is missing");
    SWAG_ERROR(Err0488, "missing [[visit]] expression                      $ expected the element to visit before [[%s]]");
    SWAG_ERROR(Err0489, "missing [[visit]] variable                        $ expected another [[visit]] variable between the [[comma]] ',' and the [[colon]] ':' $ the first [[visit]] variable is the [[value]], the second one is the [[index]]");
    SWAG_ERROR(Err0490, "missing [[while]] boolean expression              $ expected a boolean expression before [[%s]]");
    SWAG_ERROR(Err0491, "missing attribute                                 $ an [[opVisit]] function must be declared with the [[#[Swag.Macro]]] attribute");
    SWAG_ERROR(Err0492, "missing catch error                               $ expected [[try]], [[catch]] or [[assume]] to deal with the errors of [[%s]]");
    SWAG_ERROR(Err0493, "missing default value                             $ the %s needs a default value as a preceding parameter has one");
    SWAG_ERROR(Err0494, "missing dereference index                         $ an index is missing to dereference the array [[%s]] of type [[%s]] $ consider adding the index between brackets");
    SWAG_ERROR(Err0495, "missing dereference index                         $ an index is missing to dereference the slice [[%s]] of type [[%s]] $ consider adding the index between brackets");
    ...
    SWAG_ERROR(Err0503, "missing generic parameters                        $ the special function [[%s]] requires some generic parameters");
    SWAG_ERROR(Err0504, "missing generic parameters                        $ the struct [[%s]] can't be solved due to the generic field [[%s]]");
    SWAG_ERROR(Err0505, "missing generic parameters                        $ the struct [[%s]] should declare generic parameters because [[%s]] is generic");
    SWAG_ERROR(Err0506, "missing initialization                            $ a constant requires an explicit initialization");
    SWAG_ERROR(Err0507, "missing initialization                            $ a reference requires an explicit initialization");
    SWAG_ERROR(Err0508, "missing initialization                            $ a variable declared with [[let]] requires an explicit initialization");
    SWAG_ERROR(Err0509, "missing initialization                            $ an immutable variable requires an explicit initialization");
    ...
    SWAG_ERROR(Err0512, "missing interface function                        $ some functions of the interface [[%s]] are not implemented for [[%s]]");
    ...
    SWAG_ERROR(Err0523, "missing parameters                                $ the special function [[%s]] requires some parameters");
    SWAG_ERROR(Err0524, "missing return type                               $ a [[public]] function must explicitly define its return type");
    SWAG_ERROR(Err0525, "missing return type                               $ the function [[%s]] has the [[#[Swag.Discardable]]] attribute and should return something");
    SWAG_ERROR(Err0526, "missing return type                               $ the special function [[%s]] requires a return type");
    SWAG_ERROR(Err0527, "missing return type                               $ the special function [[%s]] should return a type [[%s]]");
    ...
    SWAG_ERROR(Err0530, "missing return value                              $ some control paths of %s do not return a value");
    ...
    SWAG_ERROR(Err0540, "misused UFCS                                      $ the %s [[%s]] can't be used as the first argument when calling [[%s]]");
    SWAG_ERROR(Err0541, "misused UFCS                                      $ the hidden [[with]] variable [[%s]] can't be used as the first argument when calling [[%s]]");
    SWAG_ERROR(Err0542, "non-contiguous [[#alias]]                         $ non-contiguous [[#alias]] in function [[%s]], missing [[#alias%u]]");
    SWAG_ERROR(Err0543, "non-transferable [[cvarargs]] type                $ a parameter with the type [[cvarargs]] can't be passed to another function");
    SWAG_ERROR(Err0544, "not a value                                       $ implicit reference to a static struct member of [[%s]]");
    SWAG_ERROR(Err0545, "not a value                                       $ the %s [[%s]] is not a valid value in this context");
    ...
    SWAG_ERROR(Err0555, "out of range [[#alias]] number                    $ an [[#alias]] number should be in the range [0, 31], got [[%u]] instead");
    SWAG_ERROR(Err0556, "out of range [[#mix]] number                      $ a [[#mix]] number should be in the range [0, 31], got [[%u]] instead");
    SWAG_ERROR(Err0557, "out of range [[#up]] count                        $ an [[#up]] count should be in the range [1, 255], got [[%u]] instead");
    SWAG_ERROR(Err0558, "out of range array dimensions                     $ array dimensions can't exceed [[254]]");
    ...
    SWAG_ERROR(Err0565, "out of range enum value                           $ the enum value [[%s]] exceeds the valid range of the type [[%s]]");
    SWAG_ERROR(Err0566, "out of range index                                $ the given index [[%I64u]] exceeds the maximum value [[%I64u]]");
    ...
    SWAG_ERROR(Err0568, "out of range number                               $ this literal number exceeds 64 bits and is out of allowable range");
    SWAG_ERROR(Err0569, "out of range slice                                $ the slicing bound [[%I64u]] is out of range (max is [[%I64u]])");
    SWAG_ERROR(Err0570, "out of range slice                                $ the slicing lower bound [[%I64u]] is greater than the upper bound [[%I64u]]");
    SWAG_ERROR(Err0571, "out of range slice                                $ the slicing upper bound exclusion with [[..<]] is not possible because it is zero");
    ...
    SWAG_ERROR(Err0581, "reserved name struct member                       $ the struct member name [[%s]] is reserved by the compiler $ struct member names beginning with [[item]] are reserved by the language");
    SWAG_ERROR(Err0582, "semantic cycle                                    $ a cycle has been detected during the resolution of %s [[%s]]");
    SWAG_ERROR(Err0583, "standalone expression                             $ an expression value should be used $ consider removing it");
    SWAG_ERROR(Err0584, "system error                                      $ failed to create the process [[%s]] (::CreatePipe)");
    SWAG_ERROR(Err0585, "system error                                      $ failed to create the process [[%s]]");
    ...
    SWAG_ERROR(Err0589, "too many generic parameters                       $ too many generic parameters for the special function [[%s]]");
    ...
    SWAG_ERROR(Err0593, "too many initializers                             $ too many initializers for the type [[%s]]");
    SWAG_ERROR(Err0594, "too many parameters                               $ too many parameters for [[%s]] ([[%d]] expected, [[%d]] provided)");
    SWAG_ERROR(Err0595, "tuple type mismatch                               $ the source and the requested tuples are not compatible");
    SWAG_ERROR(Err0596, "type mismatch                                     $ can't initialize type [[%s]] from type [[%s]]");
    SWAG_ERROR(Err0597, "type mismatch                                     $ conversion from a closure type to a lambda type is not allowed");
    SWAG_ERROR(Err0598, "type mismatch                                     $ conversion from a type value ([[typeinfo]]) to a compile type ([[%s]]) is not allowed");
    SWAG_ERROR(Err0599, "type mismatch                                     $ conversion from the type [[%s]] to the type [[%s]] is not allowed");
    ...
    SWAG_ERROR(Err0603, "type mismatch                                     $ type [[%s]] and type [[%s]] defined in the command line for [[%s]] are incompatible");
    SWAG_ERROR(Err0604, "type mismatch                                     $ unexpected mix of signed and unsigned type in a range expression $ lower bound and upper bound should be the same");
    SWAG_ERROR(Err0605, "type mismatch UFCS                                $ expected an UFCS argument of type [[%s]], got type [[%s]] instead");
    SWAG_ERROR(Err0606, "type mismatch argument                            $ [[#include]] requires an argument of type [[string]], got type [[%s]] instead");
    SWAG_ERROR(Err0607, "type mismatch argument                            $ [[#mixin]] requires an argument of type [[code]], got type [[%s]] instead");
    SWAG_ERROR(Err0608, "type mismatch argument                            $ [[where]] requires an argument of type [[bool]], got type [[%s]] instead");
    ...
    SWAG_ERROR(Err0616, "type mismatch argument                            $ the intrinsic [[@countof]] does not accept a tuple as an argument");
    SWAG_ERROR(Err0617, "type mismatch argument                            $ the intrinsic [[@countof]] requires an integer argument, got type [[%s]] instead");
    SWAG_ERROR(Err0618, "type mismatch argument                            $ the intrinsic [[@dataof]] does not accept a tuple as an argument");
    SWAG_ERROR(Err0619, "type mismatch argument                            $ the intrinsic [[@dataof]] does not accept an argument of type [[%s]]");
    SWAG_ERROR(Err0620, "type mismatch argument                            $ the intrinsic [[@mkany]] requires a [[typeinfo]] as a second argument, got type [[%s]] instead");
    SWAG_ERROR(Err0621, "type mismatch argument                            $ the intrinsic [[@mkcallback]] requires a pointer to function as a first argument, got type [[%s]] instead");
    SWAG_ERROR(Err0622, "type mismatch argument                            $ the intrinsic [[@mkinterface]] requires a pointer or a struct as a first argument, got type [[%s]] instead");
    SWAG_ERROR(Err0623, "type mismatch argument                            $ the intrinsic [[@mkinterface]] requires a type as a second argument, got type [[%s]] instead");
    SWAG_ERROR(Err0624, "type mismatch argument                            $ the intrinsic [[@mkinterface]] requires an interface name as a third argument, got type [[%s]] instead");
    SWAG_ERROR(Err0625, "type mismatch argument                            $ the intrinsic [[@mkstring]] requires a pointer to [[u8]] as a first argument, got type [[%s]] instead");
    SWAG_ERROR(Err0626, "type mismatch argument                            $ the intrinsic [[@runes]] requires an argument of type [[string]], got type [[%s]] instead");
    SWAG_ERROR(Err0627, "type mismatch argument const                      $ the intrinsic [[%s]] requires a mutable pointer as a first argument, got type [[%s]] instead");
    SWAG_ERROR(Err0628, "type mismatch argument const                      $ the intrinsic [[%s]] requires a mutable variable as a first argument, got type [[%s]] instead");
    SWAG_ERROR(Err0629, "type mismatch array                               $ can't declare a static array of type [[void]]");
    SWAG_ERROR(Err0630, "type mismatch array dimension                     $ the array dimension must be an integer, got type [[%s]] instead");
    SWAG_ERROR(Err0631, "type mismatch array index                         $ the array access index must be an integer, got type [[%s]] instead");
    SWAG_ERROR(Err0632, "type mismatch comparison                          $ comparison operations on tuples are not supported");
    SWAG_ERROR(Err0633, "type mismatch comparison                          $ the comparison operation [['%s']] does not accept the type [[%s]] as the left argument");
    SWAG_ERROR(Err0634, "type mismatch comparison                          $ the comparison operation [['%s']] does not accept the type [[%s]] as the right argument");
    SWAG_ERROR(Err0635, "type mismatch comparison                          $ the comparison operation [['%s']] does not accept the type [[%s]]");
    SWAG_ERROR(Err0636, "type mismatch comparison                          $ unexpected comparison of a slice and [[%s]] $ a slice comparison is only allowed with [[null]]");
    SWAG_ERROR(Err0637, "type mismatch comparison                          $ unexpected comparison of an interface and [[%s]] $ an interface comparison is only allowed with [[null]], another interface or with a [[typeinfo]]");
    SWAG_ERROR(Err0638, "type mismatch comparison                          $ unexpected comparison of the type [[any]] and the type [[%s]] $ an [[any]] comparison is only allowed with [[null]] or with a [[typeinfo]]");
    SWAG_ERROR(Err0639, "type mismatch const                               $ casting from an immutable type [[%s]] to a mutable one [[%s]] is not allowed");
    SWAG_ERROR(Err0640, "type mismatch const UFCS                          $ the UFCS argument should be mutable but is not (type is [[%s]])");
    ...
    SWAG_ERROR(Err0643, "type mismatch generic parameter                   $ tuple types are not supported for generic parameters");
    SWAG_ERROR(Err0644, "type mismatch operation                           $ the bit inversion operation [['~']] does not accept the type [[%s]]");
    SWAG_ERROR(Err0645, "type mismatch operation                           $ the operation [['%s']] does not accept the type [[%s]] as the left argument");
    SWAG_ERROR(Err0646, "type mismatch operation                           $ the operation [['%s']] does not accept the type [[%s]] as the right argument");
    SWAG_ERROR(Err0647, "type mismatch operation                           $ the operation [['%s']] does not accept the type [[%s]]");
    SWAG_ERROR(Err0648, "type mismatch operation                           $ the operation [['%s']] does not accept tuple types");
    SWAG_ERROR(Err0649, "type mismatch operation                           $ the operation [['%s']] is not allowed because the left expression is an enum type ([[%s]]) not marked with [[#[Swag.EnumFlags]]]");
    SWAG_ERROR(Err0650, "type mismatch operation                           $ the operation [['%s']] is not allowed because the right expression is an enum type ([[%s]]) not marked with [[#[Swag.EnumFlags]]]");
    SWAG_ERROR(Err0651, "type mismatch operation                           $ the operation [['%s']] requires an integer, got type [[%s]] instead");
    SWAG_ERROR(Err0652, "type mismatch pointer                             $ conversion from a value pointer [[%s]] to a block pointer [[%s]] is not allowed");
    SWAG_ERROR(Err0653, "type mismatch pointer                             $ conversion from type [[%s]] to a pointer type is not allowed");
    SWAG_ERROR(Err0654, "type mismatch return                              $ an [[#ast]] block must return a type [[string]], got type [[%s]] instead");
    SWAG_ERROR(Err0655, "type mismatch return                              $ the return value has already been deduced to be nothing, and here it's the type [[%s]]");
    SWAG_ERROR(Err0656, "type mismatch return                              $ the return value has already been deduced to have the type [[%s]], and here it's the type [[%s]]");
    SWAG_ERROR(Err0657, "type mismatch return callback                     $ the intrinsic [[@mkcallback]] does not accept a function returning the type [[%s]]");
    SWAG_ERROR(Err0658, "unclosed comment                                  $ this multi-line comment is missing its closing [['*/']]");
    ...
    SWAG_ERROR(Err0661, "unexpected [[const]]                              $ unexpected [[const]] before a lambda parameter name");
    SWAG_ERROR(Err0662, "unexpected [[default]]                            $ a [[switch]] marked with the [[#[Swag.Complete]]] attribute can't have a [[default]] statement");
    ...
    SWAG_ERROR(Err0672, "unexpected affectation                            $ unexpected usage of the assignment operator [['=']] inside an expression $ if you are trying to compare values, you should use [['==']] instead");
    SWAG_ERROR(Err0673, "unexpected aliased names                          $ unexpected variable aliases when calling [[%s]] because it's not a macro or a mixin");
    SWAG_ERROR(Err0674, "unexpected attribute return type                  $ an attribute can't have a return type starting with [[->]]");
    SWAG_ERROR(Err0675, "unexpected comparison                             $ expected an affectation with [[=]] but found the comparison operator [['==']] instead");
    SWAG_ERROR(Err0676, "unexpected default value                          $ default value for variadic parameters are not allowed");
    SWAG_ERROR(Err0677, "unexpected function body                          $ a function tagged with the [[#[Swag.Foreign]]] attribute can't have a body");
    SWAG_ERROR(Err0678, "unexpected function overload                      $ a function with the same name [[%s]] has already been declared $ if you want to declare an overload, mark all functions with [[#[Swag.Overload]]]");
    ...
    SWAG_ERROR(Err0692, "unexpected return value                           $ a function with the [[#[Swag.CalleeReturn]]] attribute can't declare a return value");
    ...
    SWAG_ERROR(Err0701, "unexpected visit name                             $ a visit specialization name is only valid to visit a struct, and here we are visiting [[%s]]");
    ...
    SWAG_ERROR(Err0709, "unknown function interface                        $ the function [[%s]] can't be found in the interface [[%s]]");
    SWAG_ERROR(Err0710, "unknown identifier                                $ the identifier [[%s]] can't be found in [[%s]] or in %s [[%s]]");
    SWAG_ERROR(Err0711, "unknown identifier                                $ the identifier [[%s]] can't be found in [[%s]]");
    SWAG_ERROR(Err0712, "unknown identifier                                $ the identifier [[%s]] can't be found in the tuple");
    SWAG_ERROR(Err0713, "unknown identifier                                $ the identifier [[%s]] can't be found");
    ...
    SWAG_ERROR(Err0721, "unknown visit                                     $ the type [[%s]] can't be visited with [[visit]] $ the special function [[opVisit]] can't be found");
    SWAG_ERROR(Err0722, "unknown visit name                                $ the visit specialization name [[%s]] can't be found in [[%s]]");
    SWAG_ERROR(Err0723, "unpack number mismatch                            $ trying to unpack [[%u]] variables but the tuple has [[%u]] fields");
    SWAG_ERROR(Err0724, "unpack number mismatch                            $ trying to unpack [[%u]] variables but the tuple has only [[%u]] fields");
    ...
    SWAG_ERROR(Err0729, "unresolved foreign call                           $ failed to load the module [[%s]] while resolving the foreign function [[%s]]");
    ...
    SWAG_ERROR(Err0733, "unsuitable struct for union                       $ the struct [[%s]] is not suitable for union because it contains an [[%s]] function");
