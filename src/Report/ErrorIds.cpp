...
void initErrors()
{
    ...
    SWAG_ERROR(Err0054, "const mismatch                                    $ the intrinsic [[%s]] requires a mutable pointer as a first argument, got [[%s]] instead");
    SWAG_ERROR(Err0055, "const mismatch                                    $ the intrinsic [[%s]] requires a mutable variable as a first argument, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0110, "forbidden copy                                    $ the copy of [[%s]] is disabled because the struct is marked with [[#[Swag.NoCopy]]]");
    SWAG_ERROR(Err0111, "generic symbol already defined                    $ the generic symbol [[%s]] has already been defined in a parent scope");
    SWAG_ERROR(Err0112, "generic type mismatch                             $ expected type [[%s]] for the generic %s, got [[%s]] instead");
    SWAG_ERROR(Err0113, "identifier expected                               $ expected a function call after [[%s]], got $$TKN$$ instead");
    SWAG_ERROR(Err0114, "immutable global variable                         $ the global variable [[%s]] is immutable at compile-time because it's initialized to 0");
    SWAG_ERROR(Err0115, "incomplete [[case]] expression list               $ expected another expression after [[,]] and before [[:]] $ [[,]] indicates a list of [[case]] expressions; provide the next expression or consider removing the [[,]]");
    SWAG_ERROR(Err0116, "incomplete [[switch]]                             $ the value [[%s.%s]] is missing");
    SWAG_ERROR(Err0117, "incomplete alias list                             $ expected another alias name after [[,]] $ either provide an additional alias name or remove the trailing [[,]]");
    SWAG_ERROR(Err0118, "incomplete arguments list                         $ expected another call argument after [[,]] $ either provide an additional argument or remove the trailing [[,]]");
    SWAG_ERROR(Err0119, "incomplete array dimensions                       $ expected another dimension after [[,]] $ either provide an additional dimension or remove the trailing [[,]]");
    SWAG_ERROR(Err0120, "incomplete binary number format                   $ binary literals require at least one [[0]] or [[1]] digit");
    SWAG_ERROR(Err0121, "incomplete character literal                      $ expected a closing back-tick [[`]] before the end of the line");
    SWAG_ERROR(Err0122, "incomplete hexadecimal number format              $ hexadecimal literals require at least one valid digit ([[0-9]], [[A-F]], [[a-f]])");
    SWAG_ERROR(Err0123, "incomplete import version                         $ the [[#import]] version has build number [[%d]] but lacks a revision");
    SWAG_ERROR(Err0124, "incomplete import version                         $ the [[#import]] version has revision [[%d]] but lacks a major version");
    SWAG_ERROR(Err0125, "incomplete indexing                               $ expected another expression after [[,]] $ either provide an additional expression or remove the trailing [[,]]");
    SWAG_ERROR(Err0126, "incomplete interface                              $ some functions of the interface [[%s]] are not implemented for [[%s]]");
    SWAG_ERROR(Err0127, "incomplete interface function                     $ the interface member [[%s]] should have at least [[self]] as the first parameter $ consider declaring the interface function with [[mtd]] instead of [[func]]");
    SWAG_ERROR(Err0128, "incomplete parameters list                        $ expected another parameter after [[,]] $ either provide an additional parameter or remove the trailing [[,]]");
    SWAG_ERROR(Err0129, "incomplete string literal                         $ expected a closing quotation mark [[\"]] before the end of the line $ if you want a multi-line string, use the syntax [[\"\"\"string\"\"\"]]");
    SWAG_ERROR(Err0130, "incomplete tuple unpacking                        $ expected another variable name after [[,]] $ either provide an additional variable name or remove the trailing [[,]]");
    SWAG_ERROR(Err0131, "incomplete variables list                         $ expected another variable name after [[,]] $ either provide an additional variable name or remove the trailing [[,]]");
    SWAG_ERROR(Err0132, "incorrect import location                         $ the [[#import]] location has an incorrect format $ expecting [[location=\"mode@accesspath\"]] where mode is [[swag]] or [[disk]]");
    SWAG_ERROR(Err0133, "index out of bounds                               $ the given index [[%I64u]] exceeds the maximum value [[%I64u]]");
    SWAG_ERROR(Err0134, "infinite [[while]]                                $ the while condition is always [[true]], which creates an infinite loop $ if you intentionally want to create an infinite loop, consider using a loop without argument [[loop { ... }]]");
    SWAG_ERROR(Err0135, "invalid [[#alias]] number                         $ a [[#alias]] variable name should end with a number, got [[%s]] instead");
    SWAG_ERROR(Err0136, "invalid [[#global]] instruction                   $ [[%s]] is not recognized as a valid [[#global]] instruction");
    SWAG_ERROR(Err0137, "invalid [[#import]] location                      $ expected a [[#import]] location path, got [[%]] instead");
    SWAG_ERROR(Err0138, "invalid [[#import]] location                      $ expected a [[#import]] version as a string, got [[%]] instead");
    SWAG_ERROR(Err0139, "invalid [[#mix]] number                           $ a [[#mix]] variable name should end with a number, got [[%s]] instead");
    SWAG_ERROR(Err0140, "invalid [[#mixin]] replacement keyword            $ a [[#mixin]] block accepts only [[break]] and [[continue]] as replacements, got $$TKN$$ instead");
    SWAG_ERROR(Err0141, "invalid [[#scope]] identifier                     $ expected the [[#scope]] identifier, got $$TKN$$ instead");
    SWAG_ERROR(Err0142, "invalid [[#up]] count                             $ expected an integer literal for the [[#up]] count, got $$TKN$$ instead");
    SWAG_ERROR(Err0143, "invalid [[#up]] count                             $ the [[#up]] count can't be zero");
    SWAG_ERROR(Err0144, "invalid [[#up]] count                             $ the [[#up]] count should be an untyped integer in the range [1, 255], got [[%s]] instead");
    SWAG_ERROR(Err0145, "invalid [[#up]] move                              $ [[#up]] can't move [[%d]] scopes up");
    SWAG_ERROR(Err0146, "invalid [[@alignof]]                              $ can't compute the alignment of a generic expression");
    SWAG_ERROR(Err0147, "invalid [[@countof]] argument                     $ the intrinsic [[@countof]] requires a positive argument, got [[%I64d]] instead");
    SWAG_ERROR(Err0148, "invalid [[@countof]] argument                     $ the intrinsic [[@countof]] requires a positive argument, got [[%d]] instead");
    SWAG_ERROR(Err0149, "invalid [[@countof]] argument                     $ the intrinsic [[@countof]] requires an integer argument, got [[%s]] instead");
    SWAG_ERROR(Err0150, "invalid [[@cvaarg]] type                          $ a variadic argument of type [[%s]] has been promoted to type [[%s]] at the call site");
    SWAG_ERROR(Err0151, "invalid [[@sizeof]]                               $ can't compute the size of a generic expression");
    SWAG_ERROR(Err0152, "invalid [[break]] scope name                      $ expected a scope name or a end of line after [[break]], got $$TKN$$ instead");
    SWAG_ERROR(Err0153, "invalid [[code]] append                           $ the %s [[%s]] wants to interpret the next statement as a [[code]] parameter but this is not possible inside a [[%s]] expression");
    SWAG_ERROR(Err0154, "invalid [[defer]] mode                            $ [[%s]] is not a valid [[defer]] special mode $ the only valid special modes are [[err]] and [[noerr]]");
    SWAG_ERROR(Err0155, "invalid [[discard]] call                          $ a function returning nothing can't be discarded");
    SWAG_ERROR(Err0156, "invalid [[discard]] call                          $ expected a discard-able call after [[discard]], got $$TKN$$ instead");
    SWAG_ERROR(Err0157, "invalid [[impl]]                                  $ expected a struct name after [[for]] but [[%s]] is %s");
    SWAG_ERROR(Err0158, "invalid [[impl]]                                  $ expected a struct name or an enum name after [[impl]] but [[%s]] is [[%s]]");
    SWAG_ERROR(Err0159, "invalid [[impl]]                                  $ expected an interface name after [[impl]] but [[%s]] is %s");
    SWAG_ERROR(Err0160, "invalid [[namealias]]                             $ can't alias a struct member");
    SWAG_ERROR(Err0161, "invalid [[opVisit]] name                          $ an [[opVisit]] variant should be a valid identifier, got [[%s]] instead");
    SWAG_ERROR(Err0162, "invalid [[orelse]] type                           $ the [[orelse]] operator does not accept a [[struct]] as an argument");
    SWAG_ERROR(Err0163, "invalid [[orelse]] type                           $ the [[orelse]] operator does not accept the type [[%s]] as an argument");
    SWAG_ERROR(Err0164, "invalid [[retval]]                                $ the [[retval]] special type is not allowed in a function that returns nothing");
    SWAG_ERROR(Err0165, "invalid [[switch]] block content                  $ expected [[case]] or [[default]], got $$TKN$$ instead");
    SWAG_ERROR(Err0166, "invalid [[switch]] type                           $ [[switch]] does not accept an expression of type [[%s]]");
    SWAG_ERROR(Err0167, "invalid [[visit]] name                            $ expected a [[visit]] specialized name, got $$TKN$$ instead");
    SWAG_ERROR(Err0168, "invalid [[visit]] type                            $ a tuple can't be visited");
    ...
    SWAG_ERROR(Err0179, "invalid address                                   $ can't take the address of a constant of type [[%s]] $ this type of constant does not have an associated memory storage");
    SWAG_ERROR(Err0180, "invalid address                                   $ can't take the address of a function parameter of type [[%s]] $ this type of parameter does not have an associated memory storage");
    SWAG_ERROR(Err0181, "invalid address                                   $ can't take the address of a function returned value of type [[%s]]");
    SWAG_ERROR(Err0182, "invalid address                                   $ can't take the address of a macro");
    SWAG_ERROR(Err0183, "invalid address                                   $ can't take the address of a mixin");
    SWAG_ERROR(Err0184, "invalid address                                   $ can't take the address of a variable declared with [[let]] $ consider using [[var]] instead of [[let]] for mutability");
    SWAG_ERROR(Err0185, "invalid address                                   $ can't take the address of an in-lined function");
    SWAG_ERROR(Err0186, "invalid address                                   $ can't take the address of the right expression");
    SWAG_ERROR(Err0187, "invalid alias name                                $ expected an identifier for name aliasing, got $$TKN$$ instead");
    SWAG_ERROR(Err0188, "invalid alias name                                $ expected the alias name after [[%s]], got $$TKN$$ instead");
    SWAG_ERROR(Err0189, "invalid align value                               $ the [[#[Swag.Align]]] argument must be a power of two, got [[%d]]");
    SWAG_ERROR(Err0190, "invalid argument                                  $ [[#include]] requires an argument of type [[string]], got [[%s]] instead");
    SWAG_ERROR(Err0191, "invalid argument                                  $ [[#mixin]] requires an argument of type [[code]], got [[%s]] instead");
    SWAG_ERROR(Err0192, "invalid argument                                  $ [[%s]] requires an argument of type [[bool]], got [[%s]] instead");
    SWAG_ERROR(Err0193, "invalid argument                                  $ invalid argument [[%s]] for the attribute [[#[Swag.Optim]]] $ the acceptable values for [[#[Swag.Optim]]] are [[bytecode]] and [[backend]]");
    SWAG_ERROR(Err0194, "invalid argument                                  $ the [[%s]] count must be an integer, got [[%s]] instead");
    SWAG_ERROR(Err0195, "invalid argument                                  $ the intrinsic [[%s]] requires a block pointer as a first argument if the number of values is greater than one (type is [[%s]])");
    SWAG_ERROR(Err0196, "invalid argument                                  $ the intrinsic [[%s]] requires a block pointer as a first argument if the number of values is variable (type is [[%s]])");
    SWAG_ERROR(Err0197, "invalid argument                                  $ the intrinsic [[%s]] requires a block pointer as a first argument, got [[%s]] instead");
    SWAG_ERROR(Err0198, "invalid argument                                  $ the intrinsic [[%s]] requires a pointer as a first argument, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0219, "invalid attribute name                            $ expected an attribute name, got $$TKN$$ instead");
    SWAG_ERROR(Err0220, "invalid attribute usage                           $ expected [[,]] for a new attribute name or [[]]] to end the list, got $$TKN$$ instead");
    SWAG_ERROR(Err0221, "invalid attribute usage                           $ [[AttributeUsage.Gen]] can only be associated with [[AttributeUsage.Struct]] or [[AttributeUsage.Enum]]");
    ...
    SWAG_ERROR(Err0238, "invalid comparison                                $ comparison operations on tuples are not supported");
    SWAG_ERROR(Err0239, "invalid comparison                                $ the comparison operation [[%s]] does not accept the type [[%s]] as the left argument");
    SWAG_ERROR(Err0240, "invalid comparison                                $ the comparison operation [[%s]] does not accept the type [[%s]] as the right argument");
    SWAG_ERROR(Err0241, "invalid comparison                                $ the comparison operation [[%s]] does not accept the type [[%s]]");
    SWAG_ERROR(Err0242, "invalid comparison                                $ unexpected comparison of a slice and [[%s]] $ a slice comparison is only allowed with [[null]]");
    SWAG_ERROR(Err0243, "invalid comparison                                $ unexpected comparison of an interface and [[%s]] $ an interface comparison is only allowed with [[null]], another interface or with a [[typeinfo]]");
    SWAG_ERROR(Err0244, "invalid comparison                                $ unexpected comparison of type [[any]] and [[%s]] $ an [[any]] comparison is only allowed with [[null]] or with a type");
    SWAG_ERROR(Err0245, "invalid compiler directive                        $ [[%s]] is not a valid compiler directive starting with [[#]]");
    SWAG_ERROR(Err0246, "invalid compiler instruction scope                $ compiler instructions do not belong to a specific scope");
    ...
    SWAG_ERROR(Err0250, "invalid constant name                             $ expected the constant name after [[const]], got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0266, "invalid enum identifier                           $ expected an enum value identifier, got $$TKN$$ instead");
    SWAG_ERROR(Err0267, "invalid enum name                                 $ expected the enum name, got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0285, "invalid expression                                $ expected an expression, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0294, "invalid function declaration                      $ expected [[func]] or [[mtd]] to declare an interface function, got [[%s]] instead $ to declare a function within an interface, use [[func]] or [[mtd]]");
    SWAG_ERROR(Err0295, "invalid function name                             $ a function name ([[%s]]) can't start with [[@]], this is reserved for intrinsics");
    SWAG_ERROR(Err0296, "invalid function name                             $ expected a function name, got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0298, "invalid generic                                   $ a generic struct instantiation of a partial type alias ([[%s]]) is not supported");
    ...
    SWAG_ERROR(Err0306, "invalid generic argument                          $ invalid usage of %s ([[%s]]) as a generic argument $ a generic argument must be a literal or a type");
    ...
    SWAG_ERROR(Err0308, "invalid generic parameter name                    $ expected an identifier as the generic name, got $$TKN$$ instead");
    SWAG_ERROR(Err0309, "invalid generic type                              $ a generic type does not accept a specific type starting with [[:]]");
    SWAG_ERROR(Err0310, "invalid hexadecimal number format                 $ hexadecimal literals can only contain digits [[0-9]], [[A-F]], and [[a-f]], got [[%s]] instead");
    SWAG_ERROR(Err0311, "invalid identifier                                $ expected an identifier, got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0315, "invalid instrinsic scope                          $ intrinsics do not belong to a specific scope");
    SWAG_ERROR(Err0316, "invalid interface conversion                      $ [[%s]] (or a [[using]] field) doesn't implement [[%s]], so struct-to-interface cast is not allowed");
    SWAG_ERROR(Err0317, "invalid interface function                        $ the first parameter should be [[self]], got [[%s]] instead $ consider declaring the interface function with [[mtd]] instead of [[func]]");
    SWAG_ERROR(Err0318, "invalid intrinsic                                 $ [[%s]] is not a valid intrinsic starting with [[@]]");
    SWAG_ERROR(Err0319, "invalid lambda call                               $ invalid lambda call because [[%s]] is not a variable, it is %s");
    SWAG_ERROR(Err0320, "invalid literal                                   $ invalid number conversion for literal [[%I64u]]");
    SWAG_ERROR(Err0321, "invalid literal suffix                            $ the symbol [[%s]] is %s and can't be used as a literal type suffix");
    SWAG_ERROR(Err0322, "invalid literal suffix                            $ the type [[%s]] can't be used as a literal type suffix");
    SWAG_ERROR(Err0323, "invalid literal suffix                            $ the type [[%s]] can't be used as a suffix for a float literal $ only [[f32]] and [[f64]] are accepted");
    SWAG_ERROR(Err0324, "invalid literal to struct conversion              $ can't initialize a type [[%s]] with [[%s]] because no corresponding [[opAffectLiteral]] can be found");
    SWAG_ERROR(Err0325, "invalid location argument                         $ invalid [[location]] mode [[%s]] $ the acceptable values are [[swag]] and [[disk]]");
    SWAG_ERROR(Err0326, "invalid logical operator                          $ expected [[%s]] for the logical test, got [[%s]] instead");
    SWAG_ERROR(Err0327, "invalid match argument                            $ invalid argument [[%s]] for the attribute [[#[Swag.Match]]] $ the acceptable values for [[#[Swag.Match]]] are [[where]] and [[self]]");
    SWAG_ERROR(Err0328, "invalid module name                               $ the [[#[Swag.Foreign]]] module name can't be empty");
    SWAG_ERROR(Err0329, "invalid module name                               $ unexpected symbol [[.]] in the [[#[Swag.Foreign]]] module name $ a [[#[Swag.Foreign]]] module name can't have an extension, consider removing it");
    SWAG_ERROR(Err0330, "invalid move                                      $ [[move]] is not applicable on an immutable expression (type is [[%s]])");
    SWAG_ERROR(Err0331, "invalid multiple variable                         $ [[using]] does not support multiple variable declarations");
    SWAG_ERROR(Err0332, "invalid name alias                                $ a name alias should not be used on %s");
    SWAG_ERROR(Err0333, "invalid named argument                            $ expected a named argument before [[:]], got [[%s]] instead");
    SWAG_ERROR(Err0334, "invalid namespace body                            $ missing the namespace body or instruction instead of [[;]]");
    SWAG_ERROR(Err0335, "invalid namespace name                            $ expected the namespace name, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0343, "invalid number prefix                             $ expected [[0x]] for hexadecimal or [[0b]] for binary, got [[%s]] instead");
    SWAG_ERROR(Err0344, "invalid opaque struct                             $ a struct marked with [[#[Swag.Opaque]]] requires a [[public]] access");
    SWAG_ERROR(Err0345, "invalid opaque struct                             $ the struct can't be marked with [[#[Swag.Opaque]]] because the whole file is exported with [[#global export]]");
    ...
    SWAG_ERROR(Err0353, "invalid operator modifier                         $ [[%s]] is not recognized as a valid instruction modifier");
    ...
    SWAG_ERROR(Err0356, "invalid parameter name                            $ expected a parameter name, got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0367, "invalid reference to [[with]] block               $ an identifier is expected after [[.]], got $$TKN$$ instead");
    SWAG_ERROR(Err0368, "invalid return type                               $ a function can't return type [[%s]]");
    SWAG_ERROR(Err0369, "invalid return type                               $ a function return of type [[void]] is invalid $ just omit the return type if you want the function to return nothing");
    ...
    SWAG_ERROR(Err0378, "invalid struct name                               $ expected the struct name, got $$TKN$$ instead");
    SWAG_ERROR(Err0379, "invalid struct operation                          $ can't affect by index a type [[%s]] to [[%s]] because no corresponding [[opIndexAffect]] can be found");
    SWAG_ERROR(Err0380, "invalid struct operation                          $ the special function [[%s]] is missing in [[%s]] to solve operator [[%s]]");
    SWAG_ERROR(Err0381, "invalid struct operation                          $ the special function [[%s]] is missing in [[%s]]");
    SWAG_ERROR(Err0382, "invalid throw type                                $ throw needs a struct or [[@err()]] as an expression, but there's nothing here");
    SWAG_ERROR(Err0383, "invalid throw type                                $ throw needs a struct or [[@err()]] as an expression, got [[%s]] instead");
    SWAG_ERROR(Err0384, "invalid top-level instruction                     $ expected a top-level instruction, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0392, "invalid type                                      $ the operator [[>>]] requires an integer, got [[%s]] instead");
    SWAG_ERROR(Err0393, "invalid type                                      $ the second parameter of [[%s]] can't be of type [[%s]] $ consider using [[opAssign]] if you want to make a copy");
    SWAG_ERROR(Err0394, "invalid type                                      $ the special function [[%s]] expects a [[bool]] as a generic parameter, got [[%s]] instead");
    SWAG_ERROR(Err0395, "invalid type                                      $ the special function [[%s]] expects a [[string]] as a generic parameter, got [[%s]] instead");
    SWAG_ERROR(Err0396, "invalid type                                      $ the type [[%s]] can't be used as a type for an attribute parameter");
    SWAG_ERROR(Err0397, "invalid type                                      $ the type [[%s]] can't be used as an argument to [[@cvaarg]]");
    SWAG_ERROR(Err0398, "invalid type                                      $ the type [[%s]] does not accept a [[const]] version");
    SWAG_ERROR(Err0399, "invalid type                                      $ the type is generic and can't be evaluated in this context");
    ...
    SWAG_ERROR(Err0402, "invalid type declaration                          $ expected a type declaration, got $$TKN$$ instead");
    SWAG_ERROR(Err0403, "invalid variable name                             $ expected a variable name, got $$TKN$$ instead");
    SWAG_ERROR(Err0404, "invalid type declaration                          $ expected the array type after its dimensions, got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0406, "invalid type suffix                               $ expected an identifier or a type after the start of a type suffix, got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0411, "invalid variable name                             $ expected the variable name after [[%s]], got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0417, "invalid visit                                     $ can't find the [[%s]] visit specialization in [[%s]]");
    SWAG_ERROR(Err0418, "invalid visit                                     $ can't visit a pointer of type [[%s]]");
    SWAG_ERROR(Err0419, "invalid visit                                     $ can't visit a variadic type by pointer");
    SWAG_ERROR(Err0420, "invalid visit                                     $ can't visit an enum content by pointer");
    SWAG_ERROR(Err0421, "invalid visit                                     $ can't visit type [[%s]]");
    SWAG_ERROR(Err0422, "invalid visit                                     $ the type [[%s]] can't be visited with [[visit]]");
    SWAG_ERROR(Err0423, "invalid void dereference                          $ dereferencing a pointer to [[void]] is not valid $ [[void]] does not have a valid size");
    SWAG_ERROR(Err0424, "literal overflow                                  $ can't negate number [[%I64d]] because [[%I64u]] is too large for type [[s64]]");
    SWAG_ERROR(Err0425, "literal overflow                                  $ can't negate number [[%d]] because [[%u]] is too large for type [[%s]]");
    ...
    SWAG_ERROR(Err0442, "misplaced [[#index]]                              $ the keyword [[#index]] is only valid within a breakable block such as [[loop]], [[visit]], [[for]], etc.");
    ...
    SWAG_ERROR(Err0454, "misplaced [[@spread]]                             $ the intrinsic [[@spread]] can only be used as a direct function argument");
    ...
    SWAG_ERROR(Err0458, "misplaced [[const]]                               $ expected [[self]] after [[const]] when declaring a function parameter, got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0465, "misplaced [[moveref]]                             $ [[moveref]] can only be used before a function argument");
    SWAG_ERROR(Err0466, "misplaced [[moveref]]                             $ unexpected [[moveref]] on a compile-time value $ [[moveref]] should be used on a pointer or a reference");
    SWAG_ERROR(Err0467, "misplaced [[moveref]]                             $ unexpected [[moveref]] on an immutable expression");
    SWAG_ERROR(Err0468, "misplaced [[moveref]]                             $ unexpected [[moveref]] on type [[%s]] $ [[moveref]] should be used on a pointer or a reference");
    SWAG_ERROR(Err0469, "misplaced [[mtd]]                                 $ [[mtd]] is only valid within a struct implementation block $ you could replace [[mtd]] with [[func]]");
    SWAG_ERROR(Err0470, "misplaced [[mtd]] in enum                         $ [[mtd]] is only valid within a struct implementation block $ you could replace [[mtd]] with [[func]]");
    SWAG_ERROR(Err0471, "misplaced [[retval]]                              $ the [[retval]] special type can only be used within a function body");
    SWAG_ERROR(Err0472, "misplaced [[self]]                                $ the [[self]] keyword is only valid within an [[impl]] block of an enum or a struct");
    SWAG_ERROR(Err0473, nullptr);
    SWAG_ERROR(Err0474, "misplaced [[using]]                               $ [[using]] can't be associated with type [[%s]] $ expected a namespace, a struct, an enum or a variable");
    SWAG_ERROR(Err0475, nullptr);
    SWAG_ERROR(Err0476, "misplaced [[using]]                               $ [[using]] is not valid on a variable with type [[%s]]");
    SWAG_ERROR(Err0477, nullptr);
    SWAG_ERROR(Err0478, "misplaced [[using]]                               $ [[using]] on a pointer variable is only valid for struct pointers, got a pointer to [[%s]] instead");
    SWAG_ERROR(Err0479, "misplaced [[using]]                               $ [[using]] on a variable is not allowed in [[%s]] scope");
    SWAG_ERROR(Err0480, "misplaced [[using]] in interface                  $ [[using]] can't be associated with an interface member");
    ...
    SWAG_ERROR(Err0488, "misplaced attribute                               $ the %s can't have the [[#[Swag.Implicit]]] attribute because it is generic");
    SWAG_ERROR(Err0489, "misplaced attribute                               $ the [[#[Swag.CalleeReturn]]] attribute can't be applied to function [[%s]] $ [[#[Swag.CalledReturn]]] can only be applied to a macro ([[#[Swag.Macro]]]) or a mixin ([[#[Swag.Mixin]]])");
    SWAG_ERROR(Err0490, "misplaced attribute                               $ the [[#[Swag.Complete]]] attribute can't be applied to function [[%s]] $ #[Swag.Complete] can only be applied to [[opAffect]] and [[opAffectLiteral]]");
    SWAG_ERROR(Err0491, "misplaced attribute                               $ the [[#[Swag.Discardable]]] attribute can only be applied to lambda variables, got [[%s]]");
    SWAG_ERROR(Err0492, "misplaced attribute                               $ the [[#[Swag.Implicit]]] attribute can't be applied to function [[%s]] $ [[#[Swag.Implicit]]] can only be applied to [[opAffect]], [[opAffectLiteral]] and [[opCast]]");
    SWAG_ERROR(Err0493, "misplaced attribute                               $ the attribute [[%s]] can only be applied to %s");
    ...
    SWAG_ERROR(Err0501, "misplaced compiler directive                      $ [[#callerlocation]] can only be used when declaring a function parameter");
    SWAG_ERROR(Err0502, "misplaced error catching                          $ [[%s]] can only be associated with function calls, but [[%s]] is %s");
    SWAG_ERROR(Err0503, "misplaced error catching                          $ [[%s]] can't be nested in a [[%s]] expression $ you should use only one [[try]], [[catch]] or [[assume]]");
    SWAG_ERROR(Err0504, "misplaced error catching                          $ treating a return error with [[%s]] can only be done inside a function, macro or mixin");
    SWAG_ERROR(Err0505, "misplaced error catching                          $ unexpected [[%s]] because the function [[%s]] can't raise an error");
    ...
    SWAG_ERROR(Err0513, "misplaced special function                        $ the struct special function [[%s]] should be declared within the corresponding [[impl]] block");
    SWAG_ERROR(Err0514, "misplaced top-level [[using]]                     $ a top-level [[using]] must be placed before any other declarations");
    SWAG_ERROR(Err0515, "misplaced tuple unpacking                         $ tuple unpacking is not allowed in %s");
    ...
    SWAG_ERROR(Err0524, "missing [[#if]] boolean expression                $ expected the boolean expression after [[#if]], got $$TKN$$ instead");
    SWAG_ERROR(Err0525, "missing [[#import]] string argument               $ expected the module name to import after [[#import]], got $$TKN$$ instead");
    SWAG_ERROR(Err0526, "missing [[#include]] string argument              $ expected the file name to include after [[#include]], got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0530, "missing [[#placeholder]] identifier               $ expected an identifier after [[#placeholder]], got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0533, "missing [[(]] before parameters                   $ expected [[(]] to start the list of parameters, got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0542, "missing [[visit]] variable                        $ expected another [[visit]] variable between [[,]] and [[:]] $ the first [[visit]] variable is the value, the second one is the index");
    SWAG_ERROR(Err0543, "missing [[while]] boolean expression              $ expected a boolean expression before [[%s]]");
    SWAG_ERROR(Err0544, "missing arguments                                 $ the %s should have arguments but none were provided");
    SWAG_ERROR(Err0545, "missing arguments                                 $ the [[#[Swag.Using]]] attribute should have at least one argument");
    ...
    SWAG_ERROR(Err0549, "missing closing [[%s]]                            $ expected symbol [[%s]], got $$TKN$$ instead");
    ...
    SWAG_ERROR(Err0558, "missing function parameters                       $ expected the function parameters between [[()]] before the start of the body [[{]] $ if you want to declare a function without parameters, consider adding [[()]] after the name");
    SWAG_ERROR(Err0559, "missing generic arguments                         $ can't instantiate %s because of missing generic arguments");
    SWAG_ERROR(Err0560, "missing generic arguments                         $ can't instantiate the generic function [[%s]] because of missing generic arguments");
    SWAG_ERROR(Err0561, "missing generic arguments                         $ can't instantiate the generic struct [[%s]] because of missing generic arguments");
    SWAG_ERROR(Err0562, "missing generic parameters                        $ expected at least one generic parameter between [[()]] $ consider removing [[()]] if this is not generic");
    SWAG_ERROR(Err0563, "missing generic parameters                        $ the special function [[%s]] expects some generic parameters");
    SWAG_ERROR(Err0564, "missing generic parameters                        $ the struct [[%s]] can't be solved due to the generic field [[%s]]");
    SWAG_ERROR(Err0565, "missing generic parameters                        $ the struct [[%s]] should declare generic parameters because [[%s]] is generic");
    SWAG_ERROR(Err0566, "missing initialization                            $ a constant requires an explicit initialization");
    SWAG_ERROR(Err0567, "missing initialization                            $ a reference requires an explicit initialization");
    SWAG_ERROR(Err0568, "missing initialization                            $ a variable declared with [[let]] requires an explicit initialization");
    SWAG_ERROR(Err0569, "missing initialization                            $ an immutable variable requires an explicit initialization");
    SWAG_ERROR(Err0570, "missing initialization                            $ expected an initialization of [[%s]] because [[%s]] does not contain a value for zero");
    SWAG_ERROR(Err0571, "missing initialization                            $ the enumeration value [[%s]] has type [[%s]] which requires an explicit initialization");
    SWAG_ERROR(Err0572, "missing intrinsic arguments                       $ expected the intrinsic arguments between [[()]]");
    ...
    SWAG_ERROR(Err0580, "missing return type                               $ the special function [[%s]] requires a return type");
    SWAG_ERROR(Err0581, "missing return type                               $ the special function [[%s]] should return a type [[%s]]");
    SWAG_ERROR(Err0582, "missing return value                              $ expected a return value because the return type has already been deduced to be [[%s]]");
    SWAG_ERROR(Err0583, "missing return value                              $ expected an expression of type [[%s]] after [[return]]");
    SWAG_ERROR(Err0584, "missing return value                              $ not all control paths of %s returns a value");
    SWAG_ERROR(Err0585, "missing return value                              $ the %s returns a type [[%s]] but lacks a [[return]] in its body");
    SWAG_ERROR(Err0586, "missing slicing upper bound                       $ expected an upper bound expression after [[..<]] $ if you want to slice to the end, consider using [[..]] instead of [[..<]]");
    ...
    SWAG_ERROR(Err0594, "non-transferable [[cvarargs]] type                $ a parameter with the type [[cvarargs]] can't be passed to another function");
    SWAG_ERROR(Err0595, "not a value                                       $ %s [[%s]] is not a valid value in this context");
    SWAG_ERROR(Err0596, "not a value                                       $ implicit reference to a static struct member of [[%s]]");
    SWAG_ERROR(Err0597, "not enough arguments                              $ not enough arguments for %s");
    SWAG_ERROR(Err0598, "not enough arguments                              $ not enough arguments to call %s");
    SWAG_ERROR(Err0599, "not enough generic arguments                      $ not enough generic arguments for %s");
    SWAG_ERROR(Err0600, "not enough generic parameters                     $ not enough generic parameters for special function [[%s]]");
    SWAG_ERROR(Err0601, "not enough initializers                           $ expected [[%d]] value(s) to initialize the array, got [[%d]] instead");
    SWAG_ERROR(Err0602, "not enough initializers                           $ expected [[%d]] value(s) to initialize the tuple, got [[%d]] instead");
    SWAG_ERROR(Err0603, "not enough parameters                             $ not enough parameters for special function [[%s]] (expected [[%d]], got [[%d]])");
    ...
    SWAG_ERROR(Err0611, "out of range character literal                    $ can't convert the character literal [[0x%x]] to [[u16]], this is out of range");
    SWAG_ERROR(Err0612, "out of range character literal                    $ can't convert the character literal [[0x%x]] to [[u8]], this is out of range");
    SWAG_ERROR(Err0613, "out of range enum value                           $ the enum value [[%s]] exceeds the valid range of type [[%s]]");
    SWAG_ERROR(Err0614, "out of range inline level                         $ can't expand [[%s]] because the limit defined with [[--limit-inline:%d]] has been reached");
    SWAG_ERROR(Err0615, "out of range number                               $ this literal number exceeds 64 bits and is out of allowable range");
    SWAG_ERROR(Err0616, "out of range slicing                              $ the slicing bound [[%I64u]] is out of range (max is [[%I64u]])");
    SWAG_ERROR(Err0617, "out of range slicing                              $ the slicing lower bound [[%I64u]] is greater than the upper bound [[%I64u]]");
    SWAG_ERROR(Err0618, "out of range slicing                              $ the slicing upper bound exclusion with [[..<]] is not possible because it is zero");
    SWAG_ERROR(Err0619, "overload mismatch                                 $ found [[%d]] overloads for [[%s]], but none matched the provided arguments");
    SWAG_ERROR(Err0620, "recursive evaluation                              $ the struct [[%s]] references itself, which is causing a recursion");
    SWAG_ERROR(Err0621, "redundant access modifier                         $ the [[%s]] access modifier is unnecessary because of [[#global export]] $ consider removing the [[public]] modifier, as [[#global export]] ensures all declarations in this file are publicly accessible");
    SWAG_ERROR(Err0622, "reserved function name                            $ the function name [[%s]] is reserved by the compiler");
    SWAG_ERROR(Err0623, "reserved identifier                               $ the identifier [[%s]] starts with [[__]] $ identifiers starting with [[__]] are typically reserved for compiler-internal or system use");
    SWAG_ERROR(Err0624, "reserved namespace name                           $ the namespace [[%s]] is reserved by the compiler");
    SWAG_ERROR(Err0625, "reserved struct member name                       $ the struct member name [[%s]] is reserved by the compiler $ struct member names beginning with [[item]] are reserved by the language");
    SWAG_ERROR(Err0626, "return type mismatch                              $ an [[#ast]] block must return a string, got [[%s]] instead");
    SWAG_ERROR(Err0627, "return type mismatch                              $ the return type has already been deduced to be [[%s]], and here it's [[%s]]");
    SWAG_ERROR(Err0628, "return type mismatch                              $ the return type has already been deduced to be nothing, and here it's [[%s]]");
    SWAG_ERROR(Err0629, "semantic cycle                                    $ a cycle has been detected during the resolution of %s [[%s]]");
    SWAG_ERROR(Err0630, "standalone expression                             $ an expression value should be used $ consider removing it");
    SWAG_ERROR(Err0631, "symbol already defined                            $ the %s [[%s]] has already been defined %s");
    SWAG_ERROR(Err0632, "symbol already defined                            $ the %s [[%s]] has already been defined in a parent scope");
    SWAG_ERROR(Err0633, "system error                                      $ failed to create the process [[%s]] (::CreatePipe)");
    SWAG_ERROR(Err0634, "system error                                      $ failed to create the process [[%s]]");
    SWAG_ERROR(Err0635, "too many arguments                                $ expected [[%d]] argument(s), got [[%d]] instead");
    SWAG_ERROR(Err0636, "too many generic arguments                        $ expected [[%d]] generic argument(s) for %s [[%s]], got [[%d]] instead");
    SWAG_ERROR(Err0637, "too many generic parameters                       $ expected one single generic parameter for special function [[%s]], got [[%d]] instead");
    SWAG_ERROR(Err0638, "too many generic parameters                       $ too many generic parameters for special function [[%s]]");
    SWAG_ERROR(Err0639, "too many initializers                             $ expected [[%d]] value(s) to initialize [[%s]], got [[%d]] instead");
    SWAG_ERROR(Err0640, "too many initializers                             $ expected [[%d]] value(s) to initialize the array, got [[%d]] instead");
    ...
    SWAG_ERROR(Err0648, "type mismatch                                     $ casting from [[%s]] to [[%s]] is not allowed");
    SWAG_ERROR(Err0649, "type mismatch                                     $ casting from [[%s]] to a pointer type is not allowed");
    SWAG_ERROR(Err0650, "type mismatch                                     $ casting from a type value ([[typeinfo]]) to a compile type ([[%s]]) is not allowed");
    SWAG_ERROR(Err0651, "type mismatch                                     $ casting from a value pointer [[%s]] to a block pointer [[%s]] is not allowed");
    SWAG_ERROR(Err0652, "type mismatch                                     $ casting from closure to lambda is not allowed");
    ...
    SWAG_ERROR(Err0655, "type mismatch                                     $ expected type [[%s]] for the UFCS argument, got [[%s]] instead");
    SWAG_ERROR(Err0656, "type mismatch                                     $ expected type [[%s]], got [[%s]] instead");
    SWAG_ERROR(Err0657, "type mismatch                                     $ expected type [[*Swag.CVaList]], got [[%s]] instead");
    SWAG_ERROR(Err0658, "type mismatch                                     $ the [[%s]] assignment requires an expression of type [[%s]], got [[%s]] instead");
    SWAG_ERROR(Err0659, "type mismatch                                     $ type [[%s]] and type [[%s]] defined in the command line for [[%s]] are incompatible");
    SWAG_ERROR(Err0660, "unauthorized function overload                    $ a function with the same name [[%s]] has already been declared $ if you want to declare an overload, mark all functions with [[#[Swag.Overload]]]");
    SWAG_ERROR(Err0661, "unexpected [[#elif]]                              $ [[#elif]] found without a preceding [[#if]] directive");
    SWAG_ERROR(Err0662, "unexpected [[#else]]                              $ [[#else]] found without a preceding [[#if]] or [[#elif]] directive");
    ...
    SWAG_ERROR(Err0676, "unexpected [[where,call)]] for struct             $ [[where:call]] is not allowed for a struct, but is only valid for functions $ consider using a single [[where]] instead");
    SWAG_ERROR(Err0677, "unexpected [[where]]                              $ a [[where]] constraint is not allowed on special function [[%s]] $ [[where]] constraints are not supported for [[opDrop]], [[opPostCopy]], and [[opPostMove]]");
    SWAG_ERROR(Err0678, "unexpected [[}]]                                  $ symbol [[}]] found without a preceding [[{]]");
    SWAG_ERROR(Err0679, "unexpected affectation                            $ unexpected usage of the assignment operator [[=]] inside an expression");
    SWAG_ERROR(Err0680, "unexpected aliased names                          $ unexpected alias when calling [[%s]] because it's not a macro or a mixin");
    SWAG_ERROR(Err0681, "unexpected attribute return type                  $ an attribute can't have a return type starting with [[->]]");
    SWAG_ERROR(Err0682, "unexpected comparison                             $ expected an affectation with [[=]] but found the comparison operator [[==]] instead");
    SWAG_ERROR(Err0683, "unexpected default value                          $ default value for variadic parameters are not allowed");
    SWAG_ERROR(Err0684, "unexpected empty expression                       $ the expression [[()]] is empty and not allowed here");
    SWAG_ERROR(Err0685, "unexpected end of comment                         $ unexpected end of multi-line comment [[*/]] after %s $ the beginning [[/*]] of the multi-line comment is missing");
    SWAG_ERROR(Err0686, "unexpected end of file within comment             $ this multi-line comment is missing its closing [[*/]]");
    SWAG_ERROR(Err0687, "unexpected function body                          $ a function tagged with the [[#[Swag.Foreign]]] attribute can't have a body");
    SWAG_ERROR(Err0688, "unexpected generic arguments                      $ expected a name without generic arguments");
    SWAG_ERROR(Err0689, "unexpected generic arguments                      $ unexpected generic arguments after %s");
    SWAG_ERROR(Err0690, "unexpected generic function                       $ the function [[%s]] appears to be generic despite the [[#[Swag.NotGeneric]]] attribute");
    SWAG_ERROR(Err0691, "unexpected generic parameters                     $ a function interface can't have generic parameters");
    SWAG_ERROR(Err0692, "unexpected generic parameters                     $ the function [[%s]] can't have generic parameters because of the [[#[Swag.NotGeneric]]] attribute");
    SWAG_ERROR(Err0693, "unexpected generic parameters                     $ the special function [[%s]] can't have generic parameters");
    ...
    SWAG_ERROR(Err0701, "unexpected return value                           $ a %s can't return something");
    ...
    SWAG_ERROR(Err0709, "unexpected type declaration                       $ unexpected type declaration after unnamed parameters");
    ...
    SWAG_ERROR(Err0717, "unknown file                                      $ the [[#load]] file [[%s]] can't be located or accessed");
    ...
    SWAG_ERROR(Err0733, "unknown relocation offset                         $ can't find the struct member [[%s]] to compute the relocation");
    SWAG_ERROR(Err0734, "unknown struct                                    $ the struct [[%s]] can't be found");
    SWAG_ERROR(Err0735, "unknown type                                      $ the type [[%s]] can't be found");
    SWAG_ERROR(Err0736, "unpacking number mismatch                         $ trying to unpack [[%u]] variable(s) but the tuple has [[%u]] field(s)");
    SWAG_ERROR(Err0737, "unpacking number mismatch                         $ trying to unpack [[%u]] variable(s) but the tuple has only [[%u]] field(s)");
    SWAG_ERROR(Err0738, "unresolved identifier                             $ the identifier [[%s]] can't be solved");
    SWAG_ERROR(Err0739, "unresolved interface                              $ the interface can't be solved due to the generic type [[%s]]");
    SWAG_ERROR(Err0740, "unsuitable struct for union                       $ the struct [[%s]] is not suitable for union because it contains an [[%s]] function");
    SWAG_ERROR(Err0741, "unsupported [[@nameof]] argument                  $ can't evaluate the [[@nameof]] argument");
    SWAG_ERROR(Err0742, "unsupported [[@stringof]] argument                $ can't evaluate the [[@stringof]] argument");
    SWAG_ERROR(Err0743, "unsupported array type                            $ can't declare a static array of type [[void]]");
    SWAG_ERROR(Err0744, "unsupported callback                              $ the intrinsic [[@mkcallback]] does not allow a function returning [[%s]]");
    SWAG_ERROR(Err0745, "unsupported callback                              $ the intrinsic [[@mkcallback]] does not allow a function with more than [[%d]] parameters, found [[%d]]");
    SWAG_ERROR(Err0746, "unsupported generic                               $ unsupported generic type deduction, this is too complicated (embedded tuples)");
    SWAG_ERROR(Err0747, "unsupported index type                            $ the array access index must be an integer, got [[%s]] instead");
    SWAG_ERROR(Err0748, "unsupported tuple type                            $ tuple types are not supported for generic parameters");
    SWAG_ERROR(Err0749, "unsupported type                                  $ the intrinsic [[@countof]] does not accept a tuple as an argument");
    SWAG_ERROR(Err0750, "unsupported type                                  $ the intrinsic [[@dataof]] does not accept a tuple as an argument");
    SWAG_ERROR(Err0751, "unsupported type                                  $ the intrinsic [[@dataof]] does not accept an argument of type [[%s]]");
    SWAG_ERROR(Err0752, "unsupported variadic parameters                   $ an inline function can't have variadic parameters");
    SWAG_ERROR(Err0753, "unused alias name                                 $ the alias name [[%s]] has no corresponding variable $ consider removing it");
