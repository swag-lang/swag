...
void initErrors()
{
    ...
    SWAG_ERROR(Err0116, "identifier expected                               $ expected a function call after [[%s]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0138, "invalid [[#alias]] number                         $ [[#alias]] variable names should end with a number, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0142, "invalid [[#mixin]] number                         $ [[#mixin]] variable names should end with a number, got [[%s]] instead");
    SWAG_ERROR(Err0143, "invalid [[#mixin]] replacement keyword            $ a [[#mixin]] block accepts only [[break]] and [[continue]] as replacements, got [[%s]] instead");
    SWAG_ERROR(Err0144, "invalid [[#scope]] identifier                     $ expected the [[#scope]] identifier, got [[%s]] instead");
    SWAG_ERROR(Err0145, "invalid [[#up]] count                             $ expected an integer literal for the [[#up]] count, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0155, "invalid [[break]] scope name                      $ expected a scope name or a end of line after [[break]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0159, "invalid [[discard]] call                          $ expected a discard-able call after [[discard]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0164, "invalid [[opVisit]] name                          $ an [[opVisit]] variant should be a valid identifier, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0168, "invalid [[switch]] block content                  $ expected [[case]] or [[default]], got [[%s]] instead");
    SWAG_ERROR(Err0169, "invalid [[switch]] type                           $ [[switch]] does not accept an expression of type [[%s]]");
    SWAG_ERROR(Err0170, "invalid [[visit]] name                            $ expected a [[visit]] specialized name, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0188, "invalid alias name                                $ expected an identifier for name aliasing, got [[%s]] instead");
    SWAG_ERROR(Err0189, "invalid alias name                                $ expected the alias name after [[%s]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0247, "invalid const [[&&]]                              $ the move reference type [[&&]] can't be combined with an immutable expression");
    SWAG_ERROR(Err0248, "invalid constant                                  $ the constant creation failed due to the generic type [[%s]]");
    SWAG_ERROR(Err0249, "invalid constant name                             $ expected the constant name after [[const]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0262, "invalid embedded instruction                      $ expected an embedded instruction or a curly block, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0265, "invalid enum identifier                           $ expected an enum value identifier, got [[%s]] instead");
    SWAG_ERROR(Err0266, "invalid enum name                                 $ expected the enum name, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0272, "invalid enum type                                 $ the enum slice type [[%s]] should be declared as [[const]] $ consider declaring the type with [[const %s]] instead");
    ...
    SWAG_ERROR(Err0280, "invalid export argument                           $ invalid argument [[%s]] for the attribute [[#[Swag.ExportType]]] $ the acceptable values for [[#[Swag.ExportType]]] are [[methods]] and [[nozero]]");
    SWAG_ERROR(Err0281, "invalid expression                                $ expected an expression after %s, got [[%s]] instead");
    SWAG_ERROR(Err0282, "invalid expression                                $ expected an expression after operator [[%s]], got [[%s]] instead $ consider adding [[(]] after [[%s]] to start an expression");
    SWAG_ERROR(Err0283, "invalid expression                                $ expected an expression, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0293, "invalid function declaration                      $ expected [[func]] or [[mtd]] to declare an interface function, got [[%s]] instead $ to declare a function within an interface, use [[func]] or [[mtd]]");
    ...
    SWAG_ERROR(Err0307, "invalid generic parameter name                    $ expected an identifier as the generic name, got [[%s]] instead");
    SWAG_ERROR(Err0308, "invalid hexadecimal number format                 $ hexadecimal literals can only contain digits [[0-9]], [[A-F]], and [[a-f]], got [[%s]] instead");
    SWAG_ERROR(Err0309, "invalid namespace body                            $ missing the namespace body or instruction instead of [[;]]");
    SWAG_ERROR(Err0310, "invalid identifier                                $ expected an identifier, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0315, "invalid interface function                        $ the first parameter should be [[self]], got [[%s]] instead $ consider declaring the interface function with [[mtd]] instead of [[func]]");
    ...
    SWAG_ERROR(Err0329, "invalid named argument                            $ expected a named argument before [[:]], got [[%s]] instead");
    SWAG_ERROR(Err0330, "invalid namespace name                            $ expected the namespace name, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0338, "invalid number prefix                             $ expected [[0x]] for hexadecimal or [[0b]] for binary, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0348, "invalid operation                                 $ the operation [[%s]] is not allowed because the left expression is an enum type ([[%s]]) not marked with [[#[Swag.EnumFlags]]]");
    SWAG_ERROR(Err0349, "invalid operation                                 $ the operation [[%s]] is not allowed because the right expression is an enum type ([[%s]]) not marked with [[#[Swag.EnumFlags]]]");
    ...
    SWAG_ERROR(Err0353, "invalid argument                                  $ invalid argument [[%s]] for the attribute [[#[Swag.Optim]]] $ the acceptable values for [[#[Swag.Optim]]] are [[bytecode]] and [[backend]]");
    ...
    SWAG_ERROR(Err0381, "invalid top-level instruction                     $ expected a top-level instruction, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0390, "invalid type                                      $ the second parameter of [[%s]] can't be of type [[%s]] $ consider using [[opAssign]] if you want to make a copy");
    ...
    SWAG_ERROR(Err0400, "invalid type declaration                          $ expected a type declaration, got [[%s]] instead");
    SWAG_ERROR(Err0401, "invalid type declaration                          $ expected a type in the tuple definition, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0409, "invalid variable name                             $ expected the variable name after [[%s]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0457, "misplaced [[const]]                               $ expected [[self]] after [[const]] when declaring a function parameter, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0472, "misplaced [[using]]                               $ [[using]] can't be associated with type [[%s]] $ expected a namespace, a struct, an enum or a variable");
    ...
    SWAG_ERROR(Err0494, "misplaced attribute                               $ unexpected start of an attribute after [[%s]] $ attributes should be placed before access modifiers; consider moving the attribute before [[%s]]");
    SWAG_ERROR(Err0495, "misplaced attribute                               $ unexpected usage of the attribute [[%s]] in this context");
    SWAG_ERROR(Err0496, "misplaced closure                                 $ a closure definition is not valid at the global level $ closures can only be defined within functions");
    ...
    SWAG_ERROR(Err0500, "misplaced error catching                          $ [[%s]] can't be nested in a [[%s]] expression $ you should use only one [[try]], [[catch]] or [[assume]]");
    ...
    SWAG_ERROR(Err0503, "misplaced function declaration                    $ functions can't be directly declared inside structs $ functions associated with structs should be declared inside [[impl]] blocks");
    ...
    SWAG_ERROR(Err0508, "misplaced special function                        $ the struct special function [[%s]] should be declared within the corresponding [[impl]] block $ an [[impl for]] block can only be used to implement interfaces");
    ...
    SWAG_ERROR(Err0517, "missing [[#foreignlib]] string argument           $ expected the library name after [[#foreignlib]], got [[%s]] instead $ use [[#foreignlib \"path/to/library\"]] to specify the library to import");
    SWAG_ERROR(Err0518, "missing [[#global testerror]] string argument     $ expected the error string to check after [[#global testerror]], got [[%s]] instead");
    SWAG_ERROR(Err0519, "missing [[#global testwarning]] string argument   $ expected the warning string to check after [[#global testwarning]], got [[%s]] instead");
    SWAG_ERROR(Err0520, "missing [[#if]] boolean expression                $ expected a boolean expression before [[%s]]");
    SWAG_ERROR(Err0521, "missing [[#import]] string argument               $ expected the module name to import after [[#import]], got [[%s]] instead");
    SWAG_ERROR(Err0522, "missing [[#include]] string argument              $ expected the file name to include after [[#include]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0526, "missing [[#placeholder]] identifier               $ expected an identifier after [[#placeholder]], got [[%s]] instead");
    SWAG_ERROR(Err0527, "missing [[#type]] type                            $ expected a valid type after [[#type]], got [[%s]] instead");
    SWAG_ERROR(Err0528, "missing [[(]] before arguments                    $ expected [[(]] to start the list of arguments, got [[%s]] instead");
    SWAG_ERROR(Err0529, "missing [[(]] before parameters                   $ expected [[(]] to start the list of parameters, got [[%s]] instead");
    SWAG_ERROR(Err0530, "missing [[:]]                                     $ a [[:]] is expected here after the [[loop]] variable name [[%s]] $ syntax is [[loop [variable:] count]]");
    SWAG_ERROR(Err0531, "missing [[case]] expression                       $ expected the [[case]] expression before [[%s]]");
    SWAG_ERROR(Err0532, "missing [[closure]] capture variable              $ expected another capture variable between [[,]] and [[|]] $ add another variable or consider removing the trailing comma");
    ...
    SWAG_ERROR(Err0538, "missing [[visit]] variable                        $ expected another [[visit]] variable between [[,]] and [[:]] $ the first [[visit]] variable is the value, the second one is the index");
    ...
    SWAG_ERROR(Err0546, "missing constant type                             $ expected a type with [[:]] or an assignment with [[=]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0549, "missing dereference index                         $ an index is missing to dereference the slice [[%s]] of type [[%s]] $ consider adding the index between brackets");
    SWAG_ERROR(Err0550, "missing end of line                               $ expected a line break or a [[;]] to close the %s, got [[%s]] instead $ consider adding a line break or separating the previous statement with a [[;]]");
    ...
    SWAG_ERROR(Err0554, "missing function parameters                       $ expected the function parameters between [[()]] before the start of the body [[{]] $ if you want to declare a function without parameters, consider adding [[()]] after the name");
    ...
    SWAG_ERROR(Err0558, "missing generic parameters                        $ expected at least one generic parameter between [[()]] $ consider removing [[()]] if this is not generic");
    ...
    SWAG_ERROR(Err0569, "missing lambda parameter type                     $ expected the type with [[:]] or an assignment with [[=]] $ when declaring lambda parameters, you should not combined inferred and explicit types");
    SWAG_ERROR(Err0570, "missing named argument                            $ the %s should be named because some arguments before are named");
    SWAG_ERROR(Err0571, "missing parameter type                            $ expected the type with [[:]] or an assignment with [[=]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0584, "missing variable type                             $ expected the type with [[:]] or an assignment with [[=]], got [[%s]] instead");
    ...
    SWAG_ERROR(Err0613, "overload mismatch                                 $ found [[%d]] overloads for [[%s]], but none matched the provided arguments");
    SWAG_ERROR(Err0614, "recursive evaluation                              $ the struct [[%s]] references itself, which is causing a recursion");
    SWAG_ERROR(Err0615, "redundant access modifier                         $ the [[%s]] access modifier is unnecessary because of [[#global export]] $ consider removing the [[public]] modifier, as [[#global export]] ensures all declarations in this file are publicly accessible");
    ...
    SWAG_ERROR(Err0672, "unexpected [[var]] in struct                      $ [[var]] is unnecessary in struct variable declarations $ struct variables should be declared as [[fieldName: Type]] without [[var]]");
    ...
    SWAG_ERROR(Err0698, "unexpected tuple unpacking name                   $ expected a variable name or [[?]] when unpacking a tuple, got [[%s]] instead");
    ...
    SWAG_ERROR(Err0718, "unknown identifier                                $ the identifier [[%s]] is preceded with [[.]] but no corresponding [[enum]] or [[with]] can be deduced");
    ...
    SWAG_ERROR(Err0747, "unused return value                               $ the return value of the function [[%s]] should be used $ if you don't need the return value, consider prefixing the call with [[discard]]");
