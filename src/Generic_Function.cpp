...
bool Generic::instantiateDefaultGenericFunc(SemanticContext* context)
{
        ...
        if (!identifier->genericParameters)
        {
            ...
            if (nodeFunc->genericParameters)
            {
                ...
                for (auto p : nodeFunc->genericParameters->childs)
                {
                    ...
                    if (!param->assignment)
                    {
                        Diagnostic diag{node->sourceFile, node->token, Fmt(Err(Err0557), identifier->resolvedSymbolName->name.c_str())};
    ...
    if (context->node->kind == AstNodeKind::Identifier)
    {
        ...
        if (idRef->resolvedSymbolOverload)
        {
            ...
            if (concreteType->isStruct())
            {
                ...
                if (contextualNode)
                {
                    Diagnostic diag{node->sourceFile, node->token, Fmt(Err(Err0557), node->token.ctext())};
    ...
    return context->report({node->sourceFile, node->token, Fmt(Err(Err0301), node->token.ctext())});
}