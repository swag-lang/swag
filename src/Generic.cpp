...
bool Generic::instantiateStruct(SemanticContext* context, AstNode* genericParameters, OneGenericMatch& match)
{
    ...
    SWAG_VERIFY(!match.genericReplaceTypes.empty(), context->report({node, Utf8::format(g_E[Err0039], node->token.ctext())}));
    ...
    SWAG_VERIFY(sourceNode->kind == AstNodeKind::StructDecl, context->report({node, Utf8::format(g_E[Err0040], node->token.ctext())}));
...
bool Generic::instantiateFunction(SemanticContext* context, AstNode* genericParameters, OneGenericMatch& match, bool selectIf)
{
    ...
    if (noReplaceTypes)
    {
        ...
        {
            if (contextualNode)
                return context->report({contextualNode, Utf8::format(g_E[Err0041], node->token.ctext())});
            return context->report({node, Utf8::format(g_E[Err0042], node->token.ctext())});
        }
    }
