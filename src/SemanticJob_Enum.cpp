...
bool SemanticJob::resolveEnum(SemanticContext* context)
{
    ...
    {
        Diagnostic  diag({node, Fmt(g_E[Err0820], node->resolvedSymbolName->name.c_str())});
        ...
        if (!assignNode && (isFlags || (valNode != firstEnumValue)))
        {
            ...
            case NativeTypeKind::U8:
                if (enumNode->computedValue->reg.u8 == UINT8_MAX)
                    return context->report({valNode, Fmt(g_E[Err0708], valNode->token.ctext())});
                if (isFlags && enumNode->computedValue->reg.u8)
                {
                    auto n = enumNode->computedValue->reg.u8;
                    SWAG_VERIFY((n & (n - 1)) == 0, context->report({valNode, Fmt(g_E[Err0709], valNode->token.ctext())}));
            ...
            case NativeTypeKind::U16:
                if (enumNode->computedValue->reg.u16 == UINT16_MAX)
                    return context->report({valNode, Fmt(g_E[Err0710], valNode->token.ctext())});
            ...
            case NativeTypeKind::U32:
                if (enumNode->computedValue->reg.u32 == UINT32_MAX)
                    return context->report({valNode, Fmt(g_E[Err0712], valNode->token.ctext())});
            ...
            case NativeTypeKind::UInt:
                if (enumNode->computedValue->reg.u64 == UINT64_MAX)
                    return context->report({valNode, Fmt(g_E[Err0714], valNode->token.ctext())});
            ...
            case NativeTypeKind::Int:
                if (enumNode->computedValue->reg.s64 <= INT64_MIN || enumNode->computedValue->reg.s64 >= INT64_MAX)
                    return context->report({valNode, Fmt(g_E[Err0719], valNode->token.ctext())});
