...
bool Parser::doAttrDecl(AstNode* parent, AstNode** result)
{
    ...
    *result       = attrNode;
    ...
    SWAG_VERIFY(token.id != TokenId::SymMinusGreat, error(token, Err(Err0679), Nte(Nte0020)));
    SWAG_VERIFY(token.id != TokenId::KwdThrow, error(token, Err(Err0674), Nte(Nte0020)));
...
bool Parser::doAttrUse(AstNode* parent, AstNode** result, bool single)
{
    ...
    *result            = attrBlockNode;
    ...
    while (token.id == TokenId::SymAttrStart)
    {
        ...
        while (token.id == TokenId::Identifier)
        {
            ...
            if (token.id != TokenId::SymRightSquare)
            {
                ...
                SWAG_VERIFY(token.id != TokenId::SymLeftParen, error(token, Err(Err0544)));
                SWAG_CHECK(checkIsIdentifier(token, Fmt(Err(Err0219), token.ctext())));
            }
        }
    ...
    // :AttrUseLastChild
    SWAG_VERIFY(!attrBlockNode->childs.empty(), error(attrBlockNode, Err(Err0264)));
