...
bool SyntaxJob::doEnum(AstNode* parent, AstNode** result)
{
    ...
    Scope* newScope = nullptr;
    {
        ...
        if (newScope->kind != ScopeKind::Enum)
        {
            ...
            Diagnostic  diag{implNode->identifier, Utf8::format(Msg0441, Scope::getNakedKindName(newScope->kind), implNode->token.text.c_str(), Scope::getNakedKindName(ScopeKind::Enum))};
            Diagnostic  note{enumNode, Utf8::format(Note027, implNode->token.text.c_str()), DiagnosticLevel::Note};
