...
bool ByteCodeRun::executeMathIntrinsic(JobContext* context, ByteCodeInstruction* ip, Register& ra, const Register& rb, const Register& rc)
{
    ...
    case ByteCodeOp::IntrinsicF32x2:
    {
        ...
        case TokenId::IntrinsicPow:
            ...
            if (isnan(ra.f32))
                return context->report({ip->node, Utf8::format(g_E[Err0417], rb.f32, rc.f32)});
            break;
        case TokenId::IntrinsicMin:
    ...
    case ByteCodeOp::IntrinsicF64x2:
    {
        ...
        case TokenId::IntrinsicPow:
            ...
            if (isnan(ra.f64))
                return context->report({ip->node, Utf8::format(g_E[Err0417], rb.f64, rc.f64)});
            break;
        case TokenId::IntrinsicMin:
    ...
    case ByteCodeOp::IntrinsicF32x1:
    {
        ...
        case TokenId::IntrinsicSqrt:
            if (rb.f32 < 0)
                return context->report(g_E[Hnt0001], {ip->node, Utf8::format(g_E[Err0425], rb.f32)});
            ra.f32 = sqrtf(rb.f32);
            if (isnan(ra.f32))
                return context->report({ip->node, Utf8::format(g_E[Err0425], rb.f32)});
            break;
        ...
        case TokenId::IntrinsicASin:
            ra.f32 = asinf(rb.f32);
            if (isnan(ra.f32))
                return context->report({ip->node, Utf8::format(g_E[Err0426], rb.f32)});
            break;
        case TokenId::IntrinsicACos:
            ra.f32 = acosf(rb.f32);
            if (isnan(ra.f32))
                return context->report({ip->node, Utf8::format(g_E[Err0427], rb.f32)});
            break;
        ...
        case TokenId::IntrinsicLog:
            ra.f32 = log(rb.f32);
            if (isnan(ra.f32))
                return context->report({ip->node, Utf8::format(g_E[Err0428], rb.f32)});
            break;
        case TokenId::IntrinsicLog2:
            ra.f32 = log2(rb.f32);
            if (isnan(ra.f32))
                return context->report({ip->node, Utf8::format(g_E[Err0423], rb.f32)});
            break;
        case TokenId::IntrinsicLog10:
            ra.f32 = log10(rb.f32);
            if (isnan(ra.f32))
                return context->report({ip->node, Utf8::format(g_E[Err0424], rb.f32)});
            break;
    ...
    case ByteCodeOp::IntrinsicF64x1:
    {
        ...
        case TokenId::IntrinsicSqrt:
            if (rb.f64 < 0)
                return context->report(g_E[Hnt0001], {ip->node, Utf8::format(g_E[Err0425], rb.f64)});
            ra.f64 = sqrt(rb.f64);
            if (isnan(ra.f64))
                return context->report({ip->node, Utf8::format(g_E[Err0425], rb.f64)});
            break;
        case TokenId::IntrinsicSin:
            ra.f64 = sin(rb.f64);
            break;
        ...
        case TokenId::IntrinsicASin:
            ra.f64 = asin(rb.f64);
            if (isnan(ra.f64))
                return context->report({ip->node, Utf8::format(g_E[Err0426], rb.f64)});
            break;
        case TokenId::IntrinsicACos:
            ra.f64 = acos(rb.f64);
            if (isnan(ra.f64))
                return context->report({ip->node, Utf8::format(g_E[Err0427], rb.f64)});
            break;
        case TokenId::IntrinsicATan:
            ra.f64 = atan(rb.f64);
            break;
        case TokenId::IntrinsicLog:
            ra.f64 = log(rb.f64);
            if (isnan(ra.f64))
                return context->report({ip->node, Utf8::format(g_E[Err0428], rb.f64)});
            break;
        case TokenId::IntrinsicLog2:
            ra.f64 = log2(rb.f64);
            if (isnan(ra.f64))
                return context->report({ip->node, Utf8::format(g_E[Err0423], rb.f64)});
            break;
        case TokenId::IntrinsicLog10:
            ra.f64 = log10(rb.f64);
            if (isnan(ra.f64))
                return context->report({ip->node, Utf8::format(g_E[Err0424], rb.f64)});
            break;
        case TokenId::IntrinsicFloor:
            ra.f64 = floor(rb.f64);
            break;
