...
SWAG_ERROR(Err0020, "missing arguments `()` when calling %s")
...
SWAG_ERROR(Err0043, "cannot instantiate the variable because the type `%s` is generic")
...
SWAG_ERROR(Err0057, "unexpected generic parameter `%d` for %s (a value is expected but a type is provided)")
...
SWAG_ERROR(Err0135, "a backtick can only be used inside an inline or a macro block")
SWAG_ERROR(Err0136, "a backtick on an identifier can only be used inside a `Swag.Macro` function or an `#inline` block")
...
SWAG_ERROR(Err0469, "cannot take the address of a constant")
...
SWAG_ERROR(Err0822, "`using` on a variable with a pointer type can only be used on a pointer to a struct (and type is `%s`)")
