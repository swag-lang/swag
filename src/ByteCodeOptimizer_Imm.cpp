...
bool ByteCodeOptimizer::optimizePassImmediate(ByteCodeOptContext* context)
{
    ...
    for (auto ip = context->bc->out; ip->op != ByteCodeOp::End; ip++)
    {
        ...
        if (flags & OPFLAG_READ_A && !(ip->hasFlag(BCI_IMM_A)))
            regs.remove(ip->a.u32);
        if (flags & OPFLAG_READ_B && !(ip->hasFlag(BCI_IMM_B)))
            regs.remove(ip->b.u32);
        if (flags & OPFLAG_READ_C && !(ip->hasFlag(BCI_IMM_C)))
            regs.remove(ip->c.u32);
        if (flags & OPFLAG_READ_D && !(ip->hasFlag(BCI_IMM_D)))
            regs.remove(ip->d.u32);
    }
