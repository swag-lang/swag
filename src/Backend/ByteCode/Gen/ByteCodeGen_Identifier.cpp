...
bool ByteCodeGen::sameStackFrame(ByteCodeGenContext* context, const SymbolOverload* overload)
{
    ...
    Diagnostic err{context->node, formErr(Err0490, Naming::kindName(overload).cstr(), overload->symbol->name.cstr())};
    ...
    if (context->node->ownerFct && context->node->ownerFct->hasAttribute(ATTRIBUTE_GENERATED_FUNC))
        err.addNote(form("referencing this runtime %s from the compile-time %s", Naming::kindName(overload).cstr(), context->node->ownerFct->getDisplayName().cstr()));
    if (overload->fromInlineParam)
        err.addNote(overload->fromInlineParam, form("this is the runtime argument corresponding to [[%s]]", overload->symbol->name.cstr()));
...
bool ByteCodeGen::emitIdentifier(ByteCodeGenContext* context)
{
    ...
    else if (resolved->hasFlag(OVERLOAD_VAR_STRUCT))
    {
        ...
        SWAG_VERIFY(!node->resultRegisterRc.empty(), Report::internalError(context->node, form("emitIdentifier, cannot reference identifier [[%s]]", identifier->token.cstr()).cstr()));
        forStruct = true;
    }
