...
bool ByteCodeGen::emitReturn(ByteCodeGenContext* context)
{
    ...
    if (node->hasOwnerInline() && node->hasSemFlag(SEMFLAG_EMBEDDED_RETURN))
        ...
        returnType = funcNode->returnType->typeInfo->getConcreteAlias();
...
bool ByteCodeGen::checkCatchError(ByteCodeGenContext* context, AstNode* srcNode, const AstNode* callNode, const AstNode* funcNode, AstNode* parent, const TypeInfoFuncAttr* typeInfoFunc)
{
    ...
    if (raiseErrors && !callNode->hasOwnerTryCatchAssume())
    {
        ...
        const Diagnostic err{callNode->token.sourceFile, callNode->token, formErr(Err0442, funcNode->token.cstr(), Naming::kindName(srcNode).cstr(), srcNode->token.cstr())};
                           ...
                           bool                freeRegistersParams)
{
    ...
    else if (varNode->typeInfo->isArray())
    {
        ...
        auto typeVar = typeArr->finalType->getConcreteAlias();
    ...
    {
        auto typeVar = varNode->typeInfo->getConcreteAlias();
        typeInfoFunc = castTypeInfo<TypeInfoFuncAttr>(typeVar, TypeInfoKind::LambdaClosure);
    }
    ...
    if (typeInfoFunc->hasFlag(TYPEINFO_VARIADIC))
    {
        if (numVariadic > SWAG_LIMIT_MAX_VARIADIC_PARAMS)
            return context->report({allParams, formErr(Err0505, SWAG_LIMIT_MAX_VARIADIC_PARAMS, numVariadic)});
    }
...
bool ByteCodeGen::emitBeforeFuncDeclContent(ByteCodeGenContext* context)
{
    ...
    if (!ownerReturnType->isVoid())
    {
        if (ownerTypeInfoFunc->returnByStackAddress())
        {
            ...
            EMIT_INST1(context, ByteCodeOp::SaveRRtoRA, funcNode->registerStoreRR);
        }
    }
    ...
    if (funcNode->stackSize > g_CommandLine.limitStackRT)
        Report::report({funcNode, formErr(Err0027, Utf8::toNiceSize(g_CommandLine.limitStackRT).cstr())});
