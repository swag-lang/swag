...
bool ByteCodeGen::emitIntrinsic(ByteCodeGenContext* context)
{
    ...
    {
        askForByteCode(context, node->resolvedSymbolOverload()->node, ASK_BC_WAIT_SEMANTIC_RESOLVED, context->bc);
        YIELD();
    }
...
bool ByteCodeGen::checkCatchError(ByteCodeGenContext* context, AstNode* srcNode, const AstNode* callNode, const AstNode* funcNode, AstNode* parent, const TypeInfoFuncAttr* typeInfoFunc)
{
    ...
    if (!raiseErrors)
    {
            ...
            parent->is(AstNodeKind::Assume))
        {
            if (!srcNode)
                srcNode = typeInfoFunc->declNode;
            const Diagnostic err{parent, parent->token, formErr(Err0396, parent->token.cstr(), srcNode->token.cstr())};
                           ...
                           bool                freeRegistersParams)
{
    ...
    askForByteCode(context, funcNode, ASK_BC_WAIT_SEMANTIC_RESOLVED, context->bc);
    YIELD();
