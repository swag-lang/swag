...
bool ByteCodeGen::skipNodes(ByteCodeGenContext* context, AstNode* node)
{
    ...
    const auto res = Ast::visit(context, node, [](ErrorContext* cxt, const AstNode* n) {
        ...
        {
            cxt->report({n->firstChild(), formErr(Err0450, n->firstChild()->token.cstr())});
...
bool ByteCodeGen::askForByteCode(const JobContext* context, AstNode* node, AskBcFlags flags, bool fromSemantic, ByteCode* caller)
{
    if (!node)
        return true;
