...
bool ByteCodeGen::emitInlineBefore(ByteCodeGenContext* context)
{
    ...
    if (returnType->isStruct() || typeInfoFunc->returnByStackAddress())
    {
    ...
    if (func->parameters)
    {
        ...
        if (numFuncParams == numCallParams)
        {
            for (uint32_t i = 0; i < numCallParams; i++)
            {
                ...
                {
                    if (overload->hasFlag(OVERLOAD_VAR_INLINE) && callParam->resultRegisterRc.countResults)
                    {
...
bool ByteCodeGen::emitLoopAfterExpr(ByteCodeGenContext* context)
{
    ...
    if (rangeNode->expressionLow->hasFlagComputedValue() && rangeNode->expressionUp->hasFlagComputedValue())
    {
        ...
        {
            Diagnostic err{rangeNode->expressionLow, formErr(Err0229, rangeNode->expressionLow->computedValue()->reg.s64, rangeNode->expressionUp->computedValue()->reg.s64)};
            err.addNote(rangeNode->expressionUp, formNte(Nte0211, rangeNode->expressionLow->computedValue()->reg.s64));
...
bool ByteCodeGen::emitSwitchCaseBeforeBlock(ByteCodeGenContext* context)
{
    ...
    else if (caseNode->whereClause && !caseNode->whereClause->computedValue()->reg.b)
    {
        // Jump to the next case
        blockNode->seekJumpNextCase = context->bc->numInstructions;
        EMIT_INST0(context, ByteCodeOp::Jump);
    ...
    if (caseNode->whereClause)
        freeRegisterRC(context, caseNode->whereClause->resultRegisterRc);
    return true;
}
