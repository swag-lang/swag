...
void ByteCodeRun::executeGetFromStackSI(ByteCodeRunContext* context, const ByteCodeInstruction* ip)
{
    ...
    {
        callInternalCompilerError(context, ip, formErr(Err0037, solved->symbol->name.c_str()));
        return;
    }
    ...
    if (solved->typeInfo->isVariadic() || solved->typeInfo->isTypedVariadic())
    {
        ...
        {
            callInternalCompilerError(context, ip, formErr(Err0037, solved->typeInfo->getDisplayNameC()));
            return;
        }
