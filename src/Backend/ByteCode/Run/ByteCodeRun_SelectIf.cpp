...
void ByteCodeRun::executeGetFromStackSI(ByteCodeRunContext* context, const ByteCodeInstruction* ip)
{
    ...
    {
        callInternalCompilerError(context, ip, formErr(Err0045, solved->symbol->name.c_str()));
        return;
    }
    ...
    if (solved->typeInfo->isVariadic() || solved->typeInfo->isTypedVariadic())
    {
        ...
        {
            callInternalCompilerError(context, ip, formErr(Err0045, solved->typeInfo->getDisplayNameC()));
            return;
        }
