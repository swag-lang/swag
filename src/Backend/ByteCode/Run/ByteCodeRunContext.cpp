...
void ByteCodeRunContext::setup(SourceFile* sf, AstNode* nd, ByteCode* nodeBC)
{
    if (!stack)
    {
        ...
        {
            stack = Allocator::allocN<uint8_t>(g_CommandLine.limitStackBC);
#ifdef SWAG_STATS
...
{
    OS::raiseException(SWAG_EXCEPTION_TO_COMPILER_HANDLER, formErr(Err0571, Utf8::toNiceSize(g_CommandLine.limitStackBC).c_str()));
}
