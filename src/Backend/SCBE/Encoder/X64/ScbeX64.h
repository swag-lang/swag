...
struct ScbeX64 final : ScbeCpu
{
    ...
    void    encodeCopy(CpuReg memRegDst, CpuReg memRegSrc, uint32_t count) override;
    ...
    void    encodeOpBinaryRegMem(CpuReg regDst, CpuReg memReg, uint64_t memOffset, CpuOp op, OpBits opBits, CpuEmitFlags emitFlags = EMITF_Zero) override;
