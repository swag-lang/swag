...
namespace
{
    CpuEncodeResult encodeAmcImm(Concat& concat, CpuReg regBase, CpuReg regMul, uint64_t mulValue, uint64_t addValue, OpBits opBitsBaseMul, uint64_t value, OpBits opBitsValue, CpuEmitFlags emitFlags)
    {
        ...
        if (regBase == CpuReg::R13)
            emitModRM(concat, addValue <= 0x7F ? ModRMMode::Displacement8 : ModRMMode::Displacement32, MODRM_REG_0, MODRM_RM_SIB);
    ...
    CpuEncodeResult encodeAmcReg(Concat& concat, CpuReg reg, OpBits opBitsReg, CpuReg regBase, CpuReg regMul, uint64_t mulValue, uint64_t addValue, OpBits opBitsBaseMul, CpuOp op, CpuEmitFlags emitFlags, bool mr)
    {
        ...
        {
            case CpuOp::LEA:
                emitSpecCPUOp(concat, 0x8D, opBitsReg);
                break;
            case CpuOp::MOVSXD:
...
CpuEncodeResult ScbeX64::encodeOpBinaryRegImm(CpuReg reg, uint64_t value, CpuOp op, OpBits opBits, CpuEmitFlags emitFlags)
{
    ...
    else if (op == CpuOp::SHL)
    {
        ...
        else
        {
            ...
            emitModRM(concat, MODRM_REG_4, reg);
            emitValue(concat, std::min(static_cast<uint32_t>(value), BackendEncoder::getNumBits(opBits) - 1), OpBits::B8);
        }
    }
