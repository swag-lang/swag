...
struct CpuFunction
{
    ...
    bool     isStackOffsetLocalParam(uint32_t offset) const { return !cc->paramByRegisterInteger.empty() && offset >= getStackOffsetParam(0) && offset <= getStackOffsetParam(static_cast<uint32_t>(cc->paramByRegisterInteger.size()) - 1); }
...
struct ScbeCpu : BackendEncoder
{
    ...
    void emitLoadRegImm64(CpuReg reg, uint64_t value, CpuEmitFlags emitFlags = EMIT_Zero);
    ...
    void emitLoadAddressAddMul(CpuReg regDst, CpuReg regSrc1, CpuReg regSrc2, uint64_t mulValue, OpBits opBits, CpuEmitFlags emitFlags = EMIT_Zero);
    ...
    void emitSetCond(CpuReg reg, CpuCondFlag setType, CpuEmitFlags emitFlags = EMIT_Zero);
