...
void SCBE_X64::emitOp(CPUReg regDst, CPUReg regSrc, CPUOp op, CPUBits numBits, CPUBits srcBits)
{
    ...
    {
        emitREX(concat, numBits, regDst, regSrc);
        if (op == CPUOp::MUL && regDst == CPUReg::RCX && regSrc == CPUReg::RAX)
        {
        ...
        else if (op == CPUOp::IMUL && regDst == CPUReg::RCX && regSrc == CPUReg::RAX)
        {
            if (numBits == CPUBits::B8 || numBits == CPUBits::B16)
             ...
             op == CPUOp::OR)
    {
        ...
        concat.addU8(getModRM(RegReg, static_cast<uint8_t>(regDst), static_cast<uint8_t>(regSrc)));
             ...
             op == CPUOp::BSR)
    {
        SWAG_ASSERT(regDst == CPUReg::RAX && regSrc == CPUReg::RAX);
        emitREX(concat, numBits, regDst, regSrc);
        concat.addU8(0x0F);
        concat.addU8(static_cast<uint8_t>(op));
        concat.addU8(0xC0);
...
void SCBE_X64::emitOp(CPUReg memReg, uint32_t memOffset, CPUReg reg, CPUOp op, CPUBits numBits, bool lock)
{
    if (numBits == CPUBits::F32)
    {
        ...
        emitLoad(CPUReg::XMM0, memReg, memOffset, CPUBits::F32);
        concat.addU8(0xF3);
        concat.addU8(0x0F);
        concat.addU8(static_cast<uint8_t>(op));
        concat.addU8(0xC1);
        emitStore(memReg, memOffset, CPUReg::XMM0, CPUBits::F32);
    }
    else if (numBits == CPUBits::F64)
    {
        SWAG_ASSERT(reg == CPUReg::XMM1);
        SWAG_ASSERT(memReg < CPUReg::R8);
        emitLoad(CPUReg::XMM0, memReg, memOffset, CPUBits::F64);
        concat.addU8(0xF2);
        concat.addU8(0x0F);
        concat.addU8(static_cast<uint8_t>(op));
        concat.addU8(0xC1);
        emitStore(memReg, memOffset, CPUReg::XMM0, CPUBits::F64);
    }
    else
    {
        SWAG_ASSERT(memReg < CPUReg::R8);
...
void SCBE_X64::emitOp(CPUReg reg, uint64_t value, CPUOp op, CPUBits numBits)
{
    ...
    if (op == CPUOp::SAR || op == CPUOp::SHR || op == CPUOp::SHL)
    {
        SWAG_ASSERT(reg == CPUReg::RAX || reg == CPUReg::RCX);
        if (value <= 0x7F)
        {
...
void SCBE_X64::emitOpInd([[maybe_unused]] CPUReg memReg, uint64_t value, CPUOp op, CPUBits numBits)
{
    ...
    if (op == CPUOp::XOR)
    {
        emitOp(memReg, value, op, numBits);
        return;
    }
