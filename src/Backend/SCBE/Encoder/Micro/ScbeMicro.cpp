...
void ScbeMicro::encodeSymbolRelocationRef(const Utf8& name)
{
    const auto inst = addInstruction(ScbeMicroOp::SymbolRelocationRef);
    inst->name      = name;
}

void ScbeMicro::encodeSymbolRelocationAddress(CpuReg reg, uint32_t symbolIndex, uint32_t offset)
{
...
void ScbeMicro::encodeSymbolRelocationValue(CpuReg reg, uint32_t symbolIndex, uint32_t offset)
{
...
void ScbeMicro::encodeSymbolGlobalString(CpuReg reg, const Utf8& str)
{
...
void ScbeMicro::encodePush(CpuReg reg)
{
    const auto inst = addInstruction(ScbeMicroOp::Push);
    inst->regA      = reg;
}

void ScbeMicro::encodePop(CpuReg reg)
{
    const auto inst = addInstruction(ScbeMicroOp::Pop);
    inst->regA      = reg;
}

void ScbeMicro::encodeRet()
{
...
void ScbeMicro::encodeLoadRegReg(CpuReg regDst, CpuReg regSrc, OpBits opBits)
{
...
void ScbeMicro::encodeLoadRegImm64(CpuReg reg, uint64_t value)
{
...
void ScbeMicro::encodeLoadRegImm(CpuReg reg, uint64_t value, OpBits opBits)
{
...
void ScbeMicro::encodeLoadRegMem(CpuReg reg, CpuReg memReg, uint64_t memOffset, OpBits opBits)
{
...
void ScbeMicro::encodeLoadSignedExtendRegMem(CpuReg reg, CpuReg memReg, uint64_t memOffset, OpBits numBitsDst, OpBits numBitsSrc)
{
...
void ScbeMicro::encodeLoadSignedExtendRegReg(CpuReg regDst, CpuReg regSrc, OpBits numBitsDst, OpBits numBitsSrc)
{
...
void ScbeMicro::encodeLoadZeroExtendRegMem(CpuReg reg, CpuReg memReg, uint64_t memOffset, OpBits numBitsDst, OpBits numBitsSrc)
{
...
void ScbeMicro::encodeLoadZeroExtendRegReg(CpuReg regDst, CpuReg regSrc, OpBits numBitsDst, OpBits numBitsSrc)
{
...
void ScbeMicro::encodeLoadAddressMem(CpuReg reg, CpuReg memReg, uint64_t memOffset)
{
...
void ScbeMicro::encodeLoadAddressAddMul(CpuReg regDst, CpuReg regSrc1, CpuReg regSrc2, uint64_t mulValue, OpBits opBits)
{
...
void ScbeMicro::encodeLoadMemReg(CpuReg memReg, uint64_t memOffset, CpuReg reg, OpBits opBits)
{
...
void ScbeMicro::encodeLoadMemImm(CpuReg memReg, uint64_t memOffset, uint64_t value, OpBits opBits)
{
...
void ScbeMicro::encodeClearReg(CpuReg reg, OpBits opBits)
{
...
void ScbeMicro::encodeClearMem(CpuReg memReg, uint64_t memOffset, uint32_t count)
{
...
void ScbeMicro::encodeSetCond(CpuReg reg, CpuCondFlag setType)
{
...
void ScbeMicro::encodeCmpRegReg(CpuReg reg0, CpuReg reg1, OpBits opBits)
{
...
void ScbeMicro::encodeCmpRegImm(CpuReg reg, uint64_t value, OpBits opBits)
{
...
void ScbeMicro::encodeCmpMemReg(CpuReg memReg, uint64_t memOffset, CpuReg reg, OpBits opBits)
{
...
void ScbeMicro::encodeCmpMemImm(CpuReg memReg, uint64_t memOffset, uint64_t value, OpBits opBits)
{
...
void ScbeMicro::encodeOpUnaryMem(CpuReg memReg, uint64_t memOffset, CpuOp op, OpBits opBits)
{
...
void ScbeMicro::encodeOpUnaryReg(CpuReg reg, CpuOp op, OpBits opBits)
{
...
void ScbeMicro::encodeJumpTable(CpuReg table, CpuReg offset, int32_t currentIp, uint32_t offsetTable, uint32_t numEntries)
{
...
void ScbeMicro::encodeJumpReg(CpuReg reg)
{
...
CpuJump ScbeMicro::encodeJump(CpuCondJump jumpType, OpBits opBits)
{
    CpuJump cpuJump;
...
void ScbeMicro::encodePatchJump(const CpuJump& jump)
{
...
void ScbeMicro::encodePatchJump(const CpuJump& jump, uint64_t offsetDestination)
{
    SWAG_ASSERT(false);
}

void ScbeMicro::encodeCopy(CpuReg regDst, CpuReg regSrc, uint32_t count)
{
...
void ScbeMicro::encodeNop()
{
    addInstruction(ScbeMicroOp::Nop);
}

void ScbeMicro::encodeCallLocal(const Utf8& symbolName)
{
    const auto inst = addInstruction(ScbeMicroOp::CallLocal);
    inst->name      = symbolName;
}

void ScbeMicro::encodeCallExtern(const Utf8& symbolName)
{
    const auto inst = addInstruction(ScbeMicroOp::CallExtern);
    inst->name      = symbolName;
}

void ScbeMicro::encodeCallReg(CpuReg reg)
{
    const auto inst = addInstruction(ScbeMicroOp::CallIndirect);
    inst->regA      = reg;
}

void ScbeMicro::encodeOpMulAdd(CpuReg regDst, CpuReg regMul, CpuReg regAdd, OpBits opBits)
{
    ...
    for (uint32_t i = 0; i < num; i++, inst++)
    {
        switch (inst->op)
        {
            ...
            case ScbeMicroOp::SymbolRelocationRef:
                encoder.encodeSymbolRelocationRef(inst->name);
                break;
            case ScbeMicroOp::SymbolRelocationAddress:
                encoder.encodeSymbolRelocationAddress(inst->regA, static_cast<uint32_t>(inst->valueA), static_cast<uint32_t>(inst->valueB));
                break;
            case ScbeMicroOp::SymbolRelocationValue:
                encoder.encodeSymbolRelocationValue(inst->regA, static_cast<uint32_t>(inst->valueA), static_cast<uint32_t>(inst->valueB));
                break;
            case ScbeMicroOp::SymbolGlobalString:
                encoder.encodeSymbolGlobalString(inst->regA, inst->name);
                break;
            case ScbeMicroOp::Push:
                encoder.encodePush(inst->regA);
                break;
            case ScbeMicroOp::Pop:
                encoder.encodePop(inst->regA);
                break;
            ...
            case ScbeMicroOp::CallLocal:
                encoder.encodeCallLocal(inst->name);
                break;
            case ScbeMicroOp::CallExtern:
                encoder.encodeCallExtern(inst->name);
                break;
            case ScbeMicroOp::CallIndirect:
                encoder.encodeCallReg(inst->regA);
                break;
            case ScbeMicroOp::JumpTable:
                encoder.encodeJumpTable(inst->regA, inst->regB, static_cast<int32_t>(inst->valueA), static_cast<uint32_t>(inst->valueB), inst->valueC);
                break;
            case ScbeMicroOp::JumpCC:
            {
                const auto cmpJump = encoder.encodeJump(inst->jumpType, inst->opBitsA);
            ...
            case ScbeMicroOp::PatchJump:
            {
                ...
                encoder.encodePatchJump(cpuJump);
                break;
            }
            case ScbeMicroOp::JumpM:
                encoder.encodeJumpReg(inst->regA);
                break;
            case ScbeMicroOp::LoadRR:
                encoder.encodeLoadRegReg(inst->regA, inst->regB, inst->opBitsA);
                break;
            case ScbeMicroOp::LoadRI64:
                encoder.encodeLoadRegImm64(inst->regA, inst->valueA);
                break;
            case ScbeMicroOp::LoadRI:
                encoder.encodeLoadRegImm(inst->regA, inst->valueA, inst->opBitsA);
                break;
            case ScbeMicroOp::LoadRM:
                encoder.encodeLoadRegMem(inst->regA, inst->regB, inst->valueA, inst->opBitsA);
                break;
            case ScbeMicroOp::LoadSignedExtendRM:
                encoder.encodeLoadSignedExtendRegMem(inst->regA, inst->regB, inst->valueA, inst->opBitsA, inst->opBitsB);
                break;
            case ScbeMicroOp::LoadSignedExtendRR:
                encoder.encodeLoadSignedExtendRegReg(inst->regA, inst->regB, inst->opBitsA, inst->opBitsB);
                break;
            case ScbeMicroOp::LoadZeroExtendRM:
                encoder.encodeLoadZeroExtendRegMem(inst->regA, inst->regB, inst->valueA, inst->opBitsA, inst->opBitsB);
                break;
            case ScbeMicroOp::LoadZeroExtendRR:
                encoder.encodeLoadZeroExtendRegReg(inst->regA, inst->regB, inst->opBitsA, inst->opBitsB);
                break;
            case ScbeMicroOp::LoadAddressM:
                encoder.encodeLoadAddressMem(inst->regA, inst->regB, inst->valueB);
                break;
            case ScbeMicroOp::LoadAddressAddMul:
                encoder.encodeLoadAddressAddMul(inst->regA, inst->regB, inst->regC, inst->valueA, inst->opBitsA);
                break;
            case ScbeMicroOp::LoadMR:
                encoder.encodeLoadMemReg(inst->regA, inst->valueA, inst->regB, inst->opBitsA);
                break;
            case ScbeMicroOp::LoadMI:
                encoder.encodeLoadMemImm(inst->regA, inst->valueA, inst->valueB, inst->opBitsA);
                break;
            case ScbeMicroOp::CmpRR:
                encoder.encodeCmpRegReg(inst->regA, inst->regB, inst->opBitsA);
                break;
            case ScbeMicroOp::CmpRI:
                encoder.encodeCmpRegImm(inst->regA, inst->valueA, inst->opBitsA);
                break;
            case ScbeMicroOp::CmpMR:
                encoder.encodeCmpMemReg(inst->regA, inst->valueA, inst->regB, inst->opBitsA);
                break;
            case ScbeMicroOp::CmpMI:
                encoder.encodeCmpMemImm(inst->regA, inst->valueA, inst->valueB, inst->opBitsA);
                break;
            case ScbeMicroOp::SetCC:
                encoder.encodeSetCond(inst->regA, inst->cpuCond);
                break;
            case ScbeMicroOp::ClearR:
                encoder.encodeClearReg(inst->regA, inst->opBitsA);
                break;
            case ScbeMicroOp::ClearM:
                encoder.encodeClearMem(inst->regA, inst->valueA, static_cast<uint32_t>(inst->valueB));
                break;
            case ScbeMicroOp::Copy:
                encoder.encodeCopy(inst->regA, inst->regB, static_cast<uint32_t>(inst->valueA));
                break;
            case ScbeMicroOp::OpUnaryM:
                encoder.encodeOpUnaryMem(inst->regA, inst->valueA, inst->cpuOp, inst->opBitsA);
                break;
            case ScbeMicroOp::OpUnaryR:
                encoder.encodeOpUnaryReg(inst->regA, inst->cpuOp, inst->opBitsA);
                break;
            ...
            case ScbeMicroOp::MulAdd:
                encoder.encodeOpMulAdd(inst->regA, inst->regB, inst->regC, inst->opBitsA);
                break;
            default:
                SWAG_ASSERT(false);
                break;
