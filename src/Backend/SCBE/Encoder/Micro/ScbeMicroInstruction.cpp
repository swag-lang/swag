...
{
    if (hasLeftOpFlag(MOF_READ_MEM | MOF_WRITE_MEM) && hasLeftOpFlag(MOF_VALUE_A) && hasLeftOpFlag(MOF_REG_A) && regA == CpuReg::Rsp)
        return static_cast<uint32_t>(valueA);
    if (hasRightOpFlag(MOF_READ_MEM | MOF_WRITE_MEM) && hasRightOpFlag(MOF_VALUE_A) && hasRightOpFlag(MOF_REG_B) && regB == CpuReg::Rsp)
        return static_cast<uint32_t>(valueA);
    return UINT32_MAX;
}