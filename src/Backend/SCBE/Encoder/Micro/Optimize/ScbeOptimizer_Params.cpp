...
void ScbeOptimizer::optimizePassParams(const ScbeMicro& out)
{
    for (uint32_t i = 0; i < out.cc->paramByRegisterInteger.size(); ++i)
    {
        if (out.cpuFct->typeFunc->numParamsRegisters() > i &&
            usedRegs[out.cc->paramByRegisterInteger[i]] == 1 &&
        ...
        {
            memToReg(out, CpuReg::Rsp, out.cpuFct->getStackOffsetParam(i), out.cc->paramByRegisterInteger[i]);
        }
    }

    /*if (usedRegs[CpuReg::R9] == 0)
    {
        memToReg(out, CpuReg::Rsp, out.cpuFct->getStackOffsetReg(0), CpuReg::R9);
    }*/
}
