...
void Scbe::emitIMMA(ScbeCpu& pp, CpuReg reg, OpBits opBits)
{
    ...
    if (ip->hasFlag(BCI_IMM_A))
        emitLoadRegImm(pp, reg, ip->a.u64, opBits);
    else
        emitLoadRegMem(pp, reg, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->a.u32), opBits);
}
...
void Scbe::emitIMMB(ScbeCpu& pp, CpuReg reg, OpBits opBits)
{
    ...
    if (ip->hasFlag(BCI_IMM_B))
        emitLoadRegImm(pp, reg, ip->b.u64, opBits);
    else
        emitLoadRegMem(pp, reg, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->b.u32), opBits);
}
...
void Scbe::emitIMMC(ScbeCpu& pp, CpuReg reg, OpBits opBits)
{
    ...
    if (ip->hasFlag(BCI_IMM_C))
        emitLoadRegImm(pp, reg, ip->c.u64, opBits);
    else
        emitLoadRegMem(pp, reg, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->c.u32), opBits);
}
...
void Scbe::emitIMMD(ScbeCpu& pp, CpuReg reg, OpBits opBits)
{
    ...
    if (ip->hasFlag(BCI_IMM_D))
        emitLoadRegImm(pp, reg, ip->d.u64, opBits);
    else
        emitLoadRegMem(pp, reg, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->d.u32), opBits);
}
...
void Scbe::emitIMMB(ScbeCpu& pp, CpuReg reg, OpBits numBitsSrc, OpBits numBitsDst, bool isSigned)
{
    ...
    if (ip->hasFlag(BCI_IMM_B))
    {
        ...
        emitLoadRegImm(pp, reg, ip->b.u64, OpBits::B32);
    }
    else if (isSigned)
    {
        emitLoadSignedExtendRegMem(pp, reg, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->b.u32), numBitsDst, numBitsSrc);
    }
    else
    {
        emitLoadZeroExtendRegMem(pp, reg, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->b.u32), numBitsDst, numBitsSrc);
    }
}
...
void Scbe::emitIMMC(ScbeCpu& pp, CpuReg reg, OpBits numBitsSrc, OpBits numBitsDst, bool isSigned)
{
    ...
    {
        SWAG_ASSERT(numBitsDst == OpBits::B16 || numBitsDst == OpBits::B32);
        emitLoadRegImm(pp, reg, ip->c.u64, OpBits::B32);
    }
    else if (isSigned)
    {
        emitLoadSignedExtendRegMem(pp, reg, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->c.u32), numBitsDst, numBitsSrc);
    }
    else
    {
        emitLoadZeroExtendRegMem(pp, reg, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->c.u32), numBitsDst, numBitsSrc);
    }
}
...
void Scbe::emitShiftRightArithmetic(ScbeCpu& pp)
{
    ...
    {
        emitLoadRegImm(pp, cc->computeRegI1, std::min(static_cast<uint32_t>(ip->b.u8), ScbeCpu::getNumBits(opBits) - 1), OpBits::B8);
        emitIMMA(pp, cc->computeRegI0, opBits);
        emitOpBinaryRegReg(pp, cc->computeRegI0, cc->computeRegI1, CpuOp::SAR, opBits);
    }
    else
    {
        emitLoadRegMem(pp, cc->computeRegI1, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->b.u32), OpBits::B32);
        emitLoadRegImm(pp, cc->computeRegI0, ScbeCpu::getNumBits(opBits) - 1, OpBits::B32);
...
void Scbe::emitShiftEqLogical(ScbeCpu& pp, CpuOp op)
{
    ...
    else
    {
        ...
        emitLoadMegReg(pp, cc->computeRegI0, 0, cc->computeRegI1, opBits);
...
void Scbe::emitBinOp(ScbeCpu& pp, CpuOp op, CpuEmitFlags emitFlags)
{
    ...
    else if (!ip->hasFlag(BCI_IMM_A | BCI_IMM_B))
    {
        ...
        emitLoadRegMem(pp, r0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->a.u32), opBits);
        emitOpBinaryRegMem(pp, r0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->b.u32), op, opBits, emitFlags);
        emitLoadMegReg(pp, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->c.u32), r0, opBits);
    }
    else
    {
        const auto r0 = isInt ? cc->computeRegI0 : cc->computeRegF0;
...
void Scbe::emitBinOpEq(ScbeCpu& pp, uint32_t offset, CpuOp op, CpuEmitFlags emitFlags)
{
    ...
    else
    {
        ...
        emitLoadRegMem(pp, r0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->a.u32), OpBits::B64);
        emitIMMB(pp, r1, opBits);
...
void Scbe::emitAddSubMul64(ScbeCpu& pp, uint64_t mulValue, CpuOp op)
{
    ...
    else
    {
        ...
        emitLoadMegReg(pp, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->c.u32), cc->computeRegI1, OpBits::B64);
    }
}
...
void Scbe::emitJumpCmpAddr(ScbeCpu& pp, CpuCondJump op, CpuReg memReg, uint64_t memOffset, OpBits opBits)
{
    ...
    {
        emitLoadRegMem(pp, cc->computeRegI0, memReg, memOffset, opBits);
...
void Scbe::emitJumpCmp2(ScbeCpu& pp, CpuCondJump op1, CpuCondJump op2, OpBits opBits)
{
    ...
    if (!ip->hasFlag(BCI_IMM_A | BCI_IMM_C))
    {
        emitLoadRegMem(pp, cc->computeRegF0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->a.u32), opBits);
...
void Scbe::emitJumpDyn(ScbeCpu& pp)
{
    ...
    emitLoadSignedExtendRegMem(pp, cc->computeRegI0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(ip->a.u32), OpBits::B64, opBits);
...
void Scbe::emitCopyVaargs(ScbeCpu& pp)
{
    ...
    if (!pp.pushRVParams.empty())
    {
        ...
        while (idxParam != UINT32_MAX)
        {
            ...
            {
                case 1:
                    emitLoadRegMem(pp, cc->computeRegI0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(reg), OpBits::B8);
                    emitLoadMegReg(pp, CpuReg::Rsp, offset, cc->computeRegI0, OpBits::B8);
                    break;
                case 2:
                    emitLoadRegMem(pp, cc->computeRegI0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(reg), OpBits::B16);
                    emitLoadMegReg(pp, CpuReg::Rsp, offset, cc->computeRegI0, OpBits::B16);
                    break;
                case 4:
                    emitLoadRegMem(pp, cc->computeRegI0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(reg), OpBits::B32);
                    emitLoadMegReg(pp, CpuReg::Rsp, offset, cc->computeRegI0, OpBits::B32);
                    break;
                case 8:
                    emitLoadRegMem(pp, cc->computeRegI0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(reg), OpBits::B64);
                    emitLoadMegReg(pp, CpuReg::Rsp, offset, cc->computeRegI0, OpBits::B64);
                    break;
                default:
                    break;
            }
        ...
        emitLoadAddressMem(pp, cc->computeRegI0, CpuReg::Rsp, pp.cpuFct->sizeStackCallParams - variadicStackSize);
    ...
    else
    {
        ...
        while (idxParam != UINT32_MAX)
        {
            emitLoadRegMem(pp, cc->computeRegI0, CpuReg::Rsp, pp.cpuFct->getStackOffsetReg(pp.pushRAParams[idxParam]), OpBits::B64);
            emitLoadMegReg(pp, CpuReg::Rsp, offset, cc->computeRegI0, OpBits::B64);
            idxParam--;
            offset += 8;
        }
