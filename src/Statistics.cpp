...
void Stats::print() const
{
    ...
    /////////////////////////
    g_Log.messageHeaderDot("workers", FMT("%u", g_ThreadMgr.numWorkers), LogColor::Header, LogColor::Value);
    ...
    if (g_CommandLine.output)
        g_Log.messageHeaderDot("output modules", FMT("%u", numGenModules.load()), LogColor::Header, LogColor::Value);
    if (g_CommandLine.test)
        g_Log.messageHeaderDot("executed #test", FMT("%u", testFunctions.load()), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("executed #run", FMT("%u", runFunctions.load()), LogColor::Header, LogColor::Value);
    ...
    g_Log.messageHeaderDot("kicked", FMT("%d %.1f%%", totalOptimsBC.load(), pc1), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("total", FMT("%u", numInstructions.load() - totalOptimsBC.load()), LogColor::Header, LogColor::Value);
    g_Log.eol();

    g_Log.messageHeaderDot("concrete types", FMT("%u", totalConcreteTypes.load()), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("concrete struct types", FMT("%u", totalConcreteStructTypes.load()), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("init ptr", FMT("%u", numInitPtr.load()), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("init func ptr", FMT("%u", numInitFuncPtr.load()), LogColor::Header, LogColor::Value);
    g_Log.eol();

    /////////////////////////
    g_Log.messageHeaderDot("cfg time", FMT("%.3fs", OS::timerToSeconds(cfgTime.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("tokenizer time", FMT("%.3fs", OS::timerToSeconds(tokenizerTime.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("syntax time", FMT("%.3fs", OS::timerToSeconds(syntaxTime.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("read files", FMT("%.3fs", OS::timerToSeconds(readFilesTime.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("semantic time", FMT("%.3fs", OS::timerToSeconds(semanticTime.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("run time", FMT("%.3fs", OS::timerToSeconds(runTime.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("run test time", FMT("%.3fs", OS::timerToSeconds(runTestTime.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("prep out 1 time", FMT("%.3fs (genfunc: %.3fs)", OS::timerToSeconds(prepOutputStage1TimeJob.load()), OS::timerToSeconds(prepOutputTimeJob_GenFunc.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("prep out 2 time", FMT("%.3fs (saveobj: %.3fs)", OS::timerToSeconds(prepOutputStage2TimeJob.load()), OS::timerToSeconds(prepOutputTimeJob_SaveObj.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("gen out time", FMT("%.3fs", OS::timerToSeconds(genOutputTimeJob.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("optim bc time", FMT("%.3fs", OS::timerToSeconds(optimBCTime.load())), LogColor::Header, LogColor::Value);
    g_Log.messageHeaderDot("total time", FMT("%.3fs", OS::timerToSeconds(g_Workspace->totalTime.load())), LogColor::Header, LogColor::Value);
    g_Log.eol();

    /////////////////////////
    g_Log.messageHeaderDot("mem max", FMT("%s", Utf8::toNiceSize(maxAllocatedMemory.load()).c_str()), LogColor::Header, LogColor::Value);
    ...
    g_Log.messageHeaderDot("mem bc stack", FMT("%s", Utf8::toNiceSize(memBcStack.load()).c_str()), LogColor::Header, LogColor::Value);
    ...
    for (const auto m : g_Workspace->modules)
    {
        ...
        g_Log.print(FMT("%u", m->byteCodeFunc.size()), LogColor::Value);
        g_Log.print(" func emit: ", LogColor::Header);
        g_Log.print(FMT("%u", m->byteCodeFuncToGen.size()), LogColor::Value);
