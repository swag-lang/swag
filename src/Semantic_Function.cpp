...
bool Semantic::resolveFuncDeclType(SemanticContext* context)
{
	...
	{
		Diagnostic diag{funcNode, funcNode->tokenName, FMT(Err(Err0488), funcNode->token.c_str())};
	...
	{
		Diagnostic diag{funcNode, funcNode->tokenName, FMT(Err(Err0490), funcNode->token.c_str())};
	...
	{
		Diagnostic diag{funcNode, funcNode->tokenName, FMT(Err(Err0487), funcNode->token.c_str())};
		auto       attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_CalleeReturn);
		auto       note = Diagnostic::note(attr, FMT(Nte(Nte0063), "attribute"));
		return context->report(diag, note);
	}
	...
	if (!funcNode->hasAstFlag(AST_FROM_GENERIC))
	{
		...
		if (funcNode->genericParameters)
		{
			if (funcNode->hasAttribute(ATTRIBUTE_NOT_GENERIC))
			{
				auto       attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_NotGeneric);
				auto       note = Diagnostic::note(attr, FMT(Nte(Nte0063), "attribute"));
				Diagnostic diag{funcNode->genericParameters, FMT(Err(Err0687), funcNode->token.c_str())};
				return context->report(diag, note);
			}
		...
		if (funcNode->hasAttribute(ATTRIBUTE_NOT_GENERIC) && funcNode->hasAstFlag(AST_IS_GENERIC))
		{
			auto       attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_NotGeneric);
			auto       note = Diagnostic::note(attr, FMT(Nte(Nte0063), "attribute"));
			Diagnostic diag{funcNode, funcNode->tokenName, FMT(Err(Err0684), funcNode->token.c_str())};
			return context->report(diag, note);
		}
		...
		if (structDecl->typeInfo->isGeneric())
		{
			for (auto c : funcNode->genericParameters->children)
			{
				...
				for (auto sc : structDecl->genericParameters->children)
				{
					...
					{
						Diagnostic diag{c, FMT(Err(Err0114), c->resolvedSymbolOverload->node->token.c_str())};
...
bool Semantic::registerFuncSymbol(SemanticContext* context, AstFuncDecl* funcNode, uint32_t symbolFlags)
{
	if (!(symbolFlags & OVERLOAD_INCOMPLETE))
	{
		...
		if (funcNode->returnType->typeInfo->isVoid() && funcNode->hasAttribute(ATTRIBUTE_DISCARDABLE))
		{
			...
			const auto       note = Diagnostic::note(attr, FMT(Nte(Nte0063), "attribute"));
			const Diagnostic diag{funcNode, funcNode->tokenName, FMT(Err(Err0573), funcNode->token.c_str())};
			return context->report(diag, note);
		}
	}
	...
	if (!funcNode->hasAstFlag(AST_FROM_GENERIC))
	{
		...
		if (funcNode->resolvedSymbolName->overloads.size() > 1 && !funcNode->hasAttribute(ATTRIBUTE_OVERLOAD))
		{
			...
			if (other)
			{
				const Diagnostic diag{funcNode, funcNode->tokenName, FMT(Err(Err0655), funcNode->token.c_str())};
...
bool Semantic::resolveReturn(SemanticContext* context)
{
	...
	if (funcNode->returnTypeDeducedNode)
	{
		...
		const CastFlags castFlags = CASTFLAG_JUST_CHECK | CASTFLAG_UN_CONST | CASTFLAG_AUTO_OP_CAST | CASTFLAG_TRY_COERCE | CASTFLAG_FOR_AFFECT;
	...
	{
		const Diagnostic  diag{child, FMT(Err(Err0623), concreteType->getDisplayNameC(), funcNode->token.c_str(), concreteType->name.c_str())};
	...
	else
	{
		...
		const CastFlags castFlags = CASTFLAG_UN_CONST | CASTFLAG_AUTO_OP_CAST | CASTFLAG_TRY_COERCE | CASTFLAG_FOR_AFFECT | CASTFLAG_PTR_REF | CASTFLAG_ACCEPT_PENDING;
...
bool Semantic::makeInline(JobContext* context, AstFuncDecl* funcDecl, AstNode* identifier)
{
	...
	while (ownerInline)
	{
		if (ownerInline->func == funcDecl)
		{
			...
			{
				const Diagnostic diag{identifier, identifier->token, FMT(Err(Err0605), identifier->token.c_str(), g_CommandLine.limitInlineLevel)};
				return context->report(diag);
			}
		}
				...
				if (it == cloneContext.usedReplaceNames.end())
				{
					if (id->identifierExtension)
					{
						for (auto& alias : id->identifierExtension->aliasNames)
						{
							if (alias.text == val)
							{
								const Diagnostic diag{id, alias, FMT(Err(Err0746), alias.c_str())};
								return context->report(diag);
							}
						}
					}
