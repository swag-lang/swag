...
SWAG_ERROR(Err0882, "capture size overflow (current size is `%u` bytes but maximum authorized is `%u`)")
SWAG_ERROR(Err0883, "expected the function parameters before `{`")
SWAG_ERROR(Err0884, "cannot capture `%s` because it's not a plain old data struct (the type contains `opDrop`, `opPostCopy` or `opPostMove`)")
...
SWAG_ERROR(Err0887, "cannot capture `%s` because capturing type `%s` is not supported")
