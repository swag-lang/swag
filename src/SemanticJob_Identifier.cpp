...
void SemanticJob::getDiagnosticForMatch(SemanticContext* context, OneTryMatch& oneTry, vector<const Diagnostic*>& result0, vector<const Diagnostic*>& result1)
{
    ...
    case MatchResult::SelectIfFailed:
    {
        ...
        note = new Diagnostic{destFuncDecl->selectIf, Utf8::format(g_E[Nte0007], destFuncDecl->selectIf->token.text.c_str()), DiagnosticLevel::Note};
...
void SemanticJob::symbolNotFoundHint(SemanticContext* context, AstNode* node, VectorNative<AlternativeScope>& scopeHierarchy, vector<Utf8>& best)
{
...
void SemanticJob::symbolErrorNotes(SemanticContext* context, VectorNative<OneTryMatch*>& overloads, AstNode* node, Diagnostic* diag, vector<const Diagnostic*>& notes)
{
    ...
    if (overloads.size() == 1 && overloads[0]->dependentVar)
    {
        ...
        {
            auto note = new Diagnostic{overloads[0]->dependentVar, g_E[Nte0013], DiagnosticLevel::Note};
    ...
    if (badUfcs && !identifier->identifierRef->startScope)
    {
        ...
        if (identifier->childParentIdx)
        {
            ...
            if (prev->resolvedSymbolName)
            {
                if (prev->typeInfo)
                {
                    auto note = new Diagnostic{prev, Utf8::format(g_E[Nte0001], prev->token.text.c_str(), SymTable::getArticleKindName(prev->resolvedSymbolName->kind), prev->typeInfo->getDisplayName().c_str()), DiagnosticLevel::Note};
                    notes.push_back(note);
                }
                else
                {
                    auto note = new Diagnostic{prev, Utf8::format(g_E[Nte0010], prev->token.text.c_str(), SymTable::getArticleKindName(prev->resolvedSymbolName->kind)), DiagnosticLevel::Note};
                    notes.push_back(note);
                }

                if (prev->resolvedSymbolOverload)
                {
                    auto note = new Diagnostic{prev->resolvedSymbolOverload->node, g_E[Nte0000], DiagnosticLevel::Note};
                    notes.push_back(note);
                }
            }
        }
...
void SemanticJob::symbolErrorRemarks(SemanticContext* context, VectorNative<OneTryMatch*>& overloads, AstNode* node, Diagnostic* diag)
{
    ...
    if (identifier->identifierRef->startScope)
    {
        ...
        {
            diag->remarks.push_back(Utf8::format(g_E[Nte0043], node->token.text.c_str(), identifier->identifierRef->typeInfo->getDisplayName().c_str()));
            for (auto s : identifier->identifierRef->startScope->childScopes)
            {
                if (s->kind == ScopeKind::Impl)
                {
                    ...
                    {
                        diag->remarks.push_back(Utf8::format(g_E[Nte0044], node->token.text.c_str(), s->getFullName().c_str()));
                    }
                }
            }
        }
...
bool SemanticJob::findIdentifierInScopes(SemanticContext* context, AstIdentifierRef* identifierRef, AstIdentifier* node)
{
    ...
    for (int oneTry = 0; oneTry < 2 && !forceEnd; oneTry++)
    {
        ...
        if (!startScope || oneTry == 1)
        {
            ...
            if (identifierRef->specFlags & AST_SPEC_IDENTIFIERREF_AUTO_SCOPE)
            {
                ...
                case TypeInfoKind::Enum:
                {
                    ...
                    addAlternativeScope(scopeHierarchy, typeEnum->scope);
                    break;
                }
                }
            }
        ...
        {
            addAlternativeScope(scopeHierarchy, startScope);
                ...
                if (addAlternative)
                {
                    ...
                    if (startScope->kind != ScopeKind::Namespace && startScope->owner->extension)
                    {
                        for (auto& as : startScope->owner->extension->alternativeScopes)
                            addAlternativeScope(scopeHierarchy, as.scope);
        ...
        if (oneTry || forceEnd || !identifierRef->startScope)
        {
            ...
            Utf8         appendMsg;
            if (identifierRef->startScope)
                addAlternativeScope(scopeHierarchy, identifierRef->startScope);
            ...
            if (identifierRef->startScope)
            {
                ...
                if (typeRef && typeRef->flags & TYPEINFO_STRUCT_IS_TUPLE)
                {
                    ...
                    auto note = new Diagnostic{identifierRef->startScope->owner, g_E[Nte0030], DiagnosticLevel::Note};
                    notes.push_back(note);
                }
                else
                {
...
void SemanticJob::collectAlternativeScopeVars(AstNode* startNode, VectorNative<AlternativeScope>& scopes, VectorNative<AlternativeScopeVar>& scopesVars)
{
    ...
    while (!toAdd.empty())
    {
        ...
        if (!done.contains(it0.scope))
        {
            ...
            addAlternativeScope(scopes, it0.scope);
...
void SemanticJob::collectAlternativeScopeHierarchy(SemanticContext* context, VectorNative<AlternativeScope>& scopes, VectorNative<AlternativeScopeVar>& scopesVars, AstNode* startNode, uint32_t flags)
{
    ...
    if (startNode->kind == AstNodeKind::Inline)
    {
        ...
        addAlternativeScope(scopes, inlineNode->parametersScope);
    }
...
void SemanticJob::addAlternativeScope(VectorNative<AlternativeScope>& scopes, Scope* scope)
{
