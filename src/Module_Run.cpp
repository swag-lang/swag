...
bool Module::computeExecuteResult(SourceFile* sourceFile, AstNode* node, JobContext* callerContext, ExecuteNodeParams* params)
{
    ...
    if (realType->kind == TypeInfoKind::Struct)
    {
        ...
        if (!count)
            return callerContext->report(Hint::isType(realType), {node, Fmt(g_E[Err0161], realType->getDisplayName().c_str())});
        ...
        if (!g_RunContext.registersRR[0].u64 || !g_RunContext.registersRR[1].u64)
            return callerContext->report(Hint::isType(realType), {node, Fmt(g_E[Err0162], realType->getDisplayName().c_str())});
    ...
    return callerContext->report({node, Fmt(g_E[Err0058], realType->getDisplayName().c_str())});
}
