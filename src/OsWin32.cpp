...
namespace OS
{
    ...
    bool doProcess(Module* module, const Utf8& cmdline, const string& currentDirectory, bool logAll, uint32_t& numErrors, LogColor logColor, const char* logPrefix)
    {
        ...
        {
            g_Log.error(Utf8::format(g_E[Err0045], cmdline.c_str()));
            return false;
        }
...

        {
            if (!CreateProcessA(nullptr,
                                ...
                                &pi))
            {
                g_Log.errorOS(Utf8::format(g_E[Err0046], cmdline.c_str()));
                return false;
            }
        }
...
#pragma pack(pop)
    ...
    void setSwagFolder(const string& folder)
    {
        ...
        {
            g_Log.error(Utf8::format(g_E[Err0048], folder.c_str()));
            OS::exit(-1);
        }
        ...
        if (!strstr(szBuffer, folder.c_str()))
        {
            ...
            {
                RegCloseKey(hKey);
                g_Log.error(Utf8::format(g_E[Err0051], folder.c_str()));
                OS::exit(-1);
            }
