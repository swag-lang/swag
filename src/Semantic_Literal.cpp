    ...
    while (pz - start < len)
    {
        ...
        case 'U':
        {
            ...
            if (cw > Utf8::MAX_ENCODED_UNICODE)
                return context->report({node->sourceFile, loc, Fmt(Err(Err0406), cw)});
            result.append(cw);
            loc.column += 8;
            continue;
        }
        ...
        return context->report({node->sourceFile, loc, Fmt(Err(Err0276), c)});
    }
...
Utf8 Semantic::checkLiteralValue(ComputedValue& computedValue, LiteralType& literalType, Register& literalValue, TypeInfo* typeSuffix, bool negApplied)
{
    ...
    case LiteralType::TT_UNTYPED_FLOAT:
        ...
        case NativeTypeKind::F64:
            ...
            break;
        default:
            return Fmt(Err(Err0322), typeSuffix->getDisplayNameC());
        }
        break;
    ...
    case LiteralType::TT_UNTYPED_INT:
        ...
        case NativeTypeKind::U8:
            if (computedValue.reg.u64 > UINT8_MAX)
                return Fmt(Fmt(Err(Err0426), computedValue.reg.u64, "u8"));
            break;
        case NativeTypeKind::U16:
            if (computedValue.reg.u64 > UINT16_MAX)
                return Fmt(Fmt(Err(Err0426), computedValue.reg.u64, "u16"));
            break;
        case NativeTypeKind::U32:
            if (computedValue.reg.u64 > UINT32_MAX)
                return Fmt(Fmt(Err(Err0426), computedValue.reg.u64, "u32"));
            break;
        case NativeTypeKind::U64:
            break;
        ...
        case NativeTypeKind::S8:
            if (computedValue.reg.s64 < INT8_MIN || computedValue.reg.s64 > INT8_MAX)
                return Fmt(Err(Err0426), computedValue.reg.s64, "s8");
            break;
        case NativeTypeKind::S16:
            if (computedValue.reg.s64 < INT16_MIN || computedValue.reg.s64 > INT16_MAX)
                return Fmt(Err(Err0426), computedValue.reg.s64, "s16");
            break;
        case NativeTypeKind::S32:
            if (computedValue.reg.s64 < INT32_MIN || computedValue.reg.s64 > INT32_MAX)
                return Fmt(Err(Err0426), computedValue.reg.s64, "s32");
            break;
        case NativeTypeKind::S64:
            break;
        ...
        case NativeTypeKind::Rune:
            if (computedValue.reg.u64 > UINT32_MAX)
                return Fmt(Fmt(Err(Err0426), computedValue.reg.u64, "rune"));
            break;
        ...
        default:
            return Fmt(Err(Err0319), computedValue.reg.u64);
        }
    ...
    case LiteralType::TT_UNTYPED_BINHEXA:
        ...
        case NativeTypeKind::S8:
            if (computedValue.reg.u64 > UINT8_MAX)
                return Fmt(Fmt(Err(Err0426), computedValue.reg.u64, "s8"));
            break;
        case NativeTypeKind::S16:
            if (computedValue.reg.u64 > UINT16_MAX)
                return Fmt(Fmt(Err(Err0426), computedValue.reg.u64, "s16"));
            break;
        case NativeTypeKind::S32:
            if (computedValue.reg.u64 > UINT32_MAX)
                return Fmt(Fmt(Err(Err0426), computedValue.reg.u64, "s32"));
            break;
        case NativeTypeKind::S64:
            if (computedValue.reg.u64 > (uint64_t) INT64_MAX + 1)
                return Fmt(Fmt(Err(Err0426), computedValue.reg.u64, "s64"));
            break;
...
bool Semantic::resolveLiteral(SemanticContext* context)
{
    ...
    if (suffix->tokenId != TokenId::NativeType)
    {
        ...
        {
            return context->report({suffix, Fmt(Err(Err0320), suffix->resolvedSymbolName->name.c_str(), Naming::aKindName(suffix->resolvedSymbolName->kind).c_str())});
        }
    }

    auto suffixType = TypeManager::concreteType(suffix->typeInfo, CONCRETE_ALIAS);
    SWAG_VERIFY(suffixType->isNative(), context->report({suffix, Fmt(Err(Err0321), suffixType->getDisplayNameC())}));
    ...
    case NativeTypeKind::F64:
        break;
    default:
        return context->report({suffix, Fmt(Err(Err0321), suffix->typeInfo->getDisplayNameC())});
    }
