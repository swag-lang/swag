        ...
        if (fct->returnType && !fct->returnType->typeInfo->isVoid())
        {
            if (!node->hasSemFlag(SEMFLAG_SCOPE_HAS_RETURN))
            {
                if (node->hasSemFlag(SEMFLAG_FCT_HAS_RETURN))
                    return context->report({fct->returnType, formErr(Err0571, fct->getDisplayNameC())});
                return context->report({fct->returnType, formErr(Err0572, fct->getDisplayNameC(), fct->returnType->typeInfo->getDisplayNameC())});
            }
        }
    }
...
bool Semantic::resolveSwitch(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node, node->token, toErr(Err0472)};
...
bool Semantic::resolveCase(SemanticContext* context)
{
    ...
    for (const auto oneExpression : node->expressions)
    {
        ...
        if (oneExpression->is(AstNodeKind::Range))
        {
            ...
            {
                return context->report({rangeNode, toErr(Err0495)});
            }
        }
...
bool Semantic::resolveVisit(SemanticContext* context)
{
    ...
    else if (typeInfo->isEnum())
    {
        if (node->hasSpecFlag(AstVisit::SPEC_FLAG_WANT_POINTER))
        {
            Diagnostic err{node, node->wantPointerToken, toErr(Err0411)};
            err.addNote(node->expression, Diagnostic::isType(node->expression->typeInfo));
            return context->report(err);
        }
...
bool Semantic::resolveIndex(SemanticContext* context)
{
    ...
    SWAG_VERIFY(ownerBreakable, context->report({node, toErr(Err0433)}));
...
bool Semantic::resolveBreak(SemanticContext* context)
{
    ...
    if (!node->label.text.empty())
    {
        ...
        SWAG_VERIFY(breakable, context->report({node->token.sourceFile, node->label, formErr(Err0714, node->label.text.c_str())}));
...
bool Semantic::resolveFallThrough(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->hasOwnerBreakable() && node->ownerBreakable()->is(AstNodeKind::Switch), context->report({node, toErr(Err0455)}));
    ...
    SWAG_VERIFY(parent && parent->is(AstNodeKind::SwitchCase), context->report({node, toErr(Err0454)}));
    node->switchCase = castAst<AstSwitchCase>(parent, AstNodeKind::SwitchCase);
    SWAG_VERIFY(node->switchCase->caseIndex != UINT32_MAX, context->report({node, toErr(Err0454)}));
    ...
    SWAG_VERIFY(node->switchCase->caseIndex < switchBlock->cases.size() - 1, context->report({node, toErr(Err0453)}));
...
bool Semantic::resolveContinue(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->hasOwnerBreakable(), context->report({node, toErr(Err0451)}));
    ...
    SWAG_VERIFY(checkBreakable, context->report({node, toErr(Err0451)}));
