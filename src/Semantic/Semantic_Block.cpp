        ...
        if (fct->returnType && !fct->returnType->typeInfo->isVoid())
        {
            if (!node->hasSemFlag(SEMFLAG_SCOPE_HAS_RETURN))
            {
                if (node->hasSemFlag(SEMFLAG_FCT_HAS_RETURN))
                    return context->report({fct->returnType, formErr(Err0527, fct->getDisplayNameC())});
                return context->report({fct->returnType, formErr(Err0528, fct->getDisplayNameC(), fct->returnType->typeInfo->getDisplayNameC())});
            }
        }
    }
...
bool Semantic::resolveSwitch(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node, node->token, toErr(Err0427)};
    ...
    if (node->hasAttribute(ATTRIBUTE_COMPLETE))
    {
        ...
        if (back->expressions.empty())
        {
            ...
            return context->report({back, back->token, toErr(Err0656)}, note);
        }
            ...
            for (const auto typeEnum : collect)
            {
                if (typeSwitch->isString())
                {
                    for (const auto one : typeEnum->values)
                    {
                        ...
                        {
                            Diagnostic err{node, node->token, formErr(Err0485, typeEnum->name.c_str(), one->name.c_str())};
...
bool Semantic::resolveCase(SemanticContext* context)
{
    ...
    for (const auto oneExpression : node->expressions)
    {
        ...
        if (oneExpression->is(AstNodeKind::Range))
        {
            ...
            {
                return context->report({rangeNode, toErr(Err0450)});
            }
        }
...
bool Semantic::resolveVisit(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node, node->extraNameToken, formErr(Err0698, Naming::aKindName(typeInfo).c_str())};
    ...
    {
        Diagnostic err{node, node->aliasNames[2], formErr(Err0686, node->aliasNames.size())};
        return context->report(err);
    }
    ...
    else if (typeInfo->isEnum())
    {
        if (node->hasSpecFlag(AstVisit::SPEC_FLAG_WANT_POINTER))
        {
            Diagnostic err{node, node->wantPointerToken, toErr(Err0352)};
            err.addNote(node->expression, Diagnostic::isType(node->expression->typeInfo));
            return context->report(err);
        }
    ...
    {
        return context->report({node->expression, toErr(Err0353)});
    }
    else
    {
        return context->report({node->expression, formErr(Err0354, typeInfo->getDisplayNameC())});
    }
...
bool Semantic::resolveIndex(SemanticContext* context)
{
    ...
    SWAG_VERIFY(ownerBreakable, context->report({node, toErr(Err0381)}));
...
bool Semantic::resolveBreak(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->hasOwnerBreakable(), context->report({node, toErr(Err0397)}));
...
bool Semantic::resolveFallThrough(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->hasOwnerBreakable() && node->ownerBreakable()->is(AstNodeKind::Switch), context->report({node, toErr(Err0404)}));
    ...
    SWAG_VERIFY(parent && parent->is(AstNodeKind::SwitchCase), context->report({node, toErr(Err0403)}));
    node->switchCase = castAst<AstSwitchCase>(parent, AstNodeKind::SwitchCase);
    SWAG_VERIFY(node->switchCase->caseIndex != UINT32_MAX, context->report({node, toErr(Err0403)}));
    ...
    SWAG_VERIFY(node->switchCase->caseIndex < switchBlock->cases.size() - 1, context->report({node, toErr(Err0402)}));
...
bool Semantic::resolveContinue(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->hasOwnerBreakable(), context->report({node, toErr(Err0400)}));
    ...
    SWAG_VERIFY(checkBreakable, context->report({node, toErr(Err0400)}));
