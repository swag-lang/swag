...
bool Semantic::resolveWhile(SemanticContext* context)
{
    ...
    {
        const Diagnostic err{node->boolExpression, toErr(Err0131)};
        ...
        if (fct->returnType && !fct->returnType->typeInfo->isVoid())
        {
            if (!node->hasSemFlag(SEMFLAG_SCOPE_HAS_RETURN))
            {
                if (node->hasSemFlag(SEMFLAG_FCT_HAS_RETURN))
                    return context->report({fct->returnType, formErr(Err0575, fct->getDisplayNameC())});
                return context->report({fct->returnType, formErr(Err0576, fct->getDisplayNameC(), fct->returnType->typeInfo->getDisplayNameC())});
            }
        }
    }
...
bool Semantic::resolveSwitch(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node, node->token, toErr(Err0474)};
    ...
    switch (typeSwitch->kind)
    {
        ...
        case TypeInfoKind::Interface:
            return context->report({node->expression, formErr(Err0163, typeSwitch->getDisplayNameC())});
    }

    SWAG_VERIFY(!node->cases.empty(), context->report({node, node->token, toErr(Err0074)}));
        ...
        for (auto expr : switchCase->expressions)
        {
            if (expr->hasFlagComputedValue())
            {
                ...
                if (typeExpr->isString())
                {
                    ...
                    if (idx != -1)
                    {
                        Diagnostic err{expr, formErr(Err0012, expr->computedValue()->text.c_str())};
                        err.addNote(valDef[idx], toNte(Nte0076));
                        return context->report(err);
                    }
                ...
                else
                {
                    ...
                    if (idx != -1)
                    {
                        const auto note = Diagnostic::note(valDef[idx], toNte(Nte0076));
                        if (expr->isConstantGenTypeInfo())
                            return context->report({expr, formErr(Err0011, expr->token.c_str())}, note);
                        if (expr->typeInfo->isEnum())
                            return context->report({expr, formErr(Err0011, expr->token.c_str())}, note);
                        if (typeExpr->isNativeInteger())
                            return context->report({expr, formErr(Err0009, expr->computedValue()->reg.u64)}, note);
                        return context->report({expr, formErr(Err0010, expr->computedValue()->reg.f64)}, note);
                    }
            ...
            {
                return checkIsConstExpr(context, expr->hasFlagComputedValue(), expr, toErr(Err0033));
            }
        }
    }
    ...
    if (node->hasAttribute(ATTRIBUTE_COMPLETE))
    {
        ...
        if (back->expressions.empty())
        {
            ...
            const auto note = Diagnostic::note(attr, formNte(Nte0066, "attribute"));
            return context->report({back, back->token, toErr(Err0660)}, note);
        }
            ...
            for (const auto typeEnum : collect)
            {
                if (typeSwitch->isString())
                {
                    for (const auto one : typeEnum->values)
                    {
                        ...
                        {
                            Diagnostic err{node, node->token, formErr(Err0113, typeEnum->name.c_str(), one->name.c_str())};
                            err.addNote(one->declNode, one->declNode->token, toNte(Nte0139));
                            return context->report(err);
                        }
                    }
                }
...
bool Semantic::resolveCase(SemanticContext* context)
{
    ...
    for (const auto oneExpression : node->expressions)
    {
        ...
        if (oneExpression->is(AstNodeKind::Range))
        {
            ...
            {
                return context->report({rangeNode, toErr(Err0497)});
            }
        }
        ...
        else
        {
            ...
            if (node->ownerSwitch->expression)
            {
                ...
                {
                    PushErrCxtStep ec(context, node->ownerSwitch->expression, ErrCxtStepKind::Note, [typeInfo] { return formNte(Nte0152, typeInfo->getDisplayNameC(), "the switch expression"); });
...
bool Semantic::resolveLoop(SemanticContext* context)
{
    ...
    if (node->expression)
    {
        ...
        if (node->expression->isNot(AstNodeKind::Range))
        {
            ...
            {
                PushErrCxtStep ec(context, node, ErrCxtStepKind::Note, [] { return toNte(Nte0024); }, true);
...
bool Semantic::resolveVisit(SemanticContext* context)
{
    ...
    if (typeInfo->isStruct())
    {
        SWAG_VERIFY(!typeInfo->isTuple(), context->report({node->expression, toErr(Err0165)}));
    ...
    {
        Diagnostic err{node, node->extraNameToken, formErr(Err0704, Naming::aKindName(typeInfo).c_str())};
    ...
    {
        Diagnostic err{node, node->aliasNames[2], formErr(Err0692, node->aliasNames.size())};
        return context->report(err);
    }
    ...
    else if (typeInfo->isVariadic() || typeInfo->isTypedVariadic())
    {
        if (node->hasSpecFlag(AstVisit::SPEC_FLAG_WANT_POINTER))
        {
            Diagnostic err{node, node->wantPointerToken, toErr(Err0409)};
    ...
    else if (typeInfo->isEnum())
    {
        if (node->hasSpecFlag(AstVisit::SPEC_FLAG_WANT_POINTER))
        {
            Diagnostic err{node, node->wantPointerToken, toErr(Err0410)};
            err.addNote(node->expression, Diagnostic::isType(node->expression->typeInfo));
            return context->report(err);
        }
    ...
    {
        return context->report({node->expression, toErr(Err0413)});
    }
    else
    {
        return context->report({node->expression, formErr(Err0414, typeInfo->getDisplayNameC())});
    }
...
bool Semantic::resolveIndex(SemanticContext* context)
{
    ...
    SWAG_VERIFY(ownerBreakable, context->report({node, toErr(Err0434)}));
...
bool Semantic::resolveBreak(SemanticContext* context)
{
    ...
    if (!node->label.text.empty())
    {
        ...
        SWAG_VERIFY(breakable, context->report({node->token.sourceFile, node->label, formErr(Err0719, node->label.text.c_str())}));
        node->setOwnerBreakable(breakable);
    }

    SWAG_VERIFY(node->hasOwnerBreakable(), context->report({node, toErr(Err0450)}));
...
bool Semantic::resolveFallThrough(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->hasOwnerBreakable() && node->ownerBreakable()->is(AstNodeKind::Switch), context->report({node, toErr(Err0457)}));
    ...
    SWAG_VERIFY(parent && parent->is(AstNodeKind::SwitchCase), context->report({node, toErr(Err0456)}));
    node->switchCase = castAst<AstSwitchCase>(parent, AstNodeKind::SwitchCase);
    SWAG_VERIFY(node->switchCase->caseIndex != UINT32_MAX, context->report({node, toErr(Err0456)}));
    ...
    SWAG_VERIFY(node->switchCase->caseIndex < switchBlock->cases.size() - 1, context->report({node, toErr(Err0455)}));
...
bool Semantic::resolveContinue(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->hasOwnerBreakable(), context->report({node, toErr(Err0453)}));
    ...
    SWAG_VERIFY(checkBreakable, context->report({node, toErr(Err0453)}));
