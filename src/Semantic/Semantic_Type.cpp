...
bool Semantic::checkTypeIsNative(SemanticContext* context, TypeInfo* leftTypeInfo, TypeInfo* rightTypeInfo, AstNode* left, AstNode* right)
{
    ...
    {
        Diagnostic err{node->token.sourceFile, node->token, formErr(Err0644, node->token.c_str(), leftTypeInfo->getDisplayNameC())};
        err.addNote(left, Diagnostic::isType(leftTypeInfo));
        return context->report(err);
    }

    Diagnostic err{node->token.sourceFile, node->token, formErr(Err0645, node->token.c_str(), rightTypeInfo->getDisplayNameC())};
...
bool Semantic::checkIsConcrete(SemanticContext* context, AstNode* node)
{
    ...
    if (node->is(AstNodeKind::TypeExpression) || node->is(AstNodeKind::TypeLambda))
        return context->report({node, toErr(Err0227)});
    ...
    {
        Diagnostic err{node, formErr(Err0541, node->resolvedSymbolOverload()->symbol->ownerTable->scope->name.c_str())};
    ...
    Diagnostic err{node, formErr(Err0542, Naming::kindName(node->resolvedSymbolName()->kind).c_str(), node->resolvedSymbolName()->name.c_str())};
        ...
        for (auto param : node->parameters->children)
        {
            ...
            if (typeParam->typeInfo->isVariadic())
            {
                ...
                if (index != node->parameters->childCount() - 1)
                    return context->report({param, toErr(Err0459)});
            }
...
bool Semantic::resolveType(SemanticContext* context)
{
    ...
    if (typeNode->typeFlags.has(TYPEFLAG_IS_CODE))
    {
        ...
        SWAG_VERIFY(typeP && typeNode->ownerFct, context->report({typeNode, formErr(Err0456, "code")}));
    ...
    {
        const auto typeP = typeNode->findParent(AstNodeKind::FuncDeclParam);
        SWAG_VERIFY(typeP && typeNode->ownerFct, context->report({typeNode, formErr(Err0456, "cvarargs")}));
    ...
    if (typeNode->identifier && !typeNode->identifier->children.empty())
    {
        ...
        if (!child->typeInfo || !child->typeInfo->isUndefined())
        {
            ...
            if (symName)
            {
                ...
                {
                    Diagnostic err{child->token.sourceFile, child->token, formErr(Err0335, child->token.c_str(), Naming::aKindName(symName->kind).c_str())};
    ...
    if (typeNode->typeFlags.has(TYPEFLAG_IS_REF))
    {
        ...
        {
            const auto typeP = typeNode->findParent(AstNodeKind::FuncDeclParam);
            SWAG_VERIFY(typeP && typeNode->ownerFct, context->report({typeNode, toErr(Err0449)}));
    ...
    if (typeNode->typeFlags.has(TYPEFLAG_IS_ARRAY))
    {
        ...
        SWAG_VERIFY(!rawType->isVoid(), context->report({typeNode->lastChild(), toErr(Err0628)}));
        ...
        for (int i = typeNode->arrayDim - 1; i >= 0; i--)
        {
            ...
            {
                SWAG_CHECK(checkIsConstExpr(context, child->hasFlagComputedValue(), child, toErr(Err0042)));
                count = child->computedValue()->reg.u32;
            }

            const auto childType = TypeManager::concreteType(child->typeInfo);
            SWAG_VERIFY(childType->isNativeInteger(), context->report({child, formErr(Err0629, child->typeInfo->getDisplayNameC())}));
            SWAG_CHECK(context->checkSizeOverflow("array", static_cast<uint64_t>(count) * rawType->sizeOf, SWAG_LIMIT_ARRAY_SIZE));
            SWAG_VERIFY(!child->isConstant0(), context->report({child, toErr(Err0174)}));
    ...
    if (typeNode->typeFlags.has(TYPEFLAG_HAS_LOC_CONST) &&
        ...
        !typeC->isStruct())
    {
        const Diagnostic err{typeNode->token.sourceFile, typeNode->locConst, formErr(Err0332, typeNode->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
