...
bool Semantic::resolveUnaryOpMinus(SemanticContext* context, AstNode* op, AstNode* child)
{
    ...
    switch (typeInfo->nativeType)
    {
        ...
        {
            Diagnostic err{node, node->token, formErr(Err0241, typeInfo->getDisplayNameC())};
        ...
        {
            Diagnostic err{node, node->token, formErr(Err0240, typeInfo->getDisplayNameC())};
            err.addNote(child, toNte(Nte0036));
            return context->report(err);
        }
        ...
        switch (typeInfo->nativeType)
        {
            case NativeTypeKind::S8:
                if (child->computedValue()->reg.s8 <= INT8_MIN)
                    return context->report({child, formErr(Err0318, child->computedValue()->reg.s8, -child->computedValue()->reg.s8, "s8")});
                child->computedValue()->reg.s64 = -child->computedValue()->reg.s8;
                break;
            case NativeTypeKind::S16:
                if (child->computedValue()->reg.s16 <= INT16_MIN)
                    return context->report({child, formErr(Err0318, child->computedValue()->reg.s16, -child->computedValue()->reg.s16, "s16")});
                child->computedValue()->reg.s64 = -child->computedValue()->reg.s16;
                break;
            case NativeTypeKind::S32:
                if (child->computedValue()->reg.s32 <= INT32_MIN)
                    return context->report({child, formErr(Err0318, child->computedValue()->reg.s32, -child->computedValue()->reg.s32, "s32")});
            ...
            case NativeTypeKind::S64:
                if (child->computedValue()->reg.s64 <= INT64_MIN)
                    return context->report({child, formErr(Err0317, child->computedValue()->reg.s64, -child->computedValue()->reg.s64)});
...
bool Semantic::resolveUnaryOpInvert(SemanticContext* context, const AstNode* child)
{
    ...
    switch (typeInfo->nativeType)
    {
        ...
        case NativeTypeKind::Any:
        {
            Diagnostic err{node, node->token, formErr(Err0631, typeInfo->getDisplayNameC())};
            err.addNote(child, toNte(Nte0036));
            return context->report(err);
        }
