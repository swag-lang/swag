...
bool Semantic::resolveUnaryOpMinus(SemanticContext* context, AstNode* op, AstNode* child)
{
    ...
    switch (typeInfo->nativeType)
    {
        ...
        {
            Diagnostic err{node, node->token, formErr(Err0336, typeInfo->getDisplayNameC())};
        ...
        {
            Diagnostic err{node, node->token, formErr(Err0337, typeInfo->getDisplayNameC())};
        ...
        switch (typeInfo->nativeType)
        {
            case NativeTypeKind::S8:
                if (child->computedValue()->reg.s8 <= INT8_MIN)
                    return context->report({child, formErr(Err0425, child->computedValue()->reg.s8, -child->computedValue()->reg.s8, "s8")});
                child->computedValue()->reg.s64 = -child->computedValue()->reg.s8;
                break;
            case NativeTypeKind::S16:
                if (child->computedValue()->reg.s16 <= INT16_MIN)
                    return context->report({child, formErr(Err0425, child->computedValue()->reg.s16, -child->computedValue()->reg.s16, "s16")});
                child->computedValue()->reg.s64 = -child->computedValue()->reg.s16;
                break;
            case NativeTypeKind::S32:
                if (child->computedValue()->reg.s32 <= INT32_MIN)
                    return context->report({child, formErr(Err0425, child->computedValue()->reg.s32, -child->computedValue()->reg.s32, "s32")});
            ...
            case NativeTypeKind::S64:
                if (child->computedValue()->reg.s64 <= INT64_MIN)
                    return context->report({child, formErr(Err0424, child->computedValue()->reg.s64, -child->computedValue()->reg.s64)});
...
bool Semantic::resolveUnaryOpInvert(SemanticContext* context, AstNode* child)
{
    ...
    switch (typeInfo->nativeType)
    {
        ...
        case NativeTypeKind::Any:
        {
            Diagnostic err{node, node->token, formErr(Err0346, typeInfo->getDisplayNameC())};
            err.addNote(child, toNte(Nte0032));
            return context->report(err);
        }
