...
bool Semantic::resolveIntrinsicTag(SemanticContext* context)
{
    ...
    switch (node->token.id)
    {
        case TokenId::IntrinsicSafety:
        {
            ...
            YIELD();
            SWAG_CHECK(checkIsConstExpr(context, front->hasFlagComputedValue(), front, toErr(Err0021), node->token.text));
            SWAG_VERIFY(front->typeInfo->isString(), context->report({front, formErr(Err0587, node->token.cstr(), front->typeInfo->getDisplayNameC())}));
            ...
            if (!done)
            {
                return context->report({front, front->token, formErr(Err0723, w.cstr())});
            }
        ...
        case TokenId::IntrinsicGetTag:
        {
            ...
            SWAG_CHECK(checkIsConstExpr(context, nameNode->hasFlagComputedValue(), nameNode, toErr(Err0021), node->token.text));
            SWAG_VERIFY(nameNode->typeInfo->isString(), context->report({nameNode, formErr(Err0587, node->token.cstr(), nameNode->typeInfo->getDisplayNameC())}));
            SWAG_VERIFY(defaultVal->hasComputedValue(), context->report({defaultVal, formErr(Err0136, Naming::aKindName(defaultVal).cstr())}));

            {
                PushErrCxtStep ec(context, typeNode, ErrCxtStepKind::Note, [] { return toNte(Nte0114); });
            ...
            if (tag)
            {
                ...
                {
                    Diagnostic err{typeNode, formErr(Err0554, typeNode->typeInfo->getDisplayNameC(), tag->type->getDisplayNameC(), tag->name.cstr())};
                    err.addNote(formNte(Nte0119, tag->cmdLine.cstr()));
...
bool Semantic::resolveIntrinsicMakeCallback(SemanticContext* context, AstNode* node)
{
    ...
    if (!typeFirst->isLambdaClosure())
        return context->report({first, formErr(Err0582, typeFirst->getDisplayNameC())});
    ...
    {
        const Diagnostic err{first, formErr(Err0497, SWAG_LIMIT_CB_MAX_PARAMS, typeFunc->parameters.size())};
        return context->report(err, Diagnostic::hereIs(typeFunc->declNode));
    }

    if (typeFunc->numReturnRegisters() > 1)
    {
        const Diagnostic err{first, formErr(Err0629, typeFunc->returnType->getDisplayNameC())};
        return context->report(err, Diagnostic::hereIs(typeFunc->declNode));
    }
...
bool Semantic::resolveIntrinsicMakeSlice(SemanticContext* context, AstNode* node, const char* name)
{
    ...
    if (!first->typeInfo->isPointer())
        return context->report({first, formErr(Err0579, name, first->typeInfo->getDisplayNameC())});
    if (!first->typeInfo->isPointerArithmetic() && !first->typeInfo->isCString())
        return context->report({first, formErr(Err0578, name, first->typeInfo->getDisplayNameC())});
...
bool Semantic::resolveIntrinsicMakeAny(SemanticContext* context, AstNode* node)
{
    ...
    // Check first parameter
    if (!first->typeInfo->isPointer())
        return context->report({first, formErr(Err0579, node->token.cstr(), first->typeInfo->getDisplayNameC())});

    const auto ptrPointer = castTypeInfo<TypeInfoPointer>(first->typeInfo, TypeInfoKind::Pointer);
    if (!ptrPointer->pointedType)
        return context->report({first, toErr(Err0134)});
    ...
    if (second->isConstantGenTypeInfo())
    {
        ...
        if (!TypeManager::makeCompatibles(context, ptrPointer->pointedType, realType, nullptr, second, CAST_FLAG_JUST_CHECK))
        {
            ...
            Diagnostic err{first, formErr(Err0561, first->typeInfo->getDisplayNameC(), realType->getDisplayNameC(), realType->getDisplayNameC(), typePointer->pointedType->getDisplayNameC())};
    ...
    if (!second->typeInfo->isPointerToTypeInfo())
        return context->report({second, formErr(Err0574, second->typeInfo->getDisplayNameC())});
...
bool Semantic::resolveIntrinsicMakeInterface(SemanticContext* context)
{
    ...
    SWAG_VERIFY(firstTypeInfo->isPointer() || firstTypeInfo->isStruct(), context->report({first, formErr(Err0580, firstTypeInfo->getDisplayNameC())}));
    SWAG_VERIFY(second->typeInfo->isPointerToTypeInfo(), context->report({second, formErr(Err0575, second->typeInfo->getDisplayNameC())}));
    const auto thirdTypeInfo = third->typeInfo->getConcreteAlias();
    SWAG_VERIFY(thirdTypeInfo->isInterface(), context->report({third, formErr(Err0586, thirdTypeInfo->getDisplayNameC())}));
...
bool Semantic::resolveIntrinsicCountOf(SemanticContext* context, AstNode* node, AstNode* expression)
{
    ...
    {
        SWAG_VERIFY(!typeInfo->isTuple(), context->report({expression, toErr(Err0567)}));
    ...
    else
    {
        ...
        SWAG_VERIFY(typeInfo->isNativeInteger(), context->report({expression, formErr(Err0584, typeInfo->getDisplayNameC())}));
        if (expression->hasFlagComputedValue())
        {
                ...
                switch (typeInfo->nativeType)
                {
                    case NativeTypeKind::S8:
                        if (expression->computedValue()->reg.s8 < 0)
                            return context->report({expression, formErr(Err0138, node->computedValue()->reg.s8)});
                        break;
                    case NativeTypeKind::S16:
                        if (expression->computedValue()->reg.s16 < 0)
                            return context->report({expression, formErr(Err0138, node->computedValue()->reg.s16)});
                        break;
                    case NativeTypeKind::S32:
                        if (expression->computedValue()->reg.s32 < 0)
                            return context->report({expression, formErr(Err0138, node->computedValue()->reg.s32)});
                        break;
                    case NativeTypeKind::S64:
                        if (expression->computedValue()->reg.s64 < 0)
                            return context->report({expression, formErr(Err0137, node->computedValue()->reg.s64)});
                        break;
                    default:
                        break;
                }
...
bool Semantic::resolveIntrinsicDataOf(SemanticContext* context, AstNode* node, AstNode* expression)
{
    ...
    {
        SWAG_VERIFY(!typeInfo->isTuple(), context->report({expression, toErr(Err0568)}));
        node->typeInfo = typeInfo;
    ...
    {
        return context->report({expression, formErr(Err0569, typeInfo->getDisplayNameC())});
    }
...
bool Semantic::resolveIntrinsicStringOf(SemanticContext* context)
{
    ...
    if (expr->hasFlagComputedValue())
    {
        if (expr->isConstantGenTypeInfo())
            ...
            return context->report({expr, toErr(Err0746)});
    }
...
bool Semantic::resolveIntrinsicNameOf(SemanticContext* context)
{
    ...
    if (expr->isConstantGenTypeInfo())
        ...
        return context->report({expr, toErr(Err0745)});
...
bool Semantic::resolveIntrinsicRunes(SemanticContext* context)
{
    ...
    SWAG_VERIFY(typeInfo->isString(), context->report({expr, formErr(Err0573, typeInfo->getDisplayNameC())}));
    node->setFlagsValueIsComputed();
...
bool Semantic::resolveIntrinsicSpread(SemanticContext* context)
{
    ...
    if (const auto pr2 = node->getParent(2); pr2->isNot(AstNodeKind::FuncCallParam))
    {
        ...
        {
            const Diagnostic err{pr2, pr2->token, toErr(Err0374)};
            return context->report(err);
        }

        const Diagnostic err{node, node->token, toErr(Err0314)};
        return context->report(err);
    }
    ...
    {
        return context->report({expr, formErr(Err0242, typeInfo->getDisplayNameC())});
    }
...
bool Semantic::resolveIntrinsicDeclType(SemanticContext* context)
{
    ...
    SWAG_VERIFY(!typeInfo->isKindGeneric(), context->report({expr, toErr(Err0238)}));
...
bool Semantic::resolveIntrinsicProperty(SemanticContext* context)
{
    ...
    switch (node->token.id)
    {
        ...
        {
            auto expr = node->firstChild();
            SWAG_VERIFY(!expr->typeInfo->isGeneric(), context->report({expr, toErr(Err0112)}));
            node->setFlagsValueIsComputed();
        ...
        {
            auto expr = node->firstChild();
            SWAG_VERIFY(!expr->typeInfo->isGeneric(), context->report({expr, toErr(Err0110)}));
            node->setFlagsValueIsComputed();
        ...
        case TokenId::IntrinsicMakeString:
        {
            ...
            if (!expr->typeInfo->isPointerTo(NativeTypeKind::U8))
                return context->report({expr, formErr(Err0581, expr->typeInfo->getDisplayNameC())});
        ...
        case TokenId::IntrinsicCVaArg:
        {
            ...
            SWAG_VERIFY(typeInfo->scopedName == "*Swag.CVaList", context->report({node->firstChild(), formErr(Err0558, "*Swag.CVaList", typeInfo->getDisplayNameC())}));

            if (node->token.is(TokenId::IntrinsicCVaStart))
            {
                SWAG_VERIFY(node->ownerFct && node->ownerFct->parameters && !node->ownerFct->parameters->children.empty(), context->report({node, node->token, toErr(Err0313)}));
                const auto typeParam = node->ownerFct->parameters->lastChild()->typeInfo;
                SWAG_VERIFY(typeParam->isCVariadic(), context->report({node, node->token, toErr(Err0313)}));
            ...
            else
            {
                ...
                SWAG_VERIFY(node->typeInfo->numRegisters() == 1, context->report({node->secondChild(), formErr(Err0244, node->typeInfo->getDisplayNameC())}));

                SWAG_VERIFY(!node->typeInfo->isNative(NativeTypeKind::F32), context->report({node->secondChild(), formErr(Err0111, node->typeInfo->getDisplayNameC(), "f64")}));
                SWAG_VERIFY(!node->typeInfo->isNative(NativeTypeKind::S8), context->report({node->secondChild(), formErr(Err0111, node->typeInfo->getDisplayNameC(), "s32")}));
                SWAG_VERIFY(!node->typeInfo->isNative(NativeTypeKind::S16), context->report({node->secondChild(), formErr(Err0111, node->typeInfo->getDisplayNameC(), "s32")}));
                SWAG_VERIFY(!node->typeInfo->isNative(NativeTypeKind::U8), context->report({node->secondChild(), formErr(Err0111, node->typeInfo->getDisplayNameC(), "u32")}));
                SWAG_VERIFY(!node->typeInfo->isNative(NativeTypeKind::U16), context->report({node->secondChild(), formErr(Err0111, node->typeInfo->getDisplayNameC(), "u32")}));
                SWAG_VERIFY(!node->typeInfo->isBool(), context->report({node->secondChild(), formErr(Err0111, node->typeInfo->getDisplayNameC(), "u32")}));
