...
bool Semantic::resolveEnum(SemanticContext* context)
{
    ...
    {
        Diagnostic err({node, node->getTokenName(), formErr(Err0519, symbolName->name.c_str())});
        for (const auto p : symbolName->nodes)
        {
            if (p != node)
            {
                const auto note   = Diagnostic::note(p, p->getTokenName(), toNte(Nte0190));
    ...
    if (node->hasAttribute(ATTRIBUTE_NO_DUPLICATE))
    {
        ...
        {
            const Diagnostic err{node->firstChild(), formErr(Err0607, rawType->getDisplayNameC())};
            return context->report(err);
        }
        ...
        for (const auto typeEnum : collect)
        {
            if (rawType->isString())
            {
                for (const auto one : typeEnum->values)
                {
                    ...
                    if (it != valText.end())
                    {
                        Diagnostic err{one->declNode, one->declNode->token, formErr(Err0520, one->name.c_str())};
                        err.addNote(it->second, it->second->getTokenName(), toNte(Nte0190));
...
bool Semantic::resolveEnumType(SemanticContext* context)
{
    ...
    switch (rawTypeInfo->kind)
    {
        ...
        case TypeInfoKind::Array:
        {
            ...
            {
                const auto       front = typeNode->firstChild();
                const Diagnostic err{front, formErr(Err0610, rawTypeInfo->getDisplayNameC(), rawTypeInfo->getDisplayNameC())};
                return context->report(err);
            }
        ...
        case TypeInfoKind::Native:
            if (rawTypeInfo->nativeType == NativeTypeKind::Any)
                return context->report({typeNode->firstChild(), formErr(Err0604, rawTypeInfo->getDisplayNameC())});
            return true;
    }

    if (rawTypeInfo->isCString())
        return context->report({typeNode->firstChild(), toErr(Err0608)});

    if (!typeNode->children.empty())
        typeNode = typeNode->firstChild();
    return context->report({typeNode, formErr(Err0604, rawTypeInfo->getDisplayNameC())});
}
...
bool Semantic::resolveSubEnumValue(SemanticContext* context)
{
    ...
    {
        const Diagnostic err{node, formErr(Err0603, node->typeInfo->getDisplayNameC())};
    ...
    {
        const Diagnostic err{node, formErr(Err0609, concreteTypeEnum->getDisplayNameC(), concreteTypeSubEnum->getDisplayNameC())};
...
bool Semantic::resolveEnumValue(SemanticContext* context)
{
    ...
    if (assignNode)
    {
        ...
        PushErrCxtStep ec{context, enumNode->type, ErrCxtStepKind::Note, [rawTypeInfo] {
                              return formNte(Nte0144, rawTypeInfo->getDisplayNameC());
                          }};
            ...
            case TypeInfoKind::Native:
                switch (rawTypeInfo->nativeType)
                {
                    ...
                    case NativeTypeKind::F64:
                        return context->report({valNode, formErr(Err0452, valNode->token.c_str(), rawTypeInfo->getDisplayNameC())});
                    default:
                        break;
                }
                break;
            ...
            switch (rawType->nativeType)
            {
                case NativeTypeKind::U8:
                    if (enumNode->computedValue()->reg.u8 == UINT8_MAX)
                        return context->report({valNode, formErr(Err0499, valNode->token.c_str(), rawType->getDisplayNameC())});
                    if (isFlags && enumNode->computedValue()->reg.u8)
                    {
                        const auto n = enumNode->computedValue()->reg.u8;
                        SWAG_VERIFY((n & n - 1) == 0, context->report({valNode, formErr(Err0732, valNode->token.c_str())}));
