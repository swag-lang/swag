...
bool Semantic::resolveEnum(SemanticContext* context)
{
    ...
    if (symbolName && symbolName->nodes.size() > 1)
    {
        ...
        for (const auto p : symbolName->nodes)
        {
            if (p != node)
            {
                const auto note   = Diagnostic::note(p, p->getTokenName(), toNte(Nte0196));
            ...
            if (rawType->isString())
            {
                for (const auto one : typeEnum->values)
                {
                    ...
                    if (it != valText.end())
                    {
                        ...
                        err.addNote(it->second, it->second->getTokenName(), toNte(Nte0196));
                        const auto val = Ast::literalToString(rawType, *one->value);
                        err.addNote(formNte(Nte0122, val.cstr()));
                        return context->report(err);
                    }
...
bool Semantic::resolveEnumValue(SemanticContext* context)
{
    ...
    if (assignNode)
    {
        ...
        PushErrCxtStep ec{context, enumNode->type, ErrCxtStepKind::Note, [rawTypeInfo] {
                              return formNte(Nte0151, rawTypeInfo->getDisplayNameC());
                          }};
