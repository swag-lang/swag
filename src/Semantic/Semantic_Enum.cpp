...
bool Semantic::resolveEnum(SemanticContext* context)
{
    ...
    {
        Diagnostic err({node, node->getTokenName(), formErr(Err0009, symbolName->name.cstr())});
    ...
    if (node->hasAttribute(ATTRIBUTE_NO_DUPLICATE))
    {
        ...
        {
            const Diagnostic err{node->firstChild(), formErr(Err0574, rawType->getDisplayNameC())};
            return context->report(err);
        }
...
bool Semantic::resolveEnumType(SemanticContext* context)
{
    ...
    if (enumNode->hasAttribute(ATTRIBUTE_ENUM_FLAGS))
    {
        ...
        if (!concreteType->isNativeInteger() || concreteType->isNativeIntegerSigned())
            return context->report({typeNode->firstChild(), formErr(Err0572, rawTypeInfo->getDisplayNameC())});
    }
    ...
    if (enumNode->hasAttribute(ATTRIBUTE_ENUM_INDEX))
    {
        ...
        if (!concreteType->isNativeInteger())
            return context->report({typeNode->firstChild(), formErr(Err0573, rawTypeInfo->getDisplayNameC())});
    }
    ...
    switch (rawTypeInfo->kind)
    {
        ...
        case TypeInfoKind::Array:
        {
            ...
            if (typeArray->count == UINT32_MAX)
            {
                ...
                const Diagnostic err{front, formErr(Err0174, rawTypeInfo->getDisplayNameC())};
                return context->report(err);
            }
            ...
            {
                const auto       front = typeNode->firstChild();
                const Diagnostic err{front, formErr(Err0577, rawTypeInfo->getDisplayNameC(), rawTypeInfo->getDisplayNameC())};
                return context->report(err);
            }
        ...
        {
            const auto       front = typeNode->firstChild();
            const Diagnostic err{front, formErr(Err0578, rawTypeInfo->getDisplayNameC(), rawTypeInfo->getDisplayNameC())};
        ...
        case TypeInfoKind::Native:
            if (rawTypeInfo->nativeType == NativeTypeKind::Any)
                return context->report({typeNode->firstChild(), formErr(Err0571, rawTypeInfo->getDisplayNameC())});
            return true;
    }

    if (rawTypeInfo->isCString())
        return context->report({typeNode->firstChild(), toErr(Err0575)});

    if (!typeNode->children.empty())
        typeNode = typeNode->firstChild();
    return context->report({typeNode, formErr(Err0571, rawTypeInfo->getDisplayNameC())});
}
...
bool Semantic::resolveSubEnumValue(SemanticContext* context)
{
    ...
    {
        const Diagnostic err{node, formErr(Err0570, node->typeInfo->getDisplayNameC())};
    ...
    {
        const Diagnostic err{node, formErr(Err0576, concreteTypeEnum->getDisplayNameC(), concreteTypeSubEnum->getDisplayNameC())};
            ...
            case TypeInfoKind::Native:
                switch (rawTypeInfo->nativeType)
                {
                    ...
                    case NativeTypeKind::F64:
                        return context->report({valNode, formErr(Err0446, valNode->token.cstr(), rawTypeInfo->getDisplayNameC())});
                    default:
                        break;
                }
                break;
            ...
            switch (rawType->nativeType)
            {
                case NativeTypeKind::U8:
                    if (enumNode->computedValue()->reg.u8 == UINT8_MAX)
                        return context->report({valNode, formErr(Err0488, valNode->token.cstr(), rawType->getDisplayNameC())});
                    if (isFlags && enumNode->computedValue()->reg.u8)
                    {
                        const auto n = enumNode->computedValue()->reg.u8;
                        SWAG_VERIFY((n & n - 1) == 0, context->report({valNode, formErr(Err0693, valNode->token.cstr())}));
