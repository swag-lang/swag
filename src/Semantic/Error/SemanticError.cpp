...
void SemanticError::errorWhereFailed(SemanticContext*, const ErrorParam& errorParam)
{
    ...
    const auto msg = formErr(Err0062, whereExpr->token.c_str(), Naming::kindName(node).c_str(), node->token.c_str(), whereExpr->token.c_str());
    const auto err = new Diagnostic{errorParam.errorNode, errorParam.errorNode->getTokenName(), msg};
    errorParam.addError(err);
    errorParam.addNote(Diagnostic::hereIs(whereExpr, formNte(Nte0182, whereExpr->token.c_str())));
}
...
void SemanticError::commonErrorNotes(SemanticContext*, const VectorNative<OneTryMatch*>& tryMatches, AstNode* node, Diagnostic* err, Vector<const Diagnostic*>& notes)
{
    ...
    {
        const auto msg  = formNte(Nte0141, tryMatches[0]->overload->symbol->getFullName().c_str());
    ...
    if (node->is(AstNodeKind::Identifier) && tryMatches.size() == 1)
    {
            ...
            overload->node->ownerStructScope->owner != identifierRef->startScope->owner)
        {
            ...
            {
                const auto msg = formNte(Nte0108,
            ...
            for (const auto s : identifierRef->startScope->childScopes)
            {
                ...
                {
                    auto msg = formNte(Nte0140, node->token.c_str(), s->getFullName().c_str());
                    err->remarks.push_back(msg);
                }
            }
        }
...
{
    Utf8 text = formErr(Err0619, node->token.c_str(), typeInfo->getDisplayNameC());
    if (msg)
    {
        text += " ";
        text += msg;
                                          ...
                                          AstNode*      otherSymbolDecl)
{
    ...
    Diagnostic err{sourceFile, token, formErr(Err0510, what.c_str(), Naming::kindName(thisKind).c_str(), thisName.c_str(), as.c_str())};
    err.addNote(otherSymbolDecl, otherSymbolDecl->getTokenName(), toNte(Nte0190));
    return context->report(err);
}
