...
void SemanticError::errorWhereFailed(SemanticContext*, const ErrorParam& errorParam)
{
    ...
    const auto msg = formErr(Err0076, whereExpr->token.c_str(), Naming::kindName(node).c_str(), node->token.c_str(), whereExpr->token.c_str());
...
{
    Utf8 text = formErr(Err0650, node->token.c_str(), typeInfo->getDisplayNameC());
    if (msg)
    {
        text += " ";
        text += msg;
                                          ...
                                          AstNode*      otherSymbolDecl)
{
    ...
    Diagnostic err{sourceFile, token, formErr(Err0005, what.c_str(), Naming::kindName(thisKind).c_str(), thisName.c_str(), as.c_str())};
