...
void SemanticError::errorWhereFailed(SemanticContext*, const ErrorParam& errorParam)
{
    ...
    errorParam.addNote(Diagnostic::hereIs(whereExpr, formNte(Nte0074, whereExpr->token.c_str())));
}
...
void SemanticError::commonErrorNotes(SemanticContext*, const VectorNative<OneTryMatch*>& tryMatches, AstNode* node, Diagnostic* err, Vector<const Diagnostic*>& notes)
{
    ...
    {
        const auto msg  = formNte(Nte0146, tryMatches[0]->overload->symbol->getFullName().c_str());
    ...
    if (node->is(AstNodeKind::Identifier) && tryMatches.size() == 1)
    {
            ...
            overload->node->ownerStructScope->owner != identifierRef->startScope->owner)
        {
            ...
            {
                const auto msg = formNte(Nte0114,
            ...
            for (const auto s : identifierRef->startScope->childScopes)
            {
                ...
                {
                    auto msg = formNte(Nte0145, node->token.c_str(), s->getFullName().c_str());
                    err->remarks.push_back(msg);
                }
            }
        }
...
{
    Utf8 text = formErr(Err0616, node->token.c_str(), typeInfo->getDisplayNameC());
    if (msg)
    {
        text += " ";
        text += msg;
                                          ...
                                          AstNode*      otherSymbolDecl)
{
    ...
    Diagnostic err{sourceFile, token, formErr(Err0509, what.c_str(), Naming::kindName(thisKind).c_str(), thisName.c_str(), as.c_str())};
    err.addNote(otherSymbolDecl, otherSymbolDecl->getTokenName(), toNte(Nte0079));
    return context->report(err);
}
