...
namespace
{
    void errorMissingNamedArgument(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        const auto msg = formErr(Err0563, Naming::niceArgumentRank(errorParam.badParamIdx).c_str());
        const auto err = new Diagnostic{errorParam.failedParam, msg};
        errorParam.addError(err);

        const auto note = Diagnostic::note(errorParam.oneTry->callParameters->children[errorParam.badParamIdx - 2], toNte(Nte0162));
    ...
    void errorInvalidNamedArgument(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        {
            errorParam.addError(new Diagnostic{isNamed, formErr(Err0708, isNamed->token.c_str(), typeInfo->getDisplayNameC())});
            if (typeInfo->declNode->hasExtraPointer(ExtraPointerKind::ExportNode))
                errorParam.addNote(Diagnostic::note(typeInfo->declNode->extraPointer<AstNode>(ExtraPointerKind::ExportNode), toNte(Nte0080)));
        ...
        {
            errorParam.addError(new Diagnostic{isNamed, formErr(Err0721, isNamed->token.c_str())});
            errorParam.addNote(Diagnostic::hereIs(errorParam.oneTry->overload));
        }
    }
    ...
    void errorDuplicatedNamedArgument(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        const auto note = Diagnostic::note(errorParam.oneTry->callParameters->children[other], toNte(Nte0178));
        errorParam.addNote(note);
    }
    ...
    void errorMissingArguments(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        const auto err      = new Diagnostic{node, formErr(Err0534, Naming::kindName(overload).c_str())};
        errorParam.addError(err);
    ...
    void errorNotEnoughArguments(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        if (!callParameters || callParameters->children.empty())
            err = new Diagnostic{node, node->token, formErr(Err0534, Naming::kindName(overload).c_str())};
        else if (errorParam.destAttrDecl)
            err = new Diagnostic{node, node->token, formErr(Err0589, niceName.c_str())};
        else
            err = new Diagnostic{node, node->token, formErr(Err0590, niceName.c_str())};
        ...
        for (uint32_t si = 0; si < match.solvedParameters.size(); si++)
        {
            ...
            if (destParam->hasSpecFlag(AstVarDecl::SPEC_FLAG_UNNAMED) || Parser::isGeneratedName(destParam->token.text))
                err->remarks.push_back(formNte(Nte0092, Naming::niceParameterRank(si + 1).c_str(), destParam->typeInfo->getDisplayNameC()));
            else
                err->remarks.push_back(formNte(Nte0094, destParam->token.c_str(), destParam->typeInfo->getDisplayNameC()));
    ...
    void errorNotEnoughGenericArguments(const SemanticContext* context, const ErrorParam& errorParam)
    {
        ...
        if (errorParam.destFuncDecl && errorParam.destFuncDecl->isSpecialFunctionName())
            err = new Diagnostic{errNode, errNode->token, formErr(Err0293, niceName.c_str(), errorParam.destFuncDecl->token.c_str())};
        else if (genericParameters)
            err = new Diagnostic{genericParameters, formErr(Err0591, niceName.c_str())};
        else
            err = new Diagnostic{errNode, errNode->token, formErr(Err0548, niceName.c_str())};
        errorParam.addError(err);
        const auto note = Diagnostic::hereIs(overload);
        errorParam.addNote(note);
        ...
        if (genericParameters && errorParam.destGenericParameters && genericParameters->childCount() < errorParam.destGenericParameters->childCount())
        {
            for (uint32_t si = genericParameters->childCount(); si < errorParam.destGenericParameters->childCount(); si++)
            {
                ...
                err->remarks.push_back(formNte(Nte0093, destParam->token.c_str()));
            }
        }
    }
    ...
    void errorTooManyArguments(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        if (errorParam.destStructDecl && errorParam.destStructDecl->typeInfo->isTuple())
            msg = formErr(Err0633, match.badSignatureInfos.badSignatureNum2, match.badSignatureInfos.badSignatureNum1);
        else if (errorParam.destStructDecl)
            msg = formErr(Err0631, match.badSignatureInfos.badSignatureNum2, errorParam.destStructDecl->typeInfo->getDisplayNameC(), match.badSignatureInfos.badSignatureNum1);
        else
            msg = formErr(Err0627, match.badSignatureInfos.badSignatureNum2, match.badSignatureInfos.badSignatureNum1);
    ...
    void errorTooManyGenericArguments(const SemanticContext* context, const ErrorParam& errorParam)
    {
        ...
        {
            const auto msg = formErr(Err0495, Naming::kindName(symbol->kind).c_str(), symbol->name.c_str());
        ...
        else
        {
            ...
            const auto msg = formErr(Err0628, match.badSignatureInfos.badSignatureNum2, Naming::kindName(symbol->kind).c_str(), symbol->name.c_str(), match.badSignatureInfos.badSignatureNum1);
            err            = new Diagnostic{errNode, msg};
        }
    ...
    void errorMismatchGenericValue(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        const auto msg = formErr(Err0296, bi.badGenMatch.c_str(),
    ...
    void errorBadGenericSignature(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        if (errorParam.destFuncDecl && bi.badSignatureParameterIdx < errorParam.destFuncDecl->genericParameters->childCount())
        {
            ...
            const auto note     = Diagnostic::note(reqParam, formNte(Nte0070, reqParam->token.c_str(), Naming::kindName(overload).c_str()));
            errorParam.addNote(note);
        }
        else
        {
    ...
    void errorBadSignature(SemanticContext* context, const ErrorParam& errorParam)
    {
        ...
        if (overload->typeInfo->isStruct())
        {
            ...
            const auto msg       = formErr(Err0647, bi.badSignatureRequestedType->getDisplayNameC(), fieldName.c_str(), bi.badSignatureGivenType->getDisplayNameC());
        ...
        if (bi.castErrorType == CastErrorType::Const)
        {
            ...
            if (callOver && callOver->hasFlag(OVERLOAD_IS_LET))
                errorParam.addNote(Diagnostic::note(callOver->node, callOver->node->token, toNte(Nte0018)));
        }

        // Generic comes from
        if (bi.genMatchFromNode)
        {
            const auto msg  = formNte(Nte0071, bi.genMatchFromNode->typeInfo->getDisplayNameC());
            ...
            if (!bi.castErrorFlags.has(CAST_FLAG_EXPLICIT) || bi.castErrorFlags.has(CAST_FLAG_COERCE))
            {
                if (bi.castErrorToType && bi.castErrorFromType && !bi.castErrorToType->isGeneric())
                {
                    if (TypeManager::makeCompatibles(context, bi.castErrorToType, bi.castErrorFromType, nullptr, nullptr, CAST_FLAG_EXPLICIT | CAST_FLAG_JUST_CHECK))
                        errorParam.addNote(Diagnostic::note(callParamNode, formNte(Nte0032, bi.castErrorToType->getDisplayNameC())));
                }
            }
        }
