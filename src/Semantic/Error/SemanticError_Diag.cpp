...
namespace
{
    void errorMissingNamedArgument(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        const auto msg = formErr(Err0469, Naming::niceArgumentRank(errorParam.badParamIdx).cstr());
        const auto err = new Diagnostic{errorParam.failedParam, msg};
        errorParam.addError(err);

        const auto note = Diagnostic::note(errorParam.oneTry->callParameters->children[errorParam.badParamIdx - 2], toNte(Nte0166));
    ...
    void errorInvalidNamedArgument(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        {
            errorParam.addError(new Diagnostic{isNamed, formErr(Err0711, isNamed->token.cstr(), typeInfo->getDisplayNameC())});
            ...
            if (typeInfo->declNode->hasExtraPointer(ExtraPointerKind::ExportNode))
                errorParam.addNote(Diagnostic::note(typeInfo->declNode->extraPointer<AstNode>(ExtraPointerKind::ExportNode), toNte(Nte0203)));
            else if (errorParam.oneTry->overload)
                errorParam.addNote(Diagnostic::hereIs(errorParam.oneTry->overload));
        }
        else
        {
            errorParam.addError(new Diagnostic{isNamed, formErr(Err0724, isNamed->token.cstr())});
    ...
    void errorDuplicatedNamedArgument(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        Utf8 msg;
        if (errorParam.destStructDecl)
            msg = formErr(Err0533, isNamed->token.cstr());
        else
            msg = formErr(Err0042, isNamed->token.cstr());
        ...
        const auto note = Diagnostic::note(errorParam.oneTry->callParameters->children[other], toNte(Nte0192));
        errorParam.addNote(note);
    }
    ...
    void errorNotEnoughArguments(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        Diagnostic* err;
        if (!callParameters)
            err = new Diagnostic{node, node->token, formErr(Err0445, Naming::kindName(overload).cstr(), node->token.cstr())};
        else if (errorParam.destAttrDecl)
            err = new Diagnostic{node, node->token, formErr(Err0495, node->token.cstr())};
        else
            err = new Diagnostic{node, node->token, formErr(Err0496, node->token.cstr())};
        ...
        for (uint32_t si = 0; si < match.solvedParameters.size(); si++)
        {
            ...
            if (destParam->hasSpecFlag(AstVarDecl::SPEC_FLAG_UNNAMED) || Parser::isGeneratedName(destParam->token.text))
                err->remarks.push_back(formNte(Nte0089, Naming::niceParameterRank(si + 1).cstr(), destParam->typeInfo->getDisplayNameC()));
            else
                err->remarks.push_back(formNte(Nte0091, destParam->token.cstr(), destParam->typeInfo->getDisplayNameC()));
    ...
    void errorNotEnoughGenericArguments(const SemanticContext* context, const ErrorParam& errorParam)
    {
        ...
        if (errorParam.destFuncDecl && errorParam.destFuncDecl->isSpecialFunctionName())
            err = new Diagnostic{errNode, errNode->token, formErr(Err0747, niceName.cstr(), errorParam.destFuncDecl->token.cstr())};
        else if (genericParameters)
            err = new Diagnostic{genericParameters, formErr(Err0497, niceName.cstr())};
        else
            err = new Diagnostic{errNode, errNode->token, formErr(Err0065, niceName.cstr())};
        errorParam.addError(err);
        const auto note = Diagnostic::hereIs(overload);
        errorParam.addNote(note);
        ...
        if (genericParameters && errorParam.destGenericParameters && genericParameters->childCount() < errorParam.destGenericParameters->childCount())
        {
            for (uint32_t si = genericParameters->childCount(); si < errorParam.destGenericParameters->childCount(); si++)
            {
                ...
                err->remarks.push_back(formNte(Nte0090, destParam->token.cstr()));
            }
        }
    }
    ...
    void errorTooManyArguments(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        if (errorParam.destStructDecl && errorParam.destStructDecl->typeInfo->isTuple())
            msg = formErr(Err0556, match.badSignatureInfos.badSignatureNum2, match.badSignatureInfos.badSignatureNum1);
        else if (errorParam.destStructDecl)
            msg = formErr(Err0554, match.badSignatureInfos.badSignatureNum2, errorParam.destStructDecl->typeInfo->getDisplayNameC(), match.badSignatureInfos.badSignatureNum1);
        else
            msg = formErr(Err0550, match.badSignatureInfos.badSignatureNum2, match.badSignatureInfos.badSignatureNum1);
    ...
    void errorTooManyGenericArguments(const SemanticContext* context, const ErrorParam& errorParam)
    {
        ...
        {
            const auto msg = formErr(Err0404, Naming::kindName(symbol->kind).cstr(), symbol->name.cstr());
        ...
        else
        {
            ...
            const auto msg = formErr(Err0551, match.badSignatureInfos.badSignatureNum2, Naming::kindName(symbol->kind).cstr(), symbol->name.cstr(), match.badSignatureInfos.badSignatureNum1);
            err            = new Diagnostic{errNode, msg};
        }
    ...
    void errorMismatchGenericValue(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        const auto msg = formErr(Err0187,
    ...
    void errorBadGenericSignature(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        {
            err = new Diagnostic{errorNode, toErr(Err0402)};
        }
        else if (match.matchFlags.has(SymbolMatchContext::MATCH_ERROR_TYPE_VALUE))
        {
            err = new Diagnostic{errorNode, toErr(Err0403)};
        }
        else
        {
            const auto msg = formErr(Err0625, bi.badSignatureRequestedType->getDisplayNameC(), niceArg.cstr(), bi.badSignatureGivenType->getDisplayNameC());
        ...
        if (errorParam.destFuncDecl && bi.badSignatureParameterIdx < errorParam.destFuncDecl->genericParameters->childCount())
        {
            ...
            const auto note     = Diagnostic::note(reqParam, formNte(Nte0193, reqParam->token.cstr(), Naming::kindName(overload).cstr()));
            errorParam.addNote(note);
        }
        else
        {
    ...
    void errorBadSignature(SemanticContext* context, const ErrorParam& errorParam)
    {
        ...
        if (overload->typeInfo->isStruct())
        {
            ...
            const auto msg       = formErr(Err0624, bi.badSignatureRequestedType->getDisplayNameC(), fieldName.cstr(), bi.badSignatureGivenType->getDisplayNameC());
        ...
        {
            const auto msg     = formErr(Err0612, bi.badSignatureGivenType->getDisplayNameC());
        ...
        {
            const auto msg = formErr(Err0570, bi.badSignatureRequestedType->getDisplayNameC(), bi.badSignatureGivenType->getDisplayNameC());
            err            = new Diagnostic{callParamNode, callParamNode->token, msg};
        }
        else
        {
            const auto msg = formErr(Err0594, bi.badSignatureRequestedType->getDisplayNameC(), bi.badSignatureGivenType->getDisplayNameC());
            err            = new Diagnostic{callParamNode, msg};
        }
        ...
        {
            const auto msg  = formNte(Nte0149, bi.genMatchFromNode->typeInfo->getDisplayNameC());
            ...
            if (!bi.castErrorFlags.has(CAST_FLAG_EXPLICIT) || bi.castErrorFlags.has(CAST_FLAG_COERCE))
            {
                if (bi.castErrorToType && bi.castErrorFromType && !bi.castErrorToType->isGeneric())
                {
                    if (TypeManager::makeCompatibles(context, bi.castErrorToType, bi.castErrorFromType, nullptr, nullptr, CAST_FLAG_EXPLICIT | CAST_FLAG_JUST_CHECK))
                        errorParam.addNote(Diagnostic::note(callParamNode, formNte(Nte0034, bi.castErrorToType->getDisplayNameC())));
                }
            }
        }
        ...
        {
            Diagnostic* note = Diagnostic::note(destParamNode, destParamNode->token, toNte(Nte0185));
            errorParam.addNote(note);
        }
        else
        {
            Diagnostic* note = Diagnostic::note(destParamNode, destParamNode->token, formNte(Nte0184, destParamNode->token.cstr()));
            errorParam.addNote(note);
        }
    }
...
void SemanticError::getDiagnosticForMatch(SemanticContext* context, OneTryMatch& oneTry, Vector<const Diagnostic*>& diagError, Vector<const Diagnostic*>& diagNote)
{
    ...
    else if (declNode->is(AstNodeKind::VarDecl))
    {
        ...
        errorParam.destLambdaDecl = castAst<AstTypeLambda>(type->declNode, AstNodeKind::TypeLambda, AstNodeKind::TypeClosure);
    ...
    switch (oneTry.symMatchContext.result)
    {
        ...
        case MatchResult::BadGenericSignature:
            ...
            if (bi.badSignatureRequestedType->isNative() || bi.badSignatureRequestedType->isStruct())
            {
                ...
                {
                    errorParam.explicitCastMsg = formNte(Nte0037, bi.badSignatureRequestedType->name.cstr());
                    break;
                }
            }
