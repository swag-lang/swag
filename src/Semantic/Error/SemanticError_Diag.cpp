...
namespace
{
    void errorMissingNamedArgument(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        const auto msg = formErr(Err0452, Naming::niceArgumentRank(errorParam.badParamIdx).cstr());
    ...
    void errorInvalidNamedArgument(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        {
            errorParam.addError(new Diagnostic{isNamed, formErr(Err0085, isNamed->token.cstr(), typeInfo->getDisplayNameC())});
        ...
        {
            errorParam.addError(new Diagnostic{isNamed, formErr(Err0098, isNamed->token.cstr())});
    ...
    void errorDuplicatedNamedArgument(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        Utf8 msg;
        if (errorParam.destStructDecl)
            msg = formErr(Err0249, isNamed->token.cstr());
        else
            msg = formErr(Err0239, isNamed->token.cstr());
    ...
    void errorNotEnoughArguments(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        Diagnostic* err;
        if (!callParameters)
            err = new Diagnostic{node, node->token, formErr(Err0394, Naming::kindName(overload).cstr(), node->token.cstr())};
        else if (errorParam.destAttrDecl)
            err = new Diagnostic{node, node->token, formErr(Err0555, node->token.cstr())};
        else
            err = new Diagnostic{node, node->token, formErr(Err0556, Naming::kindName(overload).cstr(), node->token.cstr())};
    ...
    void errorNotEnoughGenericArguments(const SemanticContext* context, const ErrorParam& errorParam)
    {
        ...
        if (errorParam.destFuncDecl && errorParam.destFuncDecl->isSpecialFunctionName())
            err = new Diagnostic{errNode, errNode->token, formErr(Err0072, niceName.cstr(), errorParam.destFuncDecl->token.cstr())};
        else if (genericParameters)
            err = new Diagnostic{genericParameters, formErr(Err0557, niceName.cstr(), overload->symbol->name.cstr())};
        else
            err = new Diagnostic{errNode, errNode->token, formErr(Err0432, niceName.cstr())};
        errorParam.addError(err);
        const auto note = Diagnostic::hereIs(overload);
        errorParam.addNote(note);
    ...
    void errorTooManyArguments(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        if (errorParam.destStructDecl && errorParam.destStructDecl->typeInfo->isTuple())
            msg = formErr(Err0600, match.badSignatureInfos.badSignatureNum2, match.badSignatureInfos.badSignatureNum1);
        else if (errorParam.destStructDecl)
            msg = formErr(Err0598, errorParam.destStructDecl->typeInfo->getDisplayNameC(), match.badSignatureInfos.badSignatureNum2, match.badSignatureInfos.badSignatureNum1);
        else
    ...
    void errorTooManyGenericArguments(const SemanticContext* context, const ErrorParam& errorParam)
    {
        ...
        {
            const auto msg = formErr(Err0554, Naming::kindName(symbol->kind).cstr(), symbol->name.cstr());
        ...
        else
        {
            ...
            const auto msg = formErr(Err0594, Naming::kindName(symbol->kind).cstr(), symbol->name.cstr(), match.badSignatureInfos.badSignatureNum2, match.badSignatureInfos.badSignatureNum1);
            err            = new Diagnostic{errNode, msg};
        }
    ...
    void errorMismatchGenericValue(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        const auto msg = formErr(Err0276,
    ...
    void errorBadGenericSignature(SemanticContext*, const ErrorParam& errorParam)
    {
        ...
        {
            err = new Diagnostic{errorNode, toErr(Err0580)};
        }
        else if (match.matchFlags.has(SymbolMatchContext::MATCH_ERROR_TYPE_VALUE))
        {
            err = new Diagnostic{errorNode, toErr(Err0581)};
        }
        else
        {
            const auto msg = formErr(Err0603, bi.badSignatureRequestedType->getDisplayNameC(), bi.badSignatureGivenType->getDisplayNameC());
    ...
    void errorBadSignature(SemanticContext* context, const ErrorParam& errorParam)
    {
        ...
        if (overload->typeInfo->isStruct())
        {
            ...
            const auto msg       = formErr(Err0605, bi.badSignatureGivenType->getDisplayNameC(), fieldName.cstr(), fieldName.cstr(), bi.badSignatureRequestedType->getDisplayNameC());
        ...
        else if (errorParam.oneTry->ufcs && bi.badSignatureParameterIdx == 0 && bi.castErrorType == CastErrorType::Const)
        {
            ...
            const auto msg   = formErr(Err0539, tname, tname, bi.badSignatureGivenType->getDisplayNameC());
        ...
        else if (errorParam.oneTry->ufcs && bi.badSignatureParameterIdx == 0)
        {
            ...
            const auto msg  = formErr(Err0640, name, bi.badSignatureGivenType->getDisplayNameC(), name, bi.badSignatureRequestedType->getDisplayNameC());
            err             = new Diagnostic{callParamNode, callParamNode->token, msg};
        }
        ...
        {
            const auto name = errorParam.oneTry->overload->node->token.cstr();
            auto       n    = formErr(Err0604, bi.badSignatureRequestedType->getDisplayNameC(), bi.badSignatureGivenType->getDisplayNameC());
        ...
        {
            const auto n = formErr(Err0604, bi.badSignatureRequestedType->getDisplayNameC(), bi.badSignatureGivenType->getDisplayNameC());
            err          = new Diagnostic{callParamNode, n};
        }
