...
namespace
{
    Diagnostic* unknownIdentifierInScope(const AstIdentifierRef* identifierRef, AstIdentifier* identifier, Vector<const Diagnostic*>& notes)
    {
        ...
        if (prevIdentifier && prevIdentifier->identifierExtension && prevIdentifier->identifierExtension->alternateEnum)
        {
            ...
            const auto msg     = formErr(Err0708, identifier->token.c_str(), altEnum->getDisplayNameC(), whereScopeName.c_str(), displayName.c_str());
        ...
        {
            err = new Diagnostic{identifier, identifier->token, formErr(Err0413, typeWhere->getDisplayNameC())};
        }
        else if (typeWhere->isStruct() && identifier->token.text.startsWith(g_LangSpec->name_opVisit))
        {
            ...
            err = new Diagnostic{visitNode, visitNode->extraNameToken, formErr(Err0408, variant.c_str(), typeWhere->getDisplayNameC())};
        }
        ...
        {
            err = new Diagnostic{identifier, identifier->token, formErr(Err0709, identifier->token.c_str(), typeWhere->getDisplayNameC())};
        }
...
bool SemanticError::unknownIdentifierError(SemanticContext* context, const AstIdentifierRef* identifierRef, AstIdentifier* identifier)
{
    ...
    switch (searchFor)
    {
        case IdentifierSearchFor::Function:
            err = new Diagnostic{identifier->token.sourceFile, identifier->token, formErr(Err0707, identifier->token.c_str())};
            break;
        ...
        case IdentifierSearchFor::Struct:
            ...
            break;
        default:
            err = new Diagnostic{identifier->token.sourceFile, identifier->token, formErr(Err0711, identifier->token.c_str())};
            break;
    }
