...
namespace
{
    Diagnostic* unknownIdentifierInScope(const AstIdentifierRef* identifierRef, AstIdentifier* identifier, Vector<const Diagnostic*>& notes)
    {
        ...
        if (prevIdentifier && prevIdentifier->identifierExtension && prevIdentifier->identifierExtension->alternateEnum)
        {
            ...
            const auto msg     = formErr(Err0718, identifier->token.cstr(), altEnum->getDisplayNameC(), whereScopeName.cstr(), displayName.cstr());
        ...
        {
            err = new Diagnostic{identifier, identifier->token, formErr(Err0709, identifier->token.cstr(), typeWhere->getDisplayNameC())};
        }
        else if (typeWhere->isStruct() && identifier->token.is(g_LangSpec->name_opVisit))
        {
            err = new Diagnostic{identifier, identifier->token, formErr(Err0736, typeWhere->getDisplayNameC())};
        }
        else if (typeWhere->isStruct() && identifier->token.text.startsWith(g_LangSpec->name_opVisit))
        {
            ...
            err = new Diagnostic{visitNode, visitNode->extraNameToken, formErr(Err0737, variant.cstr(), typeWhere->getDisplayNameC())};
        }
        else if (typeWhere->isStruct() && identifier->callParameters)
        {
            err = new Diagnostic{identifier, identifier->token, formErr(Err0723, identifier->token.cstr(), typeWhere->getDisplayNameC())};
        }
...
bool SemanticError::unknownIdentifierError(SemanticContext* context, const AstIdentifierRef* identifierRef, AstIdentifier* identifier)
{
    ...
    switch (searchFor)
    {
        case IdentifierSearchFor::Function:
            err = new Diagnostic{identifier->token.sourceFile, identifier->token, formErr(Err0714, identifier->token.cstr())};
            break;
        ...
        case IdentifierSearchFor::Type:
            err = new Diagnostic{identifier->token.sourceFile, identifier->token, formErr(Err0728, identifier->token.cstr())};
            break;
        case IdentifierSearchFor::Struct:
            err = new Diagnostic{identifier->token.sourceFile, identifier->token, formErr(Err0727, identifier->token.cstr())};
            break;
        case IdentifierSearchFor::Enum:
            err = new Diagnostic{identifier->token.sourceFile, identifier->token, formErr(Err0708, identifier->token.cstr())};
            break;
        case IdentifierSearchFor::Variable:
            err = new Diagnostic{identifier->token.sourceFile, identifier->token, formErr(Err0734, identifier->token.cstr())};
            break;
        default:
            err = new Diagnostic{identifier->token.sourceFile, identifier->token, formErr(Err0716, identifier->token.cstr())};
            break;
    }
