...
bool Semantic::resolveConditionalOp(SemanticContext* context)
{
    ...
    // Make the cast
    {
        PushErrCxtStep ec(context, rightT, ErrCxtStepKind::Note, [] { return toNte(Nte0219); });
...
bool Semantic::resolveNullConditionalOp(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node->token.sourceFile, node->token, toErr(Err0573)};
    ...
    {
        Diagnostic err{node->token.sourceFile, node->token, formErr(Err0574, typeInfo->getDisplayNameC())};
        err.addNote(expression, Diagnostic::isType(typeInfo));
        return context->report(err);
    }
...
bool Semantic::resolveRange(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node->expressionLow, formErr(Err0225, node->expressionLow->typeInfo->getDisplayNameC())};
    ...
    {
        Diagnostic err{node, node->token, toErr(Err0564)};
