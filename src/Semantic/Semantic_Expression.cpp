...
bool Semantic::resolveNullConditionalOp(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node->token.sourceFile, node->token, toErr(Err0159)};
    ...
    {
        Diagnostic err{node->token.sourceFile, node->token, formErr(Err0160, typeInfo->getDisplayNameC())};
        err.addNote(expression, Diagnostic::isType(typeInfo));
        return context->report(err);
    }
...
bool Semantic::resolveRange(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node->expressionLow, formErr(Err0349, node->expressionLow->typeInfo->getDisplayNameC())};
    ...
    {
        Diagnostic err{node, node->token, toErr(Err0650)};
