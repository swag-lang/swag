...
bool Semantic::setupFuncDeclParams(SemanticContext* context, TypeInfoFuncAttr* typeInfo, const AstNode* funcNode, AstNode* parameters, bool forGenerics)
{
    ...
    for (const auto param : parameters->children)
    {
        ...
        if (param->ownerScope->is(ScopeKind::Attribute))
        {
            ...
            {
                Diagnostic err{nodeParam->type ? nodeParam->type : nodeParam, formErr(Err0253, toTest->getDisplayNameC())};
        ...
        else if (!nodeParam->typeInfo->isCode())
        {
            if (defaultValueDone)
            {
                Diagnostic err{nodeParam, formErr(Err0447, Naming::niceParameterRank(index).cstr())};
...
bool Semantic::resolveFuncDecl(SemanticContext* context)
{
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_MAIN_FUNC))
    {
        ...
        {
            Diagnostic err{funcNode, toErr(Err0525)};
    ...
    if (funcNode->aliasMask && funcNode->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN))
    {
        ...
        {
            if ((mask & 1) == 0)
                return context->report({funcNode, funcNode->tokenName, formErr(Err0488, funcNode->token.cstr(), n)});
            mask >>= 1;
        }
    }
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_PUBLIC) && funcNode->hasAttribute(ATTRIBUTE_INLINE) && !funcNode->hasAstFlag(AST_FROM_GENERIC))
        ...
        SWAG_VERIFY(funcNode->returnType->hasSpecFlag(AstFuncDecl::SPEC_FLAG_RETURN_DEFINED), context->report({funcNode, funcNode->getTokenName(), toErr(Err0472)}));
    ...
    {
        Diagnostic err{funcNode, funcNode->getTokenName(), toErr(Err0665)};
    ...
    if (!funcNode->hasAttribute(ATTRIBUTE_GENERATED_FUNC))
    {
        ...
        {
            SWAG_VERIFY(module->is(ModuleKind::Test), context->report({funcNode, toErr(Err0316)}));
        }
    }
        ...
        if (!funcNode->content->hasAstFlag(AST_NO_SEMANTIC))
        {
            if (!funcNode->hasSemFlag(SEMFLAG_SCOPE_HAS_RETURN))
            {
                if (funcNode->hasSemFlag(SEMFLAG_FCT_HAS_RETURN))
                    return context->report({funcNode->returnType, formErr(Err0478, funcNode->getDisplayNameC())});
    ...
    if (funcNode->hasSpecFlag(AstFuncDecl::SPEC_FLAG_IMPL))
    {
        ...
        SWAG_VERIFY(implFor, context->report({funcNode->token.sourceFile, funcNode->implLoc, funcNode->implLoc, toErr(Err0336)}));
    }
...
bool Semantic::resolveFuncDeclType(SemanticContext* context)
{
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_MESSAGE_FUNC) && funcNode->parameters)
    {
        ...
        SWAG_VERIFY(paramType->isEnum(), context->report({parameters, formErr(Err0568, paramType->getDisplayNameC())}));
        paramType->computeScopedName();
        SWAG_VERIFY(paramType->scopedName == g_LangSpec->name_Swag_CompilerMsgMask, context->report({parameters, formErr(Err0568, paramType->getDisplayNameC())}));
    ...
    if (!typeNode->children.empty())
    {
        ...
        if (concrete->isVoid())
        {
            Diagnostic err{typeNode->token.sourceFile, typeNode->token.startLocation, front->token.endLocation, toErr(Err0227)};
    ...
    {
        Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0368, funcNode->token.cstr())};
    ...
    {
        Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0366, funcNode->token.cstr())};
        const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_CalleeReturn);
        err.addNote(attr, formNte(Nte0181, "attribute"));
        return context->report(err);
    }
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_IMPLICIT) && funcNode->hasAstFlag(AST_GENERIC | AST_FROM_GENERIC))
    {
        ...
        if (!ok)
        {
            Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0370, funcNode->getDisplayNameC())};
            const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_Implicit);
            err.addNote(attr, formNte(Nte0181, "attribute"));
            return context->report(err);
        }
    ...
    if (!funcNode->hasAstFlag(AST_FROM_GENERIC))
    {
        ...
        if (funcNode->genericParameters)
        {
            if (funcNode->hasAttribute(ATTRIBUTE_NOT_GENERIC))
            {
                Diagnostic err{funcNode->genericParameters, formErr(Err0672, funcNode->token.cstr())};
                const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_NotGeneric);
                err.addNote(attr, formNte(Nte0181, "attribute"));
                return context->report(err);
            }
        ...
        {
            Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0670, funcNode->token.cstr())};
            const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_NotGeneric);
            err.addNote(attr, formNte(Nte0181, "attribute"));
            return context->report(err);
        }
    ...
    {
        return context->report({typeNode->firstChild(), formErr(Err0226, typeInfo->returnType->getDisplayNameC())});
    }
        ...
        if (structDecl->typeInfo->isGeneric())
        {
            for (const auto c : funcNode->genericParameters->children)
            {
                ...
                for (const auto sc : structDecl->genericParameters->children)
                {
                    ...
                    {
                        Diagnostic err{c, formErr(Err0531, c->resolvedSymbolOverload()->node->token.cstr())};
...
bool Semantic::registerFuncSymbol(SemanticContext* context, AstFuncDecl* funcNode, OverloadFlags overFlags)
{
    if (!overFlags.has(OVERLOAD_INCOMPLETE))
    {
        ...
        {
            Diagnostic err{funcNode->returnType->firstChild(), toErr(Err0685)};
            const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_CalleeReturn);
            err.addNote(attr, formNte(Nte0181, "attribute"));
            return context->report(err);
        }
        ...
        {
            Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0475, funcNode->token.cstr())};
    ...
    if (!funcNode->hasAstFlag(AST_FROM_GENERIC))
    {
        ...
        if (symbolName->overloads.size() > 1 && !funcNode->hasAttribute(ATTRIBUTE_OVERLOAD))
        {
            ...
            if (other)
            {
                const Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0666, funcNode->token.cstr())};
    ...
    for (auto c : node->children)
    {
        ...
        if (typeField->isStruct())
        {
            ...
            if (!typeStruct->isPlainOldData())
                return context->report({c, formErr(Err0158, c->token.cstr())});
            continue;
        }

        const auto aKindName = Naming::aKindName(typeField);
        return context->report({c, formErr(Err0157, c->token.cstr(), aKindName.cstr(), aKindName.cstr())});
    }
...
bool Semantic::resolveFuncCallGenParams(SemanticContext* context)
{
    ...
    for (auto c : node->children)
    {
        ...
        {
            return context->report({c, formErr(Err0186, Naming::aKindName(symbol->kind).cstr(), symbol->name.cstr())});
        }
    }
...
bool Semantic::resolveFuncCallParam(SemanticContext* context)
{
    ...
    SWAG_VERIFY(!node->typeInfo->isCVariadic(), context->report({node, toErr(Err0489)}));
...
bool Semantic::resolveRetVal(SemanticContext* context)
{
    ...
    SWAG_VERIFY(fctDecl, context->report({node, toErr(Err0347)}));
    SWAG_VERIFY(node->ownerScope && node->ownerScope->isNot(ScopeKind::Function), context->report({node, toErr(Err0347)}));
    ...
    SWAG_VERIFY(typeFct->returnType && !typeFct->returnType->isVoid(), context->report({node, toErr(Err0346)}));
...
bool Semantic::resolveReturn(SemanticContext* context)
{
    ...
    if (funcNode->returnTypeDeducedNode)
    {
        ...
        if (node->children.empty())
        {
            ...
            {
                Diagnostic err{node, formErr(Err0476, funcReturnType->getDisplayNameC())};
        ...
        {
            Diagnostic err{child, formErr(Err0635, childType->getDisplayNameC())};
        ...
        {
            Diagnostic err{child, formErr(Err0634, child->typeInfo->getDisplayNameC(), funcNode->returnType->typeInfo->getDisplayNameC())};
            err.addNote(funcNode->returnTypeDeducedNode->firstChild(), toNte(Nte0196));
            return context->report(err);
        }
    }
    ...
    if (node->children.empty())
    {
        Diagnostic err{node, formErr(Err0477, funcNode->returnType->typeInfo->getDisplayNameC())};
    ...
    if (!concreteType->isVoid())
    {
        ...
        {
            if (funcNode->hasAttribute(ATTRIBUTE_SHARP_FUNC))
                return context->report({child, formErr(Err0686, funcNode->getDisplayNameC())});
        }
    }
    ...
    if (returnType->isVoid() && !concreteType->isVoid())
    {
        ...
        if (Parser::isGeneratedName(funcNode->token.text))
            msg = formErr(Err0689, concreteType->getDisplayNameC());
        else
            msg = formErr(Err0690, concreteType->getDisplayNameC(), funcNode->token.cstr());
...
bool Semantic::makeInline(JobContext* context, AstFuncDecl* funcDecl, AstNode* identifier)
{
    ...
    while (ownerInline)
    {
        if (ownerInline->func == funcDecl)
        {
            ...
            {
                const Diagnostic err{identifier, identifier->token, formErr(Err0510, identifier->token.cstr(), g_CommandLine.limitInlineLevel)};
                return context->report(err);
            }
        }
                ...
                if (it == cloneContext.usedReplaceNames.end())
                {
                    if (id->identifierExtension)
                    {
                        for (auto& alias : id->identifierExtension->aliasNames)
                        {
                            if (alias.text == val)
                            {
                                const Diagnostic err{id, alias, formErr(Err0754, alias.cstr())};
                                return context->report(err);
                            }
                        }
                    }
