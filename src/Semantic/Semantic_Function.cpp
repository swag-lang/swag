...
bool Semantic::setupFuncDeclParams(SemanticContext* context, TypeInfoFuncAttr* typeInfo, const AstNode* funcNode, AstNode* parameters, bool forGenerics)
{
    ...
    for (const auto param : parameters->children)
    {
        ...
        if (paramType->isCode())
            SWAG_VERIFY(funcNode->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN), context->report({paramNodeType, toErr(Err0458)}));
        ...
        if (param->ownerScope->is(ScopeKind::Attribute))
        {
            ...
            {
                Diagnostic err{nodeParam->type ? nodeParam->type : nodeParam, formErr(Err0322, toTest->getDisplayNameC())};
        ...
        {
            SWAG_VERIFY(!funcNode->hasAttribute(ATTRIBUTE_INLINE), context->report({sourceFile, nodeParam->token, toErr(Err0461)}));
            typeInfo->addFlag(TYPEINFO_VARIADIC);
            if (index != parameters->childCount())
                return context->report({nodeParam, toErr(Err0460)});
        }
        ...
        else if (!nodeParam->typeInfo->isCode())
        {
            if (defaultValueDone)
            {
                Diagnostic err{nodeParam, formErr(Err0493, Naming::niceParameterRank(index).c_str())};
...
bool Semantic::resolveFuncDecl(SemanticContext* context)
{
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_MAIN_FUNC))
    {
        ...
        {
            Diagnostic err{funcNode, toErr(Err0011)};
    ...
    if (funcNode->aliasMask && funcNode->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN))
    {
        ...
        {
            if ((mask & 1) == 0)
                return context->report({funcNode, funcNode->tokenName, formErr(Err0542, funcNode->token.c_str(), n)});
            mask >>= 1;
        }
    }
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_PUBLIC) && funcNode->hasAttribute(ATTRIBUTE_INLINE) && !funcNode->hasAstFlag(AST_FROM_GENERIC))
        ...
        SWAG_VERIFY(funcNode->returnType->hasSpecFlag(AstFuncDecl::SPEC_FLAG_RETURN_DEFINED), context->report({funcNode, funcNode->getTokenName(), toErr(Err0524)}));
    ...
    {
        Diagnostic err{funcNode, funcNode->getTokenName(), toErr(Err0677)};
    ...
    if (!funcNode->hasAttribute(ATTRIBUTE_GENERATED_FUNC))
    {
        ...
        {
            SWAG_VERIFY(module->is(ModuleKind::Test), context->report({funcNode, toErr(Err0389)}));
        }
    }
        ...
        if (!funcNode->content->hasAstFlag(AST_NO_SEMANTIC))
        {
            if (!funcNode->hasSemFlag(SEMFLAG_SCOPE_HAS_RETURN))
            {
                if (funcNode->hasSemFlag(SEMFLAG_FCT_HAS_RETURN))
                    return context->report({funcNode->returnType, formErr(Err0530, funcNode->getDisplayNameC())});
                return context->report({funcNode->returnType, formErr(Err0531, funcNode->getDisplayNameC(), funcNode->returnType->typeInfo->getDisplayNameC())});
            }
        }
    }
    ...
    if (funcNode->hasSpecFlag(AstFuncDecl::SPEC_FLAG_IMPL))
    {
        ...
        SWAG_VERIFY(implFor, context->report({funcNode->token.sourceFile, funcNode->implLoc, funcNode->implLoc, toErr(Err0664)}));
    }
...
bool Semantic::resolveFuncDeclType(SemanticContext* context)
{
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_MESSAGE_FUNC) && funcNode->parameters)
    {
        ...
        SWAG_VERIFY(paramType->isEnum(), context->report({parameters, formErr(Err0286, paramType->getDisplayNameC())}));
        paramType->computeScopedName();
        SWAG_VERIFY(paramType->scopedName == g_LangSpec->name_Swag_CompilerMsgMask, context->report({parameters, formErr(Err0286, paramType->getDisplayNameC())}));
    ...
    if (!typeNode->children.empty())
    {
        ...
        if (concrete->isVoid())
        {
            Diagnostic err{typeNode->token.sourceFile, typeNode->token.startLocation, front->token.endLocation, toErr(Err0296)};
    ...
    if (!funcNode->hasAstFlag(AST_FROM_GENERIC) && !funcNode->hasSpecFlag(AstFuncDecl::SPEC_FLAG_CHECK_ATTR))
    {
        ...
        if (funcNode->hasAttribute(ATTRIBUTE_SHARP_FUNC))
        {
            if (funcNode->hasAttribute(ATTRIBUTE_MIXIN))
            {
                Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0432, funcNode->getDisplayNameC())};
            ...
            if (funcNode->hasAttribute(ATTRIBUTE_MACRO))
            {
                Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0431, funcNode->getDisplayNameC())};
            ...
            if (funcNode->hasAttribute(ATTRIBUTE_INLINE))
            {
                Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0430, funcNode->getDisplayNameC())};
            ...
            {
                Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0433, funcNode->getDisplayNameC())};
            ...
            {
                Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0423, funcNode->getDisplayNameC())};
    ...
    {
        Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0426, funcNode->token.c_str())};
    ...
    {
        Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0429, funcNode->token.c_str())};
    ...
    {
        Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0424, funcNode->token.c_str())};
        const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_CalleeReturn);
        err.addNote(attr, formNte(Nte0066, "attribute"));
        return context->report(err);
    }
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_IMPLICIT) && funcNode->hasAstFlag(AST_GENERIC | AST_FROM_GENERIC))
    {
        ...
        if (!ok)
        {
            Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0428, funcNode->getDisplayNameC())};
            const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_Implicit);
            err.addNote(attr, formNte(Nte0066, "attribute"));
            return context->report(err);
        }
    ...
    if (!funcNode->hasAstFlag(AST_FROM_GENERIC))
    {
        ...
        if (funcNode->genericParameters)
        {
            if (funcNode->hasAttribute(ATTRIBUTE_NOT_GENERIC))
            {
                Diagnostic err{funcNode->genericParameters, formErr(Err0683, funcNode->token.c_str())};
                const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_NotGeneric);
                err.addNote(attr, formNte(Nte0066, "attribute"));
                return context->report(err);
            }
        ...
        {
            Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0681, funcNode->token.c_str())};
            const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_NotGeneric);
            err.addNote(attr, formNte(Nte0066, "attribute"));
            return context->report(err);
        }
    ...
    {
        return context->report({typeNode->firstChild(), formErr(Err0295, typeInfo->returnType->getDisplayNameC())});
    }
    ...
    if (funcNode->parameters && funcNode->parameters->childCount() == 1)
    {
        ...
        else if (funcNode->token.is(g_LangSpec->name_opDrop))
        {
            ...
            funcNode->addSemFlag(SEMFLAG_NO_PUBLIC);
            SWAG_VERIFY(!typeStruct->declNode->hasAttribute(ATTRIBUTE_CONSTEXPR), context->report({funcNode, funcNode->tokenName, formErr(Err0092, typeStruct->getDisplayNameC())}));
        }
        else if (funcNode->token.is(g_LangSpec->name_opPostCopy))
        {
            ...
            funcNode->addSemFlag(SEMFLAG_NO_PUBLIC);
            SWAG_VERIFY(!typeStruct->hasFlag(TYPEINFO_STRUCT_NO_COPY), context->report({funcNode, funcNode->tokenName, formErr(Err0093, typeStruct->getDisplayNameC())}));
        }
        ...
        if (structDecl->typeInfo->isGeneric())
        {
            for (const auto c : funcNode->genericParameters->children)
            {
                ...
                for (const auto sc : structDecl->genericParameters->children)
                {
                    ...
                    {
                        Diagnostic err{c, formErr(Err0017, c->resolvedSymbolOverload()->node->token.c_str())};
...
bool Semantic::registerFuncSymbol(SemanticContext* context, AstFuncDecl* funcNode, OverloadFlags overFlags)
{
    if (!overFlags.has(OVERLOAD_INCOMPLETE))
    {
        ...
        {
            Diagnostic err{funcNode->returnType->firstChild(), toErr(Err0691)};
            const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_CalleeReturn);
            err.addNote(attr, formNte(Nte0066, "attribute"));
            return context->report(err);
        }
        ...
        {
            Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0527, funcNode->token.c_str())};
    ...
    if (!funcNode->hasAstFlag(AST_FROM_GENERIC))
    {
        ...
        if (symbolName->overloads.size() > 1 && !funcNode->hasAttribute(ATTRIBUTE_OVERLOAD))
        {
            ...
            if (other)
            {
                const Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0678, funcNode->token.c_str())};
    ...
    for (auto c : node->children)
    {
        ...
        if (typeField->isStruct())
        {
            ...
            if (!typeStruct->isPlainOldData())
                return context->report({c, formErr(Err0184, c->token.c_str())});
            continue;
        }

        const auto aKindName = Naming::aKindName(typeField);
        return context->report({c, formErr(Err0183, c->token.c_str(), aKindName.c_str(), aKindName.c_str())});
    }
...
bool Semantic::resolveFuncCallGenParams(SemanticContext* context)
{
    ...
    for (auto c : node->children)
    {
        ...
        {
            return context->report({c, formErr(Err0219, Naming::aKindName(symbol->kind).c_str(), symbol->name.c_str())});
        }
    }
...
bool Semantic::resolveFuncCallParam(SemanticContext* context)
{
    ...
    SWAG_VERIFY(!node->typeInfo->isCVariadic(), context->report({node, toErr(Err0543)}));
...
bool Semantic::resolveRetVal(SemanticContext* context)
{
    ...
    SWAG_VERIFY(fctDecl, context->report({node, toErr(Err0412)}));
    SWAG_VERIFY(node->ownerScope && node->ownerScope->isNot(ScopeKind::Function), context->report({node, toErr(Err0412)}));
    ...
    SWAG_VERIFY(typeFct->returnType && !typeFct->returnType->isVoid(), context->report({node, toErr(Err0134)}));
...
bool Semantic::resolveReturn(SemanticContext* context)
{
    ...
    if (funcNode->returnTypeDeducedNode)
    {
        ...
        if (node->children.empty())
        {
            ...
            {
                Diagnostic err{node, formErr(Err0528, funcReturnType->getDisplayNameC())};
        ...
        {
            Diagnostic err{child, formErr(Err0654, childType->getDisplayNameC())};
        ...
        {
            Diagnostic err{child, formErr(Err0655, child->typeInfo->getDisplayNameC(), funcNode->returnType->typeInfo->getDisplayNameC())};
            err.addNote(funcNode->returnTypeDeducedNode->firstChild(), toNte(Nte0077));
            return context->report(err);
        }
    }
    ...
    if (node->children.empty())
    {
        Diagnostic err{node, formErr(Err0529, funcNode->returnType->typeInfo->getDisplayNameC())};
    ...
    if (!concreteType->isVoid())
    {
        ...
        {
            if (funcNode->hasAttribute(ATTRIBUTE_SHARP_FUNC))
                return context->report({child, formErr(Err0692, funcNode->getDisplayNameC())});
        }
    }
    ...
    if (returnType->isVoid() && !concreteType->isVoid())
    {
        ...
        if (Parser::isGeneratedName(funcNode->token.text))
            msg = formErr(Err0694, concreteType->getDisplayNameC());
        else
            msg = formErr(Err0693, concreteType->getDisplayNameC(), funcNode->token.c_str());
...
bool Semantic::makeInline(JobContext* context, AstFuncDecl* funcDecl, AstNode* identifier)
{
    ...
    while (ownerInline)
    {
        if (ownerInline->func == funcDecl)
        {
            ...
            {
                const Diagnostic err{identifier, identifier->token, formErr(Err0566, identifier->token.c_str(), g_CommandLine.limitInlineLevel)};
                return context->report(err);
            }
        }
                ...
                if (it == cloneContext.usedReplaceNames.end())
                {
                    if (id->identifierExtension)
                    {
                        for (auto& alias : id->identifierExtension->aliasNames)
                        {
                            if (alias.text == val)
                            {
                                const Diagnostic err{id, alias, formErr(Err0737, alias.c_str())};
                                return context->report(err);
                            }
                        }
                    }
