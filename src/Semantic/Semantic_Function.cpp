...
bool Semantic::setupFuncDeclParams(SemanticContext* context, TypeInfoFuncAttr* typeInfo, const AstNode* funcNode, AstNode* parameters, bool forGenerics)
{
    ...
    for (const auto param : parameters->children)
    {
        ...
        if (paramType->isCode())
            SWAG_VERIFY(funcNode->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN), context->report({paramNodeType, toErr(Err0441)}));
        ...
        if (param->ownerScope->is(ScopeKind::Attribute))
        {
            ...
            {
                Diagnostic err{nodeParam->type ? nodeParam->type : nodeParam, formErr(Err0283, toTest->getDisplayNameC())};
        ...
        {
            SWAG_VERIFY(!funcNode->hasAttribute(ATTRIBUTE_INLINE), context->report({sourceFile, nodeParam->token, toErr(Err0444)}));
            typeInfo->addFlag(TYPEINFO_VARIADIC);
            if (index != parameters->childCount())
                return context->report({nodeParam, toErr(Err0443)});
        }
        ...
        else if (!nodeParam->typeInfo->isCode())
        {
            if (defaultValueDone)
            {
                Diagnostic err{nodeParam, formErr(Err0478, Naming::niceParameterRank(index).cstr())};
...
bool Semantic::resolveFuncDecl(SemanticContext* context)
{
    ...
    if (funcNode->aliasMask && funcNode->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN))
    {
        ...
        {
            if ((mask & 1) == 0)
                return context->report({funcNode, funcNode->tokenName, formErr(Err0519, funcNode->token.cstr(), n)});
            mask >>= 1;
        }
    }
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_PUBLIC) && funcNode->hasAttribute(ATTRIBUTE_INLINE) && !funcNode->hasAstFlag(AST_FROM_GENERIC))
        ...
        SWAG_VERIFY(funcNode->returnType->hasSpecFlag(AstFuncDecl::SPEC_FLAG_RETURN_DEFINED), context->report({funcNode, funcNode->getTokenName(), toErr(Err0503)}));
    ...
    if (!funcNode->hasAttribute(ATTRIBUTE_GENERATED_FUNC))
    {
        ...
        {
            SWAG_VERIFY(module->is(ModuleKind::Test), context->report({funcNode, toErr(Err0342)}));
        }
    }
        ...
        if (!funcNode->content->hasAstFlag(AST_NO_SEMANTIC))
        {
            if (!funcNode->hasSemFlag(SEMFLAG_SCOPE_HAS_RETURN))
            {
                if (funcNode->hasSemFlag(SEMFLAG_FCT_HAS_RETURN))
                    return context->report({funcNode->returnType, formErr(Err0510, funcNode->getDisplayNameC())});
                return context->report({funcNode->returnType, formErr(Err0507, funcNode->getDisplayNameC(), funcNode->returnType->typeInfo->getDisplayNameC())});
            }
        }
    }
    ...
    if (funcNode->hasSpecFlag(AstFuncDecl::SPEC_FLAG_IMPL))
    {
        ...
        SWAG_VERIFY(implFor, context->report({funcNode->token.sourceFile, funcNode->implLoc, funcNode->implLoc, toErr(Err0362)}));
    }
...
bool Semantic::resolveFuncDeclType(SemanticContext* context)
{
    ...
    if (funcNode->hasAttribute(ATTRIBUTE_MESSAGE_FUNC) && funcNode->parameters)
    {
        ...
        SWAG_VERIFY(paramType->isEnum(), context->report({parameters, formErr(Err0582, paramType->getDisplayNameC())}));
        paramType->computeScopedName();
        SWAG_VERIFY(paramType->scopedName == g_LangSpec->name_Swag_CompilerMsgMask, context->report({parameters, formErr(Err0582, paramType->getDisplayNameC())}));
    ...
    if (!typeNode->children.empty())
    {
        ...
        if (concrete->isVoid())
        {
            Diagnostic err{typeNode->token.sourceFile, typeNode->token.startLocation, front->token.endLocation, toErr(Err0257)};
    ...
    if (!funcNode->hasAstFlag(AST_FROM_GENERIC) && !funcNode->hasSpecFlag(AstFuncDecl::SPEC_FLAG_CHECK_ATTR))
    {
        ...
        if (funcNode->hasAttribute(ATTRIBUTE_SHARP_FUNC))
        {
            if (funcNode->hasAttribute(ATTRIBUTE_MIXIN))
            {
                Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0404, funcNode->getDisplayNameC())};
            ...
            if (funcNode->hasAttribute(ATTRIBUTE_MACRO))
            {
                Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0403, funcNode->getDisplayNameC())};
            ...
            if (funcNode->hasAttribute(ATTRIBUTE_INLINE))
            {
                Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0402, funcNode->getDisplayNameC())};
            ...
            {
                Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0405, funcNode->getDisplayNameC())};
    ...
    {
        Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0398, funcNode->token.cstr())};
    ...
    {
        Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0401, funcNode->token.cstr())};
    ...
    if (!funcNode->hasAstFlag(AST_FROM_GENERIC))
    {
        ...
        if (funcNode->genericParameters)
        {
            if (funcNode->hasAttribute(ATTRIBUTE_NOT_GENERIC))
            {
                Diagnostic err{funcNode->genericParameters, formErr(Err0689, funcNode->token.cstr())};
                const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_NotGeneric);
                err.addNote(attr, formNte(Nte0181, "attribute"));
                return context->report(err);
            }
        ...
        {
            Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0687, funcNode->token.cstr())};
            const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_NotGeneric);
            err.addNote(attr, formNte(Nte0181, "attribute"));
            return context->report(err);
        }
    ...
    {
        return context->report({typeNode->firstChild(), formErr(Err0256, typeInfo->returnType->getDisplayNameC())});
    }
...
bool Semantic::registerFuncSymbol(SemanticContext* context, AstFuncDecl* funcNode, OverloadFlags overFlags)
{
    if (!overFlags.has(OVERLOAD_INCOMPLETE))
    {
        ...
        {
            Diagnostic err{funcNode->returnType->firstChild(), toErr(Err0701)};
            const auto attr = funcNode->findParentAttrUse(g_LangSpec->name_Swag_CalleeReturn);
            err.addNote(attr, formNte(Nte0181, "attribute"));
            return context->report(err);
        }
        ...
        {
            Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0506, funcNode->token.cstr())};
    ...
    if (!funcNode->hasAstFlag(AST_FROM_GENERIC))
    {
        ...
        if (symbolName->overloads.size() > 1 && !funcNode->hasAttribute(ATTRIBUTE_OVERLOAD))
        {
            ...
            if (other)
            {
                const Diagnostic err{funcNode, funcNode->tokenName, formErr(Err0683, funcNode->token.cstr())};
...
bool Semantic::resolveFuncCallGenParams(SemanticContext* context)
{
    ...
    for (auto c : node->children)
    {
        ...
        {
            return context->report({c, formErr(Err0216, Naming::aKindName(symbol->kind).cstr(), symbol->name.cstr())});
        }
    }
...
bool Semantic::resolveFuncCallParam(SemanticContext* context)
{
    ...
    SWAG_VERIFY(!node->typeInfo->isCVariadic(), context->report({node, toErr(Err0520)}));
...
bool Semantic::resolveRetVal(SemanticContext* context)
{
    ...
    SWAG_VERIFY(fctDecl, context->report({node, toErr(Err0375)}));
    SWAG_VERIFY(node->ownerScope && node->ownerScope->isNot(ScopeKind::Function), context->report({node, toErr(Err0375)}));
    ...
    SWAG_VERIFY(typeFct->returnType && !typeFct->returnType->isVoid(), context->report({node, toErr(Err0374)}));
...
bool Semantic::resolveReturn(SemanticContext* context)
{
    ...
    if (funcNode->returnTypeDeducedNode)
    {
        ...
        if (node->children.empty())
        {
            ...
            {
                Diagnostic err{node, formErr(Err0508, funcReturnType->getDisplayNameC())};
        ...
        {
            Diagnostic err{child, formErr(Err0649, childType->getDisplayNameC())};
        ...
        {
            Diagnostic err{child, formErr(Err0648, child->typeInfo->getDisplayNameC(), funcNode->returnType->typeInfo->getDisplayNameC())};
            err.addNote(funcNode->returnTypeDeducedNode->firstChild(), toNte(Nte0195));
            return context->report(err);
        }
    }
    ...
    if (node->children.empty())
    {
        Diagnostic err{node, formErr(Err0509, funcNode->returnType->typeInfo->getDisplayNameC())};
    ...
    if (!concreteType->isVoid())
    {
        ...
        {
            if (funcNode->hasAttribute(ATTRIBUTE_SHARP_FUNC))
                return context->report({child, formErr(Err0702, funcNode->getDisplayNameC())});
        }
    }
    ...
    if (returnType->isVoid() && !concreteType->isVoid())
    {
        ...
        if (Parser::isGeneratedName(funcNode->token.text))
            msg = formErr(Err0705, concreteType->getDisplayNameC());
        else
            msg = formErr(Err0706, concreteType->getDisplayNameC(), funcNode->token.cstr());
...
bool Semantic::makeInline(JobContext* context, AstFuncDecl* funcDecl, AstNode* identifier)
{
    ...
    while (ownerInline)
    {
        if (ownerInline->func == funcDecl)
        {
            ...
            {
                const Diagnostic err{identifier, identifier->token, formErr(Err0541, identifier->token.cstr(), g_CommandLine.limitInlineLevel)};
                return context->report(err);
            }
        }
                ...
                if (it == cloneContext.usedReplaceNames.end())
                {
                    if (id->identifierExtension)
                    {
                        for (auto& alias : id->identifierExtension->aliasNames)
                        {
                            if (alias.text == val)
                            {
                                const Diagnostic err{id, alias, formErr(Err0773, alias.cstr())};
                                return context->report(err);
                            }
                        }
                    }
