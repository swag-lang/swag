        ...
        {
            {
                ...
                PushErrCxtStep ec(context, declParam, ErrCxtStepKind::Note, [declParam] { return formNte(Nte0182, declParam->token.c_str()); });
...
bool Semantic::setSymbolMatchVar(SemanticContext* context, const OneMatch& oneMatch, AstIdentifierRef* idRef, AstIdentifier* identifier, SymbolOverload* overload)
{
    ...
    if (ownerFct)
    {
        if (!ownerFct->hasAttribute(ATTRIBUTE_COMPILER) && overload->node->hasAttribute(ATTRIBUTE_COMPILER) && !ownerFct->hasAstFlag(AST_IN_RUN_BLOCK))
        {
            ...
            err.addNote(overload->node, overload->node->token, formNte(Nte0159, Naming::kindName(overload->node).c_str()));
    ...
    if (typeInfo->isLambdaClosure() && identifier->callParameters)
    {
        ...
        if (!typeInfoRet->isVoid())
        {
            if (isStatementIdentifier(identifier))
            {
                ...
                {
                    const Diagnostic err(identifier, identifier->token, formErr(Err0750, overload->node->token.c_str()));
...
bool Semantic::setSymbolMatchFunc(SemanticContext* context, const OneMatch& oneMatch, AstIdentifier* identifier, const SymbolOverload* overload)
{
    ...
    if (childIdx)
    {
        ...
        {
            Diagnostic err{prev, formErr(Err0478, Naming::kindName(prev->resolvedSymbolOverload()->node).c_str(), prev->token.c_str(), identifier->token.c_str())};
    ...
    if (identifier->callParameters && !identifier->callParameters->aliasNames.empty())
    {
        if (!overload->node->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN))
        {
            ...
            Diagnostic err{cp->token.sourceFile, cp->aliasNames.front().startLocation, cp->aliasNames.back().endLocation, formErr(Err0645, identifier->token.c_str())};
    ...
    if (!returnType->isVoid())
    {
        if (isStatementIdentifier(identifier))
        {
            ...
            {
                const Diagnostic err(identifier, identifier->token, formErr(Err0748, overload->node->token.c_str()));
                return context->report(err, Diagnostic::hereIs(overload));
            }
                                ...
                                AstNode*                prevNode)
{
        ...
        !prevNode->hasAstFlag(AST_FROM_UFCS))
    {
        ...
        {
            Diagnostic err{prevNode, formErr(Err0479, prevNode->token.c_str(), symbol->name.c_str())};
        ...
        {
            Diagnostic err{prevNode, formErr(Err0478, Naming::kindName(prevNode->resolvedSymbolName()->kind).c_str(), prevNode->token.c_str(), symbol->name.c_str())};
            err.addNote(identifier->token, formNte(Nte0169, prevNode->typeInfo->getDisplayNameC()));
            err.addNote(formNte(Nte0108, Naming::kindName(prevNode->resolvedSymbolName()->kind).c_str(), prevNode->token.c_str(), symbol->name.c_str()));
            err.addNote(Diagnostic::hereIs(oneMatch.oneOverload->overload));
            err.addNote(formNte(Nte0043, Naming::kindName(prevNode->resolvedSymbolName()->kind).c_str(), identifierRef->startScope->name.c_str()));
            return context->report(err);
        }
    ...
    {
        const Diagnostic err{identifier, formErr(Err0440, symbol->name.c_str(), identifier->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
    ...
    while (true)
    {
        ...
        if (toSolveOverload.empty())
        {
            if (identifierRef->hasAstFlag(AST_SILENT_CHECK))
                return true;
            return context->report({identifier, formErr(Err0739, identifier->token.c_str())});
        }
