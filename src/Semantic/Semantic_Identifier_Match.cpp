...
bool Semantic::setSymbolMatchVar(SemanticContext* context, const OneMatch& oneMatch, AstIdentifierRef* idRef, AstIdentifier* identifier, SymbolOverload* overload)
{
    ...
    else
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{idRef, toErr(Err0611)};
            return context->report(err);
        }
    }
...
bool Semantic::setSymbolMatchFunc(SemanticContext* context, const OneMatch& oneMatch, AstIdentifier* identifier, const SymbolOverload* overload)
{
    ...
    if (childIdx)
    {
        ...
        {
            Diagnostic err{prev, formErr(Err0572, Naming::kindName(prev->resolvedSymbolOverload()->node).c_str(), prev->token.c_str(), identifier->token.c_str())};
...
bool Semantic::setSymbolMatchStruct(SemanticContext* context, OneMatch& oneMatch, AstIdentifierRef* identifierRef, AstIdentifier* identifier, const SymbolOverload* overload, TypeInfo* typeAlias)
{
    ...
    {
        const Diagnostic err{identifier, identifier->token, toErr(Err0355)};
        return context->report(err);
    }
                                ...
                                AstNode*                prevNode)
{
        ...
        !prevNode->hasAstFlag(AST_FROM_UFCS))
    {
        ...
        {
            Diagnostic err{prevNode, formErr(Err0573, prevNode->token.c_str(), symbol->name.c_str())};
        ...
        {
            Diagnostic err{prevNode, formErr(Err0572, Naming::kindName(prevNode->resolvedSymbolName()->kind).c_str(), prevNode->token.c_str(), symbol->name.c_str())};
    ...
    {
        const Diagnostic err{identifier, formErr(Err0530, symbol->name.c_str(), identifier->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
    ...
    if (symbol &&
        ...
        identifierRef->lastChild() != identifier)
    {
        const Diagnostic err{identifier, formErr(Err0531, symbol->name.c_str(), identifier->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::setMatchResult(SemanticContext* context, AstIdentifierRef* identifierRef, AstIdentifier* identifier, OneMatch& oneMatch)
{
    ...
    if (symbolKind != SymbolKind::Variable && symbolKind != SymbolKind::Function)
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{identifier, toErr(Err0611)};
            return context->report(err);
        }
    }
...
{
    SWAG_VERIFY(identifier->ownerFct, context->report({identifier, toErr(Err0423)}));
    ...
    SWAG_VERIFY(parent, context->report({parent, toErr(Err0423)}));
    ...
    {
        SWAG_VERIFY(parent->ownerFct, context->report({parent, toErr(Err0423)}));
