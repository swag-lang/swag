...
bool Semantic::setSymbolMatchVar(SemanticContext* context, const OneMatch& oneMatch, AstIdentifierRef* idRef, AstIdentifier* identifier, SymbolOverload* overload)
{
    ...
    if (ownerFct)
    {
        ...
        {
            Diagnostic err{identifier, formErr(Err0120, Naming::kindName(overload->node).cstr(), overload->node->token.cstr(), ownerFct->getDisplayNameC())};
    ...
    if (typeInfo->isLambdaClosure() && identifier->callParameters)
    {
        ...
        if (!typeInfoRet->isVoid())
        {
            if (isStatementIdentifier(identifier))
            {
                ...
                {
                    const Diagnostic err(identifier, identifier->token, formErr(Err0750, overload->node->token.cstr()));
    ...
    else
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{idRef, toErr(Err0536)};
            return context->report(err);
        }
    }
...
bool Semantic::setSymbolMatchFunc(SemanticContext* context, const OneMatch& oneMatch, AstIdentifier* identifier, const SymbolOverload* overload)
{
    ...
    if (childIdx)
    {
        ...
        {
            Diagnostic err{prev, formErr(Err0478, Naming::kindName(prev->resolvedSymbolOverload()->node).cstr(), prev->token.cstr(), identifier->token.cstr())};
    ...
    if (identifier->callParameters && identifier->callParameters->hasSpecFlag(AstFuncCallParams::SPEC_FLAG_CALL_FOR_STRUCT))
        return context->report({identifier->callParameters, formErr(Err0168, identifier->token.cstr())});
    ...
    if (identifier->callParameters && !identifier->callParameters->aliasNames.empty())
    {
        if (!overload->node->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN))
        {
            ...
            Diagnostic err{cp->token.sourceFile, cp->aliasNames.front().startLocation, cp->aliasNames.back().endLocation, formErr(Err0645, identifier->token.cstr())};
    ...
    {
        const Diagnostic err{identifier, identifier->token, formErr(Err0167, identifier->token.cstr())};
        return context->report(err, Diagnostic::hereIs(overload));
    }
    ...
    if (!returnType->isVoid())
    {
        if (isStatementIdentifier(identifier))
        {
            ...
            {
                const Diagnostic err(identifier, identifier->token, formErr(Err0748, overload->node->token.cstr()));
                return context->report(err, Diagnostic::hereIs(overload));
            }
                                ...
                                AstNode*                prevNode)
{
    ...
    {
        const Diagnostic err{prevNode, formErr(Err0157, prevNode->token.cstr(), prevNode->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
        ...
        !prevNode->hasAstFlag(AST_FROM_UFCS))
    {
        ...
        {
            Diagnostic err{prevNode, formErr(Err0479, prevNode->token.cstr(), symbol->name.cstr())};
        ...
        {
            Diagnostic err{prevNode, formErr(Err0478, Naming::kindName(prevNode->resolvedSymbolName()->kind).cstr(), prevNode->token.cstr(), symbol->name.cstr())};
    ...
    {
        const Diagnostic err{identifier, formErr(Err0440, symbol->name.cstr(), identifier->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::setMatchResult(SemanticContext* context, AstIdentifierRef* identifierRef, AstIdentifier* identifier, OneMatch& oneMatch)
{
    ...
    if (symbolKind != SymbolKind::Variable && symbolKind != SymbolKind::Function)
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{identifier, toErr(Err0536)};
            return context->report(err);
        }
    }
    ...
    while (true)
    {
        ...
        if (toSolveOverload.empty())
        {
            if (identifierRef->hasAstFlag(AST_SILENT_CHECK))
                return true;
            return context->report({identifier, formErr(Err0739, identifier->token.cstr())});
        }
