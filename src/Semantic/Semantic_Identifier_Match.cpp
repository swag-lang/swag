...
bool Semantic::setSymbolMatchVar(SemanticContext* context, const OneMatch& oneMatch, AstIdentifierRef* idRef, AstIdentifier* identifier, SymbolOverload* overload)
{
    ...
    if (ownerFct)
    {
        ...
        {
            Diagnostic err{identifier, formErr(Err0147, Naming::kindName(overload->node).c_str(), overload->node->token.c_str(), ownerFct->getDisplayNameC())};
    ...
    if (typeInfo->isLambdaClosure() && identifier->callParameters)
    {
        ...
        if (!typeInfoRet->isVoid())
        {
            if (isStatementIdentifier(identifier))
            {
                ...
                {
                    const Diagnostic err(identifier, identifier->token, formErr(Err0739, overload->node->token.c_str()));
        ...
        {
            const Diagnostic err{identifier, identifier->token, toErr(Err0129)};
            return context->report(err, Diagnostic::hereIs(overload));
        }
    ...
    else
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{idRef, toErr(Err0582)};
            return context->report(err);
        }
    }
...
bool Semantic::setSymbolMatchFunc(SemanticContext* context, const OneMatch& oneMatch, AstIdentifier* identifier, const SymbolOverload* overload)
{
    ...
    if (childIdx)
    {
        ...
        {
            Diagnostic err{prev, formErr(Err0537, Naming::kindName(prev->resolvedSymbolOverload()->node).c_str(), prev->token.c_str(), identifier->token.c_str())};
    ...
    if (identifier->callParameters && identifier->callParameters->hasSpecFlag(AstFuncCallParams::SPEC_FLAG_CALL_FOR_STRUCT))
        return context->report({identifier->callParameters, formErr(Err0231, identifier->token.c_str())});
    ...
    if (identifier->callParameters && !identifier->callParameters->aliasNames.empty())
    {
        if (!overload->node->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN))
        {
            ...
            Diagnostic err{cp->token.sourceFile, cp->aliasNames.front().startLocation, cp->aliasNames.back().endLocation, formErr(Err0667, identifier->token.c_str())};
    ...
    if (identifier->token.is(g_LangSpec->name_opDrop))
        return context->report({identifier, identifier->token, toErr(Err0093)});
    if (identifier->token.is(g_LangSpec->name_opPostCopy))
        return context->report({identifier, identifier->token, toErr(Err0094)});
    if (identifier->token.is(g_LangSpec->name_opPostMove))
        return context->report({identifier, identifier->token, toErr(Err0095)});

    // Be sure this is not a 'forward' decl
    if (funcDecl->isEmptyFct() && !funcDecl->isForeign() && !identifier->hasIntrinsicName())
    {
        const Diagnostic err{identifier, identifier->token, formErr(Err0232, identifier->token.c_str())};
        return context->report(err, Diagnostic::hereIs(overload));
    }
    ...
    if (identifier->token.text[0] != '@' && !funcDecl->isForeign())
    {
        ...
        if (ownerFct)
        {
            ...
            {
                Diagnostic err{identifier, identifier->token, formErr(Err0148, funcDecl->getDisplayNameC(), ownerFct->getDisplayNameC())};
    ...
    if (!returnType->isVoid())
    {
        if (isStatementIdentifier(identifier))
        {
            ...
            {
                const Diagnostic err(identifier, identifier->token, formErr(Err0737, overload->node->token.c_str()));
                return context->report(err, Diagnostic::hereIs(overload));
            }
...
bool Semantic::setSymbolMatchStruct(SemanticContext* context, OneMatch& oneMatch, AstIdentifierRef* identifierRef, AstIdentifier* identifier, const SymbolOverload* overload, TypeInfo* typeAlias)
{
    ...
    {
        const Diagnostic err{identifier, identifier->token, toErr(Err0312)};
        return context->report(err);
    }
                                ...
                                AstNode*                prevNode)
{
    ...
    {
        const Diagnostic err{prevNode, formErr(Err0205, prevNode->token.c_str(), prevNode->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
        ...
        !prevNode->hasAstFlag(AST_FROM_UFCS))
    {
        ...
        {
            Diagnostic err{prevNode, formErr(Err0538, prevNode->token.c_str(), symbol->name.c_str())};
        ...
        {
            Diagnostic err{prevNode, formErr(Err0537, Naming::kindName(prevNode->resolvedSymbolName()->kind).c_str(), prevNode->token.c_str(), symbol->name.c_str())};
    ...
    {
        const Diagnostic err{identifier, formErr(Err0492, symbol->name.c_str(), identifier->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
    ...
    if (symbol &&
        ...
        identifierRef->lastChild() != identifier)
    {
        const Diagnostic err{identifier, formErr(Err0493, symbol->name.c_str(), identifier->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::setMatchResult(SemanticContext* context, AstIdentifierRef* identifierRef, AstIdentifier* identifier, OneMatch& oneMatch)
{
    ...
    if (symbolKind != SymbolKind::Variable && symbolKind != SymbolKind::Function)
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{identifier, toErr(Err0582)};
            return context->report(err);
        }
    }
    ...
    switch (symbolKind)
    {
        ...
        case SymbolKind::EnumValue:
            ...
            {
                const Diagnostic err{idRef->previousResolvedNode, formErr(Err0203, idRef->previousResolvedNode->typeInfo->getDisplayNameC())};
                return context->report(err);
            }
...
{
    SWAG_VERIFY(identifier->ownerFct, context->report({identifier, toErr(Err0387)}));
    ...
    SWAG_VERIFY(parent, context->report({parent, toErr(Err0387)}));
    ...
    {
        SWAG_VERIFY(parent->ownerFct, context->report({parent, toErr(Err0387)}));
