...
bool Semantic::setSymbolMatchVar(SemanticContext* context, const OneMatch& oneMatch, AstIdentifierRef* idRef, AstIdentifier* identifier, SymbolOverload* overload)
{
    ...
    if (typeInfo->isLambdaClosure() && identifier->callParameters)
    {
        ...
        if (!typeInfoRet->isVoid())
        {
            if (isStatementIdentifier(identifier))
            {
                ...
                {
                    const Diagnostic err(identifier, identifier->token, formErr(Err0745, overload->node->token.c_str()));
    ...
    else
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{idRef, toErr(Err0622)};
            return context->report(err);
        }
    }
...
bool Semantic::setSymbolMatchFunc(SemanticContext* context, const OneMatch& oneMatch, AstIdentifier* identifier, const SymbolOverload* overload)
{
    ...
    if (childIdx)
    {
        ...
        {
            Diagnostic err{prev, formErr(Err0583, Naming::kindName(prev->resolvedSymbolOverload()->node).c_str(), prev->token.c_str(), identifier->token.c_str())};
    ...
    if (identifier->callParameters && identifier->callParameters->hasSpecFlag(AstFuncCallParams::SPEC_FLAG_CALL_FOR_STRUCT))
        return context->report({identifier->callParameters, formErr(Err0286, identifier->token.c_str())});
    ...
    if (identifier->callParameters && !identifier->callParameters->aliasNames.empty())
    {
        if (!overload->node->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN))
        {
            ...
            Diagnostic err{cp->token.sourceFile, cp->aliasNames.front().startLocation, cp->aliasNames.back().endLocation, formErr(Err0674, identifier->token.c_str())};
    ...
    {
        const Diagnostic err{identifier, identifier->token, formErr(Err0287, identifier->token.c_str())};
        return context->report(err, Diagnostic::hereIs(overload));
    }
    ...
    if (!returnType->isVoid())
    {
        if (isStatementIdentifier(identifier))
        {
            ...
            {
                const Diagnostic err(identifier, identifier->token, formErr(Err0743, overload->node->token.c_str()));
                return context->report(err, Diagnostic::hereIs(overload));
            }
...
bool Semantic::setSymbolMatchStruct(SemanticContext* context, OneMatch& oneMatch, AstIdentifierRef* identifierRef, AstIdentifier* identifier, const SymbolOverload* overload, TypeInfo* typeAlias)
{
    ...
    {
        const Diagnostic err{identifier, identifier->token, toErr(Err0368)};
        return context->report(err);
    }
                                ...
                                AstNode*                prevNode)
{
        ...
        !prevNode->hasAstFlag(AST_FROM_UFCS))
    {
        ...
        {
            Diagnostic err{prevNode, formErr(Err0584, prevNode->token.c_str(), symbol->name.c_str())};
        ...
        {
            Diagnostic err{prevNode, formErr(Err0583, Naming::kindName(prevNode->resolvedSymbolName()->kind).c_str(), prevNode->token.c_str(), symbol->name.c_str())};
    ...
    {
        const Diagnostic err{identifier, formErr(Err0541, symbol->name.c_str(), identifier->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
    ...
    if (symbol &&
        ...
        identifierRef->lastChild() != identifier)
    {
        const Diagnostic err{identifier, formErr(Err0542, symbol->name.c_str(), identifier->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::setMatchResult(SemanticContext* context, AstIdentifierRef* identifierRef, AstIdentifier* identifier, OneMatch& oneMatch)
{
    ...
    if (symbolKind != SymbolKind::Variable && symbolKind != SymbolKind::Function)
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{identifier, toErr(Err0622)};
            return context->report(err);
        }
    }
    ...
    while (true)
    {
        ...
        if (toSolveOverload.empty())
        {
            if (identifierRef->hasAstFlag(AST_SILENT_CHECK))
                return true;
            return context->report({identifier, formErr(Err0727, identifier->token.c_str())});
        }
...
{
    SWAG_VERIFY(identifier->ownerFct, context->report({identifier, toErr(Err0440)}));
    ...
    SWAG_VERIFY(parent, context->report({parent, toErr(Err0440)}));
    ...
    {
        SWAG_VERIFY(parent->ownerFct, context->report({parent, toErr(Err0440)}));
