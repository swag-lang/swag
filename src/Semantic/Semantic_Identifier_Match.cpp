...
bool Semantic::setSymbolMatchVar(SemanticContext* context, const OneMatch& oneMatch, AstIdentifierRef* idRef, AstIdentifier* identifier, SymbolOverload* overload)
{
    ...
    if (typeInfo->isLambdaClosure() && identifier->callParameters)
    {
        ...
        if (!typeInfoRet->isVoid())
        {
            if (isStatementIdentifier(identifier))
            {
                ...
                {
                    const Diagnostic err(identifier, identifier->token, formErr(Err0776, overload->node->token.cstr()));
    ...
    else
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{idRef, toErr(Err0557)};
            return context->report(err);
        }
    }
...
bool Semantic::setSymbolMatchFunc(SemanticContext* context, const OneMatch& oneMatch, AstIdentifier* identifier, const SymbolOverload* overload)
{
    ...
    if (childIdx)
    {
        ...
        {
            Diagnostic err{prev, formErr(Err0517, Naming::kindName(prev->resolvedSymbolOverload()->node).cstr(), prev->token.cstr(), identifier->token.cstr())};
    ...
    if (identifier->callParameters && identifier->callParameters->hasSpecFlag(AstFuncCallParams::SPEC_FLAG_CALL_FOR_STRUCT))
        return context->report({identifier->callParameters, formErr(Err0208, identifier->token.cstr())});
    ...
    if (identifier->callParameters && !identifier->callParameters->aliasNames.empty())
    {
        if (!overload->node->hasAttribute(ATTRIBUTE_MACRO | ATTRIBUTE_MIXIN))
        {
            ...
            Diagnostic err{cp->token.sourceFile, cp->aliasNames.front().startLocation, cp->aliasNames.back().endLocation, formErr(Err0669, identifier->token.cstr())};
    ...
    {
        const Diagnostic err{identifier, identifier->token, formErr(Err0207, identifier->token.cstr())};
        return context->report(err, Diagnostic::hereIs(overload));
    }
    ...
    if (!returnType->isVoid())
    {
        if (isStatementIdentifier(identifier))
        {
            ...
            {
                const Diagnostic err(identifier, identifier->token, formErr(Err0774, overload->node->token.cstr()));
                return context->report(err, Diagnostic::hereIs(overload));
            }
...
bool Semantic::setSymbolMatchStruct(SemanticContext* context, OneMatch& oneMatch, AstIdentifierRef* identifierRef, AstIdentifier* identifier, const SymbolOverload* overload, TypeInfo* typeAlias)
{
    ...
    {
        const Diagnostic err{identifier, identifier->token, toErr(Err0263)};
        return context->report(err);
    }
                                ...
                                AstNode*                prevNode)
{
        ...
        !prevNode->hasAstFlag(AST_FROM_UFCS))
    {
        ...
        {
            Diagnostic err{prevNode, formErr(Err0518, prevNode->token.cstr(), symbol->name.cstr())};
        ...
        {
            Diagnostic err{prevNode, formErr(Err0517, Naming::kindName(prevNode->resolvedSymbolName()->kind).cstr(), prevNode->token.cstr(), symbol->name.cstr())};
    ...
    {
        const Diagnostic err{identifier, formErr(Err0479, symbol->name.cstr(), identifier->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::setMatchResult(SemanticContext* context, AstIdentifierRef* identifierRef, AstIdentifier* identifier, OneMatch& oneMatch)
{
    ...
    if (symbolKind != SymbolKind::Variable && symbolKind != SymbolKind::Function)
    {
        if (isStatementIdentifier(identifier))
        {
            const Diagnostic err{identifier, toErr(Err0557)};
            return context->report(err);
        }
    }
    ...
    switch (symbolKind)
    {
        ...
        case SymbolKind::EnumValue:
            ...
            {
                const Diagnostic err{idRef->previousResolvedNode, formErr(Err0198, idRef->previousResolvedNode->typeInfo->getDisplayNameC())};
                return context->report(err);
            }
    ...
    while (true)
    {
        ...
        if (toSolveOverload.empty())
        {
            if (identifierRef->hasAstFlag(AST_SILENT_CHECK))
                return true;
            return context->report({identifier, formErr(Err0765, identifier->token.cstr())});
        }
...
{
    SWAG_VERIFY(identifier->ownerFct, context->report({identifier, toErr(Err0341)}));
    ...
    SWAG_VERIFY(parent, context->report({parent, toErr(Err0341)}));
    ...
    {
        SWAG_VERIFY(parent->ownerFct, context->report({parent, toErr(Err0341)}));
