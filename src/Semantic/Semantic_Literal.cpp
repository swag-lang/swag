...
bool Semantic::processLiteralString(SemanticContext* context)
{
    ...
    while (pz - start < len)
    {
        ...
        switch (c)
        {
            ...
            case 'U':
            {
                ...
                if (cw > Utf8::MAX_ENCODED_UNICODE)
                    return context->report({node->token.sourceFile, loc, formErr(Err0568, cw)});
                result.append(cw);
                loc.column += 8;
                continue;
            }
        ...
        return context->report({node->token.sourceFile, loc, formErr(Err0716, c)});
    }
...
Utf8 Semantic::checkLiteralValue(ComputedValue& computedValue, LiteralType& literalType, const Register& literalValue, TypeInfo* typeSuffix, bool negApplied)
{
    ...
    switch (literalType)
    {
        ...
        case LiteralType::TypeCharacterEscape:
        {
            ...
            if (uni.size() != 1)
                return formErr(Err0298, computedValue.text.cstr());
            ...
            else
            {
                switch (typeSuffix->nativeType)
                {
                    ...
                    case NativeTypeKind::U8:
                        if (uni[0] > UINT8_MAX)
                            return formErr(Err0565, uni[0]);
                        computedValue.reg.u8 = static_cast<uint8_t>(uni[0]);
                        break;

                    case NativeTypeKind::U16:
                        if (uni[0] > UINT16_MAX)
                            return formErr(Err0564, uni[0]);
                    ...
                    default:
                        return formErr(Err0270, typeSuffix->getDisplayNameC());
                }
            }
        ...
        case LiteralType::TypeUntypedFloat:
            switch (typeSuffix->nativeType)
            {
                ...
                case NativeTypeKind::F64:
                    ...
                    break;
                default:
                    return formErr(Err0723, typeSuffix->getDisplayNameC());
            }
            break;
        ...
        case LiteralType::TypeUntypedInt:
            switch (typeSuffix->nativeType)
            {
                ...
                default:
                    return formErr(Err0047, computedValue.reg.u64);
            }
...
bool Semantic::resolveLiteral(SemanticContext* context)
{
    ...
    if (suffix->token.isNot(TokenId::NativeType))
    {
        ...
        if (symbolName && symbolName->isNot(SymbolKind::TypeAlias))
            return context->report({suffix, formErr(Err0726, suffix->resolvedSymbolName()->name.cstr(), Naming::aKindName(symbolName->kind).cstr())});
    }

    const auto suffixType = TypeManager::concreteType(suffix->typeInfo, CONCRETE_ALIAS);
    SWAG_VERIFY(suffixType->isNative(), context->report({suffix, formErr(Err0725, suffixType->getDisplayNameC())}));
    ...
    switch (suffixType->nativeType)
    {
        ...
        case NativeTypeKind::F64:
            break;
        default:
            return context->report({suffix, formErr(Err0725, suffix->typeInfo->getDisplayNameC())});
    }
    ...
    if (!errMsg.empty())
    {
        ...
        if (suffix && suffix->typeInfo)
            err.addNote(suffix, form("this suffix forces the literal to be converted to [[%s]]", suffix->typeInfo->getDisplayNameC()));
