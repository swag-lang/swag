...
bool Semantic::processLiteralString(SemanticContext* context)
{
    ...
    while (pz - start < len)
    {
        ...
        switch (c)
        {
            ...
            case 'U':
            {
                ...
                if (cw > Utf8::MAX_ENCODED_UNICODE)
                    return context->report({node->token.sourceFile, loc, formErr(Err0261, cw)});
                result.append(cw);
                loc.column += 8;
                continue;
            }
        ...
        return context->report({node->token.sourceFile, loc, formErr(Err0707, c)});
    }
...
Utf8 Semantic::checkLiteralValue(ComputedValue& computedValue, LiteralType& literalType, const Register& literalValue, TypeInfo* typeSuffix, bool negApplied)
{
    ...
    switch (literalType)
    {
        ...
        case LiteralType::TypeCharacterEscape:
        {
            ...
            if (uni.size() != 1)
                return formErr(Err0159, computedValue.text.cstr());
            ...
            else
            {
                switch (typeSuffix->nativeType)
                {
                    ...
                    default:
                        return formErr(Err0160, typeSuffix->getDisplayNameC());
                }
            }
        ...
        case LiteralType::TypeUntypedInt:
            switch (typeSuffix->nativeType)
            {
                ...
                default:
                    return formErr(Err0200, computedValue.reg.u64);
            }
