...
bool Semantic::processLiteralString(SemanticContext* context)
{
    ...
    while (pz - start < len)
    {
        ...
        switch (c)
        {
            ...
            case 'U':
            {
                ...
                if (cw > Utf8::MAX_ENCODED_UNICODE)
                    return context->report({node->token.sourceFile, loc, formErr(Err0383, cw)});
                result.append(cw);
                loc.column += 8;
                continue;
            }
        ...
        return context->report({node->token.sourceFile, loc, formErr(Err0266, c)});
    }
...
Utf8 Semantic::checkLiteralValue(ComputedValue& computedValue, LiteralType& literalType, const Register& literalValue, TypeInfo* typeSuffix, bool negApplied)
{
    ...
    switch (literalType)
    {
        ...
        case LiteralType::TypeCharacterEscape:
        {
            ...
            if (uni.size() != 1)
                return formErr(Err0228, computedValue.text.c_str());
            ...
            else
            {
                switch (typeSuffix->nativeType)
                {
                    ...
                    case NativeTypeKind::U8:
                        if (uni[0] > UINT8_MAX)
                            return formErr(Err0593, uni[0]);
                        computedValue.reg.u8 = static_cast<uint8_t>(uni[0]);
                        break;

                    case NativeTypeKind::U16:
                        if (uni[0] > UINT16_MAX)
                            return formErr(Err0592, uni[0]);
                    ...
                    default:
                        return formErr(Err0227, typeSuffix->getDisplayNameC());
                }
            }
        ...
        case LiteralType::TypeUntypedFloat:
            switch (typeSuffix->nativeType)
            {
                ...
                case NativeTypeKind::F64:
                    ...
                    break;
                default:
                    return formErr(Err0312, typeSuffix->getDisplayNameC());
            }
            break;
            ...
            switch (typeSuffix->nativeType)
            {
                case NativeTypeKind::U8:
                    if (computedValue.reg.u64 > UINT8_MAX)
                        return form(formErr(Err0401, computedValue.reg.u64, "u8"));
                    break;
                case NativeTypeKind::U16:
                    if (computedValue.reg.u64 > UINT16_MAX)
                        return form(formErr(Err0401, computedValue.reg.u64, "u16"));
                    break;
                case NativeTypeKind::U32:
                    if (computedValue.reg.u64 > UINT32_MAX)
                        return form(formErr(Err0401, computedValue.reg.u64, "u32"));
                    break;
                case NativeTypeKind::U64:
                    break;
                ...
                case NativeTypeKind::S8:
                    if (computedValue.reg.s64 < INT8_MIN || computedValue.reg.s64 > INT8_MAX)
                        return formErr(Err0401, computedValue.reg.s64, "s8");
                    break;
                case NativeTypeKind::S16:
                    if (computedValue.reg.s64 < INT16_MIN || computedValue.reg.s64 > INT16_MAX)
                        return formErr(Err0401, computedValue.reg.s64, "s16");
                    break;
                case NativeTypeKind::S32:
                    if (computedValue.reg.s64 < INT32_MIN || computedValue.reg.s64 > INT32_MAX)
                        return formErr(Err0401, computedValue.reg.s64, "s32");
                    break;
                case NativeTypeKind::S64:
                    break;
                ...
                case NativeTypeKind::Rune:
                    if (computedValue.reg.u64 > UINT32_MAX)
                        return form(formErr(Err0401, computedValue.reg.u64, "rune"));
                    break;
                ...
                default:
                    return formErr(Err0309, computedValue.reg.u64);
            }
        ...
        case LiteralType::TypeUntypedBinHexa:
            switch (typeSuffix->nativeType)
            {
                ...
                case NativeTypeKind::S8:
                    if (computedValue.reg.u64 > UINT8_MAX)
                        return form(formErr(Err0401, computedValue.reg.u64, "s8"));
                    break;
                case NativeTypeKind::S16:
                    if (computedValue.reg.u64 > UINT16_MAX)
                        return form(formErr(Err0401, computedValue.reg.u64, "s16"));
                    break;
                case NativeTypeKind::S32:
                    if (computedValue.reg.u64 > UINT32_MAX)
                        return form(formErr(Err0401, computedValue.reg.u64, "s32"));
                    break;
                case NativeTypeKind::S64:
                    if (computedValue.reg.u64 > static_cast<uint64_t>(INT64_MAX) + 1)
                        return form(formErr(Err0401, computedValue.reg.u64, "s64"));
                    break;
...
bool Semantic::resolveLiteral(SemanticContext* context)
{
    ...
    if (suffix->token.isNot(TokenId::NativeType))
    {
        ...
        if (symbolName && symbolName->isNot(SymbolKind::TypeAlias))
            return context->report({suffix, formErr(Err0310, suffix->resolvedSymbolName()->name.c_str(), Naming::aKindName(symbolName->kind).c_str())});
    }

    const auto suffixType = TypeManager::concreteType(suffix->typeInfo, CONCRETE_ALIAS);
    SWAG_VERIFY(suffixType->isNative(), context->report({suffix, formErr(Err0311, suffixType->getDisplayNameC())}));
    ...
    switch (suffixType->nativeType)
    {
        ...
        case NativeTypeKind::F64:
            break;
        default:
            return context->report({suffix, formErr(Err0311, suffix->typeInfo->getDisplayNameC())});
    }
