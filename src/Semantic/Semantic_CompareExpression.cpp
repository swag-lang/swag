...
bool Semantic::resolveCompOpEqual(SemanticContext* context, AstNode* left, AstNode* right)
{
    ...
    if (left->hasFlagComputedValue() && right->hasFlagComputedValue())
    {
        ...
        switch (leftTypeInfo->nativeType)
        {
            ...
            default:
            {
                Diagnostic err{context->node, context->node->token, formErr(Err0638, node->token.c_str(), leftTypeInfo->getDisplayNameC())};
        ...
        (leftTypeInfo->isStruct() || rightTypeInfo->isStruct()))
    {
        ...
        {
            Diagnostic err{node, node->token, formErr(Err0314, rightTypeInfo->getDisplayNameC())};
        ...
        {
            Diagnostic err{node, node->token, formErr(Err0314, leftTypeInfo->getDisplayNameC())};
            err.addNote(Diagnostic::note(left, Diagnostic::isType(left)));
            err.addNote(Diagnostic::note(right, Diagnostic::isType(right)));
            return context->report(err);
        }
...
bool Semantic::resolveCompareExpression(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node->token.sourceFile, node->token, formErr(Err0636, node->token.c_str(), leftTypeInfo->getDisplayNameC())};
        err.addNote(left, Diagnostic::isType(leftTypeInfo));
        return context->report(err);
    }
    ...
    {
        Diagnostic err{node->token.sourceFile, node->token, formErr(Err0637, node->token.c_str(), rightTypeInfo->getDisplayNameC())};
    ...
    if (node->token.isNot(TokenId::SymEqualEqual) && node->token.isNot(TokenId::SymExclamEqual))
    {
        ...
        {
            Diagnostic err{node->token.sourceFile, node->token, toErr(Err0635)};
            if (leftTypeInfo->isTuple())
                err.addNote(left, Diagnostic::isType(leftTypeInfo));
            else
                err.addNote(right, Diagnostic::isType(rightTypeInfo));
    ...
    if (!rightTypeInfo->isPointerNull())
    {
        ...
        if (leftTypeInfo->isSlice())
        {
            Diagnostic err{node->token.sourceFile, node->token, formErr(Err0632, rightTypeInfo->getDisplayNameC())};
            err.addNote(left, Diagnostic::isType(leftTypeInfo));
            err.addNote(right, Diagnostic::isType(rightTypeInfo));
            return context->report(err);
        }
        ...
        {
            Diagnostic err{node->token.sourceFile, node->token, formErr(Err0633, rightTypeInfo->getDisplayNameC())};
            err.addNote(left, Diagnostic::isType(leftTypeInfo));
            err.addNote(right, Diagnostic::isType(rightTypeInfo));
            return context->report(err);
        }
        ...
        if (leftTypeInfo->isAny() && !rightTypeInfo->isPointerToTypeInfo())
        {
            Diagnostic err{node->token.sourceFile, node->token, formErr(Err0634, rightTypeInfo->getDisplayNameC())};
    ...
    if (node->token.isNot(TokenId::SymEqualEqual) && node->token.isNot(TokenId::SymExclamEqual))
    {
        ...
        {
            Diagnostic err{node->token.sourceFile, node->token, formErr(Err0650, node->token.c_str(), leftTypeInfo->getDisplayNameC())};
            err.addNote(left, Diagnostic::isType(leftTypeInfo));
            return context->report(err);
        }
