...
bool Semantic::collectAutoScope(SemanticContext* context, VectorNative<CollectedScope>& scopeHierarchy, AstIdentifierRef* identifierRef, const AstIdentifier* identifier)
{
    ...
    if (typeEnum.size() > 1)
    {
        Diagnostic err{identifierRef, formErr(Err0024, identifier->token.cstr())};
        bool       first = true;
    ...
    else
    {
        ...
        if (!withNodeP)
        {
            if (!hasEnum.empty())
            {
                Diagnostic err{identifierRef, formErr(Err0717, identifier->token.cstr(), hasEnum[0].second->getDisplayNameC())};
            ...
            Diagnostic err{identifierRef, formErr(Err0723, identifier->token.cstr())};
                                     ...
                                     TokenParse*                      scopeUpValue)
{
    ...
    if (startScope)
    {
        ...
        if (scopeUpMode == IdentifierScopeUpMode::Count)
        {
            for (uint64_t i = 0; i < scopeUpValue->literalValue.u8; i++)
            {
                ...
                if (!startScope && i == 0)
                {
                    const Diagnostic err{context->node, scopeUpValue->token, toErr(Err0341)};
                    return context->report(err);
                }

                if (!startScope && i)
                {
                    const Diagnostic err{context->node, scopeUpValue->token, formErr(Err0142, scopeUpValue->literalValue.u8)};
                    return context->report(err);
                }
    ...
    SWAG_VERIFY(scopeUpMode == IdentifierScopeUpMode::None, context->report({startNode, toErr(Err0341)}));
    return true;
}
