...
bool Semantic::collectAutoScope(SemanticContext* context, VectorNative<CollectedScope>& scopeHierarchy, AstIdentifierRef* identifierRef, const AstIdentifier* identifier)
{
    ...
    else
    {
        ...
        if (!withNodeP)
        {
            ...
            Diagnostic err{identifierRef, formErr(Err0714, identifier->token.c_str())};
                                     ...
                                     TokenParse*                      scopeUpValue)
{
    ...
    if (startScope)
    {
        ...
        if (scopeUpMode == IdentifierScopeUpMode::Count)
        {
            for (uint64_t i = 0; i < scopeUpValue->literalValue.u8; i++)
            {
                ...
                if (!startScope && i == 0)
                {
                    const Diagnostic err{context->node, scopeUpValue->token, toErr(Err0391)};
                    return context->report(err);
                }

                if (!startScope && i)
                {
                    const Diagnostic err{context->node, scopeUpValue->token, formErr(Err0123, scopeUpValue->literalValue.u8)};
                    return context->report(err);
                }
    ...
    SWAG_VERIFY(scopeUpMode == IdentifierScopeUpMode::None, context->report({startNode, toErr(Err0391)}));
    return true;
}
