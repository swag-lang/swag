...
bool Semantic::collectAutoScope(SemanticContext* context, VectorNative<CollectedScope>& scopeHierarchy, AstIdentifierRef* identifierRef, const AstIdentifier* identifier)
{
    ...
    else
    {
        ...
        if (!withNodeP)
        {
            if (!hasEnum.empty())
            {
                Diagnostic err{identifierRef, formErr(Err0707, identifier->token.c_str(), hasEnum[0].second->getDisplayNameC())};
            ...
            Diagnostic err{identifierRef, formErr(Err0717, identifier->token.c_str())};
                                     ...
                                     TokenParse*                      scopeUpValue)
{
    ...
    if (startScope)
    {
        ...
        if (scopeUpMode == IdentifierScopeUpMode::Count)
        {
            for (uint64_t i = 0; i < scopeUpValue->literalValue.u8; i++)
            {
                ...
                if (!startScope && i == 0)
                {
                    const Diagnostic err{context->node, scopeUpValue->token, toErr(Err0442)};
                    return context->report(err);
                }
    ...
    SWAG_VERIFY(scopeUpMode == IdentifierScopeUpMode::None, context->report({startNode, toErr(Err0442)}));
    return true;
}
