...
bool Semantic::checkCanThrow(SemanticContext* context)
{
    ...
    if (parentFct->isSpecialFunctionName())
        return context->report({node, node->token, formErr(Err0344, node->token.cstr(), node->token.cstr(), parentFct->token.cstr())});

    if (!parentFct->typeInfo->hasFlag(TYPEINFO_CAN_THROW) && !parentFct->hasAttribute(ATTRIBUTE_SHARP_FUNC))
        return context->report({node, node->token, formErr(Err0345, node->token.cstr(), node->token.cstr(), parentFct->token.cstr())});
...
bool Semantic::resolveThrow(SemanticContext* context)
{
    ...
    SWAG_VERIFY(!type->isVoid(), context->report({expr, toErr(Err0271)}));
    if (!type->isAny() || !node->hasSpecFlag(AstTryCatchAssume::SPEC_FLAG_THROW_GET_ERR))
        SWAG_VERIFY(type->isStruct(), context->report({expr, formErr(Err0270, type->getDisplayNameC())}));
