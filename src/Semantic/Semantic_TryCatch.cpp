...
bool Semantic::checkCanThrow(SemanticContext* context)
{
    ...
    if (parentFct->isSpecialFunctionName())
        return context->report({node, node->token, formErr(Err0452, node->token.c_str(), node->token.c_str(), parentFct->token.c_str())});
...
bool Semantic::checkCanCatch(SemanticContext* context)
{
    ...
    return context->report({node, node->token, formErr(Err0502, node->token.c_str(), lastChild->token.c_str(), Naming::aKindName(lastChild->resolvedSymbolName()->kind).c_str())});
}
...
bool Semantic::resolveThrow(SemanticContext* context)
{
    ...
    SWAG_VERIFY(!type->isVoid(), context->report({expr, toErr(Err0382)}));
    if (!type->isAny() || !node->hasSpecFlag(AstTryCatchAssume::SPEC_FLAG_THROW_GET_ERR))
        SWAG_VERIFY(type->isStruct(), context->report({expr, formErr(Err0383, type->getDisplayNameC())}));
