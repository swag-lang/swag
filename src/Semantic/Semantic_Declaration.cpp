...
bool Semantic::resolveUsingVar(SemanticContext* context, AstNode* varNode, TypeInfo* typeInfoVar)
{
    ...
    SWAG_ASSERT(regNode);
    SWAG_VERIFY(node->ownerFct || node->ownerScope->is(ScopeKind::Struct), context->report({node, formErr(Err0341, Naming::kindName(node->ownerScope->kind).c_str())}));
    ...
    else if (typeInfoVar->isPointer())
    {
        ...
        {
            SWAG_VERIFY(typePointer->pointedType->isStruct(), context->report({node, formErr(Err0340, typePointer->pointedType->getDisplayNameC())}));
    ...
    {
        return context->report({node, formErr(Err0342, typeInfoVar->getDisplayNameC())});
    }
...
bool Semantic::resolveUsing(SemanticContext* context)
{
    ...
    for (const auto child : node->children)
    {
        ...
        switch (typeResolved->kind)
        {
            ...
            default:
                return context->report({node, formErr(Err0343, typeResolved->getDisplayNameC())});
        }
