...
bool Semantic::resolveUsingVar(SemanticContext* context, AstNode* varNode, TypeInfo* typeInfoVar)
{
    ...
    SWAG_ASSERT(regNode);
    SWAG_VERIFY(node->ownerFct || node->ownerScope->is(ScopeKind::Struct), context->report({node, formErr(Err0355, Naming::kindName(node->ownerScope->kind).cstr())}));
    ...
    else if (typeInfoVar->isPointer())
    {
        ...
        {
            SWAG_VERIFY(typePointer->pointedType->isStruct(), context->report({node, formErr(Err0354, typePointer->pointedType->getDisplayNameC())}));
    ...
    {
        return context->report({node, formErr(Err0356, typeInfoVar->getDisplayNameC())});
    }
    ...
    switch (typeResolved->kind)
    {
        case TypeInfoKind::Pointer:
            if (!typeResolved->isPointerTo(TypeInfoKind::Struct))
                return context->report({context->node, formErr(Err0577, typeResolved->getDisplayNameC())});
            break;
        ...
        case TypeInfoKind::Enum:
            if (fromVar)
                return context->report({context->node, formErr(Err0576, typeResolved->getDisplayNameC())});
            break;
...
bool Semantic::resolveUsing(SemanticContext* context)
{
    ...
    for (const auto child : node->children)
    {
        ...
        switch (typeResolved->kind)
        {
            ...
            default:
                return context->report({node, formErr(Err0357, typeResolved->getDisplayNameC())});
        }
