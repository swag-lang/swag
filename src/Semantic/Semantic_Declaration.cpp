...
bool Semantic::resolveUsingVar(SemanticContext* context, AstNode* varNode, TypeInfo* typeInfoVar)
{
    ...
    SWAG_ASSERT(regNode);
    SWAG_VERIFY(node->ownerFct || node->ownerScope->is(ScopeKind::Struct), context->report({node, formErr(Err0408, Naming::kindName(node->ownerScope->kind).c_str())}));
    ...
    else if (typeInfoVar->isPointer())
    {
        ...
        {
            SWAG_VERIFY(typePointer->pointedType->isStruct(), context->report({node, formErr(Err0407, typePointer->pointedType->getDisplayNameC())}));
    ...
    {
        return context->report({node, formErr(Err0406, typeInfoVar->getDisplayNameC())});
    }
...
bool Semantic::resolveUsing(SemanticContext* context)
{
    ...
    for (const auto child : node->children)
    {
        ...
        switch (typeResolved->kind)
        {
            ...
            default:
                return context->report({node, formErr(Err0405, typeResolved->getDisplayNameC())});
        }
