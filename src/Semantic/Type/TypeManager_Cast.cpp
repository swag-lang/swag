...
bool TypeManager::castSubExpressionList(SemanticContext* context, AstNode* child, TypeInfo* toType, CastFlags castFlags)
{
    ...
    {
        const auto       msg = formErr(Err0627, toTypeStruct->fields.size(), toTypeStruct->getDisplayNameC(), child->childCount());
        ...
        if (!skipFirst)
        {
            ...
            if (foundItf)
            {
                if (foundField)
                {
                    ...
                    err.addNote(it.field->declNode, formNte(Nte0150, it.field->typeInfo->getDisplayNameC()));
                    err.addNote(foundField->declNode, formNte(Nte0152, foundField->typeInfo->getDisplayNameC()));
                    return context->report(err);
                }
        ...
        if (toTypeArray->count != fromSize)
        {
            if (!castFlags.has(CAST_FLAG_JUST_CHECK))
            {
                if (toTypeArray->count > fromTypeList->subTypes.size())
                    context->report({fromNode, formErr(Err0589, toTypeArray->count, fromTypeList->subTypes.size())});
                else
                    context->report({fromNode, formErr(Err0628, toTypeArray->count, fromTypeList->subTypes.size())});
            }
