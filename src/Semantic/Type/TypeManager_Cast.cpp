...
bool TypeManager::castSubExpressionList(SemanticContext* context, AstNode* child, TypeInfo* toType, CastFlags castFlags)
{
    ...
    {
        const auto       msg = formErr(Err0590, toTypeStruct->fields.size(), toTypeStruct->getDisplayNameC(), child->childCount());
    ...
    while (!stack.empty())
    {
        ...
        if (!skipFirst)
        {
            ...
            if (foundItf)
            {
                if (foundField)
                {
                    Diagnostic err{context->node, formErr(Err0027, fromTypeStruct->getDisplayNameC(), toTypeItf->name.c_str())};
        ...
        if (toTypeArray->count != fromSize)
        {
            if (!castFlags.has(CAST_FLAG_JUST_CHECK))
            {
                if (toTypeArray->count > fromTypeList->subTypes.size())
                    context->report({fromNode, formErr(Err0551, toTypeArray->count, fromTypeList->subTypes.size())});
                else
                    context->report({fromNode, formErr(Err0591, toTypeArray->count, fromTypeList->subTypes.size())});
            }
