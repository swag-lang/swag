...
bool Semantic::hasUserOp(SemanticContext* context, const Utf8& name, TypeInfoStruct* leftStruct, SymbolName** result)
{
    *result = nullptr;
    ...
    if (results.size() > 1)
    {
        ...
        err.addNote(context->node, formNte(Nte0151, name.c_str()));
...
bool Semantic::resolveUserOpAffect(SemanticContext* context, TypeInfo* leftTypeInfo, TypeInfo* rightTypeInfo, AstNode* left, AstNode* right)
{
    ...
    if (right->hasSemFlag(SEMFLAG_LITERAL_SUFFIX))
    {
        ...
        if (!symbol)
        {
            ...
            err.hint = formNte(Nte0151, g_LangSpec->name_opAffectLiteral.c_str());
            err.addNote(left->token, Diagnostic::isType(leftTypeInfo));
            err.addNote(right->firstChild(), formNte(Nte0187, suffix.c_str()));
    ...
    else
    {
        ...
        if (!symbol)
        {
            ...
            err.addNote(context->node, context->node->token, formNte(Nte0151, g_LangSpec->name_opAffect.c_str()));
...
bool Semantic::resolveUserOp(SemanticContext* context, const Utf8& name, const char* opConst, TypeInfo* opType, AstNode* left, VectorNative<AstNode*>& params, uint32_t ropFlags)
{
    ...
    if (!symbol)
    {
        ...
        if (!opConst)
        {
            ...
            err.hint = formNte(Nte0151, name.c_str());
    ...
    {
        PushErrCxtStep   ec(context, left->parent, ErrCxtStepKind::Note, [&] { return formNte(Nte0150, name.c_str(), leftType->getDisplayNameC()); }, true);
    ...
    while (true)
    {
        ...
        {
            PushErrCxtStep ec(context, left->parent, ErrCxtStepKind::Note, [name, leftType] { return formNte(Nte0150, name.c_str(), leftType->getDisplayNameC()); }, true);
