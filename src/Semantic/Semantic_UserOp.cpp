...
bool Semantic::checkFuncPrototypeOpNumParams(SemanticContext* context, const AstFuncDecl* node, AstNode* parameters, uint32_t numWanted, bool exact)
{
    ...
    {
        if (numCur > numWanted)
            return context->report({parameters->children[numWanted], formErr(Err0643, node->token.c_str(), numWanted, numCur)});
        return context->report({parameters, formErr(Err0603, node->token.c_str(), numWanted, numCur)});
    }

    if (!exact && numCur < numWanted)
    {
        return context->report({parameters, formErr(Err0604, node->token.c_str(), numWanted, numCur)});
    }
...
bool Semantic::checkFuncPrototypeOpReturnType(SemanticContext* context, AstFuncDecl* node, TypeInfo* wanted)
{
    ...
    if (wanted == nullptr)
    {
        if (returnType->isVoid())
            return context->report({node, node->getTokenName(), formErr(Err0580, node->token.c_str())});
        return true;
    }

    if (!wanted->isVoid() && returnType->isVoid())
        return context->report({node, node->getTokenName(), formErr(Err0581, node->token.c_str(), wanted->getDisplayNameC())});
...
bool Semantic::checkFuncPrototypeOpParam(SemanticContext* context, AstNode* parameters, uint32_t index, TypeInfo* wanted)
{
    ...
    if (!typeParam->isSame(wanted, CAST_FLAG_CAST))
        return context->report({parameters->children[index], formErr(Err0656, wanted->getDisplayNameC(), typeParam->getDisplayNameC())});
    return true;
}
...
bool Semantic::checkFuncPrototypeOp(SemanticContext* context, AstFuncDecl* node)
{
    ...
    if (isOpVisit && name != g_LangSpec->name_opVisit && !SWAG_IS_ALPHA(name[7]))
    {
        ...
        start.column += 7;
        const Diagnostic err{node->token.sourceFile, start, node->getTokenName().endLocation, formErr(Err0161, node->token.c_str() + 7)};
    ...
    if (!parent)
    {
        const Diagnostic err{node, node->getTokenName(), formErr(Err0513, node->token.c_str())};
        return context->report(err);
    }

    if (node->ownerScope->is(ScopeKind::Impl))
    {
        const Diagnostic err{node, node->getTokenName(), formErr(Err0512, node->token.c_str())};
        return context->report(err);
    }
    ...
    if (!node->ownerScope->owner->hasAttribute(ATTRIBUTE_PUBLIC) && node->hasAttribute(ATTRIBUTE_PUBLIC))
    {
        Diagnostic err{node, node->getTokenName(), formErr(Err0429, node->token.c_str())};
    ...
    if (parent)
    {
        ...
        SWAG_VERIFY(node->parameters, context->report({node, node->getTokenName(), formErr(Err0577, name.c_str())}));
        auto       firstGen  = node->parameters->firstChild();
        const auto firstType = firstGen->typeInfo;
        SWAG_VERIFY(firstType->isPointer(), context->report({firstGen, formErr(Err0400, name.c_str(), typeStruct->getDisplayNameC(), firstType->getDisplayNameC())}));
        const auto firstTypePtr = castTypeInfo<TypeInfoPointer>(firstType, firstType->kind);
        SWAG_VERIFY(firstTypePtr->pointedType->isSame(typeStruct, CAST_FLAG_CAST), context->report({firstGen, formErr(Err0400, name.c_str(), typeStruct->getDisplayNameC(), firstType->getDisplayNameC())}));
    }
    ...
    {
        SWAG_VERIFY(node->genericParameters, context->report({node, node->getTokenName(), formErr(Err0563, name.c_str())}));
        SWAG_VERIFY(node->genericParameters->childCount() <= 2, context->report({node->genericParameters, formErr(Err0638, name.c_str())}));
        auto firstGen = node->genericParameters->firstChild();
        SWAG_VERIFY(firstGen->hasSpecFlag(AstVarDecl::SPEC_FLAG_GENERIC_CONSTANT), context->report({firstGen, formErr(Err0307, name.c_str(), firstGen->typeInfo->getDisplayNameC())}));
        SWAG_VERIFY(firstGen->typeInfo->isSame(g_TypeMgr->typeInfoString, CAST_FLAG_CAST), context->report({firstGen, formErr(Err0395, name.c_str(), firstGen->typeInfo->getDisplayNameC())}));
    }
    else if (isOpVisit)
    {
        SWAG_VERIFY(node->genericParameters, context->report({node, node->getTokenName(), formErr(Err0563, name.c_str())}));
        SWAG_VERIFY(node->genericParameters->childCount() >= 2, context->report({node->genericParameters, formErr(Err0600, name.c_str())}));
        SWAG_VERIFY(node->genericParameters->childCount() <= 3, context->report({node->genericParameters, formErr(Err0638, name.c_str())}));
        auto firstGen = node->genericParameters->firstChild();
        SWAG_VERIFY(firstGen->hasSpecFlag(AstVarDecl::SPEC_FLAG_GENERIC_CONSTANT), context->report({firstGen, formErr(Err0307, name.c_str(), firstGen->typeInfo->getDisplayNameC())}));
        SWAG_VERIFY(firstGen->typeInfo->isSame(g_TypeMgr->typeInfoBool, CAST_FLAG_CAST), context->report({firstGen, formErr(Err0394, name.c_str(), firstGen->typeInfo->getDisplayNameC())}));
    ...
    {
        SWAG_VERIFY(!node->genericParameters, context->report({node->genericParameters, formErr(Err0693, name.c_str())}));
    }
    else
    {
        SWAG_VERIFY(!node->genericParameters || node->genericParameters->childCount() == 1, context->report({node->genericParameters, formErr(Err0637, name.c_str(), node->genericParameters->childCount())}));
    }
    ...
    else if (name == g_LangSpec->name_opAffect)
    {
        ...
        SWAG_VERIFY(!second->isSame(typeStruct, CAST_FLAG_EXACT), context->report({parameters->secondChild(), formErr(Err0393, name.c_str(), typeStruct->name.c_str())}));
    }
    ...
    else
    {
        ...
        Diagnostic err{node, node->getTokenName(), formErr(Err0622, name.c_str())};
        if (appendMsg.empty())
        {
...
bool Semantic::resolveUserOpAffect(SemanticContext* context, TypeInfo* leftTypeInfo, TypeInfo* rightTypeInfo, AstNode* left, AstNode* right)
{
    ...
    if (right->hasSemFlag(SEMFLAG_LITERAL_SUFFIX))
    {
        ...
        if (!symbol)
        {
            ...
            Diagnostic err{context->node, context->node->token, formErr(Err0324, leftTypeInfo->getDisplayNameC(), rightTypeInfo->getDisplayNameC())};
    ...
    else
    {
        ...
        if (!symbol)
        {
            ...
            Diagnostic err{right, formErr(Err0647, leftTypeInfo->getDisplayNameC(), rightTypeInfo->getDisplayNameC())};
...
bool Semantic::resolveUserOp(SemanticContext* context, const Utf8& name, const char* opConst, TypeInfo* opType, AstNode* left, VectorNative<AstNode*>& params, uint32_t ropFlags)
{
    ...
    if (!symbol)
    {
        ...
        if (!opConst)
        {
            Diagnostic err{left->parent->token.sourceFile, left->parent->token, formErr(Err0381, name.c_str(), leftType->getDisplayNameC())};
        ...
        Diagnostic err{left->parent->token.sourceFile, left->parent->token, formErr(Err0380, name.c_str(), leftType->getDisplayNameC(), opConst)};
        err.hint = formNte(Nte0144, name.c_str());
        err.addNote(left, Diagnostic::isType(leftType));
        err.addNote(note);
        return context->report(err);
    ...
    if (leftType->isGeneric())
    {
        ...
        const Diagnostic err(left, formErr(Err0560, name.c_str()));
        return context->report(err);
    }
