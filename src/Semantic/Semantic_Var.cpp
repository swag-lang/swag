...
bool Semantic::resolveTupleUnpackBefore(SemanticContext* context)
{
    ...
    if (!typeVar->isStruct())
    {
        Diagnostic err{varDecl, varDecl->token, formErr(Err0318, typeVar->getDisplayNameC())};
    ...
    {
        const Diagnostic err{varDecl, varDecl->token, toErr(Err0317)};
        return context->report(err);
    }

    if (numUnpack < typeStruct->fields.size())
    {
        Diagnostic err{varDecl, varDecl->token, formErr(Err0723, numUnpack, typeStruct->fields.size())};
    ...
    if (numUnpack > typeStruct->fields.size())
    {
        Diagnostic err{varDecl, varDecl->token, formErr(Err0724, numUnpack, typeStruct->fields.size())};
        err.addNote(varDecl->assignment, formNte(Nte0193, typeStruct->fields.size()));
        return context->report(err);
    }
...
bool Semantic::resolveVarDeclAfterType(SemanticContext* context)
{
    ...
    if (parent->is(AstNodeKind::FuncDeclParam))
    {
        ...
        {
            const Diagnostic err{varDecl, varDecl->assignToken, toErr(Err0676)};
            return context->report(err);
        }
    }
...
bool Semantic::deduceLambdaParamTypeFrom(SemanticContext* context, AstVarDecl* nodeParam, bool& lambdaExpr, bool& genericType)
{
    ...
    {
        Diagnostic err{nodeParam, formErr(Err0586, typeLambda->parameters.count, nodeParam->parent->children.count)};
...
bool Semantic::checkForMissingInitialization(SemanticContext* context, AstVarDecl* node, OverloadFlags overFlags, TypeInfo* concreteNodeType, bool isCompilerConstant)
{
    ...
    if (isCompilerConstant && !node->hasFlagComputedValue())
        return context->report({node, toErr(Err0506)});
    ...
    if (overFlags.has(OVERLOAD_CONST_ASSIGN) && node->isNot(AstNodeKind::FuncDeclParam))
    {
        ...
        return context->report({node, toErr(Err0509)});
    }
    ...
    {
        return context->report({node, toErr(Err0507)});
    }
        ...
        !node->hasAstFlag(AST_EXPLICITLY_NOT_INITIALIZED))
    {
        ...
        if (!ok)
        {
            Diagnostic err{node, formErr(Err0510, node->token.c_str(), concreteTypeEnum->getDisplayNameC())};
...
bool Semantic::checkMixAlias(SemanticContext* context, AstVarDecl* node)
{
    ...
    if (!node->hasAstFlag(AST_GENERATED) && !node->hasOwnerInline() && node->token.text.find(g_LangSpec->name_sharpmix) == 0)
    {
        ...
        if (!ownerFct)
        {
            Diagnostic err{node, toErr(Err0268)};
    ...
    if (!node->hasAstFlag(AST_GENERATED) && !node->hasOwnerInline() && node->token.text.find(g_LangSpec->name_sharpalias) == 0)
    {
        ...
        if (!ownerFct)
        {
            Diagnostic err{node, node->token, toErr(Err0268)};
...
bool Semantic::checkTypeSuffix(SemanticContext* context, const AstVarDecl* node)
{
    if (node->assignment && node->assignment->hasSemFlag(SEMFLAG_LITERAL_SUFFIX))
    {
        ...
        {
            const Diagnostic err{node->assignment->firstChild(), formErr(Err0346, node->assignment->firstChild()->token.c_str())};
            return context->report(err);
        }
    }
...
bool Semantic::resolveGlobalVar(SemanticContext* context, AstVarDecl* node, OverloadFlags& overFlags, DataSegment*& storageSegment, uint32_t& storageOffset)
{
    ...
    {
        Diagnostic err{node, node->getTokenName(), toErr(Err0420)};
...
bool Semantic::resolveVarDecl(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0435, concreteNodeType->getDisplayNameC())};
    ...
    if (!node->hasAstFlag(AST_FROM_GENERIC))
    {
        if (!isGeneric && node->assignment && (isCompilerConstant || overFlags.has(OVERLOAD_VAR_GLOBAL)))
        {
            ...
            {
                SWAG_VERIFY(!isCompilerConstant, context->report({node->assignment, toErr(Err0167)}));
            ...
            {
                SWAG_CHECK(checkIsConstExpr(context, node->assignment->hasAstFlag(AST_CONST_EXPR), node->assignment, toErr(Err0047)));
            }
        }
    }
    ...
    if (concreteNodeType && concreteNodeType->isArray())
    {
        ...
        {
            Diagnostic err{node->type, toErr(Err0165)};
    ...
    if (node->hasAstFlag(AST_EXPLICITLY_NOT_INITIALIZED))
    {
        ...
        if (isCompilerConstant)
        {
            Diagnostic err{node->assignment, toErr(Err0506)};
        ...
        if (node->hasSpecFlag(AstVarDecl::SPEC_FLAG_LET))
        {
            Diagnostic err{node->assignment, toErr(Err0508)};
            err.addNote(toNte(Nte0040));
            return context->report(err);
        }
    }
    ...
    else if (node->assignment && !node->hasAstFlag(AST_EXPLICITLY_NOT_INITIALIZED))
    {
        ...
        if (node->typeInfo->isVoid())
        {
            if (node->assignment->typeInfo->isFuncAttr() && node->assignment->resolvedSymbolOverload())
            {
                ...
                const Diagnostic err{nodeWhere, nodeWhere->token, toErr(Err0301)};
                return context->report(err, Diagnostic::hereIs(over));
            }

            const Diagnostic err{node->assignment, toErr(Err0319)};
            return context->report(err);
        }
    ...
    SWAG_VERIFY(node->typeInfo, context->report({node, formErr(Err0349, Naming::kindName(node).c_str(), node->token.c_str())}));

    // Type should be a correct one
    SWAG_VERIFY(!node->typeInfo->isPointerNull(), context->report({node->assignment ? node->assignment : node, toErr(Err0350)}));
    SWAG_VERIFY(!node->typeInfo->isVoid(), context->report({node->type ? node->type : node, toErr(Err0348)}));
