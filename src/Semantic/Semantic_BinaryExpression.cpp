...
bool Semantic::resolveBinaryOpPlus(SemanticContext* context, AstNode* left, AstNode* right)
{
    ...
    if (rightTypeInfo->isPointer())
    {
        ...
        SWAG_VERIFY(leftTypeInfo->isNativeInteger(), context->report({left, formErr(Err0296, leftTypeInfo->getDisplayNameC())}));
    ...
    switch (leftTypeInfo->nativeType)
    {
        ...
        {
            Diagnostic err{node, node->token, formErr(Err0644, node->token.c_str(), leftTypeInfo->getDisplayNameC())};
            err.addNote(left, Diagnostic::isType(leftTypeInfo));
...
bool Semantic::resolveFactorExpression(SemanticContext* context)
{
    ...
    if (leftTypeInfo->isEnum() || rightTypeInfo->isEnum())
    {
        ...
        if (leftTypeInfo->isEnum() && !leftTypeInfo->hasFlag(TYPEINFO_ENUM_FLAGS))
            return context->report({node, formErr(Err0648, node->token.c_str(), leftTypeInfo->getDisplayNameC())});
        if (rightTypeInfo->isEnum() && !rightTypeInfo->hasFlag(TYPEINFO_ENUM_FLAGS))
            return context->report({node, formErr(Err0649, node->token.c_str(), rightTypeInfo->getDisplayNameC())});
    }
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0645, node->token.c_str(), rightTypeInfo->getDisplayNameC())};
        err.addNote(right, Diagnostic::isType(rightTypeInfo));
        return context->report(err);
    }
...
bool Semantic::resolveShiftLeft(SemanticContext* context, AstNode* left, AstNode* right)
{
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0650, "<<", leftTypeInfo->getDisplayNameC())};
