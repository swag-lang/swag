...
bool Semantic::resolveBinaryOpPlus(SemanticContext* context, AstNode* left, AstNode* right)
{
    ...
    if (leftTypeInfo->isPointer())
    {
        ...
        {
            Diagnostic err{node, node->token, toErr(Err0209)};
            err.addNote(left, Diagnostic::isType(leftTypeInfo));
            return context->report(err);
        }

        SWAG_VERIFY(rightTypeInfo->isNativeInteger(), context->report({right, formErr(Err0210, rightTypeInfo->getDisplayNameC())}));
    ...
    if (rightTypeInfo->isPointer())
    {
        ...
        SWAG_VERIFY(leftTypeInfo->isNativeInteger(), context->report({left, formErr(Err0210, leftTypeInfo->getDisplayNameC())}));
    ...
    switch (leftTypeInfo->nativeType)
    {
        ...
        {
            Diagnostic err{node, node->token, formErr(Err0617, node->token.c_str(), leftTypeInfo->getDisplayNameC())};
            err.addNote(left, Diagnostic::isType(leftTypeInfo));
...
bool Semantic::resolveFactorExpression(SemanticContext* context)
{
    ...
    if (leftTypeInfo->isEnum() || rightTypeInfo->isEnum())
    {
        ...
        if (leftTypeInfo->isEnum() && !leftTypeInfo->hasFlag(TYPEINFO_ENUM_FLAGS))
            return context->report({node, formErr(Err0613, node->token.c_str(), leftTypeInfo->getDisplayNameC())});
        if (rightTypeInfo->isEnum() && !rightTypeInfo->hasFlag(TYPEINFO_ENUM_FLAGS))
            return context->report({node, formErr(Err0614, node->token.c_str(), rightTypeInfo->getDisplayNameC())});
    }
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0618, node->token.c_str(), rightTypeInfo->getDisplayNameC())};
        err.addNote(right, Diagnostic::isType(rightTypeInfo));
        return context->report(err);
    }
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0618, node->token.c_str(), rightTypeInfo->getDisplayNameC())};
        err.addNote(right, toNte(Nte0031));
        return context->report(err);
    }
...
bool Semantic::resolveShiftLeft(SemanticContext* context, AstNode* left, AstNode* right)
{
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0615, "<<", leftTypeInfo->getDisplayNameC())};
