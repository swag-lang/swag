...
bool Semantic::resolveBinaryOpPlus(SemanticContext* context, AstNode* left, AstNode* right)
{
    ...
    if (rightTypeInfo->isPointer())
    {
        ...
        SWAG_VERIFY(leftTypeInfo->isNativeInteger(), context->report({left, formErr(Err0249, leftTypeInfo->getDisplayNameC())}));
    ...
    switch (leftTypeInfo->nativeType)
    {
        ...
        {
            Diagnostic err{node, node->token, formErr(Err0636, node->token.cstr(), leftTypeInfo->getDisplayNameC())};
            err.addNote(left, Diagnostic::isType(leftTypeInfo));
    ...
    if (leftTypeInfo->isNativeFloat() && !left->typeInfoCast && !node->typeInfoCast)
    {
        ...
        switch (leftTypeInfo->nativeType)
        {
            case NativeTypeKind::S8:
                if (left->computedValue()->reg.s32 == INT8_MIN && right->computedValue()->reg.s32 == -1)
                    return context->report({node, node->token, formErr(Err0055, leftTypeInfo->getDisplayNameC())});
                if (right->computedValue()->reg.s32 == 0)
                    return context->report({right, toErr(Err0054)});
                node->computedValue()->reg.s64 = left->computedValue()->reg.s32 / right->computedValue()->reg.s32;
                break;
            case NativeTypeKind::S16:
...
bool Semantic::resolveAppend(SemanticContext* context, AstNode* left, AstNode* right)
{
    ...
    {
        PushErrCxtStep ec(context, node, ErrCxtStepKind::Note, [] { return toNte(Nte0140); }, true);
...
bool Semantic::resolveFactorExpression(SemanticContext* context)
{
    ...
    if (leftTypeInfo->isEnum() || rightTypeInfo->isEnum())
    {
        ...
        if (leftTypeInfo->isEnum() && !leftTypeInfo->hasFlag(TYPEINFO_ENUM_FLAGS))
            return context->report({node, formErr(Err0632, node->token.cstr(), leftTypeInfo->getDisplayNameC())});
        if (rightTypeInfo->isEnum() && !rightTypeInfo->hasFlag(TYPEINFO_ENUM_FLAGS))
            return context->report({node, formErr(Err0633, node->token.cstr(), rightTypeInfo->getDisplayNameC())});
    }
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0637, node->token.cstr(), rightTypeInfo->getDisplayNameC())};
        err.addNote(right, Diagnostic::isType(rightTypeInfo));
        return context->report(err);
    }
...
bool Semantic::resolveShiftLeft(SemanticContext* context, AstNode* left, AstNode* right)
{
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0634, "<<", leftTypeInfo->getDisplayNameC())};
