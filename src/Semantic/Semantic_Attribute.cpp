...
bool Semantic::checkAttribute(SemanticContext* context, AstNode* oneAttribute, AstNode* checkNode)
{
    ...
    if (!oneAttribute->typeInfo->isFuncAttr())
        return context->report({oneAttribute, formErr(Err0140, oneAttribute->typeInfo->getDisplayNameC(), Naming::aKindName(oneAttribute->typeInfo).c_str())});
    ...
    if (specificMsg)
    {
        if (nakedName == "node")
            nakedName.clear();
        Diagnostic err{oneAttribute, formErr(Err0361, oneAttribute->token.c_str())};
    ...
    Diagnostic err{oneAttribute, formErr(Err0363, oneAttribute->token.c_str(), nakedName.c_str())};
    err.addNote(Diagnostic::hereIs(oneAttribute->resolvedSymbolOverload()));
    return context->report(err);
}
    ...
    while (curAttr)
    {
        ...
        for (auto child : curAttr->children)
        {
            ...
            if (!typeInfo->attributes.hasAttribute(g_LangSpec->name_Swag_AttrMulti))
            {
                ...
                {
                    Diagnostic err{child, formErr(Err0036, child->token.c_str())};
                    return context->report(err);
                }
            ...
            if (forNode->is(AstNodeKind::AttrDecl))
            {
                ...
                if (typeAttr->attributeUsage.has(ATTR_USAGE_GEN))
                {
                    ...
                    if (!what.has(ATTR_USAGE_STRUCT | ATTR_USAGE_ENUM))
                        return context->report({child, toErr(Err0365)});
            ...
            if (it)
            {
                ...
                if (*it == ATTRIBUTE_FOREIGN)
                {
                    ...
                    SWAG_VERIFY(!attrParam->value.text.empty(), context->report({child, attrParam->token, toErr(Err0195)}));
                    SWAG_VERIFY(attrParam->value.text.find(".", 0) == -1, context->report({child, attrParam->token, toErr(Err0196)}));
                }

#define EXCLUSIVE(__a, __b) ((*it == (__a) && (flags.has(__b))) || (*it == (__b) && (flags.has(__a))))

                if (EXCLUSIVE(ATTRIBUTE_TLS, ATTRIBUTE_COMPILER))
                    return context->report({child, toErr(Err0029)});
                if (EXCLUSIVE(ATTRIBUTE_INLINE, ATTRIBUTE_NO_INLINE))
                    return context->report({child, toErr(Err0030)});
                if (EXCLUSIVE(ATTRIBUTE_MACRO, ATTRIBUTE_INLINE))
                    return context->report({child, toErr(Err0031)});
                if (EXCLUSIVE(ATTRIBUTE_MACRO, ATTRIBUTE_MIXIN))
                    return context->report({child, toErr(Err0032)});
            ...
            else if (child->token.is(g_LangSpec->name_Using))
            {
                ...
                SWAG_VERIFY(id->callParameters && !id->callParameters->children.empty(), context->report({id, formErr(Err0477, "Swag.Using")}));
                for (auto c : id->callParameters->children)
                {
                    ...
                    switch (typeChild->kind)
                    {
                        ...
                        default:
                            return context->report({c, formErr(Err0339, typeChild->getDisplayNameC())});
                    }
            ...
            else if (child->token.is(g_LangSpec->name_ExportType))
            {
                ...
                for (auto& w : what)
                {
                    ...
                    {
                        return context->report({child, attrParam->token, formErr(Err0710, w.c_str())});
                    }
                }
            }
            ...
            else if (child->token.is(g_LangSpec->name_Safety))
            {
                ...
                for (auto attr : allAttrs)
                {
                    ...
                    for (auto& w : what)
                    {
                        ...
                        if (!done)
                        {
                            return context->report({child, attrParam->token, formErr(Err0714, w.c_str())});
                        }
                    }
                }
            }
            ...
            else if (child->token.is(g_LangSpec->name_Optim))
            {
                ...
                for (auto attr : allAttrs)
                {
                    ...
                    for (auto& w : what)
                    {
                        ...
                        {
                            return context->report({child, attrParam->token, formErr(Err0713, w.c_str())});
                        }
                    }
                }
            }
            ...
            else if (child->token.is(g_LangSpec->name_Match))
            {
                ...
                for (auto attr : allAttrs)
                {
                    ...
                    for (auto& w : what)
                    {
                        ...
                        {
                            return context->report({child, attrParam->token, formErr(Err0712, w.c_str())});
                        }
                    }
                }
            }
            ...
            else if (child->token.is(g_LangSpec->name_Pack))
            {
                ...
                SWAG_VERIFY(!attrValue->reg.u8 || Math::isPowerOfTwo(attrValue->reg.u8), context->report({child, attrParam->token, formErr(Err0205, attrValue->reg.u8)}));
            }
            ...
            else if (child->token.is(g_LangSpec->name_Align))
            {
                ...
                SWAG_ASSERT(attrParam);
                auto attrValue = &attrParam->value;
                SWAG_VERIFY(Math::isPowerOfTwo(attrValue->reg.u8), context->report({child, attrParam->token, formErr(Err0131, attrValue->reg.u8)}));
            }
...
bool Semantic::resolveAttrUse(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->content || node->hasSpecFlag(AstAttrUse::SPEC_FLAG_GLOBAL), context->report({node, toErr(Err0364)}));
...
bool Semantic::resolveAttrUse(SemanticContext* context, AstAttrUse* node)
{
    for (auto child : node->children)
    {
        ...
        {
            Diagnostic err{identifier, identifier->token, formErr(Err0140, resolvedName->name.c_str(), Naming::aKindName(resolvedName->kind).c_str())};
        ...
        if (node->hasSpecFlag(AstAttrUse::SPEC_FLAG_GLOBAL))
        {
            ...
            {
                Diagnostic err{identifier, identifier->token, formErr(Err0362, resolvedName->name.c_str())};
                context->report(err, Diagnostic::hereIs(resolved));
                return false;
            }
        }
        ...
        if (identifier->callParameters)
        {
            ...
            for (auto one : identifier->callParameters->children)
            {
                ...
                SWAG_CHECK(checkIsConstExpr(context, param->hasFlagComputedValue(), param, toErr(Err0023)));
