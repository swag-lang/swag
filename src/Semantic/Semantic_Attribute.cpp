...
bool Semantic::checkAttribute(SemanticContext* context, AstNode* oneAttribute, AstNode* checkNode)
{
    ...
    if (!oneAttribute->typeInfo->isFuncAttr())
        return context->report({oneAttribute, formErr(Err0152, oneAttribute->typeInfo->getDisplayNameC(), Naming::aKindName(oneAttribute->typeInfo).cstr())});
    ...
    if (specificMsg)
    {
        if (nakedName == "node")
            nakedName.clear();
        Diagnostic err{oneAttribute, formErr(Err0377, oneAttribute->token.cstr())};
    ...
    Diagnostic err{oneAttribute, formErr(Err0379, oneAttribute->token.cstr(), nakedName.cstr())};
    err.addNote(Diagnostic::hereIs(oneAttribute->resolvedSymbolOverload()));
    return context->report(err);
}
        ...
        for (auto child : curAttr->children)
        {
            ...
            if (forNode->is(AstNodeKind::AttrDecl))
            {
                ...
                if (typeAttr->attributeUsage.has(ATTR_USAGE_GEN))
                {
                    ...
                    if (!what.has(ATTR_USAGE_STRUCT | ATTR_USAGE_ENUM))
                        return context->report({child, toErr(Err0381)});
            ...
            if (it)
            {
                ...
                if (*it == ATTRIBUTE_FOREIGN)
                {
                    ...
                    SWAG_VERIFY(!attrParam->value.text.empty(), context->report({child, attrParam->token, toErr(Err0206)}));
                    SWAG_VERIFY(attrParam->value.text.find(".", 0) == -1, context->report({child, attrParam->token, toErr(Err0207)}));
                }
...
#define EXCLUSIVE(__a, __b) ((*it == (__a) && (flags.has(__b))) || (*it == (__b) && (flags.has(__a))))
            ...
            else if (child->token.is(g_LangSpec->name_Using))
            {
                ...
                SWAG_VERIFY(id->callParameters && !id->callParameters->children.empty(), context->report({id, formErr(Err0492, "Swag.Using")}));
                for (auto c : id->callParameters->children)
                {
                    ...
                    switch (typeChild->kind)
                    {
                        ...
                        default:
                            return context->report({c, formErr(Err0354, typeChild->getDisplayNameC())});
                    }
            ...
            else if (child->token.is(g_LangSpec->name_ExportType))
            {
                ...
                for (auto& w : what)
                {
                    ...
                    {
                        return context->report({child, attrParam->token, formErr(Err0726, w.cstr())});
                    }
                }
            }
            ...
            else if (child->token.is(g_LangSpec->name_Safety))
            {
                ...
                for (auto attr : allAttrs)
                {
                    ...
                    for (auto& w : what)
                    {
                        ...
                        if (!done)
                        {
                            return context->report({child, attrParam->token, formErr(Err0730, w.cstr())});
                        }
                    }
                }
            }
            ...
            else if (child->token.is(g_LangSpec->name_Match))
            {
                ...
                for (auto attr : allAttrs)
                {
                    ...
                    for (auto& w : what)
                    {
                        ...
                        {
                            return context->report({child, attrParam->token, formErr(Err0728, w.cstr())});
                        }
                    }
                }
            }
            ...
            else if (child->token.is(g_LangSpec->name_Pack))
            {
                ...
                SWAG_VERIFY(!attrValue->reg.u8 || Math::isPowerOfTwo(attrValue->reg.u8), context->report({child, attrParam->token, formErr(Err0216, attrValue->reg.u8)}));
            }
            ...
            else if (child->token.is(g_LangSpec->name_Align))
            {
                ...
                SWAG_ASSERT(attrParam);
                auto attrValue = &attrParam->value;
                SWAG_VERIFY(Math::isPowerOfTwo(attrValue->reg.u8), context->report({child, attrParam->token, formErr(Err0143, attrValue->reg.u8)}));
            }
...
bool Semantic::resolveAttrUse(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->content || node->hasSpecFlag(AstAttrUse::SPEC_FLAG_GLOBAL), context->report({node, toErr(Err0380)}));
...
bool Semantic::resolveAttrUse(SemanticContext* context, AstAttrUse* node)
{
    for (auto child : node->children)
    {
        ...
        {
            Diagnostic err{identifier, identifier->token, formErr(Err0152, resolvedName->name.cstr(), Naming::aKindName(resolvedName->kind).cstr())};
        ...
        if (node->hasSpecFlag(AstAttrUse::SPEC_FLAG_GLOBAL))
        {
            ...
            {
                Diagnostic err{identifier, identifier->token, formErr(Err0378, resolvedName->name.cstr())};
                context->report(err, Diagnostic::hereIs(resolved));
                return false;
            }
        }
