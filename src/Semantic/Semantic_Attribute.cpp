...
bool Semantic::checkAttribute(SemanticContext* context, AstNode* oneAttribute, AstNode* checkNode)
{
    ...
    if (!oneAttribute->typeInfo->isFuncAttr())
        return context->report({oneAttribute, formErr(Err0173, oneAttribute->typeInfo->getDisplayNameC(), Naming::aKindName(oneAttribute->typeInfo).c_str())});
    ...
    if (specificMsg)
    {
        if (nakedName == "node")
            nakedName.clear();
        Diagnostic err{oneAttribute, formErr(Err0436, oneAttribute->token.c_str(), specificMsg)};
            ...
            if (forNode->is(AstNodeKind::AttrDecl))
            {
                ...
                if (typeAttr->attributeUsage.has(ATTR_USAGE_GEN))
                {
                    ...
                    if (!what.has(ATTR_USAGE_STRUCT | ATTR_USAGE_ENUM))
                        return context->report({child, toErr(Err0422)});
...
#define EXCLUSIVE(__a, __b) ((*it == (__a) && (flags.has(__b))) || (*it == (__b) && (flags.has(__a))))
            ...
            else if (child->token.is(g_LangSpec->name_Using))
            {
                ...
                for (auto c : id->callParameters->children)
                {
                    ...
                    switch (typeChild->kind)
                    {
                        ...
                        default:
                            return context->report({c, formErr(Err0414, typeChild->getDisplayNameC())});
                    }
            ...
            else if (child->token.is(g_LangSpec->name_Optim))
            {
                ...
                for (auto attr : allAttrs)
                {
                    ...
                    for (auto& w : what)
                    {
                        ...
                        {
                            return context->report({child, attrParam->token, formErr(Err0161, w.c_str())});
                        }
                    }
                }
            }
            ...
            else if (child->token.is(g_LangSpec->name_Match))
            {
                ...
                for (auto attr : allAttrs)
                {
                    ...
                    for (auto& w : what)
                    {
                        ...
                        {
                            return context->report({child, attrParam->token, formErr(Err0160, w.c_str())});
                        }
                    }
                }
            }
            ...
            else if (child->token.is(g_LangSpec->name_Align))
            {
                ...
                SWAG_ASSERT(attrParam);
                auto attrValue = &attrParam->value;
                SWAG_VERIFY(Math::isPowerOfTwo(attrValue->reg.u8), context->report({child, attrParam->token, formErr(Err0157, attrValue->reg.u8)}));
            }
...
bool Semantic::resolveAttrUse(SemanticContext* context)
{
    ...
    SWAG_VERIFY(node->content || node->hasSpecFlag(AstAttrUse::SPEC_FLAG_GLOBAL), context->report({node, toErr(Err0430)}));
...
bool Semantic::resolveAttrUse(SemanticContext* context, AstAttrUse* node)
{
    for (auto child : node->children)
    {
        ...
        {
            Diagnostic err{identifier, identifier->token, formErr(Err0173, resolvedName->name.c_str(), Naming::aKindName(resolvedName->kind).c_str())};
        ...
        if (node->hasSpecFlag(AstAttrUse::SPEC_FLAG_GLOBAL))
        {
            ...
            {
                Diagnostic err{identifier, identifier->token, formErr(Err0438, resolvedName->name.c_str())};
                context->report(err, Diagnostic::hereIs(resolved));
                return false;
            }
        }
