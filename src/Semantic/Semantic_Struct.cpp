...
bool Semantic::resolveImplForAfterFor(SemanticContext* context)
{
    ...
    if (id->resolvedSymbolName()->isNot(SymbolKind::Struct))
        return context->report({id->lastChild(), formErr(Err0483, id->resolvedSymbolName()->name.cstr(), Naming::aKindName(id->resolvedSymbolName()->kind).cstr())});
...
bool Semantic::resolveImplFor(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node->identifier, formErr(Err0692, node->identifier->token.cstr(), Naming::aKindName(typeInfo).cstr())};
    ...
    for (const auto child : children)
    {
        ...
        if (!childFct->hasSpecFlag(AstFuncDecl::SPEC_FLAG_IMPL))
        {
            ...
            if (itfSymbol)
            {
                Diagnostic err{childFct, childFct->tokenName, formErr(Err0371, childFct->token.text.cstr(), typeInterface->name.cstr())};
        ...
        if (!itfSymbol)
        {
            Diagnostic err{childFct, childFct->tokenName, formErr(Err0090, childFct->token.text.cstr(), typeInterface->getDisplayNameC())};
            if (childFct->hasSpecFlag(AstFuncDecl::SPEC_FLAG_IMPL))
        ...
        if (!typeLambda->isSame(typeFunc, CAST_FLAG_EXACT | CAST_FLAG_INTERFACE, bi))
        {
            switch (bi.matchResult)
            {
                ...
                {
                    Diagnostic err{childFct, childFct->getTokenName(), formErr(Err0306, typeBaseInterface->name.cstr(), child->token.cstr())};
                ...
                {
                    Diagnostic err{child, child->getTokenName(), formErr(Err0306, typeBaseInterface->name.cstr(), child->token.cstr())};
                ...
                {
                    Diagnostic err{childFct->returnType, formErr(Err0306, typeBaseInterface->name.cstr(), child->token.cstr())};
...
bool Semantic::resolveInterface(SemanticContext* context)
{
    ...
    for (auto child : children)
    {
        ...
        if (!node->hasAstFlag(AST_FROM_GENERIC))
        {
            ...
            {
                const Diagnostic err{varDecl->type, formErr(Err0376, child->token.cstr())};
                return context->report(err);
            }

            const auto firstParamType = typeLambda->parameters[0]->typeInfo;
            SWAG_VERIFY(firstParamType->isPointer(), context->report({typeLambda->parameters[0]->declNode, formErr(Err0718, firstParamType->getDisplayNameC())}));
            const auto firstParamPtr = castTypeInfo<TypeInfoPointer>(firstParamType, TypeInfoKind::Pointer);
            SWAG_VERIFY(firstParamPtr->pointedType == typeInterface, context->report({typeLambda->parameters[0]->declNode, formErr(Err0718, firstParamType->getDisplayNameC())}));
        }
        ...
        {
            SWAG_VERIFY(!child->typeInfo->isGeneric(), context->report({child, formErr(Err0264, node->token.cstr(), child->typeInfo->getDisplayNameC())}));
        }
        ...
        if (typeParam->attributes.hasAttribute(g_LangSpec->name_Swag_Offset))
        {
            ...
            SWAG_ASSERT(attr);
            return context->report({attr->node, attr->node->token, toErr(Err0192)});
        }
...
bool Semantic::checkImplScopes(SemanticContext* context, AstImpl* node, const Scope* scopeImpl, const Scope* scope)
{
    ...
    if (scopeImpl != scope)
    {
        Diagnostic err{node, node->token, formErr(Err0268, node->token.cstr())};
        err.addNote(Diagnostic::hereIs(node->identifier->resolvedSymbolOverload()));
...
bool Semantic::resolveImpl(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node->identifier, formErr(Err0693, node->identifier->token.cstr(), typeInfo->getDisplayNameC())};
        err.addNote(Diagnostic::hereIs(node->identifier->resolvedSymbolOverload()));
        return context->report(err);
    }

    const auto typeIdentifier = node->identifier->resolvedSymbolOverload()->typeInfo;
    SWAG_VERIFY(!typeIdentifier->isAlias(), context->report({node->identifier, toErr(Err0691)}));
...
bool Semantic::postResolveStruct(SemanticContext* context)
{
    ...
    // Verify that we have only one possible conversion
    {
        ...
        for (const auto key : usingFields | std::views::keys)
        {
            ...
            if (idx != UINT32_MAX)
            {
                Diagnostic err(key->declNode, formErr(Err0203, typeStruct->getDisplayNameC()));
...
bool Semantic::resolveStruct(SemanticContext* context)
{
    ...
    if (!sourceFile->hasFlag(FILE_GENERATED))
    {
        ...
        {
            SWAG_VERIFY(!sourceFile->hasFlag(FILE_FORCE_EXPORT), context->report({node, node->token, toErr(Err0269)}));
            SWAG_VERIFY(node->isPublic(), context->report({node, node->token, toErr(Err0380)}));
        }
    }
    ...
    for (auto child : children)
    {
        ...
        {
            Diagnostic err{node, node->getTokenName(), formErr(Err0063, node->token.cstr())};
...

    {
        ...
        for (auto child : children)
        {
            ...
            {
                SWAG_CHECK(checkIsConstExpr(context, varDecl->assignment->hasAstFlag(AST_CONST_EXPR), varDecl->assignment, toErr(Err0550)));
            ...
            if (!node->hasAstFlag(AST_GENERIC))
            {
                if (varTypeInfo->isGeneric())
                {
                    ...
                    Diagnostic err{node, node->getTokenName(), formErr(Err0679, node->token.cstr(), varDecl->token.cstr())};
            ...
            if (typeParam)
            {
                ...
                if (forceOffset)
                {
                    ...
                    if (!relocated)
                    {
                        ...
                        SWAG_ASSERT(attr);
                        return context->report({attr->node, attr->parameters[0].token, formErr(Err0102, forceOffset->text.cstr())});
                    }
                }
            }
