...
bool Semantic::resolveImplFor(SemanticContext* context)
{
    ...
    for (const auto child : children)
    {
        ...
        if (!childFct->hasSpecFlag(AstFuncDecl::SPEC_FLAG_IMPL))
        {
            ...
            if (itfSymbol)
            {
                Diagnostic err{childFct, childFct->tokenName, formErr(Err0485, childFct->token.text.c_str(), typeInterface->name.c_str())};
        ...
        if (!itfSymbol)
        {
            Diagnostic err{childFct, childFct->tokenName, formErr(Err0709, childFct->token.text.c_str(), typeInterface->name.c_str())};
            if (childFct->hasSpecFlag(AstFuncDecl::SPEC_FLAG_IMPL))
        ...
        if (!typeLambda->isSame(typeFunc, CAST_FLAG_EXACT | CAST_FLAG_INTERFACE, bi))
        {
            switch (bi.matchResult)
            {
                ...
                {
                    Diagnostic err{childFct, childFct->getTokenName(), formErr(Err0372, child->token.c_str(), typeBaseInterface->name.c_str())};
                ...
                {
                    Diagnostic err{child, child->getTokenName(), formErr(Err0372, child->token.c_str(), typeBaseInterface->name.c_str())};
                ...
                {
                    Diagnostic err{childFct->returnType, formErr(Err0372, child->token.c_str(), typeBaseInterface->name.c_str())};
...
bool Semantic::resolveInterface(SemanticContext* context)
{
    ...
    for (auto child : children)
    {
        ...
        if (typeParam->attributes.hasAttribute(g_LangSpec->name_Swag_Offset))
        {
            ...
            SWAG_ASSERT(attr);
            return context->report({attr->node, attr->node->token, toErr(Err0430)});
        }
...
bool Semantic::checkImplScopes(SemanticContext* context, AstImpl* node, const Scope* scopeImpl, const Scope* scope)
{
    ...
    if (scopeImpl != scope)
    {
        Diagnostic err{node, node->token, formErr(Err0373, node->token.c_str())};
...
bool Semantic::resolveStruct(SemanticContext* context)
{
    ...
    for (auto child : children)
    {
        ...
        {
            Diagnostic err{node, node->getTokenName(), formErr(Err0576, node->token.c_str())};
        ...
        for (auto child : children)
        {
            ...
            if (!node->hasAstFlag(AST_GENERIC))
            {
                if (varTypeInfo->isGeneric())
                {
                    ...
                    if (!node->genericParameters)
                    {
                        Diagnostic err{node, node->getTokenName(), formErr(Err0505, node->token.c_str(), varDecl->token.c_str())};
            ...
            if (typeParam)
            {
                ...
                if (forceOffset)
                {
                    ...
                    if (!relocated)
                    {
                        ...
                        SWAG_ASSERT(attr);
                        return context->report({attr->node, attr->parameters[0].token, formErr(Err0718, forceOffset->text.c_str())});
                    }
                }
            }
            ...
            if (!child->hasSpecFlag(AstVarDecl::SPEC_FLAG_AUTO_NAME))
            {
                ...
                {
                    return context->report({child, child->token, formErr(Err0581, child->token.c_str())});
                }
