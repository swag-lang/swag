        ...
        if (!identifier->genericParameters)
        {
            ...
            if (nodeFunc->genericParameters)
            {
                ...
                for (const auto p : nodeFunc->genericParameters->children)
                {
                    ...
                    if (!param->assignment)
                    {
                        const Diagnostic err{node->token.sourceFile, node->token, formErr(Err0538, identifier->resolvedSymbolName()->name.c_str())};
    ...
    if (context->node->is(AstNodeKind::Identifier))
    {
        ...
        if (idRef->resolvedSymbolOverload())
        {
            ...
            if (concreteType->isStruct())
            {
                ...
                if (contextualNode)
                {
                    const Diagnostic err{node->token.sourceFile, node->token, formErr(Err0538, node->token.c_str())};
