...
bool Generic::instantiateDefaultGenericVar(SemanticContext* context, AstVarDecl* node)
{
    ...
    return context->report({node, formErr(Err0077, node->typeInfo->getDisplayNameC())});
}

bool Generic::instantiateStruct(SemanticContext* context, AstNode* genericParameters, OneMatch& match, bool& alias)
{
    auto node = context->node;
    SWAG_VERIFY(!match.genericReplaceTypes.empty(), context->report({node, formErr(Err0215, node->token.c_str())}));
    ...
    SWAG_VERIFY(sourceNode->is(AstNodeKind::StructDecl), context->report({node, node->token, formErr(Err0213, node->token.c_str())}));
