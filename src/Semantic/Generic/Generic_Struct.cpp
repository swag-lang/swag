            ...
            if (nodeStruct->genericParameters)
            {
                ...
                if (!identifier->genericParameters)
                {
                    ...
                    for (const auto p : nodeStruct->genericParameters->children)
                    {
                        ...
                        if (!param->assignment)
                        {
                            const Diagnostic err{node->token.sourceFile, node->type->token, formErr(Err0561, typeExpr->identifier->resolvedSymbolName()->name.c_str())};
...
bool Generic::instantiateStruct(SemanticContext* context, AstNode* genericParameters, OneMatch& match, bool& alias)
{
    ...
    SWAG_VERIFY(!match.genericReplaceTypes.empty(), context->report({node, formErr(Err0302, node->token.c_str())}));
    ...
    SWAG_VERIFY(sourceNode->is(AstNodeKind::StructDecl), context->report({node, node->token, formErr(Err0298, node->token.c_str())}));
