...
bool Semantic::doExecuteCompilerNode(SemanticContext* context, AstNode* node, bool onlyConstExpr)
{
    ...
    if (!node->hasSemFlag(SEMFLAG_EXEC_RET_STACK))
    {
        ...
        if (realType && realType->isStruct() && !realType->declNode->hasAttribute(ATTRIBUTE_CONSTEXPR))
        {
            ...
            {
                Diagnostic err{node, formErr(Err0048, realType->getDisplayNameC())};
            ...
            else
            {
                ...
                if (concreteType->isSlice())
                {
                    ...
                    if (!ok)
                    {
                        const Diagnostic err{node, formErr(Err0036, typeSliceContent->getDisplayNameC())};
                        return context->report(err);
                    }
                }
                else if (!concreteType->isString())
                {
                    const Diagnostic err{node, formErr(Err0050, concreteType->getDisplayNameC())};
                    return context->report(err);
                }
...
bool Semantic::resolveCompilerWhereExpression(SemanticContext* context)
{
    ...
    if (!typeInfo->isBool())
    {
        const Diagnostic err{expression, formErr(Err0601, typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::resolveCompilerAstExpression(SemanticContext* context)
{
    ...
    const auto typeInfo   = TypeManager::concreteType(expression->typeInfo);
    SWAG_VERIFY(typeInfo->isString(), context->report({expression, formErr(Err0572, expression->typeInfo->getDisplayNameC())}));
...
bool Semantic::resolveCompilerError(SemanticContext* context)
{
    ...
    YIELD();
    SWAG_CHECK(checkIsConstExpr(context, msg->hasFlagComputedValue(), msg, formErr(Err0040, node->token.c_str())));
...
bool Semantic::resolveCompilerAssert(SemanticContext* context)
{
    ...
    if (!expr->computedValue()->reg.b)
        return context->report({node, node->token, toErr(Err0034)});
    return true;
}
...
bool Semantic::resolveCompilerMacro(SemanticContext* context)
{
    ...
    if (!node->hasOwnerInline() || node->ownerInline()->hasAttribute(ATTRIBUTE_MIXIN) || !node->ownerInline()->hasAttribute(ATTRIBUTE_MACRO))
        return context->report({node, toErr(Err0376)});
...
bool Semantic::resolveCompilerMixin(SemanticContext* context)
{
    ...
    SWAG_VERIFY(expr->typeInfo->isCode(), context->report({expr, formErr(Err0600, expr->typeInfo->getDisplayNameC())}));
...
bool Semantic::resolveCompilerInclude(SemanticContext* context)
{
    ...
    SWAG_CHECK(checkIsConstExpr(context, back->hasFlagComputedValue(), back, toErr(Err0035)));
    SWAG_VERIFY(back->typeInfo == g_TypeMgr->typeInfoString, context->report({back, formErr(Err0599, back->typeInfo->getDisplayNameC())}));
    ...
    if (!node->hasSemFlag(SEMFLAG_LOAD))
    {
        ...
        if (!std::filesystem::exists(fullFileName, err))
        {
            ...
            if (!std::filesystem::exists(fullFileName, err))
            {
                ...
                if (!std::filesystem::exists(fullFileName, err))
                    return context->report({back, formErr(Err0704, filename.c_str())});
            }
        }

        struct stat statBuf;
        const int   rc = stat(fullFileName, &statBuf);
        SWAG_VERIFY(rc == 0, context->report({back, formErr(Err0085, back->computedValue()->text.c_str())}));
...
bool Semantic::resolveCompilerSpecialValue(SemanticContext* context)
{
    ...
    switch (node->token.id)
    {
        ...
        case TokenId::CompilerCallerLocation:
            SWAG_VERIFY(node->parent->is(AstNodeKind::FuncDeclParam), context->report({node, toErr(Err0434)}));
            node->typeInfo = g_Workspace->swagScope.regTypeInfoSourceLoc;
            return true;

        case TokenId::CompilerCallerFunction:
            SWAG_VERIFY(node->parent->is(AstNodeKind::FuncDeclParam), context->report({node, toErr(Err0433)}));
            node->typeInfo = g_TypeMgr->typeInfoString;
            return true;
