        ...
        if (symbolName)
        {
            if (symbolName->is(SymbolKind::Function))
            {
                if (c->resolvedSymbolOverload())
                {
                    ...
                    {
                        const auto result = Diagnostic::note(c, c->token, formNte(Nte0123, symbolName->name.cstr()));
                        result->hint      = toNte(Nte0041);
                        return result;
                    }

                    return Diagnostic::note(c, c->token, toNte(Nte0170));
                }
            }
            ...
            return Diagnostic::note(c, toNte(Nte0084));
        }
    }
    ...
    if (!node->hasSemFlag(SEMFLAG_EXEC_RET_STACK))
    {
        ...
        if (realType && realType->isStruct() && !realType->declNode->hasAttribute(ATTRIBUTE_CONSTEXPR))
        {
            ...
            if (node->hasSpecialFuncCall())
            {
                ...
                err.hint          = formNte(Nte0158, userOp->symbol->name.cstr());
...
bool Semantic::resolveWhereConstraintExpression(SemanticContext* context)
{
    ...
    if (!typeInfo->isBool())
    {
        const Diagnostic err{expression, formErr(Err0587, typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::resolveCompilerAstExpression(SemanticContext* context)
{
    ...
    const auto typeInfo   = TypeManager::concreteType(expression->typeInfo);
    SWAG_VERIFY(typeInfo->isString(), context->report({expression, formErr(Err0643, expression->typeInfo->getDisplayNameC())}));
...
bool Semantic::resolveCompilerMacro(SemanticContext* context)
{
    ...
    if (!node->hasOwnerInline() || node->ownerInline()->hasAttribute(ATTRIBUTE_MIXIN) || !node->ownerInline()->hasAttribute(ATTRIBUTE_MACRO))
        return context->report({node, toErr(Err0336)});
...
bool Semantic::resolveCompilerMixin(SemanticContext* context)
{
    ...
    SWAG_VERIFY(expr->typeInfo->isCode(), context->report({expr, formErr(Err0586, expr->typeInfo->getDisplayNameC())}));
...
bool Semantic::resolveCompilerInclude(SemanticContext* context)
{
    ...
    SWAG_VERIFY(back->typeInfo == g_TypeMgr->typeInfoString, context->report({back, formErr(Err0585, back->typeInfo->getDisplayNameC())}));
    ...
    if (!node->hasSemFlag(SEMFLAG_LOAD))
    {
        ...
        if (!std::filesystem::exists(fullFileName, err))
        {
            ...
            if (!std::filesystem::exists(fullFileName, err))
            {
                ...
                if (!std::filesystem::exists(fullFileName, err))
                    return context->report({back, formErr(Err0720, filename.cstr())});
            }
        }
...
bool Semantic::resolveCompilerSpecialValue(SemanticContext* context)
{
    ...
    switch (node->token.id)
    {
        ...
        case TokenId::CompilerCallerLocation:
            SWAG_VERIFY(node->parent->is(AstNodeKind::FuncDeclParam), context->report({node, toErr(Err0411)}));
            node->typeInfo = g_Workspace->swagScope.regTypeInfoSourceLoc;
            return true;

        case TokenId::CompilerCallerFunction:
            SWAG_VERIFY(node->parent->is(AstNodeKind::FuncDeclParam), context->report({node, toErr(Err0410)}));
            node->typeInfo = g_TypeMgr->typeInfoString;
            return true;
