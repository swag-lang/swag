...
bool Semantic::doExecuteCompilerNode(SemanticContext* context, AstNode* node, bool onlyConstExpr)
{
    ...
    if (!node->hasSemFlag(SEMFLAG_EXEC_RET_STACK))
    {
        ...
        if (realType && realType->isStruct() && !realType->declNode->hasAttribute(ATTRIBUTE_CONSTEXPR))
        {
            ...
            {
                Diagnostic err{node, formErr(Err0218, realType->getDisplayNameC())};
            ...
            else
            {
                ...
                if (concreteType->isSlice())
                {
                    ...
                    if (!ok)
                    {
                        const Diagnostic err{node, formErr(Err0044, typeSliceContent->getDisplayNameC())};
                        return context->report(err);
                    }
                }
                else if (!concreteType->isString())
                {
                    const Diagnostic err{node, formErr(Err0048, concreteType->getDisplayNameC())};
                    return context->report(err);
                }
...
bool Semantic::resolveCompilerAstExpression(SemanticContext* context)
{
    ...
    SWAG_VERIFY(typeInfo->isString(), context->report({expression, formErr(Err0607, expression->typeInfo->getDisplayNameC())}));
...
bool Semantic::resolveCompilerError(SemanticContext* context)
{
    ...
    YIELD();
    SWAG_CHECK(checkIsConstExpr(context, msg->hasFlagComputedValue(), msg, formErr(Err0552, node->token.cstr())));
...
bool Semantic::resolveCompilerMacro(SemanticContext* context)
{
    ...
    if (!node->hasOwnerInline() || node->ownerInline()->hasAttribute(ATTRIBUTE_MIXIN) || !node->ownerInline()->hasAttribute(ATTRIBUTE_MACRO))
        return context->report({node, toErr(Err0508)});
...
bool Semantic::resolveCompilerInject(SemanticContext* context)
{
    ...
    SWAG_VERIFY(expr->typeInfo->isCode(), context->report({expr, formErr(Err0609, expr->typeInfo->getDisplayNameC())}));
...
bool Semantic::resolveCompilerInclude(SemanticContext* context)
{
    ...
    SWAG_CHECK(checkIsConstExpr(context, back->hasFlagComputedValue(), back, toErr(Err0543)));
    SWAG_VERIFY(back->typeInfo == g_TypeMgr->typeInfoString, context->report({back, formErr(Err0608, back->typeInfo->getDisplayNameC())}));
    ...
    if (!node->hasSemFlag(SEMFLAG_LOAD))
    {
        ...
        if (!std::filesystem::exists(fullFileName, err))
        {
            ...
            if (!std::filesystem::exists(fullFileName, err))
            {
                ...
                if (!std::filesystem::exists(fullFileName, err))
                    return context->report({back, formErr(Err0087, filename.cstr())});
            }
        }
...
bool Semantic::resolveCompilerTag(SemanticContext* context)
{
    ...
    switch (node->token.id)
    {
        case TokenId::CompilerSafety:
        {
            ...
            YIELD();
            SWAG_CHECK(checkIsConstExpr(context, front->hasFlagComputedValue(), front, toErr(Err0545), node->token.text));            
            SWAG_VERIFY(front->typeInfo == g_Workspace->swagScope.regTypeInfoSafetyWhat, context->report({front, formErr(Err0613, node->token.cstr(), front->typeInfo->getDisplayNameC(), node->token.cstr(), g_Workspace->swagScope.regTypeInfoSafetyWhat->getDisplayNameC())}));
            node->typeInfo = g_TypeMgr->typeInfoBool;
            node->setFlagsValueIsComputed();
        ...
        case TokenId::CompilerHasTag:
        {
            ...
            YIELD();
            SWAG_CHECK(checkIsConstExpr(context, front->hasFlagComputedValue(), front, toErr(Err0545), node->token.text));
            SWAG_VERIFY(front->typeInfo->isString(), context->report({front, formErr(Err0613, node->token.cstr(), front->typeInfo->getDisplayNameC(), node->token.cstr(), "string")}));
        ...
        case TokenId::CompilerGetTag:
        {
            ...
            SWAG_CHECK(checkIsConstExpr(context, nameNode->hasFlagComputedValue(), nameNode, toErr(Err0545), node->token.text));
            SWAG_VERIFY(nameNode->typeInfo->isString(), context->report({nameNode, formErr(Err0613, node->token.cstr(), nameNode->typeInfo->getDisplayNameC(), node->token.cstr(), "string")}));
            SWAG_VERIFY(defaultVal->hasComputedValue(), context->report({defaultVal, formErr(Err0551, Naming::aKindName(defaultVal).cstr())}));
            ...
            if (tag)
            {
                ...
                {
                    Diagnostic err{typeNode, formErr(Err0280, typeNode->typeInfo->getDisplayNameC(), tag->type->getDisplayNameC(), tag->name.cstr())};
...
bool Semantic::resolveCompilerStringOf(SemanticContext* context)
{
    ...
    if (expr->hasFlagComputedValue())
    {
        if (expr->isConstantGenTypeInfo())
            ...
            return context->report({expr, toErr(Err0259)});
    }
...
bool Semantic::resolveCompilerNameOf(SemanticContext* context)
{
    ...
    if (expr->isConstantGenTypeInfo())
        ...
        return context->report({expr, toErr(Err0258)});
...
bool Semantic::resolveCompilerTypeOf(SemanticContext* context)
{
    ...
    SWAG_VERIFY(!typeInfo->isKindGeneric(), context->report({expr, toErr(Err0074)}));
...
bool Semantic::resolveCompilerRunes(SemanticContext* context)
{
    ...
    SWAG_VERIFY(typeInfo->isString(), context->report({expr, formErr(Err0611, typeInfo->getDisplayNameC())}));
    node->setFlagsValueIsComputed();
...
bool Semantic::resolveCompilerSizeOf(SemanticContext* context)
{
    const auto node = context->node;
    auto       expr = node->firstChild();
    SWAG_VERIFY(!expr->typeInfo->isGeneric(), context->report({expr, toErr(Err0193)}));
    node->setFlagsValueIsComputed();
...
bool Semantic::resolveCompilerAlignOf(SemanticContext* context)
{
    const auto node = context->node;
    auto       expr = node->firstChild();
    SWAG_VERIFY(!expr->typeInfo->isGeneric(), context->report({expr, toErr(Err0745)}));
    node->setFlagsValueIsComputed();
