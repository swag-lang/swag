...
bool Semantic::boundCheck(SemanticContext* context, const TypeInfo* forType, AstNode* arrayNode, AstNode* arrayAccess, uint64_t maxCount)
{
    ...
    if (maxCount == 0)
    {
        if (forType->isSlice())
        {
            Diagnostic err{arrayAccess, toErr(Err0597)};
    ...
    if (idx >= maxCount)
        return context->report({arrayAccess, formErr(Err0130, idx, maxCount - 1)});
    return true;
}
...
bool Semantic::checkCanTakeAddress(SemanticContext* context, AstNode* node)
{
        ...
        !overload->typeInfo->isSlice())
    {
        if (node->isNot(AstNodeKind::IdentifierRef) || node->lastChild()->isNot(AstNodeKind::ArrayPointerIndex))
        {
            ...
            {
                const Diagnostic err{node, node->token, formErr(Err0176, overload->typeInfo->getDisplayNameC())};
                return context->report(err);
            }

            if (overload->hasFlag(OVERLOAD_CONSTANT))
            {
                const Diagnostic err{node, node->token, formErr(Err0175, overload->typeInfo->getDisplayNameC())};
                return context->report(err);
            }
        }
    }
    ...
    if (!node->hasAstFlag(AST_L_VALUE))
    {
        ...
        {
            const Diagnostic err{node, formErr(Err0173, Naming::aKindName(symbol->kind).c_str())};
            return context->report(err);
        }

        const Diagnostic err{node, formErr(Err0175, node->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::resolveMakePointer(SemanticContext* context)
{
    ...
    if (!symbol)
    {
        Diagnostic err{node, node->token, toErr(Err0182)};
    ...
    if (overload && overload->hasFlag(OVERLOAD_IS_LET))
    {
        ...
        {
            const Diagnostic err{node, node->token, toErr(Err0180)};
    ...
    if (symbol->is(SymbolKind::Function))
    {
        ...
        if (!typeInfo->isPointerRef())
        {
            ...
            Diagnostic err{node, node->token, formErr(Err0177, typeInfo->getDisplayNameC())};
...
bool Semantic::resolveArrayPointerSlicing(SemanticContext* context)
{
        ...
        !node->upperBound->hasSemFlag(SEMFLAG_ASSIGN_COMPUTED))
    {
        ...
        {
            const Diagnostic err{node->upperBound, toErr(Err0610)};
            return context->report(err);
        }
    ...
    else if (typeVar->isPointer())
    {
        ...
        {
            Diagnostic err{node, node->token, toErr(Err0347)};
    ...
    else if (typeVar->isStruct())
    {
        ...
        if (!symbol)
        {
            Diagnostic err{node->token.sourceFile, node->token, formErr(Err0359, node->array->token.c_str(), typeInfo->getDisplayNameC())};
    ...
    {
        return context->report({node->array, formErr(Err0360, node->array->typeInfo->getDisplayNameC())});
    }
    ...
    if (node->lowerBound->hasFlagComputedValue() && node->upperBound->hasFlagComputedValue())
    {
        ...
        {
            Diagnostic err{node->lowerBound, formErr(Err0609, node->lowerBound->computedValue()->reg.u64, node->upperBound->computedValue()->reg.u64)};
    ...
    if (maxBound && node->upperBound->hasFlagComputedValue())
    {
        ...
        {
            const Diagnostic err{node->upperBound, formErr(Err0608, node->upperBound->computedValue()->reg.u64, maxBound)};
            return context->report(err);
        }
    }
...
bool Semantic::resolveMoveRef(SemanticContext* context)
{
    ...
    {
        Diagnostic err(node, node->token, formErr(Err0455, front->typeInfo->getDisplayNameC()));
    ...
    if (typeInfo->isConst())
    {
        Diagnostic err(node, node->token, toErr(Err0454));
        err.addNote(front, Diagnostic::isType(front->typeInfo));
        return context->report(err);
    }
...
bool Semantic::resolveKeepRef(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0350, typeInfo->getDisplayNameC())};
...
bool Semantic::resolveArrayPointerRef(SemanticContext* context)
{
    ...
    if (!arrayType->isStruct())
    {
        ...
        {
            const Diagnostic err{arrayNode->access, formErr(Err0732, arrayNode->access->typeInfo->getDisplayNameC())};
            return context->report(err);
        }
    }
    ...
    switch (arrayType->kind)
    {
        case TypeInfoKind::Pointer:
        {
            ...
            {
                const Diagnostic err{arrayNode->array, formErr(Err0250, arrayNode->resolvedSymbolName()->name.c_str(), arrayType->getDisplayNameC())};
                return context->report(err);
            }
            ...
            {
                Diagnostic err{arrayNode->access, toErr(Err0409)};
        ...
        case TypeInfoKind::Native:
        {
            ...
            {
                Diagnostic err{arrayNode->array, formErr(Err0249, arrayType->getDisplayNameC())};
        ...
        case TypeInfoKind::Struct:
            ...
            if (arrayType->isTuple())
            {
                if (arrayNode->hasSpecFlag(AstArrayPointerIndex::SPEC_FLAG_IS_DEREF))
                {
                    Diagnostic err{arrayNode->access, toErr(Err0372)};
                    err.addNote(arrayNode->token.startLocation, arrayNode->token.endLocation, toNte(Nte0121));
                    return context->report(err);
                }
        ...
        default:
        {
            return context->report({arrayNode->array, formErr(Err0249, arrayType->getDisplayNameC())});
        }
    }
...
bool Semantic::resolveArrayPointerDeRef(SemanticContext* context)
{
    ...
    switch (arrayType->kind)
    {
        case TypeInfoKind::Pointer:
        {
            if (!arrayType->isPointerArithmetic() && !arrayNode->hasSpecFlag(AstArrayPointerIndex::SPEC_FLAG_IS_DEREF) && !context->forDebugger)
            {
                Diagnostic err{arrayNode->access, formErr(Err0250, arrayNode->resolvedSymbolName()->name.c_str(), arrayType->getDisplayNameC())};
                err.addNote(arrayNode->array, Diagnostic::isType(arrayType));
                err.addNote(toNte(Nte0110));
                return context->report(err);
            }
            ...
            if (arrayNode->array->hasFlagComputedValue() && arrayNode->access->hasFlagComputedValue())
            {
                ...
                if (!storageSegment)
                {
                    const Diagnostic err{arrayNode, toErr(Err0597)};
                    return context->report(err);
                }
        ...
        case TypeInfoKind::Struct:
        {
            ...
            {
                YIELD();
                Diagnostic err{arrayNode->access, formErr(Err0248, arrayNode->array->token.c_str(), arrayType->getDisplayNameC())};
        ...
        default:
        {
            Diagnostic err{arrayNode->array, formErr(Err0249, arrayNode->array->typeInfo->getDisplayNameC())};
            if (arrayNode->hasSpecFlag(AstArrayPointerIndex::SPEC_FLAG_IS_DEREF))
                err.addNote(arrayNode->token.startLocation, arrayNode->token.endLocation, toNte(Nte0121));
            return context->report(err);
        }
...
bool Semantic::checkInitDropCount(SemanticContext* context, const AstNode* node, AstNode* expression, AstNode* count)
{
    ...
    SWAG_VERIFY(countTypeInfo->isNativeInteger(), context->report({count, formErr(Err0191, node->token.c_str(), countTypeInfo->getDisplayNameC())}));
    ...
    if (!count->hasFlagComputedValue() || count->computedValue()->reg.u64 > 1)
    {
        if (!expression->typeInfo->isPointerArithmetic())
        {
            ...
            {
                Diagnostic err{expression, formErr(Err0192, node->token.c_str(), expression->typeInfo->getDisplayNameC())};
                err.addNote(count, formNte(Nte0140, count->computedValue()->reg.u64));
                return context->report(err);
            }

            Diagnostic err{expression, formErr(Err0193, node->token.c_str(), expression->typeInfo->getDisplayNameC())};
...
bool Semantic::resolveInit(SemanticContext* context)
{
    ...
    if (!node->count)
    {
        ...
        SWAG_VERIFY(node->expression->is(AstNodeKind::IdentifierRef), context->report({node->expression, formErr(Err0196, node->token.c_str())}));
        SWAG_VERIFY(node->expression->resolvedSymbolOverload(), context->report({node->expression, formErr(Err0196, node->token.c_str())}));
        SWAG_VERIFY(!expressionTypeInfo->isConst(), context->report({node->expression, formErr(Err0055, node->token.c_str(), expressionTypeInfo->getDisplayNameC())}));
    ...
    {
        SWAG_VERIFY(expressionTypeInfo->isPointer(), context->report({node->expression, formErr(Err0195, node->token.c_str(), expressionTypeInfo->getDisplayNameC())}));
        SWAG_VERIFY(!node->expression->typeInfo->isConst(), context->report({node->expression, formErr(Err0054, node->token.c_str(), expressionTypeInfo->getDisplayNameC())}));
    ...
    if (node->parameters)
    {
        ...
        {
            SWAG_VERIFY(node->parameters->childCount() == 1, context->report({node->parameters, formErr(Err0634, pointedType->getDisplayNameC())}));
...
bool Semantic::resolveDropCopyMove(SemanticContext* context)
{
    ...
    if (node->is(AstNodeKind::PostCopy))
    {
        ...
        {
            return context->report({node->expression, formErr(Err0098, pointedType->getDisplayNameC())});
        }
    }
