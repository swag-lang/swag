...
bool Semantic::boundCheck(SemanticContext* context, const TypeInfo* forType, AstNode* arrayNode, AstNode* arrayAccess, uint64_t maxCount)
{
    ...
    if (maxCount == 0)
    {
        if (forType->isSlice())
        {
            Diagnostic err{arrayAccess, toErr(Err0201)};
    ...
    if (idx >= maxCount)
        return context->report({arrayAccess, formErr(Err0566, idx, maxCount - 1)});
    return true;
}
...
bool Semantic::checkCanTakeAddress(SemanticContext* context, AstNode* node)
{
    ...
    if (!node->hasAstFlag(AST_L_VALUE))
    {
        ...
        {
            const Diagnostic err{node, formErr(Err0144, Naming::aKindName(symbol->kind).c_str())};
            return context->report(err);
        }

        const Diagnostic err{node, formErr(Err0146, node->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::resolveMakePointerLambda(SemanticContext* context)
{
    ...
    if (node->lambda && node->lambda->captureParameters)
    {
        ...
        SWAG_VERIFY(typeBlock->sizeOf <= maxSize, context->report({node->lambda->captureParameters, formErr(Err0574, typeBlock->sizeOf, maxSize)}));
    }
...
bool Semantic::resolveMakePointer(SemanticContext* context)
{
    ...
    if (overload && overload->hasFlag(OVERLOAD_IS_LET))
    {
        ...
        {
            const Diagnostic err{node, node->token, toErr(Err0151)};
    ...
    if (symbol->is(SymbolKind::Function))
    {
        ...
        if (!typeInfo->isPointerRef())
        {
            if (typeInfo->isVoid())
            {
                const Diagnostic err{node, node->token, formErr(Err0143, symbol->name.c_str())};
                return context->report(err, Diagnostic::hereIs(overload));
            }

            Diagnostic err{node, node->token, formErr(Err0148, typeInfo->getDisplayNameC())};
...
bool Semantic::resolveArrayPointerSlicing(SemanticContext* context)
{
        ...
        !node->upperBound->hasSemFlag(SEMFLAG_ASSIGN_COMPUTED))
    {
        ...
        {
            const Diagnostic err{node->upperBound, toErr(Err0571)};
            return context->report(err);
        }
    ...
    if (node->lowerBound->hasFlagComputedValue() && node->upperBound->hasFlagComputedValue())
    {
        ...
        {
            Diagnostic err{node->lowerBound, formErr(Err0570, node->lowerBound->computedValue()->reg.u64, node->upperBound->computedValue()->reg.u64)};
    ...
    if (maxBound && node->upperBound->hasFlagComputedValue())
    {
        ...
        {
            const Diagnostic err{node->upperBound, formErr(Err0569, node->upperBound->computedValue()->reg.u64, maxBound)};
            return context->report(err);
        }
    }
...
bool Semantic::resolveMoveRef(SemanticContext* context)
{
    ...
    {
        const Diagnostic err(node, node->token, toErr(Err0408));
        return context->report(err);
    }

    if (!typeInfo->isPointer() && !typeInfo->isPointerRef())
    {
        Diagnostic err(node, node->token, formErr(Err0410, front->typeInfo->getDisplayNameC()));
    ...
    if (typeInfo->isConst())
    {
        Diagnostic err(node, node->token, toErr(Err0409));
        err.addNote(front, Diagnostic::isType(front->typeInfo));
        return context->report(err);
    }
...
bool Semantic::resolveArrayPointerRef(SemanticContext* context)
{
    ...
    switch (arrayType->kind)
    {
        case TypeInfoKind::Pointer:
        {
            ...
            {
                const Diagnostic err{arrayNode->array, formErr(Err0199, arrayNode->resolvedSymbolName()->name.c_str(), arrayType->getDisplayNameC())};
                return context->report(err);
            }
        ...
        case TypeInfoKind::Native:
        {
            ...
            if (arrayType->nativeType == NativeTypeKind::String)
            {
                ...
                {
                    Diagnostic err{arrayNode->getParent(2), arrayNode->getParent(2)->token, formErr(Err0145, arrayType->getDisplayNameC())};
...
bool Semantic::resolveArrayPointerDeRef(SemanticContext* context)
{
    ...
    switch (arrayType->kind)
    {
        case TypeInfoKind::Pointer:
        {
            if (!arrayType->isPointerArithmetic() && !arrayNode->hasSpecFlag(AstArrayPointerIndex::SPEC_FLAG_IS_DEREF) && !context->forDebugger)
            {
                Diagnostic err{arrayNode->access, formErr(Err0199, arrayNode->resolvedSymbolName()->name.c_str(), arrayType->getDisplayNameC())};
                err.addNote(arrayNode->array, Diagnostic::isType(arrayType));
                err.addNote(toNte(Nte0110));
                return context->report(err);
            }
            ...
            if (arrayNode->array->hasFlagComputedValue() && arrayNode->access->hasFlagComputedValue())
            {
                ...
                if (!storageSegment)
                {
                    const Diagnostic err{arrayNode, toErr(Err0201)};
                    return context->report(err);
                }
        ...
        case TypeInfoKind::Struct:
        {
            ...
            {
                YIELD();
                Diagnostic err{arrayNode->access, formErr(Err0197, arrayNode->array->token.c_str(), arrayType->getDisplayNameC())};
...
bool Semantic::checkInitDropCount(SemanticContext* context, const AstNode* node, AstNode* expression, AstNode* count)
{
    ...
    SWAG_VERIFY(countTypeInfo->isNativeInteger(), context->report({count, formErr(Err0610, node->token.c_str(), countTypeInfo->getDisplayNameC())}));
...
bool Semantic::resolveInit(SemanticContext* context)
{
    ...
    {
        SWAG_VERIFY(expressionTypeInfo->isPointer(), context->report({node->expression, formErr(Err0614, node->token.c_str(), expressionTypeInfo->getDisplayNameC())}));
    ...
    if (node->parameters)
    {
        ...
        {
            SWAG_VERIFY(node->parameters->childCount() == 1, context->report({node->parameters, formErr(Err0593, pointedType->getDisplayNameC())}));
...
bool Semantic::resolveDropCopyMove(SemanticContext* context)
{
    ...
    if (node->is(AstNodeKind::PostCopy))
    {
        ...
        {
            return context->report({node->expression, formErr(Err0092, pointedType->getDisplayNameC())});
        }
    }
