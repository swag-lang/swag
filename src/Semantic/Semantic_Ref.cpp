...
bool Semantic::checkInitDropCount(SemanticContext* context, const AstNode* node, AstNode* expression, AstNode* count)
{
    ...
    SWAG_VERIFY(countTypeInfo->isNativeInteger(), context->report({count, formErr(Err0563, node->token.cstr(), countTypeInfo->getDisplayNameC(), node->token.cstr())}));
...
bool Semantic::resolveInit(SemanticContext* context)
{
    ...
    if (node->parameters)
    {
        ...
        {
            SWAG_VERIFY(node->parameters->childCount() == 1, context->report({node->parameters, formErr(Err0524, pointedType->getDisplayNameC(), node->parameters->childCount())}));
