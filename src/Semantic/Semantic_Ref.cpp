...
bool Semantic::boundCheck(SemanticContext* context, const TypeInfo* forType, const AstNode* arrayNode, AstNode* arrayAccess, uint64_t maxCount)
{
    ...
    if (maxCount == 0)
    {
        if (forType->isSlice())
        {
            Diagnostic err{arrayAccess, toErr(Err0215)};
        ...
        {
            Diagnostic err{arrayAccess, toErr(Err0215)};
            err.addNote(arrayNode, toNte(Nte0214));
            return context->report(err);
        }
    }
    ...
    if (idx >= maxCount)
        return context->report({arrayAccess, formErr(Err0512, idx, maxCount - 1)});
    return true;
}
...
bool Semantic::checkCanTakeAddress(SemanticContext* context, AstNode* node)
{
        ...
        !overload->typeInfo->isSlice())
    {
        if (node->isNot(AstNodeKind::IdentifierRef) || node->lastChild()->isNot(AstNodeKind::ArrayPointerIndex))
        {
            ...
            {
                Diagnostic err{node, node->token, toErr(Err0142)};
                err.addNote(formNte(Nte0117, node->token.cstr()));
                return context->report(err);
            }

            if (overload->hasFlag(OVERLOAD_VAR_FUNC_PARAM))
            {
                const Diagnostic err{node, node->token, formErr(Err0138, overload->typeInfo->getDisplayNameC())};
                return context->report(err);
            }

            if (overload->hasFlag(OVERLOAD_CONSTANT))
            {
                const Diagnostic err{node, node->token, formErr(Err0137, overload->typeInfo->getDisplayNameC())};
                return context->report(err);
            }
        }
    }
    ...
    if (!node->hasAstFlag(AST_L_VALUE))
    {
        ...
        {
            const Diagnostic err{node, formErr(Err0135, Naming::aKindName(symbol->kind).cstr())};
            return context->report(err);
        }

        const Diagnostic err{node, formErr(Err0137, node->typeInfo->getDisplayNameC())};
        return context->report(err);
    }
...
bool Semantic::resolveMakePointerLambda(SemanticContext* context)
{
    ...
    if (node->lambda && node->lambda->captureParameters)
    {
        ...
        SWAG_VERIFY(typeBlock->sizeOf <= maxSize, context->report({node->lambda->captureParameters, formErr(Err0014, typeBlock->sizeOf, maxSize)}));
    }
...
bool Semantic::resolveMakePointer(SemanticContext* context)
{
    ...
    if (!symbol)
    {
        Diagnostic err{node, node->token, toErr(Err0144)};
    ...
    if (overload && overload->hasFlag(OVERLOAD_IS_LET))
    {
        ...
        {
            const Diagnostic err{node, node->token, toErr(Err0142)};
    ...
    if (symbol->is(SymbolKind::Function))
    {
        ...
        if (!typeInfo->isPointerRef())
        {
            if (typeInfo->isVoid())
            {
                const Diagnostic err{node, node->token, formErr(Err0134, symbol->name.cstr())};
                return context->report(err, Diagnostic::hereIs(overload));
            }

            Diagnostic err{node, node->token, formErr(Err0139, typeInfo->getDisplayNameC())};
            err.addNote(formNte(Nte0102, Naming::aKindName(typeInfo).cstr()));
            return context->report(err, Diagnostic::hereIs(overload));
        }
    }
...
bool Semantic::resolveArrayPointerSlicing(SemanticContext* context)
{
...

    {
        PushErrCxtStep ec(context, nullptr, ErrCxtStepKind::Error, [node] {
            return formErr(Err0567, node->lowerBound->typeInfo->getDisplayNameC());
        });
        SWAG_CHECK(TypeManager::makeCompatibles(context, g_TypeMgr->typeInfoU64, nullptr, node->lowerBound, CAST_FLAG_TRY_COERCE));
    }
    {
        PushErrCxtStep ec(context, nullptr, ErrCxtStepKind::Error, [node] {
            return formErr(Err0567, node->upperBound->typeInfo->getDisplayNameC());
        });
        ...
        !node->upperBound->hasSemFlag(SEMFLAG_ASSIGN_COMPUTED))
    {
        ...
        {
            const Diagnostic err{node->upperBound, toErr(Err0516)};
            return context->report(err);
        }
    ...
    if (typeVar->isArray())
    {
        ...
        {
            Diagnostic err{node, node->token, toErr(Err0233)};
    ...
    else if (typeVar->isPointer())
    {
        ...
        {
            Diagnostic err{node, node->token, toErr(Err0223)};
    ...
    else if (typeVar->isStruct())
    {
        ...
        if (!symbol)
        {
            Diagnostic err{node->token.sourceFile, node->token, formErr(Err0232, node->array->token.cstr(), typeInfo->getDisplayNameC())};
            err.hint = formNte(Nte0158, g_LangSpec->name_opSlice.cstr());
    ...
    {
        return context->report({node->array, formErr(Err0234, node->array->typeInfo->getDisplayNameC())});
    }
    ...
    if (node->lowerBound->hasFlagComputedValue() && node->upperBound->hasFlagComputedValue())
    {
        ...
        {
            Diagnostic err{node->lowerBound, formErr(Err0515, node->lowerBound->computedValue()->reg.u64, node->upperBound->computedValue()->reg.u64)};
            err.addNote(node->upperBound, toNte(Nte0212));
            return context->report(err);
        }
    }
    ...
    if (maxBound && node->upperBound->hasFlagComputedValue())
    {
        ...
        {
            const Diagnostic err{node->upperBound, formErr(Err0514, node->upperBound->computedValue()->reg.u64, maxBound)};
            return context->report(err);
        }
    }
...
bool Semantic::resolveMoveRef(SemanticContext* context)
{
    ...
    {
        const Diagnostic err(node, node->token, toErr(Err0340));
        return context->report(err);
    }

    if (!typeInfo->isPointer() && !typeInfo->isPointerRef())
    {
        Diagnostic err(node, node->token, formErr(Err0342, front->typeInfo->getDisplayNameC()));
    ...
    if (typeInfo->isConst())
    {
        Diagnostic err(node, node->token, toErr(Err0341));
        err.addNote(front, Diagnostic::isType(front->typeInfo));
        return context->report(err);
    }
...
bool Semantic::resolveKeepRef(SemanticContext* context)
{
    ...
    {
        Diagnostic err{node, node->token, formErr(Err0227, typeInfo->getDisplayNameC())};
        ...
        {
            err.hint = toNte(Nte0139);
            err.addNote(front, formNte(Nte0051, front->token.cstr()));
            return context->report(err);
        }
...
bool Semantic::resolveArrayPointerRef(SemanticContext* context)
{
    ...
    if (const auto pr2 = arrayNode->getParent(2); pr2->isNot(AstNodeKind::MakePointer))
    {
        if (baseType->isConst())
        {
            Diagnostic err{pr2, pr2->token, formErr(Err0094, baseType->getDisplayNameC())};
    ...
    if (!arrayType->isStruct())
    {
        ...
        {
            const Diagnostic err{arrayNode->access, formErr(Err0601, arrayNode->access->typeInfo->getDisplayNameC())};
            return context->report(err);
        }
    }
    ...
    switch (arrayType->kind)
    {
        case TypeInfoKind::Pointer:
        {
            ...
            {
                const Diagnostic err{arrayNode->array, formErr(Err0219, arrayNode->resolvedSymbolName()->name.cstr(), arrayType->getDisplayNameC())};
                return context->report(err);
            }
            ...
            {
                Diagnostic err{arrayNode->access, toErr(Err0281)};
        ...
        case TypeInfoKind::Native:
        {
            ...
            if (arrayType->nativeType == NativeTypeKind::String)
            {
                ...
                {
                    Diagnostic err{arrayNode->getParent(2), arrayNode->getParent(2)->token, formErr(Err0136, arrayType->getDisplayNameC())};
            ...
            {
                Diagnostic err{arrayNode->array, formErr(Err0170, arrayType->getDisplayNameC())};
                if (arrayNode->hasSpecFlag(AstArrayPointerIndex::SPEC_FLAG_IS_DEREF))
                    err.addNote(arrayNode->token.startLocation, arrayNode->token.endLocation, toNte(Nte0164));
                return context->report(err);
            }
        ...
        case TypeInfoKind::Struct:
            ...
            if (arrayType->isTuple())
            {
                if (arrayNode->hasSpecFlag(AstArrayPointerIndex::SPEC_FLAG_IS_DEREF))
                {
                    Diagnostic err{arrayNode->access, toErr(Err0247)};
                    err.addNote(arrayNode->token.startLocation, arrayNode->token.endLocation, toNte(Nte0164));
                    return context->report(err);
                }
        ...
        default:
        {
            return context->report({arrayNode->array, formErr(Err0170, arrayType->getDisplayNameC())});
        }
    }
...
bool Semantic::resolveArrayPointerDeRef(SemanticContext* context)
{
    ...
    switch (arrayType->kind)
    {
        case TypeInfoKind::Pointer:
        {
            if (!arrayType->isPointerArithmetic() && !arrayNode->hasSpecFlag(AstArrayPointerIndex::SPEC_FLAG_IS_DEREF) && !context->forDebugger)
            {
                Diagnostic err{arrayNode->access, formErr(Err0219, arrayNode->resolvedSymbolName()->name.cstr(), arrayType->getDisplayNameC())};
                err.addNote(arrayNode->array, Diagnostic::isType(arrayType));
                err.addNote(toNte(Nte0104));
                return context->report(err);
            }
            ...
            if (arrayNode->array->hasFlagComputedValue() && arrayNode->access->hasFlagComputedValue())
            {
                ...
                if (!storageSegment)
                {
                    const Diagnostic err{arrayNode, toErr(Err0215)};
                    return context->report(err);
                }
        ...
        case TypeInfoKind::Struct:
        {
            ...
            {
                YIELD();
                Diagnostic err{arrayNode->access, formErr(Err0168, arrayNode->array->token.cstr(), arrayType->getDisplayNameC())};
                err.hint = formNte(Nte0158, g_LangSpec->name_opIndex.cstr());
                err.addNote(arrayNode->array, Diagnostic::isType(arrayType));
                return context->report(err);
            }
        ...
        default:
        {
            Diagnostic err{arrayNode->array, formErr(Err0170, arrayNode->array->typeInfo->getDisplayNameC())};
            if (arrayNode->hasSpecFlag(AstArrayPointerIndex::SPEC_FLAG_IS_DEREF))
                err.addNote(arrayNode->token.startLocation, arrayNode->token.endLocation, toNte(Nte0164));
            return context->report(err);
        }
...
bool Semantic::checkInitDropCount(SemanticContext* context, const AstNode* node, AstNode* expression, AstNode* count)
{
    ...
    SWAG_VERIFY(countTypeInfo->isNativeInteger(), context->report({count, formErr(Err0596, node->token.cstr(), countTypeInfo->getDisplayNameC())}));
    ...
    if (!count->hasFlagComputedValue() || count->computedValue()->reg.u64 > 1)
    {
        if (!expression->typeInfo->isPointerArithmetic())
        {
            ...
            {
                Diagnostic err{expression, formErr(Err0587, node->token.cstr(), expression->typeInfo->getDisplayNameC())};
                err.addNote(count, formNte(Nte0136, count->computedValue()->reg.u64));
                return context->report(err);
            }

            Diagnostic err{expression, formErr(Err0588, node->token.cstr(), expression->typeInfo->getDisplayNameC())};
...
bool Semantic::resolveInit(SemanticContext* context)
{
    ...
    if (!node->count)
    {
        ...
        SWAG_VERIFY(node->expression->is(AstNodeKind::IdentifierRef), context->report({node->expression, formErr(Err0147, node->token.cstr())}));
        SWAG_VERIFY(node->expression->resolvedSymbolOverload(), context->report({node->expression, formErr(Err0147, node->token.cstr())}));
        SWAG_VERIFY(!expressionTypeInfo->isConst(), context->report({node->expression, formErr(Err0614, node->token.cstr(), expressionTypeInfo->getDisplayNameC())}));
    ...
    {
        SWAG_VERIFY(expressionTypeInfo->isPointer(), context->report({node->expression, formErr(Err0590, node->token.cstr(), expressionTypeInfo->getDisplayNameC())}));
        SWAG_VERIFY(!node->expression->typeInfo->isConst(), context->report({node->expression, formErr(Err0613, node->token.cstr(), expressionTypeInfo->getDisplayNameC())}));
    ...
    if (node->parameters)
    {
        ...
        {
            SWAG_VERIFY(node->parameters->childCount() == 1, context->report({node->parameters, formErr(Err0557, pointedType->getDisplayNameC())}));
            const auto child = node->parameters->firstChild();

            {
                PushErrCxtStep ec(context, node->expression, ErrCxtStepKind::Note, [node] { return formNte(Nte0116, node->expression->typeInfo->getDisplayNameC()); });
...
bool Semantic::resolveDropCopyMove(SemanticContext* context)
{
    ...
    if (node->is(AstNodeKind::PostCopy))
    {
        ...
        {
            return context->report({node->expression, formErr(Err0084, pointedType->getDisplayNameC())});
        }
    }
