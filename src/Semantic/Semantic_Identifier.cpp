...
bool Semantic::resolveNameAlias(SemanticContext* context)
{
    ...
    if (back->resolvedSymbolName()->is(SymbolKind::Variable))
    {
        ...
        if (back->is(AstNodeKind::IdentifierRef))
        {
            ...
            for (const auto& c : back->children)
            {
                ...
                {
                    SWAG_VERIFY(cptVar == 0, context->report({back, toErr(Err0344)}));
                    cptVar++;
                }
            }
        }
    ...
    {
        Diagnostic err{back, formErr(Err0362, Naming::aKindName(symbolName->kind).cstr())};
        ...
        {
            err.addNote(node, node->kwdLoc, formNte(Nte0061, Naming::aKindName(symbolName->kind).cstr()));
        }
    ...
    for (const auto& dep : toCheck)
    {
        ...
        if (dependentVar)
        {
            if (dep.node->isGeneratedSelf())
            {
                ...
                err.addNote(dep.node->ownerFct, dep.node->ownerFct->token, toNte(Nte0140));
                ...
                if (parentIdx != parent->parent->childCount() - 1)
                {
                    ...
                    if (brother->is(AstNodeKind::Statement))
                    {
                        ...
                        switch (brotherParent->kind)
                        {
                            ...
                            {
                                const auto       msg = formErr(Err0123, Naming::kindName(overload).cstr(), overload->node->token.cstr(), brotherParent->token.cstr());
                                              ...
                                              AstNode*              ufcsFirstParam)
{
    ...
    if (callParameters || ufcsFirstParam)
    {
            ...
            !TypeManager::concretePtrRefType(symbol->overloads[0]->typeInfo, CONCRETE_FORCE_ALIAS)->isLambdaClosure())
        {
            ...
            {
                const Diagnostic err{identifier, identifier->token, formErr(Err0179, identifier->token.cstr(), symbol->overloads[0]->typeInfo->getDisplayNameC())};
                return context->report(err, Diagnostic::hereIs(symbol->overloads[0]));
            }

            const Diagnostic err{identifier, identifier->token, formErr(Err0180, identifier->token.cstr(), Naming::aKindName(symbol->kind).cstr())};
            return context->report(err, Diagnostic::hereIs(symbol->overloads[0]));
        }
...
bool Semantic::fillMatchContextGenericParameters(SemanticContext* context, SymbolMatchContext& symMatchContext, AstIdentifier* node, const SymbolOverload* overload)
{
    ...
    if (genericParameters)
    {
            ...
            symbolKind != SymbolKind::TypeAlias)
        {
            ...
            Diagnostic err{genericParameters, formErr(Err0667, Naming::aKindName(symbol->kind).cstr())};
            err.addNote(node, node->token, formNte(Nte0129, node->token.cstr(), Naming::aKindName(symbol->kind).cstr()));
...
bool Semantic::resolveIdentifier(SemanticContext* context, AstIdentifier* identifier, ResolveIdFlags riFlags)
{
    ...
    {
        SWAG_VERIFY(identifier == identifierRef->firstChild(), context->report({identifier, toErr(Err0224)}));
