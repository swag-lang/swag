...
bool Semantic::resolveNameAlias(SemanticContext* context)
{
        ...
        symbolName->isNot(SymbolKind::Variable))
    {
        ...
        {
            err.addNote(node, node->kwdLoc, formNte(Nte0061, Naming::aKindName(symbolName->kind).c_str()));
        }
                                              ...
                                              AstNode*              ufcsFirstParam)
{
    ...
    if (callParameters || ufcsFirstParam)
    {
            ...
            !TypeManager::concretePtrRefType(symbol->overloads[0]->typeInfo, CONCRETE_FORCE_ALIAS)->isLambdaClosure())
        {
            ...
            {
                const Diagnostic err{identifier, identifier->token, formErr(Err0169, identifier->token.c_str(), symbol->overloads[0]->typeInfo->getDisplayNameC())};
                return context->report(err, Diagnostic::hereIs(symbol->overloads[0]));
            }

            const Diagnostic err{identifier, identifier->token, formErr(Err0170, identifier->token.c_str(), Naming::aKindName(symbol->kind).c_str())};
            return context->report(err, Diagnostic::hereIs(symbol->overloads[0]));
        }
...
bool Semantic::fillMatchContextGenericParameters(SemanticContext* context, SymbolMatchContext& symMatchContext, AstIdentifier* node, const SymbolOverload* overload)
{
    ...
    if (genericParameters)
    {
            ...
            symbolKind != SymbolKind::TypeAlias)
        {
            ...
            Diagnostic err{genericParameters, formErr(Err0659, Naming::aKindName(symbol->kind).c_str())};
            err.addNote(node, node->token, formNte(Nte0129, node->token.c_str(), Naming::aKindName(symbol->kind).c_str()));
