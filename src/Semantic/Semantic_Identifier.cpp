...
bool Semantic::resolveNameAlias(SemanticContext* context)
{
    ...
    if (back->resolvedSymbolName()->is(SymbolKind::Variable))
    {
        ...
        if (back->is(AstNodeKind::IdentifierRef))
        {
            ...
            for (const auto& c : back->children)
            {
                ...
                {
                    SWAG_VERIFY(cptVar == 0, context->report({back, toErr(Err0157)}));
                    cptVar++;
                }
            }
        }
    ...
    {
        Diagnostic err{back, formErr(Err0326, Naming::aKindName(symbolName->kind).c_str())};
        ...
        {
            err.addNote(node, node->kwdLoc, formNte(Nte0026, Naming::aKindName(symbolName->kind).c_str()));
        }
...
bool Semantic::getUsingVar(SemanticContext* context, AstIdentifierRef* identifierRef, const AstIdentifier* /*node*/, const SymbolOverload* overload, AstNode** result, AstNode** resultLeaf)
{
    ...
    for (const auto& dep : toCheck)
    {
        ...
        if (dependentVar)
        {
            ...
            {
                Diagnostic err{dependentVar, formErr(Err0014, dependentVar->typeInfo->getDisplayNameC())};
                err.addNote(dep.node->ownerFct, dep.node->ownerFct->token, toNte(Nte0123));
                err.addNote(toNte(Nte0036));
                return context->report(err);
            }

            Diagnostic err{dep.node, formErr(Err0014, dependentVar->typeInfo->getDisplayNameC())};
                ...
                if (parentIdx != parent->parent->childCount() - 1)
                {
                    ...
                    if (brother->is(AstNodeKind::Statement))
                    {
                        ...
                        switch (brotherParent->kind)
                        {
                            ...
                            {
                                const auto       msg = formErr(Err0150, Naming::kindName(overload).c_str(), overload->node->token.c_str(), brotherParent->token.c_str());
                                              ...
                                              AstNode*              ufcsFirstParam)
{
    ...
    if (callParameters || ufcsFirstParam)
    {
            ...
            !TypeManager::concretePtrRefType(symbol->overloads[0]->typeInfo, CONCRETE_FORCE_ALIAS)->isLambdaClosure())
        {
            ...
            {
                const Diagnostic err{identifier, identifier->token, formErr(Err0284, identifier->token.c_str(), symbol->overloads[0]->typeInfo->getDisplayNameC())};
                return context->report(err, Diagnostic::hereIs(symbol->overloads[0]));
            }

            const Diagnostic err{identifier, identifier->token, formErr(Err0283, identifier->token.c_str(), Naming::aKindName(symbol->kind).c_str())};
            return context->report(err, Diagnostic::hereIs(symbol->overloads[0]));
        }
    ...
    if (callParameters)
    {
        ...
        for (uint32_t i = 0; i < childCount; i++)
        {
            ...
            if (i != childCount - 1)
            {
                ...
                {
                    Diagnostic err{oneParam, toErr(Err0502)};
...
bool Semantic::fillMatchContextGenericParameters(SemanticContext* context, SymbolMatchContext& symMatchContext, AstIdentifier* node, const SymbolOverload* overload)
{
    ...
    if (genericParameters)
    {
            ...
            symbolKind != SymbolKind::TypeAlias)
        {
            ...
            err.addNote(node, node->token, formNte(Nte0132, node->token.c_str(), Naming::aKindName(symbol->kind).c_str()));
...
bool Semantic::resolveIdentifier(SemanticContext* context, AstIdentifier* identifier, ResolveIdFlags riFlags)
{
    ...
    {
        SWAG_VERIFY(identifier == identifierRef->firstChild(), context->report({identifier, toErr(Err0358)}));
