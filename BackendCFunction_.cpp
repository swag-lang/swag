...
bool BackendC::emitFunctionBody(Concat& concat, Module* moduleToGen, ByteCode* bc)
{
    ...
    for (uint32_t i = 0; i < bc->numInstructions; i++, ip++)
    {
        ...
        {
            if (moduleToGen->buildParameters.buildCfg->backendC.writeSourceCode || g_CommandLine.debug)
            {
        ...
        if (moduleToGen->buildParameters.buildCfg->backendC.writeByteCodeInstruction || g_CommandLine.debug)
        {
            CONCAT_FIXED_STR(concat, "/* ");
