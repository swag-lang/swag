...
void printStats()
{
    ...
    g_Log.messageHeaderCentered("swag version", format("%d.%d.%d", SWAG_BUILD_VERSION, SWAG_BUILD_REVISION, SWAG_BUILD_NUM));
    g_Log.messageHeaderCentered("frontend time", format("%.3fs", g_Stats.frontendTime.count()));
    if (g_CommandLine.backendOutput)
        g_Log.messageHeaderCentered("backend time", format("%.3fs", g_Stats.backendTime.count()));
    g_Log.messageHeaderCentered("total time", format("%.3fs", g_Stats.totalTime.count()));
    g_Log.messageHeaderCentered("workers", format("%d", g_Stats.numWorkers));
    g_Log.messageHeaderCentered("modules", format("%d", g_Stats.numModules.load()));
    g_Log.messageHeaderCentered("files", format("%d", g_Stats.numFiles.load()));
    g_Log.messageHeaderCentered("lines", format("%d", g_Stats.numLines.load()));
    g_Log.messageHeaderCentered("lines/s", format("%d", (int) (g_Stats.numLines.load() / g_Stats.totalTime.count())));
    g_Log.messageHeaderCentered("instructions", format("%d", g_Stats.numInstructions.load()));
    if (g_CommandLine.backendOutput)
        g_Log.messageHeaderCentered("output modules", format("%d", g_Stats.numGenModules.load()));
    if (g_CommandLine.test)
        g_Log.messageHeaderCentered("test functions", format("%d", g_Stats.testFunctions.load()));
    if (g_Workspace.numErrors)
        g_Log.messageHeaderCentered("errors", format("%d", g_Workspace.numErrors.load()), LogColor::Red);
