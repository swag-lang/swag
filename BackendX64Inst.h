...
namespace BackendX64Inst
{
    ...
    {
        pp.concat.addString("\x48\x8B\x87", 3); // mov rax, [rdi + ?]
        pp.concat.addU32(stackOffset);
    }
    ...
    {
        pp.concat.addString("\x48\x8B\x9F", 3); // mov rbx, [rdi + ?]
        pp.concat.addU32(stackOffset);
    }
    ...
    {
        pp.concat.addString("\x48\x8B\x8F", 3); // mov rcx, [rdi + ?]
        pp.concat.addU32(stackOffset);
    }
