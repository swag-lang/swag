...
bool SemanticJob::checkFuncPrototypeOp(SemanticContext* context, AstFuncDecl* node)
{
    ...
    {
        SWAG_VERIFY(parameters && parameters->childs.size() == 2, context->report({node, node->token, format("invalid number of arguments for special function '%s'", name.c_str())}));
        SWAG_VERIFY(returnType && returnType->typeInfo->isSame(g_TypeMgr.typeInfoS32, 0), context->report({returnType, format("invalid return type for special function '%s' ('s32' expected, '%s' provided)", name.c_str(), returnType->typeInfo->name.c_str())}));
    }
    ...
    {
        SWAG_VERIFY(parameters && parameters->childs.size() == 1, context->report({node, node->token, format("invalid number of arguments for special function '%s'", name.c_str())}));
        SWAG_VERIFY(returnType && returnType->typeInfo->isSame(g_TypeMgr.typeInfoConstPVoid, ISSAME_EXACT), context->report({returnType, format("invalid return type for special function '%s' ('const *void' expected, '%s' provided)", name.c_str(), returnType->typeInfo->name.c_str())}));
    }
    ...
    else if (name == "opAffect")
    {
        SWAG_VERIFY(parameters && parameters->childs.size() == 2, context->report({node, node->token, format("invalid number of arguments for special function '%s'", name.c_str())}));
        SWAG_VERIFY(returnType->typeInfo == g_TypeMgr.typeInfoVoid, context->report({returnType, format("invalid return type for special function '%s' ('void' expected, '%s' provided)", name.c_str(), returnType->typeInfo->name.c_str())}));
        SWAG_VERIFY(!parameters->childs[1]->typeInfo->isSame(typeStruct, ISSAME_EXACT), context->report({parameters->childs[1], format("invalid parameter '2' for special function '%s' (cannot be of type '%s')", name.c_str(), typeStruct->name.c_str())}));
    }
    else if (name == "opIndex")
    {
        ...
        for (int i = 1; i < parameters->childs.size(); i++)
            SWAG_VERIFY(parameters->childs[i]->typeInfo->isSame(g_TypeMgr.typeInfoU32, 0), context->report({parameters->childs[i], format("invalid parameter '%d' for special function '%s' ('u32' expected, '%s' provided)", i + 1, name.c_str(), parameters->childs[i]->typeInfo->name.c_str())}));
    }
