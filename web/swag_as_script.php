...
<div class="container">
<p><h1 id="Script file">Script file</h1></p>
<p>Instead of a workspace, Swag can also be used to build and run a simple script file, thanks to the fact that the compiler can run anything at compile time.No executable will be generated, the compiler will do all the job.To create a new script file with the special extension 'swgs':</br>
</p>
<pre><code><span style="color:#0">$ swag new -f:myScript
=> script file 'myScript.swgs' has been created
=> type 'swag script -f:myScript.swgs' to run that script</span></code></pre><p></br>
</p>
...
You can then run your script with the 'script' command.</br>
</p>
<pre><code><span style="color:#0">$ swag script -f:myScript
</span><span style="color:#3BC3A7">Hello</span><span style="color:#0"> world !</span></code></pre><p></br>
You can also just specify the script file <b>with the extension</b> as a command.</br>
</p>
<pre><code><span style="color:#0">$ swag myScript.swgs
</span><span style="color:#3BC3A7">Hello</span><span style="color:#0"> world !</span></code></pre><p></br>
You will find a bunch of small scripts in <code>swag/bin/examples/scripts</code>.To run one of them from the console, go to the folder and type for example <code>swag pendulum.swgs</code></br>
<h2 id="Dependencies">Dependencies</h2></p>
<p></br>
You can add external dependencies, and they will be compiled and used as native code.</br>
</p>
...
A special hidden workspace (in the Swag cache folder) will be created to contain all the corresponding native code.</br>
</p>
<ul><li>To locate the Swag cache folder, add <code>--verbose-path</code> to the command line.</li>
...
<p></br>
If your script is divided in more than one single file, you can add <code>#load <filename></code> in the <code>#dependencies</code> block.</br>
</p>
<pre><code><span style="color:#FF6A00">#dependencies</span><span style="color:#0">
{
...
<p></br>
The compiler comes with a <b>bytecode debugger</b> that can be used to trace and debug compile time execution.Add <code>@breakpoint()</code> in your code when you want the debugger to trigger.</br>
