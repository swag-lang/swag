...
<div>This <code>swag-lang/swag/bin/reference/language</code> module provides a brief explanation of the language basic syntax and usage without the need of the <a href="libraries.php">Swag standard modules</a> (<code>Std</code>).</br>
</br>
...
As <code>reference/language</code> is written as a test module, you can run it with :</br>
</div>
<pre><code><span style="color:#0">swag test --workspace:c:/swag-lang/swag/bin/reference
swag test -w:c:/swag-lang/swag/bin/reference</span></code></pre><div>This will run all the test modules of the specified workspace (including this one). You can also omit the <code>--workspace</code> parameter (or <code>-w</code> in its short form) if you run Swag directly from the workspace folder.</br>
</br>
Note that if you want to compile and run a single module in the workspace, you can specify it with the <code>--module</code> (<code>-m</code>) parameter.</br>
</div>
<pre><code><span style="color:#0">swag test -w:c:/swag-lang/swag/bin/reference -m:test_language</span></code></pre><div></div>
...
<code>#main</code> is the <b>program entry point</b>, a special compiler function (that's why the name starts with <code>#</code>). It must be defined only once for a native executable.</br>
<code>@print</code> is an <b>intrinsic</b>, a special built-in function (that's why the name starts with <code>@</code>). It is part of the <a href="std/swag.runtime.html">compiler runtime</a> which comes with the compiler.</br>
</div>
...
<div>Next, a version that this time uses the <code>Console.print</code> function in the <a href="std/std.core.html">Std.Core</a> module.</br>
The <code>Std.Core</code> module would have to be imported in order to be used, but let's keep it simple.</br>
</div>
<pre><code><span style="color:#FF6A00">#main</span><span style="color:#0">
{
...
<div>A <code>#run</code> block is executed at <b>compile time</b>, and can make Swag behaves like a kind of a <b>scripting language</b>.</br>
...
</span><span style="color:#FF6A00">#main</span><span style="color:#0">
{
    ...
    </span><span style="color:#6A9955">// nestedFunc() can be called at compile time because it is marked with the 'Swag.ConstExpr' attribute.</span><span style="color:#0">
...
</span><span style="color:#FF6A00">#main</span><span style="color:#0">
{
    ...
    {
        sb := </span><span style="color:#3BC3A7">StrConv</span><span style="color:#0">.</span><span style="color:#3BC3A7">StringBuilder</span><span style="color:#0">{}
...
{
    </span><span style="color:#6A9955">// #run will force the call of mySillyFunction() at compile time even if it's not marked with #[Swag.ConstExpr]</span><span style="color:#0">
...
<div>All source files in Swag have the <code>.swg</code> extension, except if you write a simple script with the <code>.swgs</code> extension.</br>
They must be encoded in <b>UTF8</b>.</br>
</br>
In Swag you cannot compile a single file (with the exception of <code>.swgs</code> script files). The source code is organized in a <b>workspace</b> which contains one or multiple <b>modules</b>.</br>
</br>
For example, <code>Std</code> is a workspace that contains all the Swag standard modules.</br>
</br>
A module is a <code>dll</code> (under windows) or an executable, and a workspace can include many of them.</br>
...
</span><span style="color:#3186CD">const</span><span style="color:#0"> </span><span style="color:#3BC3A7">B</span><span style="color:#0"> = </span><span style="color:#6A9955">/* false */</span><span style="color:#0"> </span><span style="color:#3186CD">true</span></code></pre><div>Nested comments are supported.</br>
</div>
...
</span><span style="color:#3186CD">const</span><span style="color:#0"> this_1_is_2_also__3_valid = </span><span style="color:#74A35B">0</span></code></pre><div>But your identifiers cannot start with two underscores. This is reserved by the compiler.</br>
</div>
<pre><code><span style="color:#6A9955">// const __this_is_invalid = 0</span></code></pre><div>Note that some identifiers may start with <code>#</code>. This indicates a <b>compiler special keyword</b>.</br>
</div>
<pre><code><span style="color:#7F7F7F">#assert</span><span style="color:#0">
</span><span style="color:#FF6A00">#run</span><span style="color:#0">
</span><span style="color:#FF6A00">#main</span></code></pre><div></br>
Some identifiers can also start with <code>@</code>. This indicates an <b>intrinsic</b> function.</br>
</div>
...
</span><span style="color:#B4B44A">@print</span><span style="color:#0">()</span></code></pre><div></div>
...
</span><span style="color:#ED9A11">cvarargs</span></code></pre><div><h3 id="Language keywords">Language keywords</br>
</h3></div>
...
</span><span style="color:#3186CD">dref</span></code></pre><div><h3 id="Compiler keywords">Compiler keywords</br>
</h3></div>
...
</span><span style="color:#FF6A00">#run</span></code></pre><div><h3 id="Intrinsics libc">Intrinsics libc</br>
</h3></div>
...
</span><span style="color:#F2470C">@trunc</span></code></pre><div><h3 id="Other intrinsics">Other intrinsics</br>
</h3></div>
...
</span><span style="color:#B4B44A">@typeof</span></code></pre><div><h3 id="Modifiers">Modifiers</br>
</h3></div>
<pre><code><span style="color:#0">,over
,nodrop
,bit
,move
,moveraw</span></code></pre><div></div>

<h2 id="006_semicolon">Semicolon</h2>
<div>In Swag, there's no need to end a statement with <code>;</code> like in C/C++. Most of the time a <code>end of line</code> is enough.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(y == x)     </span><span style="color:#6A9955">// Verify that y is equal to x.</span></code></pre><div>The semicolons are not mandatory, but it's possible to use them if you want.</br>
</div>
...
x, y += </span><span style="color:#74A35B">1</span><span style="color:#0">;</span></code></pre><div>Semicolons can be usefull if you want to do multiple things on the same line.</br>
</div>
<pre><code><span style="color:#3186CD">var</span><span style="color:#0"> x = </span><span style="color:#74A35B">0</span><span style="color:#0">; </span><span style="color:#3186CD">var</span><span style="color:#0"> y = </span><span style="color:#74A35B">0</span><span style="color:#0">
...
<pre><code><span style="color:#6A9955">// Here we declare a constant 'A' and initialize it with 'B', which is not yet known (neither its value or its type).</span><span style="color:#0">
...
</span><span style="color:#3186CD">const</span><span style="color:#0"> </span><span style="color:#3BC3A7">C</span><span style="color:#0">: </span><span style="color:#ED9A11">u64</span><span style="color:#0"> = </span><span style="color:#74A35B">1</span></code></pre><div>In this test, we call the function <code>functionDeclaredLater</code> before it is known. This is fine.</br>
</div>
...
{
}</span></code></pre><div>Note that the order is not relevant in the same file, but it is also irrelevant across multiple files. You can for example call a function in one file and declare it in another one. Ordrer does not matter !</br>
</div>

<h2 id="010_basic_types">Basic types</h2>
<div>
</br>
 These are all signed integers types <code>s8</code>, <code>s16</code>, <code>s32</code> and <code>s64</code>.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> b: </span><span style="color:#ED9A11">s16</span><span style="color:#0"> = -</span><span style="color:#74A35B">2</span><span style="color:#0">     </span><span style="color:#6A9955">// 16 bits signed integer</span><span style="color:#0">
    </span><span style="color:#3186CD">var</span><span style="color:#0"> c: </span><span style="color:#ED9A11">s32</span><span style="color:#0"> = -</span><span style="color:#74A35B">3</span><span style="color:#0">     </span><span style="color:#6A9955">// 32 bits signed integer</span><span style="color:#0">
    </span><span style="color:#3186CD">var</span><span style="color:#0"> d: </span><span style="color:#ED9A11">s64</span><span style="color:#0"> = -</span><span style="color:#74A35B">4</span><span style="color:#0">     </span><span style="color:#6A9955">// 64 bits signed integer</span><span style="color:#0">

    </span><span style="color:#6A9955">// Remember that the instrinsic '@assert' will raise an error at runtime if the enclosed expression is false.</span><span style="color:#0">
 ...
 These are all unsigned integers types <code>u8</code>, <code>u16</code>, <code>u32</code> and <code>u64</code>.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> b: </span><span style="color:#ED9A11">u16</span><span style="color:#0"> = </span><span style="color:#74A35B">2</span><span style="color:#0">      </span><span style="color:#6A9955">// 16 bits unsigned integer</span><span style="color:#0">
    </span><span style="color:#3186CD">var</span><span style="color:#0"> c: </span><span style="color:#ED9A11">u32</span><span style="color:#0"> = </span><span style="color:#74A35B">3</span><span style="color:#0">      </span><span style="color:#6A9955">// 32 bits unsigned integer</span><span style="color:#0">
    </span><span style="color:#3186CD">var</span><span style="color:#0"> d: </span><span style="color:#ED9A11">u64</span><span style="color:#0"> = </span><span style="color:#74A35B">4</span><span style="color:#0">      </span><span style="color:#6A9955">// 64 bits unsigned integer</span><span style="color:#0">
 ...
 These are all floating point types <code>f32</code> and <code>f64</code>.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> b: </span><span style="color:#ED9A11">f64</span><span style="color:#0"> = </span><span style="color:#74A35B">3.14159</span><span style="color:#0">    </span><span style="color:#6A9955">// 64 bits floating point value</span><span style="color:#0">
 ...
 The boolean type <code>bool</code>.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> b: </span><span style="color:#ED9A11">bool</span><span style="color:#0"> = </span><span style="color:#3186CD">false</span><span style="color:#0">     </span><span style="color:#6A9955">// Stored in 1 byte</span><span style="color:#0">
 ...
 The <code>string</code> type. Strings are <b>UTF8</b>, and are stored as two 64 bits (the pointer to the value and the string length in bytes). Note that a string literal ends with a null character like in C.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 The <code>rune</code> type uses the string syntax, postfix with the type <code>rune</code>. It's a 32 bits unicode code point.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div><h3 id="Type reflection">Type reflection</br>
</h3>Swag has <b>type reflection</b> at <b>compile time</b> and at <b>runtime</b>. We will see that later in more details.</br>
</div>
<div>
</br>
 You can use <code>@decltype</code> to create a type based on an expression.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> b: </span><span style="color:#B4B44A">@decltype</span><span style="color:#0">(a) = </span><span style="color:#74A35B">1</span><span style="color:#0">     </span><span style="color:#6A9955">// 'b' will have the same type as 'a'</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    y := </span><span style="color:#ED9A11">bool</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">const</span><span style="color:#0"> b: </span><span style="color:#ED9A11">u32</span><span style="color:#0"> = </span><span style="color:#74A35B">0</span><span style="color:#0">x</span><span style="color:#3BC3A7">FFFF</span><span style="color:#0">           </span><span style="color:#6A9955">// Hexadecimal, with '0x'</span><span style="color:#0">
    </span><span style="color:#3186CD">const</span><span style="color:#0"> c: </span><span style="color:#ED9A11">u32</span><span style="color:#0"> = </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">00001111</span><span style="color:#0">       </span><span style="color:#6A9955">// Binary, with '0b'</span><span style="color:#0">
 ...
 You can separate the digits with the <code>_</code> character.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">const</span><span style="color:#0"> b: </span><span style="color:#ED9A11">u32</span><span style="color:#0"> = </span><span style="color:#74A35B">0</span><span style="color:#0">x</span><span style="color:#3BC3A7">F_F_F_F</span><span style="color:#0">
    </span><span style="color:#3186CD">const</span><span style="color:#0"> c: </span><span style="color:#ED9A11">u32</span><span style="color:#0"> = </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">0000</span><span style="color:#0">_</span><span style="color:#74A35B">1111</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(a == </span><span style="color:#74A35B">123456</span><span style="color:#0">)
 ...
 The default type of an hexadecimal number or a binary number is <code>u32</code> or <code>u64</code> depending on its value.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#6A9955">// The compiler will deduce that the type of 'a' is 'u32'.</span><span style="color:#0">
    </span><span style="color:#3186CD">const</span><span style="color:#0"> a = </span><span style="color:#74A35B">0</span><span style="color:#0">x</span><span style="color:#3BC3A7">FF</span><span style="color:#0">
    ...
    </span><span style="color:#6A9955">// The compiler will deduce that the type of 'b' is 'u64' because the constant is too big for 32 bits.</span><span style="color:#0">
    </span><span style="color:#3186CD">const</span><span style="color:#0"> b = </span><span style="color:#74A35B">0</span><span style="color:#0">x</span><span style="color:#3BC3A7">F_FFFFF_FFFFFF</span><span style="color:#0">
    ...
    </span><span style="color:#3186CD">const</span><span style="color:#0"> c = </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">00000001</span><span style="color:#0">
    ...
    </span><span style="color:#3186CD">const</span><span style="color:#0"> d = </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">00000001</span><span style="color:#0">_</span><span style="color:#74A35B">00000001</span><span style="color:#0">_</span><span style="color:#74A35B">00000001</span><span style="color:#0">_</span><span style="color:#74A35B">00000001</span><span style="color:#0">_</span><span style="color:#74A35B">00000001</span><span style="color:#0">
 ...
 A boolean is <code>true</code> or <code>false</code>. Note again that as constants are known at compile time, we can  use <code>#assert</code> to check the values.</br>
</div>
<pre><code><span style="color:#0">{
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> b = </span><span style="color:#74A35B">0.11</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(b == </span><span style="color:#74A35B">0.11</span><span style="color:#0">)

    </span><span style="color:#3186CD">var</span><span style="color:#0"> c = </span><span style="color:#74A35B">15e2</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(c == </span><span style="color:#74A35B">15e2</span><span style="color:#0">)

    </span><span style="color:#3186CD">var</span><span style="color:#0"> d = </span><span style="color:#74A35B">15e</span><span style="color:#0">+</span><span style="color:#74A35B">2</span><span style="color:#0">
 ...
 By default, a floating point value is <code>f32</code>, not <code>f64</code> (aka <code>double</code>) like in C/C++.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">var</span><span style="color:#0"> a = </span><span style="color:#74A35B">1.5</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(a == </span><span style="color:#74A35B">1.5</span><span style="color:#0">)
...
}</span></code></pre><div><h3 id="Postfix">Postfix</br>
...
</span><span style="color:#6A9955">// Here we have the first usage of the 'short declaration form' with ':='.</span><span style="color:#0">
</span><span style="color:#6A9955">// 'b' is a new variable of type 'u8' initialized with '10'.</span><span style="color:#0">
b := </span><span style="color:#74A35B">10</span><span style="color:#0">'</span><span style="color:#ED9A11">u8</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">const</span><span style="color:#0"> b = </span><span style="color:#BB6643">"this are some cyrillic characters: ӜИ"</span><span style="color:#0">
    </span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> b == </span><span style="color:#BB6643">"this are some cyrillic characters: ӜИ"</span><span style="color:#0">
}</span></code></pre><div>
</br>
...
}</span></code></pre><div>You cannot index a string to get a rune, except in ascii strings. This is because we didn't want the runtime to come with the cost of UTF8 encoding/decoding. But note that the <code>Std.Core</code> module will have all you need to manipulate UTF8 strings.</br>
</div>
<div>
</br>
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#6A9955">// Here, the 'X' character in the middle does not have the index '1', because the chinese character before is</span><span style="color:#0">
    </span><span style="color:#6A9955">// encoded in UTF8 with more than 1 byte.</span><span style="color:#0">
...
}</span></code></pre><div>Multiple adjacent strings are compiled as one.</br>
</div>
...
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> a == </span><span style="color:#BB6643">"this is a string"</span></code></pre><div>You can concatenate some values if the values are known at compile time. Use the <code>++</code> operator for that.</br>
</div>
...
</span><span style="color:#3186CD">var</span><span style="color:#0"> c = </span><span style="color:#BB6643">"the devil number is not "</span><span style="color:#0"> ++ (b + </span><span style="color:#74A35B">1</span><span style="color:#0">) ++ </span><span style="color:#BB6643">"!"</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(d == </span><span style="color:#BB6643">"they are 4 apples in 4 baskets"</span><span style="color:#0">)</span></code></pre><div>A string can be <code>null</code> if not defined.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(a == </span><span style="color:#3186CD">null</span><span style="color:#0">)</span></code></pre><div><h3 id="Escape sequence">Escape sequence</br>
</h3>A string and a rune can contain some <i>escape sequences</i> to specify special characters.</br>
</div>
<div>
</br>
 An escape sequence starts with <code>\</code>.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div><h3 id="Raw string">Raw string</br>
</h3>A <i>raw string</i> does not transform the escape sequences inside it.</br>
</div>
<div>
</br>
 A raw string starts and ends with the character <code>@</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 A raw string can be declared on several lines because <code>eol</code> is now part of the string.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#6A9955">// Every blanks before the ending mark `"@` will be removed from every other lines, so the string before is equivalent to :</span><span style="color:#0">
    </span><span style="color:#6A9955">//</span><span style="color:#0">
...
}</span></code></pre><div><h3 id="Multiline string">Multiline string</br>
</h3>A multiline string starts and ends with <code>"""</code>. Unlike raw strings, they still evaluate escape sequences.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 In a multiline or a raw string, if you end a line with <code>\</code>, the following <eol> will <b>not</b> be part of the string.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div><h3 id="Character">Character</br>
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> b = </span><span style="color:#BB6643">"1"</span><span style="color:#0">'</span><span style="color:#ED9A11">u16</span><span style="color:#0">
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> c = </span><span style="color:#BB6643">"2"</span><span style="color:#0">'</span><span style="color:#ED9A11">u32</span><span style="color:#0">
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> d = </span><span style="color:#BB6643">"3"</span><span style="color:#0">'</span><span style="color:#ED9A11">u64</span><span style="color:#0">
...
}</span></code></pre><div><h3 id="@stringof and @nameof">@stringof and @nameof</br>
</h3>You can use the instrinsic <code>@stringof</code> to return at compile time the result of a constant expression as a string.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 You can also use <code>@nameof</code> to return the name of a variable, function etc.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">const</span><span style="color:#0"> </span><span style="color:#3BC3A7">X</span><span style="color:#0"> = </span><span style="color:#74A35B">1</span><span style="color:#0">
...
<div>To declare a variable, use the <code>var</code> keyword, followed by <code>:</code> and then the type.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>If you don't assign a value, then the variable will be initialized with its default value (0). So a variable is <b>always</b> initialized.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(c == </span><span style="color:#74A35B">0</span><span style="color:#0">)</span></code></pre><div>But if you really do not want the variable to be initialized, you can assign it with <code>undefined</code>. To use with care, of course, but this is sometimes necessary to avoid the initialization cost.</br>
</div>
...
</span><span style="color:#3186CD">var</span><span style="color:#0"> b: </span><span style="color:#ED9A11">string</span><span style="color:#0"> = </span><span style="color:#3186CD">undefined</span></code></pre><div>We have seen that the type is optional in the declaration if it can be deduced from the assignment.</br>
</div>
<div>
</br>
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> b = </span><span style="color:#BB6643">"string"</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(b == </span><span style="color:#BB6643">"string"</span><span style="color:#0">)
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> c = </span><span style="color:#74A35B">1.5</span><span style="color:#0">'</span><span style="color:#ED9A11">f64</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> c = </span><span style="color:#74A35B">1.5</span><span style="color:#0">, d = </span><span style="color:#BB6643">"string"</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(c == </span><span style="color:#74A35B">1.5</span><span style="color:#0">)
...
}</span></code></pre><div>And in fact even <code>var</code> is optional if you use the short form with <code>:=</code>.</br>
</div>
<pre><code><span style="color:#0">a := </span><span style="color:#74A35B">1.5</span><span style="color:#0">
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(a == </span><span style="color:#74A35B">1.5</span><span style="color:#0">)
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(a) == </span><span style="color:#ED9A11">f32</span><span style="color:#0">

b := </span><span style="color:#BB6643">"string"</span><span style="color:#0">
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(b == </span><span style="color:#BB6643">"string"</span><span style="color:#0">)
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(b) == </span><span style="color:#ED9A11">string</span><span style="color:#0">

c := </span><span style="color:#74A35B">1.5</span><span style="color:#0">'</span><span style="color:#ED9A11">f64</span><span style="color:#0">
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(c == </span><span style="color:#74A35B">1.5</span><span style="color:#0">)
...
<div>If you use <code>const</code> instead of <code>var</code>, the value must be known by the compiler. There's no memory footprint if the type is a value or a string.</br>
</div>
<pre><code><span style="color:#6A9955">// This are constants and not variables. So they cannot be changed after the declaration.</span><span style="color:#0">
...
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> b == </span><span style="color:#BB6643">"string"</span></code></pre><div>Constants can have more than just simple types. In that case, there's a memory footprint, because those constants are stored in the data segment. But that means also you could take the address of such constants at runtime.</br>
</div>
<div>
</br>
 This is our first static array. It contains '3' elements, and the type of the elements is <code>s32</code>.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    ptr := &a[</span><span style="color:#74A35B">0</span><span style="color:#0">] </span><span style="color:#6A9955">// Take the address of the first element</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    x = x / </span><span style="color:#74A35B">2</span><span style="color:#0">
    x = x % </span><span style="color:#74A35B">2</span><span style="color:#0"> </span><span style="color:#6A9955">// Modulo</span><span style="color:#0">
    x = x ^ </span><span style="color:#74A35B">2</span><span style="color:#0"> </span><span style="color:#6A9955">// Xor</span><span style="color:#0">
    x = x & </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">0000</span><span style="color:#0">_</span><span style="color:#74A35B">0001</span><span style="color:#0">'</span><span style="color:#ED9A11">s32</span><span style="color:#0"> </span><span style="color:#6A9955">// Bitmask and</span><span style="color:#0">
    x = x | </span><span style="color:#3186CD">cast</span><span style="color:#0">(</span><span style="color:#ED9A11">s32</span><span style="color:#0">) </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">0000</span><span style="color:#0">_</span><span style="color:#74A35B">0001</span><span style="color:#0"> </span><span style="color:#6A9955">// Bitmask or</span><span style="color:#0">
    x = x << </span><span style="color:#74A35B">1</span><span style="color:#0"> </span><span style="color:#6A9955">// Shift bits left</span><span style="color:#0">
    x = x >> </span><span style="color:#74A35B">1</span><span style="color:#0"> </span><span style="color:#6A9955">// Shift bits right</span><span style="color:#0">
}</span></code></pre><div>
</br>
...
<pre><code><span style="color:#0">{
    ...
    x %= </span><span style="color:#74A35B">2</span><span style="color:#0">
    x ^= </span><span style="color:#74A35B">2</span><span style="color:#0">
    x |= </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">0000</span><span style="color:#0">_</span><span style="color:#74A35B">0001</span><span style="color:#0">'</span><span style="color:#ED9A11">s32</span><span style="color:#0">
    x &= </span><span style="color:#3186CD">cast</span><span style="color:#0">(</span><span style="color:#ED9A11">s32</span><span style="color:#0">) </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">0000</span><span style="color:#0">_</span><span style="color:#74A35B">0001</span><span style="color:#0">
    x <<= </span><span style="color:#74A35B">1</span><span style="color:#0">
    x >>= </span><span style="color:#74A35B">1</span><span style="color:#0">
}</span></code></pre><div>
</br>
...
<pre><code><span style="color:#0">{
    ...
    x = !x      </span><span style="color:#6A9955">// Invert boolean</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(x == </span><span style="color:#3186CD">false</span><span style="color:#0">)

    y := </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">0000</span><span style="color:#0">_</span><span style="color:#74A35B">0001</span><span style="color:#0">'</span><span style="color:#ED9A11">u8</span><span style="color:#0">
    y = ~y      </span><span style="color:#6A9955">// Invert bits</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(y == </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">1111</span><span style="color:#0">_</span><span style="color:#74A35B">1110</span><span style="color:#0">)

    z := </span><span style="color:#74A35B">1</span><span style="color:#0">
    z = -z      </span><span style="color:#6A9955">// Negative</span><span style="color:#0">
...
{
    x := </span><span style="color:#74A35B">5</span><span style="color:#0">
    y := </span><span style="color:#74A35B">10</span><span style="color:#0">
...
}</span></code></pre><div><h3 id="Logical operators">Logical operators</br>
</h3>This is <b>not</b> <code>&&</code> and <code>||</code> like in C/C++, but <code>and</code> and <code>or</code>.</br>
</div>
...
a = (</span><span style="color:#74A35B">1</span><span style="color:#0"> > </span><span style="color:#74A35B">10</span><span style="color:#0">) </span><span style="color:#B040BE">or</span><span style="color:#0"> (</span><span style="color:#74A35B">10</span><span style="color:#0"> < </span><span style="color:#74A35B">1</span><span style="color:#0">)</span></code></pre><div><h3 id="Ternary operator">Ternary operator</br>
</h3>The ternary operator will test an expression, and will return a value depending on the result of the test.</br>
<code>A = Expression ? B : C</code> will return <code>B</code> if the expression is true, and will return <code>C</code> if the expression is false.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(y == </span><span style="color:#74A35B">666</span><span style="color:#0">)</span></code></pre><div><h3 id="Spaceshift operator">Spaceshift operator</br>
</h3>Operator <code><=></code> will return -1, 0 or 1 if the expression on the left is lower, equal or greater than the expression on the right. The returned type is <code>s32</code>.</br>
</div>
...
</span><span style="color:#3BC3A7">A</span><span style="color:#0"> <=> </span><span style="color:#3BC3A7">B</span><span style="color:#0"> == </span><span style="color:#74A35B">1</span><span style="color:#0">  </span><span style="color:#B040BE">if</span><span style="color:#0"> </span><span style="color:#3BC3A7">A</span><span style="color:#0"> > </span><span style="color:#3BC3A7">B</span></code></pre><div></div>
<pre><code><span style="color:#0">{
...
{
    x1 := </span><span style="color:#74A35B">10</span><span style="color:#0"> <=> </span><span style="color:#74A35B">20</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(x1 == -</span><span style="color:#74A35B">1</span><span style="color:#0">)
    x2 := </span><span style="color:#74A35B">20</span><span style="color:#0"> <=> </span><span style="color:#74A35B">10</span><span style="color:#0">
...
}</span></code></pre><div><h3 id="Null-coalescing operator">Null-coalescing operator</br>
</h3>The operator <code>orelse</code> will return the left expression if it is not zero, otherwise it will return the right expression.</br>
</div>
<div>
</br>
 Works with strings, pointers and structures with the <code>opData</code> special function (we'll see that later).</br>
</div>
<pre><code><span style="color:#0">{
    a := </span><span style="color:#BB6643">"string1"</span><span style="color:#0">
    b := </span><span style="color:#BB6643">"string2"</span><span style="color:#0">
...
}</span></code></pre><div><h3 id="Type promotion">Type promotion</br>
...
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(</span><span style="color:#74A35B">0</span><span style="color:#0">'</span><span style="color:#ED9A11">s8</span><span style="color:#0"> + </span><span style="color:#74A35B">1</span><span style="color:#0">'</span><span style="color:#ED9A11">u16</span><span style="color:#0">) == </span><span style="color:#ED9A11">u16</span><span style="color:#0">   </span><span style="color:#6A9955">// Priority to bigger type also</span><span style="color:#0">

</span><span style="color:#6A9955">// This means that a 8/16 bits operation (like an addition) can more easily overflow if you do not take care.</span><span style="color:#0">
</span><span style="color:#6A9955">// In that case, you can use the ',up' modifier on the operation, which will promote the type to at least 32 bits. The operation will be done accordingly.</span><span style="color:#0">
...
<div>
 ...
 Use <code>cast(type)</code> to cast from one type to another.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 Use <code>acast</code> to automatically cast to the expression on the left.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>Use <code>cast,bit</code> to convert a native type to another without converting the value. Works only if the two types are of the same size.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(y == </span><span style="color:#74A35B">0</span><span style="color:#0">x</span><span style="color:#74A35B">3</span><span style="color:#0">f</span><span style="color:#74A35B">800000</span><span style="color:#0">)
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#3186CD">cast</span><span style="color:#0">,</span><span style="color:#FF6A00">bit</span><span style="color:#0">(</span><span style="color:#ED9A11">u32</span><span style="color:#0">) </span><span style="color:#74A35B">1.0</span><span style="color:#0"> == </span><span style="color:#74A35B">0</span><span style="color:#0">x</span><span style="color:#74A35B">3</span><span style="color:#0">f</span><span style="color:#74A35B">800000</span><span style="color:#0">
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#3186CD">cast</span><span style="color:#0">,</span><span style="color:#FF6A00">bit</span><span style="color:#0">(</span><span style="color:#ED9A11">f32</span><span style="color:#0">) </span><span style="color:#74A35B">0</span><span style="color:#0">x</span><span style="color:#74A35B">3</span><span style="color:#0">f</span><span style="color:#74A35B">800000</span><span style="color:#0"> == </span><span style="color:#74A35B">1.0</span></code></pre><div>Swag can sometimes cast from one type to another for you. This is an <i>implicit</i> cast.</br>
</div>
<div>
</br>
...
<div>Remember that dynamic arrays are part of the <code>Std.Core</code> module. Here we are only talking about native static arrays.</br>
</div>
<div>
</br>
...
<pre><code><span style="color:#0">{
 ...
 You can get the number of elements of an array with <code>@countof</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 You can get the address of the array with <code>@dataof</code>.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">var</span><span style="color:#0"> array: [</span><span style="color:#74A35B">2</span><span style="color:#0">] </span><span style="color:#ED9A11">s32</span><span style="color:#0">
    ptr0 := </span><span style="color:#B4B44A">@dataof</span><span style="color:#0">(array)
    ...
    ptr1 := &array[</span><span style="color:#74A35B">0</span><span style="color:#0">]
...
<pre><code><span style="color:#0">{
    ...
    array1 := [</span><span style="color:#BB6643">"10"</span><span style="color:#0">, </span><span style="color:#BB6643">"20"</span><span style="color:#0">, </span><span style="color:#BB6643">"30"</span><span style="color:#0">]
...
<pre><code><span style="color:#0">{
 ...
 But for speed, you can force the array to be not initialized with <code>undefined</code>.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>Of course an array can have multiple dimensions.</br>
</div>
<div>
</br>
...
}</span></code></pre><div>You can initialize a whole array variable (but not a constant) with one single value. Only basic types are accepted (integers, float, string, bool, rune).</br>
</div>
...
<div>A slice is a pointer on a buffer of datas, and a <code>u64</code> to count the number of elements. Unlike a static array, its value can be changed at runtime.</br>
</div>
<div>
</br>
...
<pre><code><span style="color:#0">{
 ...
 At runtime, <code>@dataof</code> will return the address of the values, <code>@countof</code> the number of elements.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">var</span><span style="color:#0"> a: </span><span style="color:#3186CD">const</span><span style="color:#0"> [..] </span><span style="color:#ED9A11">u32</span><span style="color:#0"> = [</span><span style="color:#74A35B">10</span><span style="color:#0">, </span><span style="color:#74A35B">20</span><span style="color:#0">, </span><span style="color:#74A35B">30</span><span style="color:#0">, </span><span style="color:#74A35B">40</span><span style="color:#0">, </span><span style="color:#74A35B">50</span><span style="color:#0">]
    count := </span><span style="color:#B4B44A">@countof</span><span style="color:#0">(a)
    addr  := </span><span style="color:#B4B44A">@dataof</span><span style="color:#0">(a)
 ...
 You can create a slice with your own <code>pointer</code> and <code>count</code> using <code>@mkslice</code>.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> slice: [..] </span><span style="color:#ED9A11">u32</span><span style="color:#0"> = </span><span style="color:#B4B44A">@mkslice</span><span style="color:#0">(&array[</span><span style="color:#74A35B">0</span><span style="color:#0">] + </span><span style="color:#74A35B">2</span><span style="color:#0">, </span><span style="color:#74A35B">2</span><span style="color:#0">)
 ...
 For a string, the slice must be <code>const</code> because a string is immutable.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div><h3 id="The slicing operator">The slicing operator</br>
</h3></div>
<div>
</br>
 Instead of <code>@mkslice</code>, you can slice something with the <code>..</code> operator. For example you can slice a string.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">var</span><span style="color:#0"> str = </span><span style="color:#BB6643">"string"</span><span style="color:#0">
 ...
 The upper limit is <b>included</b> by default. If you want to exclude it, use <code>..<</code> insteand of <code>..</code>.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">var</span><span style="color:#0"> str = </span><span style="color:#BB6643">"string"</span><span style="color:#0">
    </span><span style="color:#3186CD">var</span><span style="color:#0"> slice = str[</span><span style="color:#74A35B">1.</span><span style="color:#0">.<</span><span style="color:#74A35B">3</span><span style="color:#0">]
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> slice = str[..</span><span style="color:#74A35B">2</span><span style="color:#0">]        </span><span style="color:#6A9955">// Index 2 is included</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    slice := arr[</span><span style="color:#74A35B">2.</span><span style="color:#0">.</span><span style="color:#74A35B">3</span><span style="color:#0">]
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(slice[</span><span style="color:#74A35B">0</span><span style="color:#0">] == </span><span style="color:#74A35B">30</span><span style="color:#0">)
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(slice[</span><span style="color:#74A35B">1</span><span style="color:#0">] == </span><span style="color:#74A35B">40</span><span style="color:#0">)
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#B4B44A">@countof</span><span style="color:#0">(slice) == </span><span style="color:#74A35B">2</span><span style="color:#0">)
...
<pre><code><span style="color:#0">{
    arr := [</span><span style="color:#74A35B">10</span><span style="color:#0">, </span><span style="color:#74A35B">20</span><span style="color:#0">, </span><span style="color:#74A35B">30</span><span style="color:#0">, </span><span style="color:#74A35B">40</span><span style="color:#0">]
    slice1 := arr[</span><span style="color:#74A35B">1.</span><span style="color:#0">.</span><span style="color:#74A35B">3</span><span style="color:#0">]
    ...
    slice2 := slice1[</span><span style="color:#74A35B">1.</span><span style="color:#0">.</span><span style="color:#74A35B">2</span><span style="color:#0">]
...
<pre><code><span style="color:#0">{
    arr := [</span><span style="color:#74A35B">10</span><span style="color:#0">, </span><span style="color:#74A35B">20</span><span style="color:#0">, </span><span style="color:#74A35B">30</span><span style="color:#0">, </span><span style="color:#74A35B">40</span><span style="color:#0">]
    ptr := &arr[</span><span style="color:#74A35B">2</span><span style="color:#0">]
    slice := ptr[</span><span style="color:#74A35B">0.</span><span style="color:#0">.</span><span style="color:#74A35B">1</span><span style="color:#0">]
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(slice[</span><span style="color:#74A35B">0</span><span style="color:#0">] == </span><span style="color:#74A35B">30</span><span style="color:#0">)
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(slice[</span><span style="color:#74A35B">1</span><span style="color:#0">] == </span><span style="color:#74A35B">40</span><span style="color:#0">)
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#B4B44A">@countof</span><span style="color:#0">(slice) == </span><span style="color:#74A35B">2</span><span style="color:#0">)
}</span></code></pre>
...
<div>
 ...
 A pointer to a <b>single element</b> is declared with <code>*</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 A pointer can be <code>null</code> (i know some of you will collapse here).</br>
</div>
<pre><code><span style="color:#0">{
 ...
 You can take the address of something with <code>&</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 You can get the pointed value with <code>dref</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 Pointers can be <code>const</code>.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div><h3 id="Multiple values pointers">Multiple values pointers</br>
</h3>If you want to enable <b>pointer arithmetic</b>, and make a pointer to <b>multiple values</b>, declare your pointer with <code>^</code> instead of <code>*</code>.</br>
</div>
<div>
</br>
 <code>ptr</code> is a pointer to a memory block of <code>u8</code>.</br>
</div>
<pre><code><span style="color:#0">{
...
<pre><code><span style="color:#0">{
    ...
    ptr := &x[</span><span style="color:#74A35B">1</span><span style="color:#0">]
...
<pre><code><span style="color:#0">{
    ...
    ptr := &arr[</span><span style="color:#74A35B">0</span><span style="color:#0">]
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(ptr) == ^</span><span style="color:#ED9A11">s32</span><span style="color:#0">)

    </span><span style="color:#6A9955">// The type of 'ptr' is ^s32, so it can be dereferenced by index</span><span style="color:#0">
    value1 := ptr[</span><span style="color:#74A35B">0</span><span style="color:#0">]
    ...
    value2 := ptr[</span><span style="color:#74A35B">1</span><span style="color:#0">]
    ...
    value := </span><span style="color:#3186CD">dref</span><span style="color:#0"> ptr
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(myRef == </span><span style="color:#74A35B">42</span><span style="color:#0">)</span></code></pre><div>When an affectation is done outside of an initialization, you will change the pointed value, and not the reference itself.</br>
</div>
<pre><code><span style="color:#0">x := </span><span style="color:#74A35B">42</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(x == </span><span style="color:#74A35B">66</span><span style="color:#0">)</span></code></pre><div>If you want to change the reference and not the pointed value, you can use <code>ref</code> in the affectation.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(myRef == </span><span style="color:#74A35B">1000</span><span style="color:#0">)</span></code></pre><div>Most of the time, you have to take the address of a variable to make a reference to it. The only exception are function parameters, if the reference is <code>const</code>. In that case, taking the address is not necessary</br>
</div>
...
}</span></code></pre><div>This is usefull for structs for examples, as you can directly pass a literal to a function.</br>
</div>
...
}</span></code></pre><div>Note that declaring a tuple type or a struct type is equivalent to a constant reference.</br>
</div>
...
<div><code>any</code> is a specific type that can store every other types. <code>any</code> is <b>not a variant</b>. It's a dynamic typed reference to an existing value.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 <code>any</code> is in fact a pointer to the value, and a <code>typeinfo</code>. <code>@dataof</code> can be used to retrieve the pointer to the value.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 <code>@typeof</code> will give you the type <code>any</code>, but <code>@kindof</code> will give you the real underlying type. In that case, <code>@kindof</code> is evaluted at runtime.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 You can declare an array with multiple types, with <code>any</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 An <code>any</code> can be set to null, and tested against null.</br>
</div>
<pre><code><span style="color:#0">{
...
<div>
 ...
 A tuple is an anonymous structure, aka a struct literal. Syntax is <code>{}</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 Tuple values have default names to access them, in the form of <code>itemX</code> where <code>X</code> is the field rank.</br>
</div>
<pre><code><span style="color:#0">{
...
<pre><code><span style="color:#0">{
 ...
 A tuple is an expression, where names of fields can be omitted. But the automatic name <code>itemX</code> is always valid.</br>
</div>
<pre><code><span style="color:#0">{
...
<pre><code><span style="color:#0">{
    ...
    y := </span><span style="color:#74A35B">666</span><span style="color:#0">
    t := {x, y}
...
}</span></code></pre><div><h3 id="Tuple unpacking">Tuple unpacking</br>
</h3></div>
<div>
</br>
...
<pre><code><span style="color:#0">{
    ...
    tuple2 := {</span><span style="color:#BB6643">"name"</span><span style="color:#0">, </span><span style="color:#3186CD">true</span><span style="color:#0">}
    (name, value) := tuple2
 ...
 You can ignore a tuple field by naming the variable <code>?</code>.</br>
</div>
<pre><code><span style="color:#0">{
    tuple1 := {x: </span><span style="color:#74A35B">1.0</span><span style="color:#0">, y: </span><span style="color:#74A35B">2.0</span><span style="color:#0">}
    (x, ?) := tuple1
...
<div>
 ...
 Enums values, unlike C/C++, must end with <code>;</code> or an <code>eol</code> (like a normal Swag declaration).</br>
</div>
<pre><code><span style="color:#0">{
 ...
 By default, an enum is of type <code>s32</code>.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    type := </span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(</span><span style="color:#3BC3A7">Values</span><span style="color:#0">)
 ...
 <code>@kindof</code> will return the underlying type of the enum.</br>
</div>
<pre><code><span style="color:#0">{
...
<pre><code><span style="color:#0">{
 ...
 <code>@countof</code> returns the number of values of an enum.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 You can use the keyword <code>using</code> for an enum.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div><h3 id="Enum as flags">Enum as flags</br>
...
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#3BC3A7">MyFlags</span><span style="color:#0">.</span><span style="color:#3BC3A7">A</span><span style="color:#0"> == </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">00000001</span><span style="color:#0">
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#3BC3A7">MyFlags</span><span style="color:#0">.</span><span style="color:#3BC3A7">B</span><span style="color:#0"> == </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">00000010</span><span style="color:#0">
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#3BC3A7">MyFlags</span><span style="color:#0">.</span><span style="color:#3BC3A7">C</span><span style="color:#0"> == </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">00000100</span><span style="color:#0">
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#3BC3A7">MyFlags</span><span style="color:#0">.</span><span style="color:#3BC3A7">D</span><span style="color:#0"> == </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">00001000</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(value == </span><span style="color:#74A35B">0</span><span style="color:#0">b</span><span style="color:#74A35B">00000110</span><span style="color:#0">)
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(value & </span><span style="color:#3BC3A7">MyFlags</span><span style="color:#0">.</span><span style="color:#3BC3A7">C</span><span style="color:#0"> == </span><span style="color:#3BC3A7">MyFlags</span><span style="color:#0">.</span><span style="color:#3BC3A7">C</span><span style="color:#0">)</span></code></pre><div><h3 id="Enum of arrays">Enum of arrays</br>
...
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#3BC3A7">Value</span><span style="color:#0">.</span><span style="color:#3BC3A7">B</span><span style="color:#0">[</span><span style="color:#74A35B">1</span><span style="color:#0">] == </span><span style="color:#74A35B">20</span></code></pre><div><h3 id="Enum of slices">Enum of slices</br>
...
x := </span><span style="color:#3BC3A7">Value</span><span style="color:#0">.</span><span style="color:#3BC3A7">A</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(y[</span><span style="color:#74A35B">1</span><span style="color:#0">] == </span><span style="color:#74A35B">20</span><span style="color:#0">)</span></code></pre><div><h3 id="Enum type inference">Enum type inference</br>
</h3></div>
<div>
</br>
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">var</span><span style="color:#0"> x: </span><span style="color:#3BC3A7">Values</span><span style="color:#0"> = </span><span style="color:#3BC3A7">Values</span><span style="color:#0">.</span><span style="color:#3BC3A7">A</span><span style="color:#0">

    </span><span style="color:#6A9955">// But in fact 'Values' is not necessary because it can be deduced</span><span style="color:#0">
    </span><span style="color:#3186CD">var</span><span style="color:#0"> y: </span><span style="color:#3BC3A7">Values</span><span style="color:#0"> = </span><span style="color:#3BC3A7">A</span><span style="color:#0">

    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(x == y)
}</span></code></pre><div>
</br>
 The enum type is not necessary in a <code>case</code> expression of a <code>switch</code> block (it is deduced from the switch expression).</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">enum</span><span style="color:#0"> </span><span style="color:#3BC3A7">Values</span><span style="color:#0"> { </span><span style="color:#3BC3A7">A</span><span style="color:#0">; </span><span style="color:#3BC3A7">B</span><span style="color:#0">; }
    x := </span><span style="color:#3BC3A7">Values</span><span style="color:#0">.</span><span style="color:#3BC3A7">A</span><span style="color:#0">
 ...
 In an expression, and if the enum name can be deduced, you can omit it and use the <code>.Value</code> syntax.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">enum</span><span style="color:#0"> </span><span style="color:#3BC3A7">Values</span><span style="color:#0"> { </span><span style="color:#3BC3A7">A</span><span style="color:#0">; </span><span style="color:#3BC3A7">B</span><span style="color:#0">; }
...
}</span></code></pre><div>By type reflection, you can loop/visit all values of an enum.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">enum</span><span style="color:#0"> </span><span style="color:#3BC3A7">RGB</span><span style="color:#0"> { </span><span style="color:#3BC3A7">R</span><span style="color:#0">; </span><span style="color:#3BC3A7">G</span><span style="color:#0">; </span><span style="color:#3BC3A7">B</span><span style="color:#0">; }
...
<div><code>impl</code> can be used to declare some stuff in the scope of an enum. The keyword <code>self</code> represents the enum value.</br>
</div>
<pre><code><span style="color:#3186CD">enum</span><span style="color:#0"> </span><span style="color:#3BC3A7">RGB</span><span style="color:#0"> { </span><span style="color:#3BC3A7">R</span><span style="color:#0">; </span><span style="color:#3BC3A7">G</span><span style="color:#0">; </span><span style="color:#3BC3A7">B</span><span style="color:#0">; }</span></code></pre><div>Note the <code>impl enum</code> syntax. We'll see later that <code>impl</code> is also used for structs.</br>
</div>
...
}</span></code></pre><div>You can also specify more than one name. Here <code>C</code> will be a namespace inside <code>B</code> which is itself inside <code>A</code>.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#3BC3A7">A</span><span style="color:#0">.</span><span style="color:#3BC3A7">B</span><span style="color:#0">.</span><span style="color:#3BC3A7">C</span><span style="color:#0">.</span><span style="color:#FF6A00">a</span><span style="color:#0">() == </span><span style="color:#74A35B">2</span><span style="color:#0">)</span></code></pre><div>You can also put <code>using</code> in front of the namespace to be able to access the content without scoping in the <b>current</b> file.</br>
</div>
...
</span><span style="color:#3186CD">const</span><span style="color:#0"> </span><span style="color:#3BC3A7">C</span><span style="color:#0"> = </span><span style="color:#3BC3A7">FileSymbol</span><span style="color:#0"> </span><span style="color:#6A9955">// No need to specify 'Private' because of the 'using'</span></code></pre><div>This is equivalent to <code>#scopefile</code>, but you don't have to specify a name, the compiler will generate it for you.</br>
</div>
...
</span><span style="color:#3186CD">const</span><span style="color:#0"> </span><span style="color:#3BC3A7">D</span><span style="color:#0"> = </span><span style="color:#3BC3A7">OtherSymbol</span></code></pre><div>All symbols from a Swag source file are exported to other files of the same module. So using <code>#scopefile</code> can protect from name conflicts.</br>
</div>

<h2 id="050_if">If</h2>
<div>A basic test with <code>if</code>. Curlies are optional. The expression doesn't need to be enclosed with <code>()</code> unlike C/C++.</br>
</div>
...
}</span></code></pre><div>You can also at the same time declare and test one variable in an <code>if</code> expression. <code>var</code> or <code>const</code> is mandatory in that case.</br>
</div>
...
<div><code>loop</code> are used to iterate a given amount of time.</br>
</div>
<div>
</br>
...
<pre><code><span style="color:#0">{
 ...
 The intrinsic <code>@index</code> returns the current index of the loop (starting at 0).</br>
</div>
<pre><code><span style="color:#0">{
...
<pre><code><span style="color:#0">{
    cpt  := </span><span style="color:#74A35B">0</span><span style="color:#0">
    cpt1 := </span><span style="color:#74A35B">0</span><span style="color:#0">
...
}</span></code></pre><div><code>loop</code> can be used on every types that accept the <code>@countof</code> intrinsic. So you can loop on a slice, an array, a string... and we'll see later, even on a struct.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(cpt == </span><span style="color:#74A35B">10</span><span style="color:#0">+</span><span style="color:#74A35B">20</span><span style="color:#0">+</span><span style="color:#74A35B">30</span><span style="color:#0">+</span><span style="color:#74A35B">40</span><span style="color:#0">)</span></code></pre><div><b>Warning !</b></br>
On a string, it will loop for each byte, <b>not</b> runes (if a rune is encoded in more than one byte). If you want to iterate on all runes, you will have to use the <code>Std.Core</code> module.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(cpt == </span><span style="color:#74A35B">3</span><span style="color:#0">)</span></code></pre><div><code>break</code> and <code>continue</code> can be used inside a loop.</br>
</div>
<div>
</br>
 You can exit a loop with <code>break</code>.</br>
</div>
<pre><code><span style="color:#0">{
    cpt := </span><span style="color:#74A35B">0</span><span style="color:#0">
 ...
 You can force to return to the loop evaluation with <code>continue</code>.</br>
</div>
<pre><code><span style="color:#0">{
    cpt := </span><span style="color:#74A35B">0</span><span style="color:#0">
    </span><span style="color:#B040BE">loop</span><span style="color:#0"> x: </span><span style="color:#74A35B">10</span><span style="color:#0">
...
}</span></code></pre><div>Loop can also be used to iterate on a range of signed values.</br>
</div>
<div>
</br>
 Syntax is <code>lower bound..upper bound</code></br>
</div>
<pre><code><span style="color:#0">{
    count := </span><span style="color:#74A35B">0</span><span style="color:#0">
    sum   := </span><span style="color:#74A35B">0</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
 ...
 You can exclude the last value with the <code>..<</code> syntax.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>A loop without an expression but with a block is infinite. This is equivalent to <code>while true {}</code>.</br>
</div>
...
<pre><code><span style="color:#0">{
    ...
    {
        a := </span><span style="color:#B4B44A">@alias1</span><span style="color:#0">    </span><span style="color:#6A9955">// This is the index</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    result := </span><span style="color:#74A35B">0</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
 ...
 You can visit with a pointer to the value, and not the value itself, by adding <code>&</code> before the value name.</br>
</div>
<pre><code><span style="color:#0">{
...
<div>
 ...
 <code>for</code> accepts a <i>start statement</i>, an <i>expression to test</i>, and an <i>ending statement</i>. This is in fact the same as the C/C++ <code>for</code>.</br>
</div>
<pre><code><span style="color:#0">{
    cpt := </span><span style="color:#74A35B">0</span><span style="color:#0">
    </span><span style="color:#B040BE">for</span><span style="color:#0"> i := </span><span style="color:#74A35B">0</span><span style="color:#0">; i < </span><span style="color:#74A35B">10</span><span style="color:#0">; i += </span><span style="color:#74A35B">1</span><span style="color:#0">
        cpt += </span><span style="color:#74A35B">1</span><span style="color:#0">
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(cpt == </span><span style="color:#74A35B">10</span><span style="color:#0">)

    </span><span style="color:#6A9955">// ';' can be replaced by a newline (like always)</span><span style="color:#0">
    </span><span style="color:#B040BE">for</span><span style="color:#0"> i := </span><span style="color:#74A35B">0</span><span style="color:#0">
    ...
    </span><span style="color:#B040BE">for</span><span style="color:#0"> { i := </span><span style="color:#74A35B">0</span><span style="color:#0">; cpt = </span><span style="color:#74A35B">0</span><span style="color:#0">; }
        i < </span><span style="color:#74A35B">10</span><span style="color:#0">
        i += </span><span style="color:#74A35B">1</span><span style="color:#0">
    {
        cpt += </span><span style="color:#74A35B">1</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    cpt1 := </span><span style="color:#74A35B">0</span><span style="color:#0">'</span><span style="color:#ED9A11">u64</span><span style="color:#0">
    </span><span style="color:#B040BE">for</span><span style="color:#0"> i := </span><span style="color:#74A35B">10</span><span style="color:#0">; i < </span><span style="color:#74A35B">15</span><span style="color:#0">; i += </span><span style="color:#74A35B">1</span><span style="color:#0">
...
<div>
 ...
 <code>while</code> is a loop that runs <b>until the expression is false</b>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 You can also <code>break</code> and <code>continue</code> inside a <code>while</code>.</br>
</div>
<pre><code><span style="color:#0">{
    i := </span><span style="color:#74A35B">0</span><span style="color:#0">
    </span><span style="color:#B040BE">while</span><span style="color:#0"> i < </span><span style="color:#74A35B">10</span><span style="color:#0">
...
<div>
 ...
 <code>switch</code> works like in C/C++, except that no <code>break</code> is necessary (except if the <code>case</code> is empty). That means that there's no automatic <code>fallthrough</code> from one case to another.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    ch := </span><span style="color:#BB6643">"A"</span><span style="color:#0">'</span><span style="color:#ED9A11">rune</span><span style="color:#0">
 ...
 You can put multiple values on the same <code>case</code>.</br>
</div>
<pre><code><span style="color:#0">{
    value := </span><span style="color:#74A35B">6</span><span style="color:#0">
    </span><span style="color:#B040BE">switch</span><span style="color:#0"> value
...
</span><span style="color:#6A9955">// `switch` works with every types that accept the `==` operator.</span></code></pre><div>
</br>
...
<pre><code><span style="color:#0">{
 ...
 If you want to pass from one case to another like in C/C++, use <code>fallthrough</code>.</br>
</div>
<pre><code><span style="color:#0">{
    value := </span><span style="color:#74A35B">6</span><span style="color:#0">
    </span><span style="color:#B040BE">switch</span><span style="color:#0"> value
 ...
 <code>break</code> can be used to exit the current <code>case</code> statement.</br>
</div>
<pre><code><span style="color:#0">{
    value := </span><span style="color:#74A35B">6</span><span style="color:#0">
    </span><span style="color:#B040BE">switch</span><span style="color:#0"> value
 ...
 A <code>case</code> statement cannot be empty. Use <code>break</code> if you want to do nothing.</br>
</div>
<pre><code><span style="color:#0">{
    value := </span><span style="color:#74A35B">6</span><span style="color:#0">
    </span><span style="color:#B040BE">switch</span><span style="color:#0"> value
...
</span><span style="color:#6A9955">// `switch` can be marked with `Swag.Complete` to force all the cases to be covered.</span></code></pre><div>
</br>
...
<pre><code><span style="color:#0">{
    ...
    color := </span><span style="color:#3BC3A7">Color</span><span style="color:#0">.</span><span style="color:#3BC3A7">Red</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    value  := </span><span style="color:#74A35B">6</span><span style="color:#0">
    value1 := </span><span style="color:#BB6643">"true"</span><span style="color:#0">
 ...
 When used on an <code>any</code> variable, switch is done on the underlying variable type.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 A <code>switch</code> can also be used with a (constant) range of values.</br>
</div>
<pre><code><span style="color:#0">{
...
<pre><code><span style="color:#0">{
 ...
 A <code>case</code> expression doesn't need to be constant, except if the switch is marked with <code>Swag.Complete</code>.</br>
</div>
<pre><code><span style="color:#0">{
...
<pre><code><span style="color:#0">{
 ...
 By default, <code>break</code> will exit the parent scope only.</br>
</div>
<pre><code><span style="color:#0">{
    cpt := </span><span style="color:#74A35B">0</span><span style="color:#0">
    </span><span style="color:#B040BE">loop</span><span style="color:#0"> </span><span style="color:#74A35B">10</span><span style="color:#0">
...
<div>
 ...
 This is a <code>struct</code> declaration. <code>var</code> is not necessary for the fields.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>A function can take an argument of type <code>struct</code>. No copy is done (this is equivalent to a const reference in C++).</br>
</div>
...
<div>Like for an enum, <code>impl</code> is used to declare some stuff in the scope of a struct.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#3BC3A7">MyStruct</span><span style="color:#0">.</span><span style="color:#3BC3A7">MyConst</span><span style="color:#0">)</span></code></pre><div>You can have multiple <code>impl</code> blocks. The difference with a namespace is that <code>self</code> and <code>Self</code> are defined inside an impl block. They refere to the corresponding type.</br>
</div>
...
}</span></code></pre><div>If you declare your function with <code>mtd</code> (method) instead of <code>func</code>, then the first parameter is forced to be <code>using self</code>.</br>
If you declare your function with <code>mtdc</code> (method const) instead of <code>func</code>, then the first parameter is forced to be <code>const using self</code>.</br>
Other than that, it's exactly the same. So this is just <b>syntaxic sugar</b> to avoid repeating the <code>using self</code>.</br>
</div>
<pre><code><span style="color:#3186CD">impl</span><span style="color:#0"> </span><span style="color:#3BC3A7">MyStruct</span><span style="color:#0">
{
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(c.</span><span style="color:#FF6A00">returnZ</span><span style="color:#0">() == </span><span style="color:#74A35B">20</span><span style="color:#0">)</span></code></pre><div>All functions in an impl block can be retrieved by reflection, as long as the struct is declared with <code>#[Swag.ExportType("methods")] (by default, methods are not exported).</br>
</div>
...
t := </span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(</span><span style="color:#3BC3A7">MyStruct</span><span style="color:#0">)
...
<div>A struct can have special operations in the <code>impl</code> block. This operations are predefined, and known by the compiler.This is the way to go to <b>overload operators</b> for example.</br>
</div>
...
</span><span style="color:#3186CD">impl</span><span style="color:#0"> </span><span style="color:#3BC3A7">Struct</span><span style="color:#0">
{
    ...
    </span><span style="color:#6A9955">// Example: x := cast(OneType) v</span><span style="color:#0">
...
<div><code>opAffect</code> is a way of assigning to a struct with <code>=</code>.</br>
...
}</span></code></pre><div>Here the variable <code>v</code> will not be initialized prior to the affectation. This is more optimal, as there's only one initialization.</br>
</div>
...
</span><span style="color:#FF6A00">toto</span><span style="color:#0">(</span><span style="color:#74A35B">5</span><span style="color:#0">'</span><span style="color:#ED9A11">u16</span><span style="color:#0">)</span></code></pre><div>A structure can be converted to a constant by a call to opAffect if the function is marked with <code>Swag.ConstExpr</code>.</br>
</div>
...
<div>Swag accepts copy and move semantics for structures. In this examples, we use a <code>Vector3</code> to illustrate, even if of course that kind of struct does not need a move semantic, as there's no heap involved.</br>
</div>
...
}</span></code></pre><pre><code><span style="color:#0">a := </span><span style="color:#3BC3A7">Vector3</span><span style="color:#0">{}
b := </span><span style="color:#3BC3A7">Vector3</span><span style="color:#0">{</span><span style="color:#74A35B">100</span><span style="color:#0">, </span><span style="color:#74A35B">200</span><span style="color:#0">, </span><span style="color:#74A35B">300</span><span style="color:#0">}
...
a =,moveraw b
a =,nodrop,moveraw b</span></code></pre><div><code>moveref</code> can be used instead of <code>ref</code> in a function parameter to declare a move semantic intention.</br>
</div>
...
a := </span><span style="color:#3BC3A7">Vector3</span><span style="color:#0">{</span><span style="color:#74A35B">1</span><span style="color:#0">, </span><span style="color:#74A35B">2</span><span style="color:#0">, </span><span style="color:#74A35B">3</span><span style="color:#0">}
...
<div>You can visit a struct value if a macro <code>opVisit</code> has been defined. This is the equivalent of an <b>iterator</b>.</br>
</div>
<pre><code><span style="color:#3186CD">struct</span><span style="color:#0"> </span><span style="color:#3BC3A7">MyStruct</span><span style="color:#0">
{
    ...
    </span><span style="color:#3186CD">func</span><span style="color:#0">(ptr: </span><span style="color:#ED9A11">bool</span><span style="color:#0">) </span><span style="color:#B4B44A">opVisit</span><span style="color:#0">(</span><span style="color:#ED9A11">self</span><span style="color:#0">, stmt: </span><span style="color:#ED9A11">code</span><span style="color:#0">)
    {
        ...
        </span><span style="color:#B040BE">loop</span><span style="color:#0"> idx: </span><span style="color:#74A35B">3</span><span style="color:#0">
        {
            ...
            </span><span style="color:#7F7F7F">#macro</span><span style="color:#0">
            {
                ...
                </span><span style="color:#B4B44A">@alias1</span><span style="color:#0"> := </span><span style="color:#B4B44A">@index</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(cpt == </span><span style="color:#74A35B">3</span><span style="color:#0">)</span></code></pre><div>You can have variants of <code>opVisit</code> by specifying an <b>additional name</b>.</br>
</div>
<pre><code><span style="color:#3186CD">impl</span><span style="color:#0"> </span><span style="color:#3BC3A7">MyStruct</span><span style="color:#0">
{
    </span><span style="color:#7F7F7F">#[Swag.Macro]</span><span style="color:#0">
...
<div>
 ...
 You can force the layout of a field with the <code>Swag.Offset</code> attribute.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">struct</span><span style="color:#0"> </span><span style="color:#3BC3A7">MyStruct</span><span style="color:#0">
    {
...
<div>You can also control the struct layout with two attributes: <code>#[Swag.Pack] and </code>#[Swag.Align].</br>
</div>
<div>
</br>
...
<pre><code><span style="color:#0">{
    ...
    v := </span><span style="color:#3BC3A7">MyUnion</span><span style="color:#0">{x: </span><span style="color:#74A35B">666</span><span style="color:#0">}
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(v.y == </span><span style="color:#74A35B">666</span><span style="color:#0">)
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(v.z == </span><span style="color:#74A35B">666</span><span style="color:#0">)
}</span></code></pre>
...
{
    x, y, z: </span><span style="color:#ED9A11">f32</span><span style="color:#0">
}</span></code></pre><div>Here we declare an interface, with two functions <code>set</code> and <code>reset</code>.</br>
</div>
...
}</span></code></pre><div>You can implement an interface for any given struct with <code>impl</code> and <code>for</code>.</br>
For example here, we implement interface <code>IReset</code> for struct <code>Point2</code>.</br>
</div>
...
}</span></code></pre><div>And we implement interface <code>IReset</code> also for struct <code>Point3</code>.</br>
</div>
...
<div>A function declaration usually starts with the <code>func</code> keyword followed by the function name.</br>
</div>
<pre><code><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">toto</span><span style="color:#0">()
{
}</span></code></pre><div>If the function needs to return a value, you must add <code>-></code> followed by the type.</br>
</div>
...
}</span></code></pre><div>The return type can be deduced in case of a simple expression, by using <code>=></code> instead of <code>-></code>.</br>
Here the return type will be deduced to be <code>s32</code>.</br>
</div>
<pre><code><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">sum</span><span style="color:#0">(x, y: </span><span style="color:#ED9A11">s32</span><span style="color:#0">) => x + y</span></code></pre><div>A short version exists, in case of a function returning nothing.</br>
</div>
<pre><code><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">print</span><span style="color:#0">(val: </span><span style="color:#ED9A11">string</span><span style="color:#0">) = </span><span style="color:#B4B44A">@print</span><span style="color:#0">(val)</span></code></pre><div>Parameters are specified after the function name, between parenthesis.</br>
Here we declare two parameters <code>x</code> and <code>y</code> of type <code>s32</code>, and one last parameter of type <code>f32</code>.</br>
</div>
...
}</span></code></pre><div>Parameters can have default values.</br>
</div>
...
}</span></code></pre><div>The type of the parameters can be deduced if you specify a default value.</br>
Here <code>x</code> and <code>y</code> have the type <code>f32</code> because <code>0.0</code> is a 32 bits floating point literal.</br>
</div>
...
}</span></code></pre><pre><code><span style="color:#6A9955">// Functions can be nested inside other functions. This are not closure but just functions in a sub scope.</span><span style="color:#0">
</span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">sub</span><span style="color:#0">(x, y: </span><span style="color:#ED9A11">s32</span><span style="color:#0">) => x - y</span></code></pre><div>
</br>
 Simple call.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>You can name parameters, and don't have to respect parameters order in that case.</br>
</div>
...
{
    x1 := </span><span style="color:#FF6A00">sub</span><span style="color:#0">(x: </span><span style="color:#74A35B">5</span><span style="color:#0">, y: </span><span style="color:#74A35B">2</span><span style="color:#0">)
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(x1 == </span><span style="color:#74A35B">3</span><span style="color:#0">)
    x2 := </span><span style="color:#FF6A00">sub</span><span style="color:#0">(y: </span><span style="color:#74A35B">5</span><span style="color:#0">, x: </span><span style="color:#74A35B">2</span><span style="color:#0">)
...
}</span></code></pre><div><h4 id="Multiple return values">Multiple return values</br>
</h4></div>
<div>
</br>
...
<pre><code><span style="color:#0">{
    ...
    result := </span><span style="color:#FF6A00">myFunction</span><span style="color:#0">()
    ...
    (z, w) := </span><span style="color:#FF6A00">myFunction</span><span style="color:#0">()
...
result := </span><span style="color:#FF6A00">returns2</span><span style="color:#0">()
...
(x, y) := </span><span style="color:#FF6A00">returns2</span><span style="color:#0">()
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(x == </span><span style="color:#74A35B">1</span><span style="color:#0">)
...
</span><span style="color:#3186CD">var</span><span style="color:#0"> ptr0: </span><span style="color:#3186CD">func</span><span style="color:#0">() = &myFunction0
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#FF6A00">ptr1</span><span style="color:#0">(</span><span style="color:#74A35B">2</span><span style="color:#0">) == </span><span style="color:#74A35B">4</span><span style="color:#0">)</span></code></pre><div>A lambda can be null.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(lambda == </span><span style="color:#3186CD">null</span><span style="color:#0">)</span></code></pre><div>You can use a lambda as a function parameter type.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#FF6A00">toDo</span><span style="color:#0">(</span><span style="color:#74A35B">4</span><span style="color:#0">, &square) == </span><span style="color:#74A35B">16</span><span style="color:#0">)</span></code></pre><div><h4 id="Anonymous functions">Anonymous functions</br>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#FF6A00">cb</span><span style="color:#0">(</span><span style="color:#74A35B">4</span><span style="color:#0">) == </span><span style="color:#74A35B">64</span><span style="color:#0">)</span></code></pre><div>Anonymous functions can be passed as parameters to another function.</br>
</div>
<pre><code><span style="color:#3186CD">alias</span><span style="color:#0"> callback = </span><span style="color:#3186CD">func</span><span style="color:#0">(</span><span style="color:#ED9A11">s32</span><span style="color:#0">)-></span><span style="color:#ED9A11">s32</span><span style="color:#0">
</span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">toDo</span><span style="color:#0">(value: </span><span style="color:#ED9A11">s32</span><span style="color:#0">, ptr: callback)-></span><span style="color:#ED9A11">s32</span><span style="color:#0"> => </span><span style="color:#FF6A00">ptr</span><span style="color:#0">(value)
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#FF6A00">toDo</span><span style="color:#0">(</span><span style="color:#74A35B">4</span><span style="color:#0">, </span><span style="color:#3186CD">func</span><span style="color:#0">(x: </span><span style="color:#ED9A11">s32</span><span style="color:#0">)-></span><span style="color:#ED9A11">s32</span><span style="color:#0"> { </span><span style="color:#B040BE">return</span><span style="color:#0"> x - x; }) == </span><span style="color:#74A35B">0</span><span style="color:#0">)</span></code></pre><div>The types of the parameters can be deduced.</br>
</div>
<pre><code><span style="color:#3186CD">alias</span><span style="color:#0"> callback = </span><span style="color:#3186CD">func</span><span style="color:#0">(</span><span style="color:#ED9A11">s32</span><span style="color:#0">)-></span><span style="color:#ED9A11">s32</span><span style="color:#0">
</span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">toDo</span><span style="color:#0">(value: </span><span style="color:#ED9A11">s32</span><span style="color:#0">, ptr: callback)-></span><span style="color:#ED9A11">s32</span><span style="color:#0"> => </span><span style="color:#FF6A00">ptr</span><span style="color:#0">(value)
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#FF6A00">toDo</span><span style="color:#0">(</span><span style="color:#74A35B">4</span><span style="color:#0">, </span><span style="color:#3186CD">func</span><span style="color:#0">(x) { </span><span style="color:#B040BE">return</span><span style="color:#0"> x - x; }) == </span><span style="color:#74A35B">0</span><span style="color:#0">)</span></code></pre><div>When you affect a lambda to a variable, the type of parameters and the return type can also be omitted, as they will be deduced from the variable type.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#FF6A00">fct</span><span style="color:#0">(</span><span style="color:#74A35B">20</span><span style="color:#0">, </span><span style="color:#74A35B">120</span><span style="color:#0">))</span></code></pre><div>Lambdas can have default parameters values.</br>
</div>
<pre><code><span style="color:#0">{
...
<div>Swag supports a limited set of the <code>closure</code> concept.</br>
</br>
Only a given amount of bytes of capture are possible (for now 48 bytes). That way there's never an hidden allocation.</br>
Another limitation is that you can only capture 'simple' variables (no struct with <code>opDrop</code>, <code>opPostCopy</code>, <code>opPostMove</code> for example).</br>
</div>
<div>
</br>
 A closure is declared like a lambda, with the captured variables between <code>|...|</code> before the function parameters.</br>
</div>
<pre><code><span style="color:#0">{
    a := </span><span style="color:#74A35B">125</span><span style="color:#0">
    b := </span><span style="color:#74A35B">521</span><span style="color:#0">

    </span><span style="color:#6A9955">// Capture 'a' and 'b'</span><span style="color:#0">
    </span><span style="color:#3186CD">var</span><span style="color:#0"> fct: </span><span style="color:#3186CD">closure</span><span style="color:#0">() = </span><span style="color:#3186CD">closure</span><span style="color:#0">|a, b|()
    {
 ...
 You can also capture by pointer with <code>&</code> (otherwise it's a copy).</br>
</div>
<pre><code><span style="color:#0">{
    a := </span><span style="color:#74A35B">125</span><span style="color:#0">

    </span><span style="color:#6A9955">// Capture 'a' by pointer</span><span style="color:#0">
    </span><span style="color:#3186CD">var</span><span style="color:#0"> fct: </span><span style="color:#3186CD">closure</span><span style="color:#0">() = </span><span style="color:#3186CD">closure</span><span style="color:#0">|&a|()
    {
 ...
 You can also capture by reference with <code>ref</code>.</br>
</div>
<pre><code><span style="color:#0">{
    a := </span><span style="color:#74A35B">125</span><span style="color:#0">

    </span><span style="color:#6A9955">// Capture 'a' by reference</span><span style="color:#0">
    fct := </span><span style="color:#3186CD">closure</span><span style="color:#0">|</span><span style="color:#3186CD">ref</span><span style="color:#0"> a|()
    {
        </span><span style="color:#6A9955">// We can change the value of the local variable 'a'</span><span style="color:#0">
        a += </span><span style="color:#74A35B">1</span><span style="color:#0">
    }
...
<pre><code><span style="color:#0">{
    ...
    {
        res := </span><span style="color:#74A35B">0</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    fct := </span><span style="color:#FF6A00">getInc</span><span style="color:#0">()
...
<pre><code><span style="color:#0">{
 ...
 A mixin accepts parameters of type <code>code</code>.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.Mixin]</span><span style="color:#0">
 ...
 When the last parameter of a mixin is of type <code>code</code>, the caller can declare that code in a statement just after the call.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.Mixin]</span><span style="color:#0">
 ...
 You can use the special name <code>@alias</code> to create a named alias for an identifier.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.Mixin]</span><span style="color:#0">
    ...
    a, b := </span><span style="color:#74A35B">0</span><span style="color:#0">
...

{
    ...
    {
        </span><span style="color:#3186CD">var</span><span style="color:#0"> </span><span style="color:#B4B44A">@alias0</span><span style="color:#0"> = value
    }
 ...
 You can declare special variables named <code>@mixin?</code>. Those variables will have a unique name each time the mixin is used. So the same mixin, even if it declares local variables, can be used multiple time in the same scope.</br>
</div>
<pre><code><span style="color:#0">{
...
<div>
 ...
 A macro, like a mixin, is declared like a function, but with the attribute <code>Swag.Macro</code>.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.Macro]</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    {
        a := </span><span style="color:#74A35B">666</span><span style="color:#0">    </span><span style="color:#6A9955">// 'a' is declared in the scope of 'myMacro'</span><span style="color:#0">
    }
 ...
 But you can force an identifier to be found outside of the scope of the macro with <code>#up</code>.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.Macro]</span><span style="color:#0">
    </span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">myMacro</span><span style="color:#0">()
 ...
 Like a mixin, macro accepts <code>code</code> parameters.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.Macro]</span><span style="color:#0">
 ...
 You can use <code>#macro</code> inside a macro to force the code after to be in the same scope of the caller. That is, no <code>#up</code> is necessary to reference variables of the caller.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.Macro]</span><span style="color:#0">
    </span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">myMacro</span><span style="color:#0">(what: </span><span style="color:#ED9A11">code</span><span style="color:#0">)
    {
        ...
        a := </span><span style="color:#74A35B">666</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    </span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">repeat</span><span style="color:#0">(count: </span><span style="color:#ED9A11">s32</span><span style="color:#0">, what: </span><span style="color:#ED9A11">code</span><span style="color:#0">)
    {
        ...
        </span><span style="color:#B040BE">while</span><span style="color:#0"> a < count
        {
            ...
            {
                index := </span><span style="color:#C3973B">#up</span><span style="color:#0"> a
                </span><span style="color:#7F7F7F">#mixin</span><span style="color:#0"> </span><span style="color:#C3973B">#up</span><span style="color:#0"> what
            }
 ...
 When you need <code>break</code> in the user code to break outside of a multi loop.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.Macro]</span><span style="color:#0">
 ...
 In a macro, you can use special variables named <code>@alias<num></code>. Note that this is also valid for mixins.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.Macro]</span><span style="color:#0">
    ...
    {
        </span><span style="color:#B4B44A">@alias0</span><span style="color:#0"> := v
        </span><span style="color:#B4B44A">@alias1</span><span style="color:#0"> := v * </span><span style="color:#74A35B">2</span><span style="color:#0">
...
<div>
 ...
 A function can take a variable number of arguments with <code>...</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 In that case, <code>parameters</code> is a slice of <code>any</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 If all variadic parameters are of the same type, you can force it. Parameters then won't be of type <code>any</code>.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>You can spread the content of an array or a slice to variadic parameters with <code>@spread</code>.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">sum</span><span style="color:#0">(params: </span><span style="color:#ED9A11">s32</span><span style="color:#0">...)-></span><span style="color:#ED9A11">s32</span><span style="color:#0">
    {
        ...
        total := </span><span style="color:#74A35B">0</span><span style="color:#0">
    ...
    arr := [</span><span style="color:#74A35B">1</span><span style="color:#0">, </span><span style="color:#74A35B">2</span><span style="color:#0">, </span><span style="color:#74A35B">3</span><span style="color:#0">, </span><span style="color:#74A35B">4</span><span style="color:#0">]
    res := </span><span style="color:#FF6A00">sum</span><span style="color:#0">(</span><span style="color:#B4B44A">@spread</span><span style="color:#0">(arr)) </span><span style="color:#6A9955">// is equivalent to sum(1, 2, 3, 4)</span><span style="color:#0">
    ...
    res1 := </span><span style="color:#FF6A00">sum</span><span style="color:#0">(</span><span style="color:#B4B44A">@spread</span><span style="color:#0">(arr[</span><span style="color:#74A35B">1.</span><span style="color:#0">.</span><span style="color:#74A35B">2</span><span style="color:#0">])) </span><span style="color:#6A9955">// is equivalent to sum(2, 3)</span><span style="color:#0">
...
<div><i>ufcs</i> stands for <i>uniform function call syntax</i>. It allows every functions to be called with a <code>param.func()</code> form when the first parameter of <code>func()</code> is of the same type as <code>param</code>.</br>
</div>
<pre><code><span style="color:#0">{
...
<div>A function marked with <code>Swag.ConstExpr</code> can be executed by the compiler if it can.</br>
</div>
<pre><code><span style="color:#7F7F7F">#[Swag.ConstExpr]</span><span style="color:#0">
</span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">sum</span><span style="color:#0">(x, y: </span><span style="color:#ED9A11">f32</span><span style="color:#0">) => x + y</span></code></pre><div>Here <code>G</code> will be baked to 3 by the compiler.</br>
</div>
...
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#3BC3A7">G</span><span style="color:#0"> == </span><span style="color:#74A35B">3</span></code></pre><div>If a function is not <code>ConstExpr</code>, you can force the compile time call with <code>#run</code>.</br>
</div>
...
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#FF6A00">sum</span><span style="color:#0">(</span><span style="color:#74A35B">1</span><span style="color:#0">, </span><span style="color:#74A35B">2</span><span style="color:#0">, </span><span style="color:#74A35B">3</span><span style="color:#0">) == </span><span style="color:#74A35B">6</span></code></pre><div>Note that in Swag, there is no implicit cast for function parameters. So you must always specify the right type.</br>
</div>
...
</span><span style="color:#6A9955">// res0 := over(1, 2)</span><span style="color:#0">

res0 := </span><span style="color:#FF6A00">over</span><span style="color:#0">(</span><span style="color:#74A35B">1</span><span style="color:#0">'</span><span style="color:#ED9A11">s32</span><span style="color:#0">, </span><span style="color:#74A35B">2</span><span style="color:#0">'</span><span style="color:#ED9A11">s32</span><span style="color:#0">)
...
</span><span style="color:#3186CD">discard</span><span style="color:#0"> </span><span style="color:#FF6A00">sum</span><span style="color:#0">(</span><span style="color:#74A35B">2</span><span style="color:#0">, </span><span style="color:#74A35B">3</span><span style="color:#0">)</span></code></pre><div>If a function authorizes the caller to not use its return value, because it's not that important, it can be marked with <code>Swag.Discardable</code>.</br>
</div>
...
<div>Inside a function, you can use the <code>retval</code> type which is an alias to the function return type.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>But <code>retval</code> will also make a direct reference to the caller storage, to avoid an unnecessary copy (if possible). So this is mostly a hint for the compiler, and usefull when the function returns a complexe type like a struct, a tuple or an array.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    (r, g, b) := </span><span style="color:#FF6A00">getWhite</span><span style="color:#0">()
...
<pre><code><span style="color:#0">{
    ...
    arr := </span><span style="color:#FF6A00">toto</span><span style="color:#0">()
...
Put a special attribute <code>Swag.Foreign</code> before the function prototype, and specify the module name where the function is located.</br>
</br>
The module name can be a swag compiled module, or an external system module (where the location depends on the OS).</br>
</div>
<div>In the case below, the function is located in <code>kernel32.dll</code> (under windows)</br>
</div>
...
}</span></code></pre><div>Note that in the case of a system module, you will have to declare somewhere the imported library too.</br>
</br>
<code>#foreignlib</code> is here to force a link to the given library (when generating executables).</br>
</div>
...
All intrinsics start with <code>@</code>, which is reserved for them.</br>
</div>
<pre><code><span style="color:#7F7F7F">#global</span><span style="color:#0"> skip</span></code></pre><div><h3 id="Base">Base</br>
</h3></div>
...
</span><span style="color:#B4B44A">@compilerwarning</span><span style="color:#0">()</span></code></pre><div><h3 id="Buildin">Buildin</br>
</h3></div>
...
</span><span style="color:#B4B44A">@err</span></code></pre><div><h3 id="Memory related">Memory related</br>
</h3></div>
...
</span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#F2470C">@strlen</span><span style="color:#0">(value: </span><span style="color:#3186CD">const</span><span style="color:#0"> *</span><span style="color:#ED9A11">u8</span><span style="color:#0">)-></span><span style="color:#ED9A11">u64</span><span style="color:#0">;</span></code></pre><div><h3 id="Atomic operations">Atomic operations</br>
</h3></div>
...
</span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#F2470C">@atomcmpxchg</span><span style="color:#0">(addr: *</span><span style="color:#ED9A11">u64</span><span style="color:#0">, compareTo, exchangeWith: </span><span style="color:#ED9A11">u64</span><span style="color:#0">)-></span><span style="color:#ED9A11">u64</span><span style="color:#0">;</span></code></pre><div><h3 id="Math">Math</br>
</h3></div>
...
</h3><code>@init</code> can be used to reinitialize a variable/array to the default value.</br>
</div>
<div>
</br>
...
}</span></code></pre><div>When called on a struct, the struct will be restored to the values defined in it.</br>
</div>
...
}</span></code></pre><div><h3 id="@drop">@drop</br>
</h3>Note that for a struct, this will <b>not</b> call <code>opDrop</code>, so this is mostly useful to initialize a plain old data.</br>
</br>
...
<div>A function can be generic by specifying some parameters after <code>func</code>.</br>
...
}</span></code></pre><div>Generic types can be deduced from parameters, so <code>func'type()</code> is not always necessary.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#FF6A00">myFunc</span><span style="color:#0">(</span><span style="color:#74A35B">2.0</span><span style="color:#0">'</span><span style="color:#ED9A11">f32</span><span style="color:#0">) == </span><span style="color:#74A35B">4.0</span><span style="color:#0">)     </span><span style="color:#6A9955">// T is deduced to be f32</span></code></pre><div>You can also specify constants as generic parameters.</br>
</div>
<div>
</br>
 <code>N</code> is a constant a type <code>s32</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 <code>const</code> can also be omitted, as an identifier followed by a type definition is considered to be a constant and not a type.</br>
</div>
<pre><code><span style="color:#0">{
...
<pre><code><span style="color:#0">{
 ...
 You can ommit the type if you declare the constant with <code>const</code>. It will be deduced from the assignment expression.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>You can mix types and constants.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>Like functions, a struct can also be generic.</br>
</div>
<pre><code><span style="color:#0">{
...
</h4>On a function, you can use <code>#validif</code> to check if the usage of the function is correct.</br>
</br>
If the <code>#validif</code> expression returns false, then the function will not be considered for the call. If there's no other overload to match, then the compiler will raise an error.</br>
</br>
The <code>#validif</code> expression is evaluated <b>only once</b>, whatever the call, so it is typically used to check generic parameters.</br>
</div>
...
{
    total := </span><span style="color:#74A35B">0</span><span style="color:#0">'</span><span style="color:#3BC3A7">T</span><span style="color:#0">
...
res1 := sum'</span><span style="color:#ED9A11">s32</span><span style="color:#0">(</span><span style="color:#74A35B">1</span><span style="color:#0">, </span><span style="color:#74A35B">2</span><span style="color:#0">)
...
</span><span style="color:#6A9955">// res1 := sum'f32(1, 2)</span></code></pre><div>You can use <code>#validif</code> to make a kind of a generic specialisation.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#FF6A00">isNull</span><span style="color:#0">(</span><span style="color:#74A35B">0.001</span><span style="color:#0">'</span><span style="color:#ED9A11">f32</span><span style="color:#0">))</span></code></pre><div>Instead of a single expression, <code>#validif</code> can be followed by a block that returns a <code>bool</code>.</br>
</div>
...
}</span></code></pre><div>By using <code>@compilererror</code>, you can then trigger your own errors at compile time if the type is incorrect.</br>
</div>
...
</span><span style="color:#6A9955">// x := sum'f32(1, 2)</span></code></pre><div><code>#validif</code> can also be used on a generic struct. Unlike functions, if the expression failed, then you will have an error right away because there's no overload in the case of structures.</br>
</div>
...
</span><span style="color:#6A9955">//var v: Point's32</span></code></pre><div><h4 id="Multiple evaluations">Multiple evaluations</br>
</h4>Instead of <code>#validif</code>, you can use <code>#validifx</code>. <code>#validifx</code> is evaluated for <b>each</b> call, so it can be used to check parameters, as long as they can be <b>evaluated at compile time</b>.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    x1 := </span><span style="color:#FF6A00">div</span><span style="color:#0">(</span><span style="color:#74A35B">1</span><span style="color:#0">, </span><span style="color:#74A35B">1</span><span style="color:#0">)

    </span><span style="color:#6A9955">// Error at compile time, division by zero.</span><span style="color:#0">

    </span><span style="color:#6A9955">// x2 := div(1, 0)</span><span style="color:#0">
}
...
<div>Swag provides also a simple way of checking generic parameters, without the need of <code>#validif</code>.</br>
...
</span><span style="color:#6A9955">// res1 := sum'f32(1, 2)</span></code></pre><div>The type constraint can be any compile time expression, as long as the resulting type is <code>bool</code>.</br>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#FF6A00">myFunc</span><span style="color:#0">(</span><span style="color:#3186CD">false</span><span style="color:#0">) == </span><span style="color:#3186CD">false</span><span style="color:#0">)</span></code></pre><div>Works also for structs.</br>
</div>
<pre><code><span style="color:#7F7F7F">#[Swag.ConstExpr]</span><span style="color:#0">
...
<div><code>defer</code> is used to call an expression when the current scope is left. It's purely compile time, so it does not evaluate until the block is left.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 <code>defer</code> can also be used with a block.</br>
</div>
<pre><code><span style="color:#0">{
    v := </span><span style="color:#74A35B">0</span><span style="color:#0">
    </span><span style="color:#B040BE">defer</span><span style="color:#0">
 ...
 <code>defer</code> expressions are called when leaving the corresponding scope, even with <code>return</code>, <code>break</code>, <code>continue</code> etc., and even inside a <code>loop/while/for</code> etc.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 <code>defer</code> are executed in reverse order of their declaration.</br>
</div>
<pre><code><span style="color:#0">{
    x := </span><span style="color:#74A35B">1</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
    ...
    resource := </span><span style="color:#3186CD">false</span><span style="color:#0">
    </span><span style="color:#B040BE">loop</span><span style="color:#0"> </span><span style="color:#74A35B">10</span><span style="color:#0">
    {
...
<div>
 ...
 <code>using</code> brings the scope of a namespace, a struct or an enum in the current one.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">enum</span><span style="color:#0"> </span><span style="color:#3BC3A7">RGB</span><span style="color:#0"> { </span><span style="color:#3BC3A7">R</span><span style="color:#0">; </span><span style="color:#3BC3A7">G</span><span style="color:#0">; </span><span style="color:#3BC3A7">B</span><span style="color:#0">; }
 ...
 <code>using</code> can also be used with a variable</br>
</div>
<pre><code><span style="color:#0">{
 ...
 You can declare a variable with <code>using</code> just before.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">struct</span><span style="color:#0"> </span><span style="color:#3BC3A7">Point</span><span style="color:#0"> { x: </span><span style="color:#ED9A11">s32</span><span style="color:#0">; y: </span><span style="color:#ED9A11">s32</span><span style="color:#0">; }
...
}</span></code></pre><div><h4 id="For a function parameter">For a function parameter</br>
</h4><code>using</code> applied to a function parameter can be seen as the equivalent of the hidden <code>this</code> in C++.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(pt.y == </span><span style="color:#74A35B">1</span><span style="color:#0">)</span></code></pre><div><h4 id="For a field">For a field</br>
</h4><code>using</code> can also be used with a field inside a struct.</br>
</div>
<pre><code><span style="color:#3186CD">struct</span><span style="color:#0"> </span><span style="color:#3BC3A7">Point2</span><span style="color:#0">
{
    x, y: </span><span style="color:#ED9A11">s32</span><span style="color:#0">
...
<div>You can use <code>with</code> to avoid repeating the same variable again and again. You can then access fields with a simple <code>.</code>.</br>
</div>
...
</span><span style="color:#3186CD">impl</span><span style="color:#0"> </span><span style="color:#3BC3A7">Point</span><span style="color:#0">
{
 ...
 <code>with</code> on a variable.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">var</span><span style="color:#0"> pt: </span><span style="color:#3BC3A7">Point</span><span style="color:#0">
...
<pre><code><span style="color:#0">{
 ...
 Instead of an identifier name, <code>with</code> also accepts a variable declaration.</br>
</div>
<pre><code><span style="color:#0">{
...
<div>
 ...
 <code>alias</code> is used to make a shortcut to another name or type.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#3186CD">enum</span><span style="color:#0"> </span><span style="color:#3BC3A7">RGB</span><span style="color:#0"> { </span><span style="color:#3BC3A7">R</span><span style="color:#0">; </span><span style="color:#3BC3A7">G</span><span style="color:#0">; </span><span style="color:#3BC3A7">B</span><span style="color:#0">; }
    </span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#3BC3A7">RGB</span><span style="color:#0">.</span><span style="color:#3BC3A7">R</span><span style="color:#0"> == </span><span style="color:#74A35B">0</span><span style="color:#0">)
...
<pre><code><span style="color:#0">{
 ...
 A type alias can be marked with the <code>Swag.Strict</code> attribute. In that case, implicit cast is not done, but explicit cast is still possible.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div>The caller will then have to deal with the error in some way. It can <code>catch</code> it, and test (or not) its value with the <code>@err</code> intrinsic. The execution will continue at the call site.</br>
</div>
...
}</span></code></pre><div>The caller can stop the execution with <code>try</code>, and return to its own caller with the same error raised. The function must then also be marked with <code>throw</code>.</br>
</br>
Here, the caller of <code>myFunc1</code> will have to deal with the error at its turn.</br>
</div>
...
}</span></code></pre><div>The caller can also panic if an error is raised, with <code>assume</code>.</br>
</br>
</div>
...
}</span></code></pre><div>Note that you can use a block instead of one single statement (this does not create a scope).</br>
</div>
<pre><code><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#FF6A00">myFunc3</span><span style="color:#0">() </span><span style="color:#3186CD">throw</span><span style="color:#0">
{
    ...
    {
        cpt0 := </span><span style="color:#FF6A00">count</span><span style="color:#0">(</span><span style="color:#BB6643">"filename"</span><span style="color:#0">)
        cpt1 := </span><span style="color:#FF6A00">count</span><span style="color:#0">(</span><span style="color:#BB6643">"other filename"</span><span style="color:#0">)
    }

    </span><span style="color:#3186CD">assume</span><span style="color:#0">
    {
        cpt2 := </span><span style="color:#FF6A00">count</span><span style="color:#0">(</span><span style="color:#BB6643">"filename"</span><span style="color:#0">)
        cpt3 := </span><span style="color:#FF6A00">count</span><span style="color:#0">(</span><span style="color:#BB6643">"other filename"</span><span style="color:#0">)
    }
    ...
    </span><span style="color:#3186CD">catch</span><span style="color:#0">
    {
        ...
        cpt5 := </span><span style="color:#FF6A00">count</span><span style="color:#0">(</span><span style="color:#BB6643">"other filename"</span><span style="color:#0">)
    }
...
}</span></code></pre><div>When a function is marked with <code>throw</code>, the <code>try</code> for a function call is automatic if not specified. That means that most of the time it's not necessary to specify it.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#B4B44A">@err</span><span style="color:#0"> == </span><span style="color:#BB6643">"error"</span><span style="color:#0">)</span></code></pre><div><h3 id="defer">defer</br>
</h3><code>defer</code> can have parameters like <code>defer(err)</code> or <code>defer(noerr)</code> to control if it should be executed depending on the error status.</br>
</div>
...
<div>Swag comes with a bunch of safety checks which can be activated by module/function/instruction with the <code>#[Swag.Safety] attribute.</br>
</br>
...
<div class="blockquote">Swag comes with four predefined configurations : <code>debug</code>, <code>fast-debug</code>, <code>fast-compile</code> and <code>release</code>. Safety checks are disabled in <code>fast-compile</code> and <code>release</code>.</br>
</div>
<div></div>
...
</span><span style="color:#6A9955">// x += 1      // This would overflow, and panic, because we lose informations</span></code></pre><div>But if you know what your are doing, you can use a special version of those operators, which will not panic. Add the <code>,over</code> modifier after the operation. This will disable safety checks.</br>
</div>
<pre><code><span style="color:#0">x := </span><span style="color:#74A35B">255</span><span style="color:#0">'</span><span style="color:#ED9A11">u8</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(x == </span><span style="color:#74A35B">0</span><span style="color:#0">)</span></code></pre><div>You can also use <code>#[Swag.Overflow(true)] to authorize overflow on a more global scale.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(x == </span><span style="color:#74A35B">0</span><span style="color:#0">)</span></code></pre><div>For 8 or 16 bits, you can promote an operation to 32 bits by using ',up'.</br>
</div>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(x) == </span><span style="color:#ED9A11">u32</span><span style="color:#0">)</span></code></pre><div>Swag will also check that a cast does not lose information.</br>
</div>
...
y1 := </span><span style="color:#3186CD">cast</span><span style="color:#0">,</span><span style="color:#FF6A00">over</span><span style="color:#0">(</span><span style="color:#ED9A11">s8</span><span style="color:#0">) x1  </span><span style="color:#6A9955">// But ',over' can be used on the cast operation too</span><span style="color:#0">
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(y1 == -</span><span style="color:#74A35B">1</span><span style="color:#0">)

x2 := -</span><span style="color:#74A35B">1</span><span style="color:#0">'</span><span style="color:#ED9A11">s8</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(y2 == </span><span style="color:#74A35B">255</span><span style="color:#0">)</span></code></pre><div>Rember that you can disable these safety checks with the corresponding attribute.</br>
</div>
<pre><code><span style="color:#7F7F7F">#[Swag.Overflow(true)]</span></code></pre><pre><code><span style="color:#0">x := </span><span style="color:#74A35B">255</span><span style="color:#0">'</span><span style="color:#ED9A11">u8</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(x == </span><span style="color:#74A35B">127</span><span style="color:#0">)</span></code></pre><div><h3 id="#[Swag.Safety("any", true)]">#[Swag.Safety("any", true)]</br>
...
</span><span style="color:#6A9955">//@assert(z == 0)</span></code></pre><div><h3 id="#[Swag.Safety("boundcheck", true)]">#[Swag.Safety("boundcheck", true)]</br>
...
</span><span style="color:#6A9955">//@assert(x[idx] == 1)     // '10' is out of range, will panic</span></code></pre><div>Safety when indexing a slice.</br>
</div>
...
</span><span style="color:#6A9955">//@assert(x[idx] == 1)      // '10' is out of range, will panic</span></code></pre><div>Safety when slicing a sized value.</br>
</div>
<pre><code><span style="color:#3186CD">var</span><span style="color:#0"> x: </span><span style="color:#3186CD">const</span><span style="color:#0"> [..] </span><span style="color:#ED9A11">s32</span><span style="color:#0"> = [</span><span style="color:#74A35B">0</span><span style="color:#0">, </span><span style="color:#74A35B">1</span><span style="color:#0">, </span><span style="color:#74A35B">2</span><span style="color:#0">]
...
</span><span style="color:#6A9955">//@assert(slice[0] == "s"'u8)</span></code></pre><div><h3 id="#[Swag.Safety("math", true)]">#[Swag.Safety("math", true)]</br>
...
</span><span style="color:#6A9955">//@print(z)</span></code></pre><div>Swag will also check for invalid arguments on some math intrinsics.</br>
</div>
...
</span><span style="color:#6A9955">//@acos(2'f32)</span></code></pre><div><h3 id="#[Swag.Safety("switch", true)]">#[Swag.Safety("switch", true)]</br>
...
<div><code>#assert</code> is a static assert (at compile time).</br>
</div>
<pre><code><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#3186CD">true</span></code></pre><div><code>@defined(SYMBOL)</code> returns true, at compile time, if the given symbol exists in the current context.</br>
</div>
...
</span><span style="color:#3186CD">var</span><span style="color:#0"> </span><span style="color:#3BC3A7">Global</span><span style="color:#0"> = </span><span style="color:#74A35B">0</span></code></pre><div>A static <code>#if/#elif/#else</code>, with an expression that can be evaluated at compile time.</br>
</div>
...
{
}</span></code></pre><div><code>#error</code> to raise a compile time error, and <code>#warning</code> to raise a compile time warning.</br>
</div>
...
}</span></code></pre><div>A more complicated <code>#assert</code>.</br>
</div>
<pre><code><span style="color:#7F7F7F">#[Swag.ConstExpr]</span><span style="color:#0">
...
</h4><code>#test</code> is a special function than can be used in the 'tests/' folder of the workspace.</br>
They will be executed only if swag is running in test mode.</br>
</div>
<div><h4 id="#main">#main</br>
</h4><code>#main</code> is the program entry point.</br>
Can only be defined once per module.</br>
</div>
<pre><code><span style="color:#FF6A00">#main</span><span style="color:#0">
{
}</span></code></pre><div><h4 id="#init">#init</br>
</h4><code>#init</code> will be called at runtime, during the module initialization.</br>
...
{
}</span></code></pre><div><h4 id="#drop">#drop</br>
</h4><code>#drop</code> will be called at runtime, when module is unloaded.</br>
...
{
}</span></code></pre><div><h4 id="#premain">#premain</br>
</h4><code>#premain</code> will be called after all modules have done their #init code, but before the #main function is called.</br>
</div>
<pre><code><span style="color:#FF6A00">#premain</span><span style="color:#0">
{
}</span></code></pre>
<h3 id="183__run">Run</h3>
<div><code>#run</code> is a special function that will be called at compile time.</br>
It can be used to precompute some global values for example.</br>
</div>
<pre><code><span style="color:#3186CD">var</span><span style="color:#0"> </span><span style="color:#3BC3A7">G</span><span style="color:#0">: [</span><span style="color:#74A35B">5</span><span style="color:#0">] </span><span style="color:#ED9A11">f32</span><span style="color:#0"> = </span><span style="color:#3186CD">undefined</span></code></pre><div>Initialize <code>G</code> with <code>[1,2,4,8,16] at compile time.</br>
</div>
<pre><code><span style="color:#FF6A00">#run</span><span style="color:#0">
{
...
}</span></code></pre><div><code>#test</code> are executed after #run, even at compile time (during testing).</br>
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(</span><span style="color:#3BC3A7">G</span><span style="color:#0">[</span><span style="color:#74A35B">4</span><span style="color:#0">] == </span><span style="color:#74A35B">16</span><span style="color:#0">)</span></code></pre><div><code>#run</code> can also be used as an expression, to call for example a function not marked with <code>#[Swag.ConstExpr].</br>
</div>
...
{
    result := </span><span style="color:#74A35B">0</span><span style="color:#0">'</span><span style="color:#ED9A11">s32</span><span style="color:#0">
    </span><span style="color:#B040BE">visit</span><span style="color:#0"> v: values
        result += v
    </span><span style="color:#B040BE">return</span><span style="color:#0"> result
}</span></code></pre><div><code>#run</code> can also be used as an expression block.</br>
The return type is deduced from the <code>return</code> statement.</br>
</div>
...
<div>A bunch of <code>#global</code> can start a source file.</br>
</div>
...
<div>A global variable can be tagged with <code>#[Swag.Tls] to store it in the thread local storage (one copy per thread).</br>
</div>
<pre><code><span style="color:#7F7F7F">#[Swag.Tls]</span><span style="color:#0">
</span><span style="color:#3186CD">var</span><span style="color:#0"> </span><span style="color:#3BC3A7">G</span><span style="color:#0"> = </span><span style="color:#74A35B">0</span></code></pre><div>
</br>
...
}</span></code></pre><div>A global variable can also be marked as <code>#[Swag.Compiler]. That kind of variable will not be exported to the runtime and can only be used in compile time code.</br>
</div>
...
<div>User attributes are declared like functions, but with the <code>attr</code> keyword before instead of <code>func</code>.</br>
</div>
<pre><code><span style="color:#3186CD">using</span><span style="color:#0"> </span><span style="color:#3BC3A7">Swag</span><span style="color:#0">
</span><span style="color:#3186CD">attr</span><span style="color:#0"> </span><span style="color:#3BC3A7">AttributeA</span><span style="color:#0">()</span></code></pre><div>Like functions, attributes can have parameters.</br>
</div>
<pre><code><span style="color:#3186CD">attr</span><span style="color:#0"> </span><span style="color:#3BC3A7">AttributeB</span><span style="color:#0">(x, y: </span><span style="color:#ED9A11">s32</span><span style="color:#0">, z: </span><span style="color:#ED9A11">string</span><span style="color:#0">)</span></code></pre><div>So attributes can also have default values.</br>
</div>
<pre><code><span style="color:#3186CD">attr</span><span style="color:#0"> </span><span style="color:#3BC3A7">AttributeBA</span><span style="color:#0">(x: </span><span style="color:#ED9A11">s32</span><span style="color:#0">, y: </span><span style="color:#ED9A11">string</span><span style="color:#0"> = </span><span style="color:#BB6643">"string"</span><span style="color:#0">)</span></code></pre><div>You can define a usage before the attribute definition to restrict its usage.</br>
</div>
<pre><code><span style="color:#7F7F7F">#[AttrUsage(AttributeUsage.Function)]</span><span style="color:#0">
</span><span style="color:#3186CD">attr</span><span style="color:#0"> </span><span style="color:#3BC3A7">AttributeC</span><span style="color:#0">()</span></code></pre><div>To use an attribute, the syntax is <code>#[attribute, attribute...]. It should be placed <b>before</b> the thing you want to tag.</br>
</div>
...
{
}</span></code></pre><div>You can declare multiple usages.</br>
</div>
...
{
}</span></code></pre><div>Finaly, attributes can be retrieved at runtime thanks to <b>type reflection</b>.</br>
</div>
...
All are located in the reserved <code>Swag</code> namespace.</br>
</div>
...
<div>In Swag, types are also values that can be inspected at compile time or at runtime. The two main intrinsics for this are <code>@typeof</code> and <code>@kindof</code>.</br>
</div>
<div>
</br>
 You can get the type of an expression with <code>@typeof</code>, or just with the type itself (<b>types are also values</b>).</br>
</div>
<pre><code><span style="color:#0">{
    ...
    ptr2 := </span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(</span><span style="color:#ED9A11">s16</span><span style="color:#0">)
    ...
    ptr3 := </span><span style="color:#ED9A11">s32</span><span style="color:#0">
    ...
    ptr4 := </span><span style="color:#ED9A11">s64</span><span style="color:#0">
 ...
 The return result of <code>@typeof</code> is a const pointer to a <code>Swag.TypeInfo</code> kind of structure. This is an alias for the <code>typeinfo</code> type.</br>
</div>
<pre><code><span style="color:#0">{
    ...
    ptr1 := </span><span style="color:#C3973B">#type</span><span style="color:#0"> [</span><span style="color:#74A35B">2</span><span style="color:#0">] </span><span style="color:#ED9A11">s32</span><span style="color:#0">
 ...
 The <code>TypeInfo</code> structure contains a different enum value for each type.</br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div><h3 id="@decltype">@decltype</br>
</h3></div>
<div>
</br>
 <code>@decltype</code> can be used to transform a <code>typeinfo</code> to a real compiler type. This is the opposite of <code>@typeof</code> or <code>@kindof</code>.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 <code>@decltype</code> can evaluate a constexpr expression that returns a <code>typeinfo</code> to determine the real type.</br>
</div>
<pre><code><span style="color:#0">{
    </span><span style="color:#7F7F7F">#[Swag.ConstExpr]</span><span style="color:#0">
...
<div><code>#message</code> is a special function that will be called by the compiler when something specific occurs during the build. The parameter of <code>#message</code> is a mask that tells the compiler when to call the function.</br>
</div>
<div>With the <code>Swag.CompilerMsgMask.SemFunctions</code> flag, for example, <code>#message</code> will be called each time a function of the module <b>has been typed</b>. You can then use <code>getMessage()</code> in the <code>@compiler()</code> interface to retrieve some informations about the reason of the call.</br>
</div>
...
</span><span style="color:#3186CD">func</span><span style="color:#0"> </span><span style="color:#3BC3A7">XXTestFunc3</span><span style="color:#0">() {}</span></code></pre><div>The compiler will call the following function after the semantic pass. So after <b>all the functions</b> of the module have been parsed.</br>
</div>
...
}</span></code></pre><div>This will be called for every global variables of the module.</br>
</div>
<pre><code><span style="color:#FF6A00">#message</span><span style="color:#0">(</span><span style="color:#3BC3A7">Swag</span><span style="color:#0">.</span><span style="color:#3BC3A7">CompilerMsgMask</span><span style="color:#0">.</span><span style="color:#3BC3A7">SemGlobals</span><span style="color:#0">)
{
    itf := </span><span style="color:#B4B44A">@compiler</span><span style="color:#0">()
    msg := itf.</span><span style="color:#FF6A00">getMessage</span><span style="color:#0">()
}</span></code></pre><div>This will be called for every global types of the module (structs, enums, interfaces...).</br>
</div>
...
<div>The most simple way to produce a string which contains the Swag code to compile is with an <code>#ast</code> block. An <code>#ast</code> block is executed at compile time and the string it returns will be compiled <b>inplace</b>.</br>
</div>
<div>
</br>
 The <code>#ast</code> can be a simple expression with the string to compile.</br>
</div>
<pre><code><span style="color:#0">{
 ...
 Or it can be a block, with an explicit <code>return</code></br>
</div>
<pre><code><span style="color:#0">{
...
}</span></code></pre><div><h4 id="Struct and enums">Struct and enums</br>
</h4><code>#ast</code> can for example be used to generate the content of a <code>struct</code> or <code>enum</code>.</br>
</div>
<pre><code><span style="color:#3186CD">struct</span><span style="color:#0"> </span><span style="color:#3BC3A7">MyStruct</span><span style="color:#0">
{
    </span><span style="color:#FF6A00">#ast</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(v.y == </span><span style="color:#74A35B">666</span><span style="color:#0">)</span></code></pre><div>It works with generics too, and can be mixed with static declarations.</br>
</div>
...
</span><span style="color:#7F7F7F">#assert</span><span style="color:#0"> </span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(v1.z) == </span><span style="color:#ED9A11">string</span></code></pre><div><code>#ast</code> needs to return a <i>string like</i> value, which can of course be dynamically constructed.</br>
</div>
<pre><code><span style="color:#7F7F7F">#[Swag.Compiler]</span><span style="color:#0">
...
{
    len := </span><span style="color:#74A35B">0</span><span style="color:#0">
...
</span><span style="color:#B4B44A">@assert</span><span style="color:#0">(v.z == </span><span style="color:#74A35B">3</span><span style="color:#0">)</span></code></pre><div><h4 id="For example">For example</br>
</h4>This is a real life example of an <code>#ast</code> usage from the <code>Std.Core</code> module. Here we generate a structure which contains all the fields of an original other structure, but where the types are forced to be <code>bool</code>.</br>
</div>
<pre><code><span style="color:#3186CD">struct</span><span style="color:#0">(</span><span style="color:#3BC3A7">T</span><span style="color:#0">) </span><span style="color:#3BC3A7">IsSet</span><span style="color:#0">
{
    </span><span style="color:#FF6A00">#ast</span><span style="color:#0">
    {
        ...
        str := </span><span style="color:#3BC3A7">StrConv</span><span style="color:#0">.</span><span style="color:#3BC3A7">StringBuilder</span><span style="color:#0">{}

        </span><span style="color:#6A9955">// We get the type of the generic parameter `T`</span><span style="color:#0">
        typeof := </span><span style="color:#B4B44A">@typeof</span><span style="color:#0">(</span><span style="color:#3BC3A7">T</span><span style="color:#0">)
...
</h4><code>#ast</code> can also be called at the global scope.</br>
</div>
...
}</span></code></pre><div>But be aware that you must use <code>#placeholder</code> in case you are generating global symbols that can be used by something else in the code. This will tell Swag that <i>this symbol</i> will exist at some point, so please wait for it to <i>exist</i> before complaining.</br>
</div>
<pre><code><span style="color:#7F7F7F">#placeholder</span><span style="color:#0"> myGeneratedConst   </span><span style="color:#6A9955">// Symbol `myGeneratedConst` will be generated</span></code></pre><div>Here for example, thanks to the <code>#placeholder</code>, the <code>#assert</code> will wait for the symbol <code>myGeneratedConst</code> to be replaced with its real content.</br>
</div>
...
<div>The other method to compile generated code is to use the function <code>compileString()</code> in the <code>@compiler()</code> interface. Of course this should be called at compile time, and mostly during a <code>#message</code> call.</br>
</br>
Here is a real life example from the <code>Std.Ogl</code> module (opengl wrapper), which uses <code>#message</code> to track functions marked with a specific <b>user attribute</b> <code>Ogl.Extension</code>, and generates some code for each function that has been found.</br>
</div>
...
}</span></code></pre><div>The following will be used to track the functions with that specific attribute.</br>
</div>
...
</span><span style="color:#3186CD">var</span><span style="color:#0"> g_Functions: </span><span style="color:#3BC3A7">Array</span><span style="color:#0">'</span><span style="color:#3BC3A7">OneFunc</span></code></pre><div>This <code>#message</code> will be called for each function of the <code>Ogl</code> module.</br>
</div>
...
}</span></code></pre><div>We will generate a <code>glInitExtensions</code> global function, so we register it as a place holder.</br>
</div>
<pre><code><span style="color:#7F7F7F">#placeholder</span><span style="color:#0"> glInitExtensions</span></code></pre><div>This is called once all functions of the module have been typed, and this is the main code generation.</br>
</div>
<pre><code><span style="color:#FF6A00">#message</span><span style="color:#0">(</span><span style="color:#3BC3A7">CompilerMsgMask</span><span style="color:#0">.</span><span style="color:#3BC3A7">PassAfterSemantic</span><span style="color:#0">)
{
    ...
    {
        typeFunc := </span><span style="color:#3186CD">cast</span><span style="color:#0">(</span><span style="color:#3186CD">const</span><span style="color:#0"> *</span><span style="color:#3BC3A7">TypeInfoFunc</span><span style="color:#0">) e.type
    ...
    itf := </span><span style="color:#B4B44A">@compiler</span><span style="color:#0">()
    str := builderVars.</span><span style="color:#FF6A00">toString</span><span style="color:#0">()
...
<div>The Swag compiler can generate documentation for all the modules of a given workspace.</br>
</div>
<pre><code><span style="color:#0">swag doc -w:myWorkspaceFolder</span></code></pre><div></div>
<div>The documentation comment needs to be placed just before a function, struct or enum.</br>
</div>
...
}</span></code></pre><div>For constants or enum values, the document comment is the one declared at the end of the line.</br>
</div>
...
}</span></code></pre><div>Some other markdown markers are also supported.</br>
</div>
...
}</span></code></pre><div>You can create a reference to something of the current package with [name]or [name1.name2 etc.]</br>
</div>
