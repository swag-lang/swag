...
<p>### Under windows 10/11 You should register the location of the swag compiler (<code class="incode">swag.exe</code>) in the PATH environment variable to be able to call it from everywhere. </p>
...
<p>### To compile your workspace </p>
<p>$ swag build -w:first             Workspace first [fast-debug-windows-x86_64]              Building first                  Done 0.067s </p>
<p>You can omit the workspace name (<code class="incode">-w:first</code> or <code class="incode">--workspace:first</code>) if you call the compiler directly from the workspace folder. This command will compile all modules in <code class="incode">modules/</code> and <code class="incode">examples/</code>. </p>
...
<p>### Note on Windows Defender realtime protection It's activated by default under Windows 10, and runs each time you launch an executable or a process. This can increase the compile time of your project, so consider excluding your Swag folder from it ! </p>
...
<h2>Content of the Swag folder </h2>
<ul>
...
<h2>The Swag language </h2>
<p>It contains the list of all that can be done with the language, in the form of small tests (in fact it's not really exhaustive, but should be...). </p>
