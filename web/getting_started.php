...
<div class="container">
<p><h1 id="Your first install">Your first install</h1></p>
<p><a href="https://github.com/swag-lang/swag/releases">Download</a> the latest release from github, and unzip it in a folder. Of course a <i>SSD</i> is better.</br>
<h3 id="Under windows 10/11">Under windows 10/11</h3></p>
<p>You should register the location of the swag compiler (<code>swag.exe</code>) in the PATH environment variable to be able to call it from everywhere.</br>
You can open a Powershell window, and run the following code :</br>
</p>
...
<p></br>
The compile unit of swag is a <b>workspace</b> which contains a variable number of <b>modules</b>.A module will compile to a dynamic library or an executable.</br>
To create a fresh new workspace named <i>first</i> :</br>
</p>
...
=> type 'swag run -w:</span><span style="color:#3BC3A7">F</span><span style="color:#0">:\first' to build </span><span style="color:#B040BE">and</span><span style="color:#0"> run that module</span></code></pre><p></br>
This will also create a simple executable module <i>first</i> to print "Hello world !".</br>
<i>F:/first/modules/first/src/main.swg</i></br>
</p>
...
A workspace contains a predefined number of sub folders:</br>
</p>
<ul><li><code>modules/</code> contains all the modules (sub folders) of that workspace.</li>
...
<p></br>
A module is also organized in a predefined way:</br>
</p>
<ul><li><code>moduleName/</code> the folder name of the module is used to create output files.</li>
...
<p></br>
A module always contains a special file named <code>module.swg</code>. This file is used to configure the module, and is <b>mandatory</b>.</br>
...
<pre><code><span style="color:#0">$ swag build -w:first
                 ...
                 </span><span style="color:#3BC3A7">Done</span><span style="color:#0"> </span><span style="color:#74A35B">0.067</span><span style="color:#0">s</span></code></pre><p></br>
You can omit the workspace name (<code>-w:first</code> or <code>--workspace:first</code>) if you call the compiler directly from the workspace folder.This command will compile all modules in <code>modules/</code> and <code>examples/</code>.</br>
You can also build and run your workspace.</br>
</p>
<pre><code><span style="color:#0">$ swag run -w:first
                 ...
                 </span><span style="color:#3BC3A7">Done</span><span style="color:#0"> </span><span style="color:#74A35B">0.093</span><span style="color:#0">s</span></code></pre><p></br>
<h3 id="Note on Windows Defender realtime protection">Note on Windows Defender realtime protection</h3></p>
<p>It's activated by default under Windows 10, and runs each time you launch an executable or a process.This can increase the compile time of your project, so consider excluding your Swag folder from it !</br>
<a href="https://support.microsoft.com/en-us/windows/add-an-exclusion-to-windows-security-811816c0-4dfd-af4a-47e4-c301afe13b26#:~:text=Go%20to%20Start%20%3E%20Settings%20%3E%20Update,%2C%20file%20types%2C%20or%20process">Reference</a></br>
<h1 id="Content of the Swag folder">Content of the Swag folder</h1></p>
<p>The Swag folder contains the compiler <code>swag.exe</code>, but also a bunch of sub folders.</br>
</p>
<ul><li><code>examples/</code> is a workspace with various code examples (mostly for testing).</li>
...
<p>You should take a look at the <code>reference/</code> sub folder in the Swag directory, or to the corresponding <a href="language.php">documentation</a>.It contains the list of all that can be done with the language, in the form of small tests (in fact it's not really exhaustive, but should be...).</br>
It's a good starting point to familiarize yourself with the language.</br>
And as this is a normal Swag workspace, you could also build and test it with <code>swag test -w:swag/reference</code>.</br>
