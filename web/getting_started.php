...
<p><h3 id="Under windows 10/11">Under windows 10/11</h3></p>
<p>You should register the location of the swag compiler (<code>swag.exe</code>) in the PATH environment variable to be able to call it from everywhere.</p>
...
<p><h1 id="Your first project">Your first project</h1></p>
<p></p>
...
=> type 'swag run -w:</span><span style="color:#3BC3A7">F</span><span style="color:#0">:\first' to build </span><span style="color:#B040BE">and</span><span style="color:#0"> run that module</span></code></pre><p></p>
...
<p></p>
<p>A module always contains a special file named <code>module.swg</code>. This file is used to configure the module, and is <b>mandatory</b>.</p>
<p><h3 id="To compile your workspace">To compile your workspace</h3></p>
<p></p>
<pre><code><span style="color:#0">$ swag build -w:first
                 ...
                 </span><span style="color:#3BC3A7">Done</span><span style="color:#0"> </span><span style="color:#74A35B">0.067</span><span style="color:#0">s</span></code></pre><p></p>
<p>You can omit the workspace name (<code>-w:first</code> or <code>--workspace:first</code>) if you call the compiler directly from the workspace folder.This command will compile all modules in <code>modules/</code> and <code>examples/</code>.</p>
...
<pre><code><span style="color:#0">$ swag run -w:first
                 ...
                 </span><span style="color:#3BC3A7">Done</span><span style="color:#0"> </span><span style="color:#74A35B">0.093</span><span style="color:#0">s</span></code></pre><p></p>
<p><h3 id="Note on Windows Defender realtime protection">Note on Windows Defender realtime protection</h3></p>
...
<p><h1 id="Content of the Swag folder">Content of the Swag folder</h1></p>
<p>The Swag folder contains the compiler <code>swag.exe</code>, but also a bunch of sub folders.</p>
<p></p>
...
<p></p>
<p><h1 id="The Swag language">The Swag language</h1></p>
<p>You should take a look at the <code>reference/</code> sub folder in the Swag directory, or to the corresponding <a href="language.php">documentation</a>.It contains the list of all that can be done with the language, in the form of small tests (in fact it's not really exhaustive, but should be...).</p>
<p>It's a good starting point to familiarize yourself with the language.</p>
<p>And as this is a normal Swag workspace, you could also build and test it with <code>swag test -w:swag/reference</code>.</p>
<p>You will also find some small examples (mostly written for tests) in <code>swag/bin/examples/modules</code>.To build and run one of them from the console, go to the workspace folder (<code>/examples</code>) and type for example <code>swag run -m:wnd</code>.</p>
</div>
</div>
